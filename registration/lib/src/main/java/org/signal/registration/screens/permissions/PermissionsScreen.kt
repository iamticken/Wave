...
package org.wave.registration.screens.permissions
...
import org.wave.registration.screens.util.MockMultiplePermissionsState
import org.wave.registration.screens.util.MockPermissionsState
import org.wave.registration.test.TestTags
...
fun PermissionsScreen(
  ...
  Column(
    ...
    Text(
      text = "Wave needs the following permissions to provide the best experience:",
...
private fun getPermissionDescriptions(permissions: List<String>): List<String> {
  return buildList {
    ...
    if (permissions.any { it == Manifest.permission.READ_CONTACTS || it == Manifest.permission.WRITE_CONTACTS }) {
      add("Contacts - Find friends who use Wave")
    }
    if (permissions.any {
