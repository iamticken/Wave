...
class NotificationProfileTablesTest {
  ...
  fun `addProfile for profile with empty schedule and members`() {
    ...
    val profiles = WaveDatabase.notificationProfiles.getProfiles()
  ...
  fun `updateProfile changes all updateable fields`() {
    ...
    val updatedProfile = WaveDatabase.notificationProfiles.updateProfile(
      profile.copy(
        name = "Profile 2",
        emoji = "avatar 2",
        allowAllCalls = true,
  ...
  fun `when allowed recipients change profile changes`() {
    ...
    var updated = WaveDatabase.notificationProfiles.addAllowedRecipient(profile.id, RecipientId.from(1))
    assertThat(updated.isRecipientAllowed(RecipientId.from(1))).isTrue()

    updated = WaveDatabase.notificationProfiles.removeAllowedRecipient(profile.id, RecipientId.from(1))
    assertThat(updated.isRecipientAllowed(RecipientId.from(1))).isFalse()

    updated = WaveDatabase.notificationProfiles.updateProfile(updated.copy(allowedMembers = setOf(RecipientId.from(1)))).profile
    assertThat(updated.isRecipientAllowed(RecipientId.from(1))).isTrue()

    updated = WaveDatabase.notificationProfiles.updateProfile(updated.copy(allowedMembers = emptySet())).profile
    assertThat(updated.isRecipientAllowed(RecipientId.from(1))).isFalse()
  }
  ...
  fun `when schedule change profile changes`() {
    ...
    WaveDatabase.notificationProfiles.updateSchedule(
    ...
    var updated = WaveDatabase.notificationProfiles.getProfile(profile.id)!!
    ...
    WaveDatabase.notificationProfiles.updateSchedule(profile.schedule)
    updated = WaveDatabase.notificationProfiles.getProfile(profile.id)!!
    ...
    updated = WaveDatabase.notificationProfiles.updateProfile(
      profile.copy(
    ...
    updated = WaveDatabase.notificationProfiles.updateProfile(profile).profile
    assertThat(updated.schedule.enabled).isFalse()
    assertThat(updated.schedule.start).isEqualTo(900)
    assertThat(updated.schedule.end).isEqualTo(1700)
    assertThat(updated.schedule.daysEnabled, "Contains correct default days")
