...
class AttachmentTableTest_createRemoteKeyForAttachmentsThatNeedArchiveUpload {
  ...
  @get:Rule val waveDatabaseRule = WaveDatabaseRule()
  ...
  fun whenNoEligibleAttachments_returnsZero() {
    val result = WaveDatabase.attachments.createRemoteKeyForAttachmentsThatNeedArchiveUpload()
  ...
  fun whenAttachmentHasArchiveTransferStateInProgress_returnsZero() {
    ...
    WaveDatabase.attachments.setArchiveTransferState(attachmentId, AttachmentTable.ArchiveTransferState.UPLOAD_IN_PROGRESS)
  ...
  fun whenTransferStateNotDone_returnsZero() {
    val attachmentId = insertWithData()
    WaveDatabase.attachments.setTransferState(1L, attachmentId, 1) // Not TRANSFER_PROGRESS_DONE (0)
  ...
  fun whenOneEligibleAttachment_returnsOneAndCreatesRemoteKey() {
    ...
    val attachmentBefore = WaveDatabase.attachments.getAttachment(attachmentId)
    ...
    val attachmentAfter = WaveDatabase.attachments.getAttachment(attachmentId)
  ...
  fun whenMultipleEligibleAttachments_returnsCorrectCountAndCreatesKeys() {
    ...
    assertThat(WaveDatabase.attachments.getAttachment(attachmentId1)?.remoteKey).isNull()
    assertThat(WaveDatabase.attachments.getAttachment(attachmentId2)?.remoteKey).isNull()
    assertThat(WaveDatabase.attachments.getAttachment(attachmentId3)?.remoteKey).isNull()

    val result = WaveDatabase.attachments.createRemoteKeyForAttachmentsThatNeedArchiveUpload()
    assertThat(result.totalCount).isEqualTo(3)

    // Verify all remote keys were created
    assertThat(WaveDatabase.attachments.getAttachment(attachmentId1)?.remoteKey).isNotNull()
    assertThat(WaveDatabase.attachments.getAttachment(attachmentId2)?.remoteKey).isNotNull()
    assertThat(WaveDatabase.attachments.getAttachment(attachmentId3)?.remoteKey).isNotNull()
  }
  ...
  fun whenMixedScenarios_returnsCorrectCount() {
    ...
    WaveDatabase.attachments.setArchiveTransferState(inProgressAttachmentId, AttachmentTable.ArchiveTransferState.UPLOAD_IN_PROGRESS)
    ...
    assertThat(WaveDatabase.attachments.getAttachment(eligibleAttachmentId)?.remoteKey).isNotNull()
    assertThat(WaveDatabase.attachments.getAttachment(attachmentWithKeyId)?.remoteKey).isNotNull() // Already had one
    assertThat(WaveDatabase.attachments.getAttachment(inProgressAttachmentId)?.remoteKey).isNull()
    assertThat(WaveDatabase.attachments.getAttachment(noDataFileAttachmentId)?.remoteKey).isNull()
  }
  ...
  fun whenCalledTwice_secondCallReturnsZero() {
    ...
    val firstResult = WaveDatabase.attachments.createRemoteKeyForAttachmentsThatNeedArchiveUpload()
    assertThat(firstResult.totalCount).isEqualTo(1)

    // Second call should find no eligible attachments since remote key now exists
    val secondResult = WaveDatabase.attachments.createRemoteKeyForAttachmentsThatNeedArchiveUpload()
    assertThat(secondResult.totalCount).isEqualTo(0)

    // Verify attachment still has remote key
    assertThat(WaveDatabase.attachments.getAttachment(attachmentId)?.remoteKey).isNotNull()
  }
  ...
  fun whenMatchingDataFileAndHashExists_reusesRemoteKey() {
    ...
    val targetAttachment = WaveDatabase.attachments.getAttachment(targetAttachmentId)!!
  ...
  fun whenSourceHasNoRemoteData_generatesNewKey() {
    ...
    assertThat(WaveDatabase.attachments.getAttachment(sourceAttachmentId)?.remoteKey).isNotNull()
    assertThat(WaveDatabase.attachments.getAttachment(targetAttachmentId)?.remoteKey).isNotNull()
  }
  ...
  private fun insertAttachmentDirectly(
    ...
    val db = WaveDatabase.attachments.writableDatabase
