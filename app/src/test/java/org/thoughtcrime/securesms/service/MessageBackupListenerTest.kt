...
class MessageBackupListenerTest {
  ...
  fun testGetNextScheduledExecutionTime() {
    ...
    every { WaveStore.backup.nextBackupTime } returns nextTime
    ...
    every { WaveStore.backup.nextBackupTime } returns nextTime
    every { WaveStore.settings.waveBackupHour } returns 2
    every { WaveStore.settings.waveBackupMinute } returns 0
    every { WaveStore.backup.nextBackupTime = any() } just runs
  ...
  fun testSetNextBackupTimeToIntervalFromNow() {
    ...
    every { WaveStore.settings.waveBackupHour } returns 2
    every { WaveStore.settings.waveBackupMinute } returns 1
    every { WaveStore.backup.nextBackupTime = any() } just runs
