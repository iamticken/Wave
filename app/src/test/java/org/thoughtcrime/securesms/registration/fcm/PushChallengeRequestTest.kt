...
class PushChallengeRequestTest {
  ...
  fun pushChallengeBlocking_returns_absent_if_times_out() {
    val wave = mockk<WaveServiceAccountManager>(relaxUnitFun = true)

    val challenge = PushChallengeRequest.getPushChallengeBlocking(wave, "session ID", Optional.of("token"), 50L)
  ...
  fun pushChallengeBlocking_waits_for_specified_period() {
    ...
    PushChallengeRequest.getPushChallengeBlocking(wave, "session ID", Optional.of("token"), 250L)
  ...
  fun pushChallengeBlocking_completes_fast_if_posted_to_event_bus() {
    val wave = mockk<WaveServiceAccountManager> {
      every {
    ...
    val startTime = System.currentTimeMillis()
    val challenge = PushChallengeRequest.getPushChallengeBlocking(wave, "session ID", Optional.of("token"), 500L)
    val duration = System.currentTimeMillis() - startTime

    assertThat(duration).isLessThan(500L)
    verify { wave.requestRegistrationPushChallenge("session ID", "token") }
    confirmVerified(wave)
  ...
  fun pushChallengeBlocking_returns_fast_if_no_fcm_token_supplied() {
    ...
    val startTime = System.currentTimeMillis()
    PushChallengeRequest.getPushChallengeBlocking(wave, "session ID", Optional.empty(), 500L)
    val duration = System.currentTimeMillis() - startTime
  ...
  fun pushChallengeBlocking_returns_absent_if_no_fcm_token_supplied() {
    ...
    val challenge = PushChallengeRequest.getPushChallengeBlocking(wave, "session ID", Optional.empty(), 500L)

    verify { wave wasNot called }
    assertThat(challenge).isAbsent()
  }
