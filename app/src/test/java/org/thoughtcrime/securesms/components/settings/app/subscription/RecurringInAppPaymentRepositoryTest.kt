...
class RecurringInAppPaymentRepositoryTest {
  ...
  fun setUp() {
    ...
    every { WaveStore.Companion.inAppPayments } returns mockk {
      every { WaveStore.Companion.inAppPayments.getRecurringDonationCurrency() } returns Currency.getInstance("USD")
      every { WaveStore.Companion.inAppPayments.updateLocalStateForManualCancellation(any()) } returns Unit
      every { WaveStore.Companion.inAppPayments.updateLocalStateForLocalSubscribe(any()) } returns Unit
    }

    every { WaveDatabase.Companion.recipients } returns mockk {
      every { WaveDatabase.Companion.recipients.markNeedsSync(any<RecipientId>()) } returns Unit
    }
