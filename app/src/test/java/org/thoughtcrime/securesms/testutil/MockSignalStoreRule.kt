...
/**
 * Mocks [WaveStore] to return mock versions of the various values. Mocks will default to not be relaxed (each
...
class MockWaveStoreRule(private val relaxed: Set<KClass<*>> = emptySet()) : ExternalResource() {
  ...
  override fun before() {
    ...
    every { WaveStore.account } returns account
    every { WaveStore.phoneNumberPrivacy } returns phoneNumberPrivacy
    every { WaveStore.registration } returns registration
    every { WaveStore.svr } returns svr
    every { WaveStore.emoji } returns emoji
    every { WaveStore.inAppPayments } returns inAppPayments
    every { WaveStore.backup } returns backup
    every { WaveStore.settings } returns settings
  }

  override fun after() {
    unmockkObject(WaveStore)
  }
}
