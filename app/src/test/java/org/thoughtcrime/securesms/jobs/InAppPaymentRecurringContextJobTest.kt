...
class InAppPaymentRecurringContextJobTest {
  ...
  fun setUp() {
    ...
    every { mockWaveStore.account.isRegistered } returns true
    every { mockWaveStore.account.isLinkedDevice } returns false
    every { mockWaveStore.inAppPayments.setLastEndOfPeriod(any()) } returns Unit
  ...
  fun `Given a CREATED IAP, when I onAdded, then I expect PENDING`() {
    ...
    job.onAdded()
    val updatedIap = WaveDatabase.inAppPayments.getById(iap.id)
  ...
  fun `Given user has donor entitlement already, when I run, then I do not expect receipt request`() {
    ...
    every { AppDependencies.waveServiceAccountManager.whoAmI } returns mockk {
