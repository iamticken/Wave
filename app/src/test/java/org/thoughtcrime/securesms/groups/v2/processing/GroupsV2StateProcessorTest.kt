...
import org.wave.core.util.Hex.fromStringCondensed
...
import org.thoughtcrime.securesms.logging.CustomWaveProtocolLogger
...
class GroupsV2StateProcessorTest {
  ...
  fun setUp() {
    ...
    every { WaveStore.internal.gv2IgnoreP2PChanges } returns false

    Log.initialize(SystemOutLogger())
    WaveProtocolLoggerProvider.setProvider(CustomWaveProtocolLogger())
    ...
    every { AppDependencies.waveServiceAccountManager.getGroupsV2Api() } returns groupsV2API
    ...
    every { WaveDatabase.groups } returns groupTable
    every { WaveDatabase.recipients } returns recipientTable
    every { WaveDatabase.threads } returns threadTable
  ...
  fun updateFromServerIfUnableToApplyP2PChange() {
    given {
      ...
      serverState(
        revision = 2,
        title = "Breaking Wave for Science",
        members = selfAndOthers
      )
      changeSet {
        changeLog(2) {
    ...
    val signedChange = DecryptedGroupChange(
      revision = 2,
      newTitle = DecryptedString("Breaking Wave for Science")
    )
  ...
  fun `when freshly added to a group, with additional group changes after being added, then only update from server at the revision we were added, and then schedule pulling additional changes later`() {
    given {
      changeSet {
        ...
        changeLog(3) {
          change {
            setNewTitle("Breaking Wave for Science")
          }
        }
      }
      apiCallParameters(2, true)
    ...
    assertThat(result.latestServer)
      ...
      .given {
        ...
        assertThat(it.title, "title matches that as it was in revision added").isEqualTo("Baking Wave for Science")
      }
   ...
   */
  @Test
  fun ignore403sWithoutWaveTimestampHeader() {
    given {
      localState(
        active = false,
        revision = 5,
