...
import org.wave.spinner.Plugin
import org.wave.spinner.PluginResult
...
class StorageServicePlugin : Plugin {
  ...
  override fun get(parameters: Map<String, List<String>>): PluginResult {
    ...
    val waveStorageRecords = when (val result = repository.readStorageRecords(storageServiceKey, manifest.recordIkm, manifest.storageIds)) {
    ...
    for (record in waveStorageRecords) {
