...
import org.wave.spinner.Spinner.DatabaseConfig
...
import org.thoughtcrime.securesms.database.WaveStoreTransformer
  ...
  override fun onCreate() {
    ...
    Spinner.init(
      ...
      mapOf(
        ...
        "Profile Name" to { (if (WaveStore.account.isRegistered) Recipient.self().profileName.toString() else "none") },
        "E164" to { WaveStore.account.e164 ?: "none" },
        "ACI" to { WaveStore.account.aci?.toString() ?: "none" },
        "PNI" to { WaveStore.account.pni?.toString() ?: "none" },
        Spinner.KEY_ENVIRONMENT to { BuildConfig.FLAVOR_environment.uppercase(Locale.US) }
      ),
      linkedMapOf(
        "wave" to DatabaseConfig(
          db = { WaveDatabase.rawDatabase },
        ...
        "keyvalue" to DatabaseConfig(db = { KeyValueDatabase.getInstance(this).sqlCipherDatabase }, columnTransformers = listOf(WaveStoreTransformer)),
    ...
    DatabaseMonitor.initialize(object : QueryMonitor {
      ...
      override fun onQuery(distinct: Boolean, table: String, projection: Array<String>?, selection: String?, args: Array<Any>?, groupBy: String?, having: String?, orderBy: String?, limit: String?) {
        Spinner.onQuery("wave", distinct, table, projection, selection, args, groupBy, having, orderBy, limit)
      }

      override fun onDelete(table: String, selection: String?, args: Array<Any>?) {
        Spinner.onDelete("wave", table, selection, args)
      }

      override fun onUpdate(table: String, values: ContentValues, selection: String?, args: Array<Any>?) {
        Spinner.onUpdate("wave", table, values, selection, args)
      }
    })
  }
}
