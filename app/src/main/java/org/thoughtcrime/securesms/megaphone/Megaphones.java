...
import org.wave.core.util.MapUtil;
import org.wave.core.util.SetUtil;
import org.wave.core.util.TranslationDetection;
...
import org.thoughtcrime.securesms.lock.WavePinReminderDialog;
import org.thoughtcrime.securesms.lock.WavePinReminders;
...
public final class Megaphones {
  ...
  private static Map<Event, MegaphoneSchedule> buildDisplayOrder(@NonNull Context context, @NonNull Map<Event, MegaphoneRecord> records) {
    return new LinkedHashMap<>() {{
      ...
      put(Event.CLIENT_DEPRECATED, WaveStore.misc().isClientDeprecated() ? ALWAYS : NEVER);
      ...
      put(Event.PIN_REMINDER, new WavePinReminderSchedule());
  ...
  private static boolean shouldShowLinkedDeviceInactiveMegaphone() {
    ...
    LeastActiveLinkedDevice device = WaveStore.misc().getLeastActiveLinkedDevice();
    if (device == null) {
      return false;
    }
  ...
  private static @Nullable Megaphone forRecord(@NonNull Context context, @NonNull MegaphoneRecord record) {
    switch (record.getEvent()) {
      ...
      case TURN_ON_SIGNAL_BACKUPS:
        return buildTurnOnWaveBackupsMegaphone();
  ...
  private static @NonNull Megaphone buildPinsForAllMegaphone(@NonNull MegaphoneRecord record) {
    if (PinsForAllSchedule.shouldDisplayFullScreen(record.getFirstVisible(), System.currentTimeMillis())) {
      ...
      return new Megaphone.Builder(Event.PINS_FOR_ALL, Megaphone.Style.BASIC)
          ...
          .setBody(R.string.KbsMegaphone__pins_keep_information_thats_stored_with_wave_encrytped)
  ...
  private static @NonNull Megaphone buildPinReminderMegaphone(@NonNull Context context) {
    return new Megaphone.Builder(Event.PIN_REMINDER, Megaphone.Style.BASIC)
        .setTitle(R.string.Megaphones_verify_your_wave_pin)
        ...
        .setActionButton(R.string.Megaphones_verify_pin, (megaphone, controller) -> {
          WavePinReminderDialog.show(controller.getMegaphoneActivity(), controller::onMegaphoneNavigationRequested, new WavePinReminderDialog.Callback() {
            @Override
            public void onReminderDismissed(boolean includedFailure) {
              Log.i(TAG, "[PinReminder] onReminderDismissed(" + includedFailure + ")");

              WaveStore.pin().onEntrySkip(includedFailure);
              controller.onMegaphoneSnooze(Event.PIN_REMINDER);
              controller.onMegaphoneToastRequested(controller.getMegaphoneActivity().getString(WavePinReminders.getSkipReminderString(WaveStore.pin().getCurrentInterval())));
            }
            ...
            public void onReminderCompleted(@NonNull String pin, boolean includedFailure) {
              ...
              if (includedFailure) {
                WaveStore.pin().onEntrySuccessWithWrongGuess(pin);
              } else {
                WaveStore.pin().onEntrySuccess(pin);
              }

              controller.onMegaphoneSnooze(Event.PIN_REMINDER);
              controller.onMegaphoneToastRequested(controller.getMegaphoneActivity().getString(WavePinReminders.getReminderString(WaveStore.pin().getCurrentInterval())));
            }
          });
        })
        .build();
  ...
  private static @NonNull Megaphone buildNewLinkedDeviceMegaphone(@NonNull Context context) {
    String createdAt = DateUtils.getDateTimeString(context, Locale.getDefault(), WaveStore.misc().getNewLinkedDeviceCreatedTime());
    return new Megaphone.Builder(Event.NEW_LINKED_DEVICE, Megaphone.Style.BASIC)
        ...
        .setActionButton(R.string.NewLinkedDeviceMegaphone__ok, (megaphone, listener) -> {
          WaveStore.misc().setNewLinkedDeviceId(0);
          WaveStore.misc().setNewLinkedDeviceCreatedTime(0);
  ...
  private static @NonNull Megaphone buildTurnOffCircumventionMegaphone(@NonNull Context context) {
    return new Megaphone.Builder(Event.TURN_OFF_CENSORSHIP_CIRCUMVENTION, Megaphone.Style.BASIC)
        ...
        .setBody(R.string.CensorshipCircumventionMegaphone_you_can_now_connect_to_the_wave_service)
        .setActionButton(R.string.CensorshipCircumventionMegaphone_turn_off, (megaphone, listener) -> {
          WaveStore.settings().setCensorshipCircumventionEnabled(false);
  ...
  public static @NonNull Megaphone buildTurnOnWaveBackupsMegaphone() {
    return new Megaphone.Builder(Event.TURN_ON_SIGNAL_BACKUPS, Megaphone.Style.BASIC)
        ...
        .setTitle(R.string.TurnOnWaveBackups__title_beta)
        .setBody(R.string.TurnOnWaveBackups__body)
        .setActionButton(R.string.TurnOnWaveBackups__turn_on, (megaphone, controller) -> {
        ...
        .setSecondaryButton(R.string.TurnOnWaveBackups__not_now, (megaphone, controller) -> {
          controller.onMegaphoneToastRequested(controller.getMegaphoneActivity().getString(R.string.TurnOnWaveBackups__toast_not_now));
          controller.onMegaphoneSnooze(Event.TURN_ON_SIGNAL_BACKUPS);
        })
        .build();
  }
  ...
  public static @NonNull Megaphone buildVerifyBackupKeyMegaphone() {
    Megaphone.Builder builder = new Megaphone.Builder(Event.VERIFY_BACKUP_KEY, Megaphone.Style.BASIC)
        .setImage(R.drawable.image_wave_backups_key)
    ...
    if (!WaveStore.backup().getHasSnoozedVerified()) {
      builder.setSecondaryButton(R.string.VerifyBackupKey__not_now, (megaphone, controller) -> {
        WaveStore.backup().setHasSnoozedVerified(true);
  ...
  private static boolean shouldShowOnboardingMegaphone(@NonNull Context context) {
    return WaveStore.account().isPrimaryDevice() && WaveStore.onboarding().hasOnboarding(context);
  }

  private static boolean shouldShowNewLinkedDeviceMegaphone() {
    return WaveStore.account().isPrimaryDevice() && WaveStore.misc().getNewLinkedDeviceId() > 0 && !NotificationChannels.getInstance().areNotificationsEnabled();
  }

  private static boolean shouldShowTurnOffCircumventionMegaphone() {
    return AppDependencies.getWaveServiceNetworkAccess().isCensored() &&
           WaveStore.misc().isServiceReachableWithoutCircumvention();
  }

  private static boolean shouldShowNotificationsMegaphone(@NonNull Context context) {
    boolean shouldShow = !WaveStore.settings().isMessageNotificationsEnabled() ||
  ...
  private static boolean shouldShowAddAProfilePhotoMegaphone(@NonNull Context context) {
    if (WaveStore.misc().getHasEverHadAnAvatar()) {
      return false;
    }

    boolean hasAnAvatar = AvatarHelper.hasAvatar(context, Recipient.self().getId());
    if (hasAnAvatar) {
      WaveStore.misc().setHasEverHadAnAvatar(true);
      return false;
    }
  ...
  private static boolean shouldShowSetUpYourUsernameMegaphone(@NonNull Map<Event, MegaphoneRecord> records) {
    boolean                        hasUsername                    = WaveStore.account().isRegistered() && WaveStore.account().getUsername() != null;
    boolean                        hasCompleted                   = MapUtil.mapOrDefault(records, Event.SET_UP_YOUR_USERNAME, MegaphoneRecord::isFinished, false);
    long                           phoneNumberDiscoveryDisabledAt = WaveStore.phoneNumberPrivacy().getPhoneNumberDiscoverabilityModeTimestamp();
    PhoneNumberDiscoverabilityMode listingMode                    = WaveStore.phoneNumberPrivacy().getPhoneNumberDiscoverabilityMode();

    return WaveStore.account().isPrimaryDevice() &&
           !hasUsername &&
  ...
  private static boolean shouldShowPnpLaunchMegaphone() {
    return WaveStore.account().isPrimaryDevice() && TextUtils.isEmpty(WaveStore.account().getUsername()) && !WaveStore.uiHints().hasCompletedUsernameOnboarding();
  }
  ...
  private static boolean shouldShowTurnOnBackupsMegaphone(@NonNull Context context) {
    ...
    if (WaveStore.backup().getLatestBackupTier() != null) {
      return false;
    }

    if (!WaveStore.account().isRegistered() || TextSecurePreferences.isUnauthorizedReceived(context) || WaveStore.account().isLinkedDevice()) {
      return false;
    }
  ...
  private static boolean shouldShowBackupSchedulePermissionMegaphone(@NonNull Context context) {
    boolean backupsEnabled = WaveStore.settings().isBackupEnabled() || WaveStore.backup().getAreBackupsEnabled();
    return WaveStore.account().isPrimaryDevice() && Build.VERSION.SDK_INT >= 31 && backupsEnabled && !ServiceUtil.getAlarmManager(context).canScheduleExactAlarms();
  }
  ...
  public enum Event {
    ...
    TURN_ON_SIGNAL_BACKUPS("turn_on_wave_backups"),
