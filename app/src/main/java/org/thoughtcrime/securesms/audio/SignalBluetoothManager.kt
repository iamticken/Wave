 ...
 * reports that to the [org.thoughtcrime.securesms.webrtc.audio.WaveAudioManager], and then handles connecting/disconnecting
 * to the device if requested by [org.thoughtcrime.securesms.webrtc.audio.WaveAudioManager].
 */
@SuppressLint("MissingPermission") // targetSdkVersion is still 30 (https://issuetracker.google.com/issues/201454155)
class WaveBluetoothManager(
  private val context: Context,
  private val audioDeviceUpdatedListener: AudioDeviceUpdatedListener,
  private val handler: WaveAudioHandler
) {
  ...
  companion object {
    private val TAG = Log.tag(WaveBluetoothManager::class.java)
