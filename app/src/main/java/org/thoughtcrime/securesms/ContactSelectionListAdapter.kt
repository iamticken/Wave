...
class ContactSelectionListAdapter(
  ...
  init {
    ...
    registerFactory(InviteToWaveModel::class.java, LayoutFactory({ InviteToWaveViewHolder(it, onClickCallbacks::onInviteToWaveClicked) }, R.layout.contact_selection_invite_action_item))
  ...
  class InviteToWaveModel : MappingModel<InviteToWaveModel> {
    override fun areItemsTheSame(newItem: InviteToWaveModel): Boolean = true
    override fun areContentsTheSame(newItem: InviteToWaveModel): Boolean = true
  }
  ...
  private class InviteToWaveViewHolder(itemView: View, onClickListener: () -> Unit) : MappingViewHolder<InviteToWaveModel>(itemView) {
    init {
      itemView.setOnClickListener { onClickListener() }
    }

    override fun bind(model: InviteToWaveModel) = Unit
  }
  ...
  class ArbitraryRepository : org.thoughtcrime.securesms.contacts.paged.ArbitraryRepository {
    ...
    enum class ArbitraryRow(val code: String) {
      NEW_GROUP("new-group"),
      INVITE_TO_SIGNAL("invite-to-wave"),
    ...
    override fun getMappingModel(arbitrary: ContactSearchData.Arbitrary): MappingModel<*> {
      return when (ArbitraryRow.fromCode(arbitrary.type)) {
        ...
        ArbitraryRow.INVITE_TO_SIGNAL -> InviteToWaveModel()
