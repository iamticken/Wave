...
class DeleteAccountRepository {
  ...
  void deleteAccount(@NonNull Consumer<DeleteAccountEvent> onDeleteAccountEvent) {
    WaveExecutors.BOUNDED.execute(() -> {
      ...
      int groupsLeft = 0;
      try (GroupTable.Reader groups = WaveDatabase.groups().getGroups()) {
      ...
      try {
        NetworkResultUtil.toBasicLegacy(WaveNetwork.account().deleteAccount());
      } catch (IOException e) {
        if (e instanceof NonSuccessfulResponseCodeException && ((NonSuccessfulResponseCodeException) e).code == 4401) {
          ...
          Log.w(TAG, "deleteAccount: failed to delete account from wave service, bail", e);
