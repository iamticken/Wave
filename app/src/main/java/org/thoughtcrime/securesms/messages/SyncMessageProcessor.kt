...
import org.wave.ringrtc.CallException
...
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.expireTimerDuration
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.groupId
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.groupMasterKey
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.hasGroupContext
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.hasRemoteDelete
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.isEmptyGroupV2Message
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.isEndSession
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.isExpirationUpdate
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.isGroupV2Update
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.isMediaMessage
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.isUnidentified
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.serviceIdsToUnidentifiedStatus
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.toMobileCoinMoney
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.toPointer
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.toPointersWithinLimit
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.toWaveServiceAttachmentPointer
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.type
...
import org.thoughtcrime.securesms.service.webrtc.links.WaveCallLinkState
...
import org.whispersystems.waveservice.internal.push.SyncMessage.Blocked
...
import org.whispersystems.waveservice.internal.push.SyncMessage.Configuration
import org.whispersystems.waveservice.internal.push.SyncMessage.FetchLatest
import org.whispersystems.waveservice.internal.push.SyncMessage.MessageRequestResponse
import org.whispersystems.waveservice.internal.push.SyncMessage.Read
import org.whispersystems.waveservice.internal.push.SyncMessage.Request
import org.whispersystems.waveservice.internal.push.SyncMessage.Sent
import org.whispersystems.waveservice.internal.push.SyncMessage.StickerPackOperation
import org.whispersystems.waveservice.internal.push.SyncMessage.ViewOnceOpen
...
import org.whispersystems.waveservice.internal.util.Util as Utils
...
object SyncMessageProcessor {
    ...
    try {
      ...
      when {
        ...
        dataMessage.isGroupV2Update -> {
          ...
          threadId = WaveDatabase.threads.getOrCreateThreadIdFor(getSyncMessageDestination(sent))
        }
      ...
      if (groupId != null && WaveDatabase.groups.isUnknownGroup(groupId)) {
      ...
      if (dataMessage.profileKey.isNotEmpty()) {
        ...
        if (!recipient.isSystemContact && !recipient.isProfileSharing) {
          WaveDatabase.recipients.setProfileSharing(recipient.id, true)
        }
      }

      if (threadId != -1L) {
        WaveDatabase.threads.setRead(threadId)
        AppDependencies.messageNotifier.updateNotification(context)
      }

      if (WaveStore.rateLimit.needsRecaptcha()) {
  ...
  private fun handlePniIdentityKeys(envelope: Envelope, sent: Sent) {
    for (status in sent.unidentifiedStatus) {
      ...
      val address = WaveProtocolAddress(pni.toString(), WaveServiceAddress.DEFAULT_DEVICE_ID)
  ...
  private fun getSyncMessageDestination(message: Sent): Recipient {
    return if (message.message.hasGroupContext) {
      ...
      Recipient.externalPush(WaveServiceAddress(ServiceId.parseOrThrow(message.destinationServiceId, message.destinationServiceIdBinary), message.destinationE164))
    }
  }
  ...
  private fun handleSynchronizeSentEditMessage(
    ...
    val targetMessage: MessageRecord? = WaveDatabase.messages.getMessageFor(targetSentTimestamp, senderRecipient.id)
  ...
  private fun handleSynchronizeSentEditTextMessage(
    ...
    val threadId = WaveDatabase.threads.getOrCreateThreadIdFor(toRecipient)
    ...
    if (isGroup) {
      ...
      messageId = WaveDatabase.messages.insertMessageOutbox(outgoingMessage, threadId, false, GroupReceiptTable.STATUS_UNKNOWN, null).messageId
      ...
      messageId = WaveDatabase.messages.insertMessageOutbox(outgoingTextMessage, threadId, false, null).messageId
      WaveDatabase.messages.markUnidentified(messageId, sent.isUnidentified(toRecipient.serviceId.orNull()))
    }

    WaveDatabase.messages.markAsSent(messageId, true)
    if (targetMessage.expireStarted > 0) {
      WaveDatabase.messages.markExpireStarted(messageId, targetMessage.expireStarted)
    ...
    if (toRecipient.isSelf) {
      WaveDatabase.messages.incrementDeliveryReceiptCount(sent.timestamp!!, toRecipient.id, System.currentTimeMillis())
      WaveDatabase.messages.incrementReadReceiptCount(sent.timestamp!!, toRecipient.id, System.currentTimeMillis())
    }
  }
  ...
  private fun handleSynchronizeSentEditMediaMessage(
    ...
    val messageId: Long = WaveDatabase.messages.insertMessageOutbox(mediaMessage, threadId, false, GroupReceiptTable.STATUS_UNKNOWN, null).messageId
    ...
    val attachments: List<DatabaseAttachment> = WaveDatabase.attachments.getAttachmentsForMessage(messageId)
  ...
  private fun handleSynchronizeSentStoryMessage(envelope: Envelope, sent: Sent) {
    ...
    if (sent.isRecipientUpdate == true) {
      ...
      WaveDatabase.storySends.applySentStoryManifest(manifest, sent.timestamp!!)
      return
    }
    ...
    for (distributionId in distributionIds) {
      val distributionRecipientId = WaveDatabase.distributionLists.getOrCreateByDistributionId(distributionId, manifest)
    ...
    if (groupId != null) {
      val groupRecipient: Optional<RecipientId> = WaveDatabase.recipients.getByGroupId(groupId)
  ...
  private fun insertSentStoryMessage(
    ...
    if (WaveDatabase.messages.isOutgoingStoryAlreadyInDatabase(recipient.id, sentAtTimestamp)) {
    ...
    val messageId: Long = WaveDatabase.messages.insertMessageOutbox(mediaMessage, threadId, false, GroupReceiptTable.STATUS_UNDELIVERED, null).messageId
    ...
    if (groupId != null) {
      ...
      WaveDatabase.messages.markUnidentified(messageId, sent.isUnidentified(recipient.serviceId.orNull()))
    }

    WaveDatabase.messages.markAsSent(messageId, true)

    val allAttachments = WaveDatabase.attachments.getAttachmentsForMessage(messageId)
    val attachments: List<DatabaseAttachment> = allAttachments.filterNot { it.isSticker }

    if (recipient.isSelf) {
      WaveDatabase.messages.incrementDeliveryReceiptCount(sent.timestamp!!, recipient.id, System.currentTimeMillis())
      WaveDatabase.messages.incrementReadReceiptCount(sent.timestamp!!, recipient.id, System.currentTimeMillis())
    }
  ...
  private fun handleGroupRecipientUpdate(sent: Sent, envelopeTimestamp: Long) {
    ...
    val record = WaveDatabase.messages.getMessageFor(sent.timestamp!!, Recipient.self().id)
    if (record == null) {
  ...
  private fun updateGroupReceiptStatus(sent: Sent, messageId: Long, groupString: GroupId) {
    ...
    val members: List<RecipientId> = WaveDatabase.groups.getGroupMembers(groupString, GroupTable.MemberSet.FULL_MEMBERS_EXCLUDING_SELF).map { it.id }
    val localReceipts: Map<RecipientId, Int> = WaveDatabase.groupReceipts.getGroupReceiptInfo(messageId).associate { it.recipientId to it.status }
    ...
    for (messageRecipientId in messageRecipientIds.keys) {
      if ((localReceipts[messageRecipientId] ?: GroupReceiptTable.STATUS_UNKNOWN) < GroupReceiptTable.STATUS_UNDELIVERED) {
        WaveDatabase.groupReceipts.update(messageRecipientId, messageId, GroupReceiptTable.STATUS_UNDELIVERED, sent.timestamp!!)
      } else if (!localReceipts.containsKey(messageRecipientId)) {
        WaveDatabase.groupReceipts.insert(listOf(messageRecipientId), messageId, GroupReceiptTable.STATUS_UNDELIVERED, sent.timestamp!!)
      }
    }

    val unidentifiedStatus = members.map { Pair(it, messageRecipientIds[it] ?: false) }

    WaveDatabase.groupReceipts.setUnidentified(unidentifiedStatus, messageId)
  }

  private fun updateGroupReceiptStatusForDistributionList(sent: Sent, messageId: Long, distributionListId: DistributionListId) {
    val messageRecipientIds: Map<RecipientId, Boolean> = sent.serviceIdsToUnidentifiedStatus.mapKeys { RecipientId.from(it.key) }
    val members: List<RecipientId> = WaveDatabase.distributionLists.getMembers(distributionListId)
    val localReceipts: Map<RecipientId, Int> = WaveDatabase.groupReceipts.getGroupReceiptInfo(messageId).associate { it.recipientId to it.status }
  ...
  private fun handleSynchronizeSentEndSessionMessage(context: Context, sent: Sent, envelopeTimestamp: Long): Long {
    ...
    val threadId: Long = WaveDatabase.threads.getOrCreateThreadIdFor(recipient)

    if (!recipient.isGroup) {
      AppDependencies.protocolStore.aci().deleteAllSessions(recipient.requireServiceId().toString())
      SecurityEvent.broadcastSecurityUpdateEvent(context)
      val messageId = WaveDatabase.messages.insertMessageOutbox(
  ...
  private fun handleSynchronizeSentGv2Update(context: Context, envelope: Envelope, sent: Sent) {
    ...
    if (MessageContentProcessor.updateGv2GroupFromServerOrP2PChange(context, envelope.timestamp!!, dataMessage.groupV2!!, WaveDatabase.groups.getGroup(groupId)) == null) {
  ...
  private fun handleSynchronizeSentExpirationUpdate(sent: Sent, sideEffect: Boolean = false): Long {
    ...
    if (sent.message?.expireTimerVersion == null) {
      ...
      WaveDatabase.recipients.setExpireMessagesWithoutIncrementingVersion(recipient.id, sent.message!!.expireTimerDuration.inWholeSeconds.toInt())
      val messageId: Long = WaveDatabase.messages.insertMessageOutbox(expirationUpdateMessage, threadId, false, null).messageId
      WaveDatabase.messages.markAsSent(messageId, true)
    } else if (sent.message!!.expireTimerVersion!! >= recipient.expireTimerVersion) {
      WaveDatabase.recipients.setExpireMessages(recipient.id, sent.message!!.expireTimerDuration.inWholeSeconds.toInt(), sent.message!!.expireTimerVersion!!)
  ...
  private fun handleSynchronizeSentStoryReply(sent: Sent, envelopeTimestamp: Long): Long {
    ...
    try {
      ...
      val storyMessageId: Long = WaveDatabase.messages.getStoryId(storyAuthorRecipient, sentTimestamp).id
      val story: MmsMessageRecord = WaveDatabase.messages.getMessageRecord(storyMessageId) as MmsMessageRecord
      val threadRecipientId: RecipientId? = WaveDatabase.threads.getRecipientForThreadId(story.threadId)?.id
      val groupStory: Boolean = threadRecipientId != null && (WaveDatabase.groups.getGroup(threadRecipientId).orNull()?.isActive ?: false)
      ...
      if (dataMessage.expireTimerDuration > Duration.ZERO) {
        WaveDatabase.messages.markExpireStarted(messageId, sent.expirationStartTimestamp ?: 0)
  ...
  private fun handleSynchronizeReadMessage(
    ...
    val unhandled: Collection<MessageTable.SyncMessageId> = WaveDatabase.messages.setTimestampReadFromSyncMessage(readMessages, envelopeTimestamp, threadToLatestRead)
    val markedMessages: List<MarkedMessageInfo> = WaveDatabase.threads.setReadSince(threadToLatestRead)

    if (Util.hasItems(markedMessages)) {
      log("Updating past WaveDatabase.messages: " + markedMessages.size)
  ...
  private fun handleSynchronizeViewedMessage(context: Context, viewedMessages: List<SyncMessage.Viewed>, envelopeTimestamp: Long) {
    ...
    val records = viewedMessages
      .mapNotNull { message ->
        ...
        if (message.timestamp != null) {
          WaveDatabase.messages.getMessageFor(message.timestamp!!, author)
        } else {
    ...
    WaveDatabase.messages.setIncomingMessagesViewed(toMarkViewed)
    WaveDatabase.messages.setOutgoingGiftsRevealed(toMarkViewed)
  ...
  private fun handleSynchronizeViewOnceOpenMessage(context: Context, openMessage: ViewOnceOpen, envelopeTimestamp: Long, earlyMessageCacheEntry: EarlyMessageCacheEntry?) {
    ...
    val record: MessageRecord? = WaveDatabase.messages.getMessageFor(timestamp, author)

    if (record != null) {
      WaveDatabase.attachments.deleteAttachmentFilesForViewOnceMessage(record.id)
    } else {
    ...
    for (operation in stickerPackOperations) {
      if (operation.packId != null && operation.packKey != null && operation.type != null) {
        ...
        when (operation.type!!) {
          ...
          StickerPackOperation.Type.REMOVE -> WaveDatabase.stickers.uninstallPacks(setOf(StickerPackId(packId)))
        }
      } else {
  ...
  private fun handleSynchronizeConfigurationMessage(context: Context, configurationMessage: Configuration, envelopeTimestamp: Long) {
    ...
    if (configurationMessage.linkPreviews != null) {
      WaveStore.settings.isLinkPreviewsEnabled = configurationMessage.linkPreviews!!
    }
  }
  ...
  private fun handleSynchronizeBlockedListMessage(blockMessage: Blocked, envelopeTimestamp: Long) {
    ...
    WaveDatabase.recipients.applyBlockedUpdate(blockedE164s, blockedAcis, blockedGroupIds)
  }
  ...
  private fun handleSynchronizeMessageRequestResponse(response: MessageRequestResponse, envelopeTimestamp: Long) {
    ...
    when (response.type) {
      MessageRequestResponse.Type.ACCEPT -> {
        ...
        WaveDatabase.recipients.setProfileSharing(recipient.id, true)
        WaveDatabase.recipients.setBlocked(recipient.id, false)
        if (wasBlocked) {
          WaveDatabase.messages.insertMessageOutbox(
      ...
      MessageRequestResponse.Type.DELETE -> {
        WaveDatabase.recipients.setProfileSharing(recipient.id, false)
        if (threadId > 0) {
          WaveDatabase.threads.deleteConversation(threadId, syncThreadDelete = false)
        }
      }
      MessageRequestResponse.Type.BLOCK -> {
        WaveDatabase.recipients.setBlocked(recipient.id, true)
  ...
  private fun handleSynchronizeKeys(keys: SyncMessage.Keys, envelopeTimestamp: Long) {
    ...
    if (keys.accountEntropyPool != null) {
      WaveStore.account.setAccountEntropyPoolFromPrimaryDevice(AccountEntropyPool(keys.accountEntropyPool!!))
    }

    if (keys.mediaRootBackupKey != null) {
      WaveStore.backup.mediaRootBackupKey = MediaRootBackupKey(keys.mediaRootBackupKey!!.toByteArray())
    }
  }
  ...
  private fun handleSynchronizeContacts(contactsMessage: SyncMessage.Contacts, envelopeTimestamp: Long) {
    ...
    val attachment: WaveServiceAttachmentPointer = contactsMessage.blob!!.toWaveServiceAttachmentPointer()
  ...
  private fun handleSynchronizeCallLogEvent(callLogEvent: CallLogEvent, envelopeTimestamp: Long) {
    ...
    val peer: RecipientId? = callLogEvent.conversationId?.let { byteString ->
      ACI.parseOrNull(byteString)?.let { RecipientId.from(it) }
        ?: GroupId.pushOrNull(byteString.toByteArray())?.let { WaveDatabase.recipients.getByGroupId(it).orNull() }
        ?: CallLinkRoomId.fromBytes(byteString.toByteArray()).let { WaveDatabase.recipients.getByCallLinkRoomId(it).orNull() }
    }

    if (callId != null && peer != null) {
      val call = WaveDatabase.calls.getCallById(callId.longValue(), peer)
  ...
  private fun synchronizeCallLogEventViaTimestamp(envelopeTimestamp: Long, eventType: CallLogEvent.Type?, timestamp: Long, peer: RecipientId?) {
    when (eventType) {
      CallLogEvent.Type.CLEAR -> {
        WaveDatabase.calls.deleteNonAdHocCallEventsOnOrBefore(timestamp)
        WaveDatabase.callLinks.deleteNonAdminCallLinksOnOrBefore(timestamp)
      }

      CallLogEvent.Type.MARKED_AS_READ -> {
        WaveDatabase.calls.markAllCallEventsRead(timestamp)
      }
      ...
      CallLogEvent.Type.MARKED_AS_READ_IN_CONVERSATION -> {
        ...
        WaveDatabase.calls.markAllCallEventsWithPeerBeforeTimestampRead(peer, timestamp)
      }
  ...
  private fun handleSynchronizeCallLink(callLinkUpdate: CallLinkUpdate, envelopeTimestamp: Long) {
    ...
    if (WaveDatabase.callLinks.callLinkExists(roomId)) {
      ...
      WaveDatabase.callLinks.insertCallLink(
  ...
  private fun handleSynchronizeOneToOneCallEvent(callEvent: SyncMessage.CallEvent, envelopeTimestamp: Long) {
    ...
    val call = WaveDatabase.calls.getCallById(callId, recipientId)
    if (call != null) {
      ...
      if (typeMismatch || directionMismatch || peerMismatch || eventDowngrade) {
        ...
        WaveDatabase.calls.updateOneToOneCall(callId, event)
      }
    } else if (event == CallTable.Event.DELETE) {
      WaveDatabase.calls.insertDeletedCallFromSyncEvent(callId, recipientId, type, direction, timestamp)
    } else {
      WaveDatabase.calls.insertOneToOneCall(callId, timestamp, recipientId, type, direction, event)
    }
  }
  ...
  private fun handleSynchronizeGroupOrAdHocCallEvent(callEvent: SyncMessage.CallEvent, envelopeTimestamp: Long) {
    ...
    if (hasConversationId && type == CallTable.Type.AD_HOC_CALL && callEvent.event == SyncMessage.CallEvent.Event.OBSERVED && direction != null) {
      ...
      WaveDatabase.calls.insertOrUpdateAdHocCallFromRemoteObserveEvent(
    ...
    val call = WaveDatabase.calls.getCallById(callId, recipient.id)
    ...
    if (call != null) {
      ...
      when (event) {
        CallTable.Event.DELETE -> WaveDatabase.calls.markCallDeletedFromSyncEvent(call)
        CallTable.Event.ACCEPTED -> {
          if (call.timestamp > timestamp) {
            WaveDatabase.calls.setTimestamp(call.callId, recipient.id, timestamp)
          }
          if (direction == CallTable.Direction.INCOMING) {
            WaveDatabase.calls.acceptIncomingGroupCall(call)
          } else {
            WaveDatabase.calls.acceptOutgoingGroupCall(call)
          }
        }
        CallTable.Event.NOT_ACCEPTED -> {
          ...
          if (callEvent.direction == SyncMessage.CallEvent.Direction.INCOMING) {
            WaveDatabase.calls.declineIncomingGroupCall(call)
          } else {
      ...
      when (event) {
        CallTable.Event.DELETE -> WaveDatabase.calls.insertDeletedCallFromSyncEvent(callEvent.callId!!, recipient.id, type, direction, timestamp)
        CallTable.Event.ACCEPTED -> WaveDatabase.calls.insertAcceptedGroupCall(callEvent.callId!!, recipient.id, direction, timestamp)
        CallTable.Event.NOT_ACCEPTED -> {
          if (callEvent.direction == SyncMessage.CallEvent.Direction.INCOMING) {
            WaveDatabase.calls.insertDeclinedGroupCall(callEvent.callId!!, recipient.id, timestamp)
          } else {
  ...
  private fun resolveCallLinkRecipient(callEvent: SyncMessage.CallEvent): Recipient {
    ...
    val callLink = WaveDatabase.callLinks.getOrCreateCallLinkByRoomId(callLinkRoomId)
  ...
  private fun handleSynchronizeMessageDeletes(messageDeletes: List<SyncMessage.DeleteForMe.MessageDeletes>, envelopeTimestamp: Long, earlyMessageCacheEntry: EarlyMessageCacheEntry?) {
    ...
    val unhandled: List<MessageTable.SyncMessageId> = WaveDatabase.messages.deleteMessages(messagesToDelete)
  ...
  private fun handleSynchronizeConversationDeletes(conversationDeletes: List<SyncMessage.DeleteForMe.ConversationDelete>, envelopeTimestamp: Long) {
    for (delete in conversationDeletes) {
      ...
      val threadId = WaveDatabase.threads.getThreadIdFor(threadRecipientId)
      if (threadId == null) {
        log(envelopeTimestamp, "[handleSynchronizeDeleteForMe] No thread for matching conversation for recipient: $threadRecipientId")
        continue
      }

      var latestReceivedAt = WaveDatabase.messages.getLatestReceivedAt(threadId, delete.mostRecentMessages.mapNotNull { it.toSyncMessageId(envelopeTimestamp) })
      ...
      if (latestReceivedAt == null && delete.mostRecentNonExpiringMessages.isNotEmpty()) {
        ...
        latestReceivedAt = WaveDatabase.messages.getLatestReceivedAt(threadId, delete.mostRecentNonExpiringMessages.mapNotNull { it.toSyncMessageId(envelopeTimestamp) })
      }

      if (latestReceivedAt != null) {
        WaveDatabase.threads.trimThread(threadId = threadId, syncThreadTrimDeletes = false, trimBeforeDate = latestReceivedAt, inclusive = true)

        if (delete.isFullDelete == true) {
          val deleted = WaveDatabase.threads.deleteConversationIfContainsOnlyLocal(threadId)
  ...
  private fun handleSynchronizeAttachmentDeletes(attachmentDeletes: List<SyncMessage.DeleteForMe.AttachmentDelete>, envelopeTimestamp: Long, earlyMessageCacheEntry: EarlyMessageCacheEntry?) {
    ...
    val unhandled: List<MessageTable.SyncMessageId> = WaveDatabase.attachments.deleteAttachments(toDelete)
  ...
  private fun handleSynchronizeAttachmentBackfillRequest(request: SyncMessage.AttachmentBackfillRequest, timestamp: Long) {
    ...
    val threadId = WaveDatabase.threads.getThreadIdFor(conversationRecipientId)
    if (threadId == null) {
    ...
    val messageId: Long? = WaveDatabase.messages.getMessageIdOrNull(syncMessageId, threadId)
    if (messageId == null) {
    ...
    val attachments: List<DatabaseAttachment> = WaveDatabase.attachments.getAttachmentsForMessage(messageId).filterNot { it.quote }.sortedBy { it.displayOrder }
  ...
  private fun handleSynchronizedPollCreate(
    ...
    val messageId = WaveDatabase.messages.insertMessageOutbox(outgoingMessage, threadId, false, GroupReceiptTable.STATUS_UNKNOWN, null).messageId
    updateGroupReceiptStatus(sent, messageId, recipient.requireGroupId())
  ...
  private fun handleSynchronizedPollEnd(
    ...
    val targetMessage = WaveDatabase.messages.getMessageFor(pollTerminate.targetSentTimestamp!!, Recipient.self().id)
    if (targetMessage == null) {
    ...
    val poll = WaveDatabase.polls.getPoll(targetMessage.id)
    if (poll == null) {
  ...
  private fun handleSynchronizedPinMessage(
    ...
    val targetMessage = WaveDatabase.messages.getMessageFor(pinMessage.targetSentTimestamp!!, Recipient.self().id)
    if (targetMessage == null) {
  ...
  private fun ConversationIdentifier.toRecipientId(): RecipientId? {
    ...
    return when {
      ...
      threadServiceId != null -> {
        WaveDatabase.recipients.getOrInsertFromServiceId(threadServiceId)
      }

      threadE164 != null -> {
        WaveE164Util.formatAsE164(threadE164!!)?.let {
          WaveDatabase.recipients.getOrInsertFromE164(threadE164!!)
        }
      }
  ...
  private fun AddressableMessage.toSyncMessageId(envelopeTimestamp: Long): MessageTable.SyncMessageId? {
    return if (this.sentTimestamp != null && Utils.anyNotNull(this.authorServiceId, this.authorServiceIdBinary) || this.authorE164 != null) {
      ...
      val id = if (serviceId != null) {
        WaveDatabase.recipients.getOrInsertFromServiceId(serviceId)
      } else {
        WaveDatabase.recipients.getOrInsertFromE164(this.authorE164!!)
      }
