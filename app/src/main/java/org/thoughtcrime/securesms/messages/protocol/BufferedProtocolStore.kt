...
class BufferedProtocolStore private constructor(
  private val aciStore: Pair<ServiceId, BufferedWaveServiceAccountDataStore>,
  private val pniStore: Pair<ServiceId, BufferedWaveServiceAccountDataStore>
) {

  fun get(serviceId: ServiceId): BufferedWaveServiceAccountDataStore {
  ...
  fun getAciStore(): BufferedWaveServiceAccountDataStore {
  ...
  companion object {
    fun create(): BufferedProtocolStore {
      ...
      val pni = WaveStore.account.requirePni()

      return BufferedProtocolStore(
        aciStore = aci to BufferedWaveServiceAccountDataStore(aci),
        pniStore = pni to BufferedWaveServiceAccountDataStore(pni)
      )
    }
  }
}
