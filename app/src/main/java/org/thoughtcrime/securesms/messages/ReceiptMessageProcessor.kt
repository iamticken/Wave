...
object ReceiptMessageProcessor {
  ...
  private fun handleDeliveryReceipt(
    ...
    val missingTargetTimestamps: Set<Long> = WaveDatabase.messages.incrementDeliveryReceiptCounts(deliveryReceipt.timestamp, senderRecipientId, envelope.timestamp!!, stopwatch)
    ...
    WaveDatabase.pendingPniSignatureMessages.acknowledgeReceipts(senderRecipientId, deliveryReceipt.timestamp, metadata.sourceDeviceId)
    stopwatch?.split("pni-signatures")

    WaveDatabase.messageLog.deleteEntriesForRecipient(deliveryReceipt.timestamp, senderRecipientId, metadata.sourceDeviceId)
    stopwatch?.split("msl")
  ...
  private fun handleReadReceipt(
    ...
    val missingTargetTimestamps: Set<Long> = WaveDatabase.messages.incrementReadReceiptCounts(readReceipt.timestamp, senderRecipientId, envelope.timestamp!!)
  ...
  private fun handleViewedReceipt(
    ...
    val storyViewedReceipts = WaveStore.story.viewedReceiptsEnabled
    ...
    val missingTargetTimestamps: Set<Long> = if (readReceipts && storyViewedReceipts) {
      WaveDatabase.messages.incrementViewedReceiptCounts(viewedReceipt.timestamp, senderRecipientId, envelope.timestamp!!)
    } else if (readReceipts) {
      WaveDatabase.messages.incrementViewedNonStoryReceiptCounts(viewedReceipt.timestamp, senderRecipientId, envelope.timestamp!!)
    } else {
      WaveDatabase.messages.incrementViewedStoryReceiptCounts(viewedReceipt.timestamp, senderRecipientId, envelope.timestamp!!)
    }

    val foundTargetTimestamps: Set<Long> = viewedReceipt.timestamp.toSet() - missingTargetTimestamps.toSet()
    WaveDatabase.messages.updateViewedStories(foundTargetTimestamps)
