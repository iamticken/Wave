...
class MediaPreviewV2Fragment :
  ...
  private fun initializeToolbar(toolbar: MaterialToolbar) {
    ...
    toolbar.setTitleTextAppearance(requireContext(), R.style.Wave_Text_TitleMedium)
    toolbar.setSubtitleTextAppearance(requireContext(), R.style.Wave_Text_BodyMedium)
  ...
  private fun getSubTitleText(mediaRecord: MediaTable.MediaRecord): CharSequence {
    ...
    val onSurfaceColor = ContextCompat.getColor(requireContext(), R.color.wave_colorOnSurface)
  ...
  private fun canRemotelyDelete(attachment: DatabaseAttachment): Boolean {
    ...
    val attachmentCount = WaveDatabase.attachments.getAttachmentsForMessage(mmsId).size
    return attachmentCount <= 1 && MessageConstraintsUtil.isValidRemoteDeleteSend(listOf(WaveDatabase.messages.getMessageRecord(mmsId)), System.currentTimeMillis())
  }
