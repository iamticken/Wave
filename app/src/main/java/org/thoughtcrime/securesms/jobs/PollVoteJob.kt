...
class PollVoteJob(
  ...
  override fun run(): Result {
    ...
    val poll = WaveDatabase.polls.getPoll(messageId)
    if (poll == null) {
  ...
  private fun deliver(conversationRecipient: Recipient, destinations: List<Recipient>, targetAuthor: Recipient, targetSentTimestamp: Long, poll: PollRecord): List<Recipient> {
    val votes = WaveDatabase.polls.getVotes(poll.id, poll.allowMultipleVotes, voteCount)
    ...
    if (groupResult.completed.isNotEmpty() || destinations.isEmpty()) {
      if (isRemoval) {
        WaveDatabase.polls.markPendingAsRemoved(
          pollId = poll.id,
        ...
        WaveDatabase.polls.markPendingAsAdded(
          pollId = poll.id,
          voterId = Recipient.self().id.toLong(),
          voteCount = voteCount,
          messageId = poll.messageId,
  ...
  override fun onFailure() {
    ...
    val pollId = WaveDatabase.polls.getPollId(messageId)
    if (pollId == null) {
      Log.w(TAG, "Poll no longer exists")
      return
    }

    WaveDatabase.polls.removePendingVote(pollId, optionId, voteCount, messageId)
  }
  ...
  private fun buildPollVote(
    ...
    voteCount: Int
  ): WaveServiceDataMessage.PollVote {
    return WaveServiceDataMessage.PollVote(
