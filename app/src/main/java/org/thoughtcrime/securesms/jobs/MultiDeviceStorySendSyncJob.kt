...
class MultiDeviceStorySendSyncJob private constructor(parameters: Parameters, private val sentTimestamp: Long, private val deletedMessageId: Long) : BaseJob(parameters) {
  ...
  override fun onRun() {
    val updateManifest = WaveDatabase.storySends.getLocalManifest(sentTimestamp)
    val recipientsSet: Set<WaveServiceStoryMessageRecipient> = updateManifest.toRecipientsSet()
    val transcriptMessage: WaveServiceSyncMessage = WaveServiceSyncMessage.forSentTranscript(buildSentTranscript(recipientsSet))
    val sendMessageResult = AppDependencies.waveServiceMessageSender.sendSyncMessage(transcriptMessage)
    ...
    WaveDatabase.messages.deleteRemotelyDeletedStory(deletedMessageId)
  }
  ...
  private fun buildSentTranscript(recipientsSet: Set<WaveServiceStoryMessageRecipient>): SentTranscriptMessage {
    return SentTranscriptMessage(
      Optional.of(WaveServiceAddress(Recipient.self().requireAci())),
      sentTimestamp,
      Optional.empty(),
      0,
      emptyMap(),
