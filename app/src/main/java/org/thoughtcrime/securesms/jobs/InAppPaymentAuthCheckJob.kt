...
import org.wave.donations.json.StripePaymentIntent
import org.wave.donations.json.StripeSetupIntent
...
class InAppPaymentAuthCheckJob private constructor(parameters: Parameters) : BaseJob(parameters), StripeApi.PaymentIntentFetcher, StripeApi.SetupIntentHelper {
  ...
  companion object {
    ...
    @JvmStatic
    fun enqueueIfNeeded() {
      if (WaveDatabase.inAppPayments.hasWaitingForAuth()) {
  ...
  override fun onRun() {
    ...
    val unauthorizedInAppPayments = WaveDatabase.inAppPayments.getAllWaitingForAuth()
  ...
  private fun migrateLegacyData() {
    val pending3DSData = WaveStore.inAppPayments.consumePending3DSData()
    if (pending3DSData != null) {
      Log.i(TAG, "Found legacy data. Performing migration.", true)

      WaveDatabase.inAppPayments.insert(
  ...
  private fun checkOneTimePayment(inAppPayment: InAppPaymentTable.InAppPayment): CheckResult<Unit> {
    ...
    val updatedPayment = WaveDatabase.inAppPayments.getById(inAppPayment.id)
  ...
  private fun checkRecurringPayment(inAppPayment: InAppPaymentTable.InAppPayment): CheckResult<Unit> {
    ...
    Log.d(TAG, "Set default payment method via Wave service.", true)

    WaveDatabase.inAppPaymentSubscribers.setPaymentMethod(subscriber.subscriberId, inAppPayment.data.paymentMethodType)
    ...
    try {
      ...
      if (updateLevelResponse.status in listOf(200, 204)) {
        ...
        with(WaveDatabase.inAppPayments.getById(inAppPayment.id)!!) {
  ...
  private fun markChecked(inAppPayment: InAppPaymentTable.InAppPayment) {
    val fresh = WaveDatabase.inAppPayments.getById(inAppPayment.id)
    WaveDatabase.inAppPayments.update(
