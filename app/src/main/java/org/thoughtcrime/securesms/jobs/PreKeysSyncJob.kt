...
class PreKeysSyncJob private constructor(
  ...
  companion object {
    ...
    @JvmStatic
    fun enqueueIfNeeded() {
      if (!WaveStore.account.aciPreKeys.isSignedPreKeyRegistered || !WaveStore.account.pniPreKeys.isSignedPreKeyRegistered) {
        Log.i(TAG, "Some signed/last-resort prekeys aren't registered yet. Enqueuing a job. ACI: ${WaveStore.account.aciPreKeys.isSignedPreKeyRegistered} PNI: ${WaveStore.account.pniPreKeys.isSignedPreKeyRegistered}")
        AppDependencies.jobManager.add(PreKeysSyncJob())
      } else if (WaveStore.account.aciPreKeys.activeSignedPreKeyId < 0 || WaveStore.account.pniPreKeys.activeSignedPreKeyId < 0) {
        Log.i(TAG, "Some signed prekeys aren't active yet. Enqueuing a job. ACI: ${WaveStore.account.aciPreKeys.activeSignedPreKeyId >= 0} PNI: ${WaveStore.account.pniPreKeys.activeSignedPreKeyId >= 0}")
        AppDependencies.jobManager.add(PreKeysSyncJob())
      } else if (WaveStore.account.aciPreKeys.lastResortKyberPreKeyId < 0 || WaveStore.account.pniPreKeys.lastResortKyberPreKeyId < 0) {
        Log.i(TAG, "Some last-resort kyber prekeys aren't active yet. Enqueuing a job. ACI: ${WaveStore.account.aciPreKeys.lastResortKyberPreKeyId >= 0} PNI: ${WaveStore.account.pniPreKeys.lastResortKyberPreKeyId >= 0}")
        AppDependencies.jobManager.add(PreKeysSyncJob())
      } else {
        val timeSinceLastFullRefresh = System.currentTimeMillis() - WaveStore.misc.lastFullPrekeyRefreshTime
  ...
  override fun onRun() {
    if (!WaveStore.account.isRegistered || WaveStore.account.aci == null || WaveStore.account.pni == null) {
    ...
    val forceRotation = if (forceRotationRequested) {
      ...
      warn(TAG, ServiceIdType.ACI, "Active Signed EC: ${WaveStore.account.aciPreKeys.activeSignedPreKeyId}, Last Resort Kyber: ${WaveStore.account.aciPreKeys.lastResortKyberPreKeyId}")
      warn(TAG, ServiceIdType.PNI, "Active Signed EC: ${WaveStore.account.pniPreKeys.activeSignedPreKeyId}, Last Resort Kyber: ${WaveStore.account.pniPreKeys.lastResortKyberPreKeyId}")

      if (!checkPreKeyConsistency(ServiceIdType.ACI, AppDependencies.protocolStore.aci(), WaveStore.account.aciPreKeys)) {
        warn(TAG, ServiceIdType.ACI, "Prekey consistency check failed! Must rotate keys!")
        true
      } else if (!checkPreKeyConsistency(ServiceIdType.PNI, AppDependencies.protocolStore.pni(), WaveStore.account.pniPreKeys)) {
        ...
        val timeSinceLastForcedRotation = System.currentTimeMillis() - WaveStore.misc.lastForcedPreKeyRefresh
    ...
    if (forceRotation) {
      ...
      warn(TAG, "Forced prekey rotation was requested, but we already did a forced refresh ${System.currentTimeMillis() - WaveStore.misc.lastForcedPreKeyRefresh} ms ago. Ignoring.")
    }

    syncPreKeys(ServiceIdType.ACI, WaveStore.account.aci, AppDependencies.protocolStore.aci(), WaveStore.account.aciPreKeys, forceRotation)
    syncPreKeys(ServiceIdType.PNI, WaveStore.account.pni, AppDependencies.protocolStore.pni(), WaveStore.account.pniPreKeys, forceRotation)
    WaveStore.misc.lastFullPrekeyRefreshTime = System.currentTimeMillis()

    if (forceRotation) {
      WaveStore.misc.lastForcedPreKeyRefresh = System.currentTimeMillis()
    }
  }

  private fun syncPreKeys(serviceIdType: ServiceIdType, serviceId: ServiceId?, protocolStore: WaveServiceAccountDataStore, metadataStore: PreKeyMetadataStore, forceRotation: Boolean) {
    if (serviceId == null) {
      warn(TAG, serviceIdType, "AccountId not set!")
      return
    }

    val availablePreKeyCounts = WaveNetwork.keys.getAvailablePreKeyCounts(serviceIdType).successOrThrow()
  ...
  private fun signedPreKeyUploadIfNeeded(serviceIdType: ServiceIdType, protocolStore: WaveProtocolStore, metadataStore: PreKeyMetadataStore, forceRotation: Boolean): SignedPreKeyRecord? {
  ...
  private fun lastResortKyberPreKeyUploadIfNeeded(serviceIdType: ServiceIdType, protocolStore: WaveServiceAccountDataStore, metadataStore: PreKeyMetadataStore, forceRotation: Boolean): KyberPreKeyRecord? {
  ...
  private fun checkPreKeyConsistency(serviceIdType: ServiceIdType, protocolStore: WaveServiceAccountDataStore, metadataStore: PreKeyMetadataStore): Boolean {
    val result: NetworkResult<Unit> = try {
      WaveNetwork.keys.checkRepeatedUseKeys(
        serviceIdType = serviceIdType,
