...
class ResetSvrGuessCountJob private constructor(
  ...
  override fun run(): Result {
    SvrRepository.operationLock.withLock {
      ...
      val svr3Result = if (svr3Complete) {
        ...
        resetGuessCount(
          svr = AppDependencies.waveServiceAccountManager.getSecureValueRecoveryV3(AppDependencies.libwaveNetwork),
          serializedChangeSession = serializedChangeSessionV3,
          pin = pin,
          masterKey = masterKey,
          changeSessionSaver = { serializedChangeSessionV3 = it.serialize() },
          authTokenSaver = { WaveStore.svr.appendSvr3AuthTokenToList(it.asBasic()) }
        )
      }
      ...
      return if (svr2Complete) {
        ...
        resetGuessCount(
          svr = AppDependencies.waveServiceAccountManager.getSecureValueRecoveryV2(BuildConfig.SVR2_MRENCLAVE),
          serializedChangeSession = serializedChangeSessionV2,
          pin = pin,
          masterKey = masterKey,
          changeSessionSaver = { serializedChangeSessionV2 = it.serialize() },
          authTokenSaver = { WaveStore.svr.appendSvr2AuthTokenToList(it.asBasic()) }
        )
      }
    }
  }
  ...
  private fun resetGuessCount(
    ...
    val session: PinChangeSession = if (serializedChangeSession != null) {
      svr.resumePinChangeSession(pin, WaveStore.svr.masterKey, serializedChangeSession)
    } else {
