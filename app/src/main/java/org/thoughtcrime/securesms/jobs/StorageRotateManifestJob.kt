...
class StorageRotateManifestJob private constructor(parameters: Parameters) : Job(parameters) {
  ...
  override fun run(): Result {
    ...
    val restoreKey: StorageKey? = WaveStore.storageService.storageKeyForInitialDataRestore
    if (restoreKey == null) {
    ...
    val currentManifest: WaveStorageManifest = when (val result = repository.getStorageManifest(restoreKey)) {
    ...
    return when (val result = repository.writeUnchangedManifest(storageServiceKey, manifestWithNewVersion)) {
      StorageServiceRepository.WriteStorageRecordsResult.Success -> {
        ...
        WaveStore.storageService.manifest = manifestWithNewVersion
