...
class ArchiveAttachmentReconciliationJob private constructor(
  ...
  companion object {
    ...
    fun enqueueIfRetryAllowed(forced: Boolean) {
      if (WaveStore.backup.archiveAttachmentReconciliationAttempts < 3) {
        WaveStore.backup.archiveAttachmentReconciliationAttempts++
  ...
  override fun run(): Result {
    ...
    if (WaveStore.backup.lastAttachmentReconciliationTime < 0) {
      ...
      WaveStore.backup.lastAttachmentReconciliationTime = System.currentTimeMillis()
      return Result.success()
    }

    val timeSinceLastSync = System.currentTimeMillis() - WaveStore.backup.lastAttachmentReconciliationTime
    ...
    snapshotVersion = snapshotVersion ?: WaveDatabase.backupMediaSnapshots.getCurrentSnapshotVersion()
  ...
  private fun syncDataFromCdn(snapshotVersion: Long): Result? {
    ...
    Log.d(TAG, "BEFORE:\n" + WaveDatabase.attachments.debugGetAttachmentStats().shortPrettyString(), true)
    stopwatch.split("stats-before")

    val mediaObjectsThatMayNeedReUpload = WaveDatabase.backupMediaSnapshots.getMediaObjectsLastSeenOnCdnBeforeSnapshotVersion(snapshotVersion)
    ...
    if (mayNeedReUploadCount > 0) {
      ...
      mediaObjectsThatMayNeedReUpload.forEach { mediaObjectCursor ->
        ...
        if (entry.isThumbnail) {
          ...
          val wasReset = WaveDatabase.attachments.resetArchiveThumbnailTransferStateByPlaintextHashAndRemoteKeyIfNecessary(entry.plaintextHash, entry.remoteKey)
          if (wasReset) {
          ...
          val wasReset = WaveDatabase.attachments.resetArchiveTransferStateByPlaintextHashAndRemoteKeyIfNecessary(entry.plaintextHash, entry.remoteKey)
          if (wasReset) {
            newBackupJobRequired = true
            bookkeepingErrorCount++
          } else {
      ...
      if (internalUser && mediaIdsThatNeedUpload.isNotEmpty()) {
        ...
        val matchingAttachments = WaveDatabase.attachments.debugGetAttachmentsForMediaIds(mediaIdsThatNeedUpload, limit = 10_000)
        ...
        matchingAttachments.forEach { pair ->
          ...
          if (isThumbnail) {
            val thumbnailTransferState = WaveDatabase.attachments.getArchiveThumbnailTransferState(attachment.attachmentId)
    ...
    WaveStore.backup.remoteStorageGarbageCollectionPending = false
    WaveStore.backup.lastAttachmentReconciliationTime = System.currentTimeMillis()
  ...
  private fun syncCdnPage(archivedItemPage: ArchiveGetMediaItemsResponse, currentSnapshotVersion: Long): Set<ArchivedMediaObject> {
    ...
    WaveDatabase.backupMediaSnapshots.markSeenOnRemote(
      mediaIdBatch = mediaObjects.map { it.mediaId },
      snapshotVersion = currentSnapshotVersion
    )

    val mediaOnRemoteButNotLocal = WaveDatabase.backupMediaSnapshots.getMediaObjectsThatCantBeFound(mediaObjects)
    val mediaObjectsOnBothRemoteAndLocal = mediaObjects - mediaOnRemoteButNotLocal

    val cdnMismatches = WaveDatabase.backupMediaSnapshots.getMediaObjectsWithNonMatchingCdn(mediaObjectsOnBothRemoteAndLocal)
    if (cdnMismatches.isNotEmpty()) {
      ...
      for (mismatch in cdnMismatches) {
        WaveDatabase.attachments.setArchiveCdnByPlaintextHashAndRemoteKey(mismatch.plaintextHash, mismatch.remoteKey, mismatch.cdn)
      }
    }
  ...
  private fun validateAndDeleteFromRemote(deletes: Set<ArchivedMediaObject>): Result? {
    ...
    val validatedDeletes = WaveDatabase.attachments.getMediaObjectsThatCantBeFound(deletes)
