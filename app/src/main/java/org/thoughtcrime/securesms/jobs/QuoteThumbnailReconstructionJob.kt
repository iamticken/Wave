...
class QuoteThumbnailReconstructionJob private constructor(params: Parameters) : Job(params) {
  ...
  override fun run(): Result {
    val quoteAttachment = WaveDatabase.attachments.getNewestQuotePendingReconstruction()
    ...
    val message = WaveDatabase.messages.getMessageRecordOrNull(quoteAttachment.mmsId)
    if (message == null) {
      ...
      WaveDatabase.attachments.clearQuotePendingReconstruction(quoteAttachment.attachmentId)
    ...
    if (messageAge > RemoteConfig.messageQueueTime.milliseconds) {
      ...
      WaveDatabase.attachments.clearAllQuotesPendingReconstruction()
      return Result.success()
    }

    val targetMessage = WaveDatabase.messages.getMessageFor(message.getQuote()!!.id, message.getQuote()!!.author)
    ...
    val targetAttachment = WaveDatabase.attachments.getAttachmentsForMessage(targetMessage.id).firstOrNull { MediaUtil.isImageOrVideoType(it.contentType) && it.uri != null }
    ...
    val thumbnailData = WaveDatabase.attachments.generateQuoteThumbnail(DecryptableUri(targetAttachment.uri!!), targetAttachment.contentType, quiet = true)
    ...
    WaveDatabase.attachments.applyReconstructedQuoteData(quoteAttachment.attachmentId, thumbnailData)
  ...
  override fun onFailure() {
    activeQuoteAttachment?.let { attachment ->
      ...
      WaveDatabase.attachments.clearQuotePendingReconstruction(attachment.attachmentId)
