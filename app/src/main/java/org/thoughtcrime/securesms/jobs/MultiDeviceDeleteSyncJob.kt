...
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.pad
...
import org.whispersystems.waveservice.internal.push.SyncMessage.DeleteForMe
...
class MultiDeviceDeleteSyncJob private constructor(
  ...
  companion object {
    ...
    private fun createMessageDeletes(messageRecords: Collection<MessageRecord>): List<DeleteSyncJobData.AddressableMessage> {
      return messageRecords.mapNotNull { message ->
        val threadRecipient = WaveDatabase.threads.getRecipientForThreadId(message.threadId)
  ...
  private fun syncDelete(deleteForMe: DeleteForMe): Boolean {
    ...
    return try {
      ...
      AppDependencies.waveServiceMessageSender.sendSyncMessage(syncMessageContent, true, Optional.empty()).isSuccess
    } catch (e: IOException) {
