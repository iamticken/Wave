  ...
  private static final class ForegroundServiceController implements AppForegroundObserver.Listener, AutoCloseable {
    ...
    public void onBackground() {
      ...
      if (Build.VERSION.SDK_INT < 31) {
        notificationController.set(GenericForegroundService.startForegroundTaskDelayed(context, context.getString(R.string.BackgroundMessageRetriever_checking_for_messages), 300, R.drawable.ic_wave_refresh));
      } else {
        try {
          notificationController.set(GenericForegroundService.startForegroundTask(context, context.getString(R.string.BackgroundMessageRetriever_checking_for_messages), NotificationChannels.getInstance().OTHER, R.drawable.ic_wave_refresh));
