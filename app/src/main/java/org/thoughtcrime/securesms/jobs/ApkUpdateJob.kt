...
import org.wave.core.util.getDownloadManager
...
class ApkUpdateJob private constructor(parameters: Parameters) : BaseJob(parameters) {
  ...
  public override fun onRun() {
    ...
    if (shouldUpdate(getCurrentAppVersionCode(), updateDescriptor, WaveStore.apkUpdate.lastApkUploadTime, Environment.IS_WEBSITE)) {
      Log.i(TAG, "Newer version code available. Current: (versionCode: ${getCurrentAppVersionCode()}, uploadTime: ${WaveStore.apkUpdate.lastApkUploadTime}), Update: (versionCode: ${updateDescriptor.versionCode}, uploadTime: ${updateDescriptor.uploadTimestamp})")
      ...
      Log.d(TAG, "Version code is the same or older than our own. Current: (versionCode: ${getCurrentAppVersionCode()}, uploadTime: ${WaveStore.apkUpdate.lastApkUploadTime}), Update: (versionCode: ${updateDescriptor.versionCode}, uploadTime: ${updateDescriptor.uploadTimestamp})")
    }

    WaveStore.apkUpdate.lastSuccessfulCheck = System.currentTimeMillis()
  }
  ...
  private fun getDownloadStatus(uri: String, remoteDigest: ByteArray): DownloadStatus {
    val pendingDownloadId: Long = WaveStore.apkUpdate.downloadId
    val pendingDigest: ByteArray? = WaveStore.apkUpdate.digest

    if (pendingDownloadId == -1L || pendingDigest == null || !MessageDigest.isEqual(pendingDigest, remoteDigest)) {
      WaveStore.apkUpdate.clearDownloadAttributes()
  ...
  private fun handleDownloadStart(uri: String?, versionName: String?, digest: ByteArray, uploadTimestamp: Long) {
    ...
    val downloadRequest = DownloadManager.Request(Uri.parse(uri)).apply {
      ...
      setTitle("Downloading Wave update")
      setDescription("Downloading Wave $versionName")
      setDestinationInExternalFilesDir(context, null, "wave-update.apk")
    ...
    WaveStore.apkUpdate.setDownloadAttributes(downloadId, digest, uploadTimestamp)
  }
  ...
  private fun deleteExistingDownloadedApks(context: Context) {
    ...
    for (file in directory.listFiles() ?: emptyArray()) {
      if (file.name.startsWith("wave-update")) {
        if (file.delete()) {
