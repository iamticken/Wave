...
public class RefreshOwnProfileJob extends BaseJob {
  ...
  protected void onRun() throws Exception {
    if (!WaveStore.account().isRegistered() || TextUtils.isEmpty(WaveStore.account().getE164())) {
      Log.w(TAG, "Not yet registered!");
      return;
    }

    if ((WaveStore.svr().hasPin() || WaveStore.account().restoredAccountEntropyPool()) && !WaveStore.svr().hasOptedOut() && WaveStore.storageService().getLastSyncTime() == 0) {
      Log.i(TAG, "Registered with PIN or AEP but haven't completed storage sync yet.");
      return;
    }

    if (!WaveStore.registration().hasUploadedProfile() && WaveStore.account().isPrimaryDevice()) {
    ...
    WaveServiceProfile profile              = profileAndCredential.getProfile();
    ...
    WaveStore.registration().setHasDownloadedProfile(true);
  ...
  private static WaveServiceProfile.RequestType getRequestType(@NonNull Recipient recipient) {
    return ExpiringProfileCredentialUtil.isValid(recipient.getExpiringProfileKeyCredential()) ? WaveServiceProfile.RequestType.PROFILE
                                                                                              : WaveServiceProfile.RequestType.PROFILE_AND_CREDENTIAL;
  }
  ...
  private void setProfileName(@Nullable String encryptedName) {
    try {
      ...
      if (!profileName.isEmpty()) {
        ...
        WaveDatabase.recipients().setProfileName(Recipient.self().getId(), profileName);
      } else {
  ...
  private void setProfileAbout(@Nullable String encryptedAbout, @Nullable String encryptedEmoji) {
    try {
      ...
      WaveDatabase.recipients().setAbout(Recipient.self().getId(), plaintextAbout, plaintextEmoji);
    } catch (InvalidCiphertextException | IOException e) {
      Log.w(TAG, e);
    }
  }
  ...
  private void setProfileCapabilities(@Nullable WaveServiceProfile.Capabilities capabilities) {
    ...
    WaveDatabase.recipients().setCapabilities(Recipient.self().getId(), capabilities);
  }
  ...
  private void ensurePhoneNumberSharingIsCorrect(@Nullable String phoneNumberSharingCiphertext) {
    ...
    try {
      ...
      if (remotePhoneNumberSharing == RecipientTable.PhoneNumberSharingState.UNKNOWN || remotePhoneNumberSharing.getEnabled() != WaveStore.phoneNumberPrivacy().isPhoneNumberSharingEnabled()) {
  ...
  private static void checkUsernameIsInSync() {
    ...
    try {
      String localUsername = WaveStore.account().getUsername();

      WhoAmIResponse whoAmIResponse     = AppDependencies.getWaveServiceAccountManager().getWhoAmI();
    ...
    try {
      UsernameLinkComponents localUsernameLink = WaveStore.account().getUsernameLink();

      if (localUsernameLink != null) {
        byte[]                remoteEncryptedUsername = NetworkResultUtil.toBasicLegacy(WaveNetwork.username().getEncryptedUsernameFromLinkServerId(localUsernameLink.getServerId()));
        ...
        if (!remoteUsername.getUsername().equals(WaveStore.account().getUsername())) {
  ...
  private void setProfileBadges(@Nullable List<WaveServiceProfile.Badge> badges) throws IOException {
    if (badges == null) {
      return;
    }
    ...
    Set<String> remoteDonorBadgeIds = badges.stream()
                                            ...
                                            .map(WaveServiceProfile.Badge::getId)
                                            .collect(Collectors.toSet());
    ...
    if (!remoteHasSubscriptionBadges && localHasSubscriptionBadges) {
      ...
      WaveStore.inAppPayments().setExpiredBadge(mostRecentExpiration);
      ...
      Badge badge = WaveStore.inAppPayments().getExpiredBadge();
    ...
    if (!remoteHasGiftBadges && localHasGiftBadges) {
      ...
      WaveStore.inAppPayments().setExpiredGiftBadge(mostRecentExpiration);
    ...
    boolean userHasVisibleBadges   = badges.stream().anyMatch(WaveServiceProfile.Badge::isVisible);
    ...
    if (userHasVisibleBadges && userHasInvisibleBadges) {
      boolean displayBadgesOnProfile = WaveStore.inAppPayments().getDisplayBadgesOnProfile();
      ...
      WaveDatabase.recipients().setBadges(Recipient.self().getId(), updatedBadges);
    } else {
      WaveDatabase.recipients().setBadges(Recipient.self().getId(), appBadges);
    }
  }
