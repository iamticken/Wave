...
public class AutomaticSessionResetJob extends BaseJob {
  ...
  protected void onRun() throws Exception {
    ...
    if (RemoteConfig.automaticSessionReset()) {
      ...
      DeviceLastResetTime resetTimes         = WaveDatabase.recipients().getLastSessionResetTimes(recipientId);
      ...
      if (timeSinceLastReset > resetInterval) {
        ...
        WaveDatabase.recipients().setLastSessionResetTime(recipientId, setLastResetTime(resetTimes, deviceId, System.currentTimeMillis()));
  ...
  private void insertLocalMessage() {
    MessageTable.InsertResult result = WaveDatabase.messages().insertChatSessionRefreshedMessage(recipientId, deviceId, sentTimestamp - 1);
