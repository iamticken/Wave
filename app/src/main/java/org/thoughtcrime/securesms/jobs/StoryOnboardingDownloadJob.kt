...
class StoryOnboardingDownloadJob private constructor(parameters: Parameters) : BaseJob(parameters) {
  ...
  companion object {
    ...
    fun enqueueIfNeeded() {
      if (WaveStore.story.hasDownloadedOnboardingStory || WaveStore.story.userHasViewedOnboardingStory) {
  ...
  override fun onRun() {
    if (WaveStore.story.hasDownloadedOnboardingStory) {
      Log.i(TAG, "Already downloaded onboarding story. Exiting.")
      return
    }

    val releaseChannelRecipientId = WaveStore.releaseChannel.releaseChannelRecipientId
    ...
    WaveDatabase.messages.getAllStoriesFor(releaseChannelRecipientId, -1).use { reader ->
      reader.forEach { messageRecord ->
        WaveDatabase.messages.deleteMessage(messageRecord.id)
      }
    }
    ...
    val threadId = WaveDatabase.threads.getOrCreateThreadIdFor(Recipient.resolved(releaseChannelRecipientId))
    ...
    if (insertResults.size != ONBOARDING_IMAGE_COUNT) {
      ...
      insertResults.forEach {
        WaveDatabase.messages.deleteMessage(it.messageId)
      }
    ...
    WaveStore.story.hasDownloadedOnboardingStory = true

    Log.i(TAG, "Enqueueing download jobs...")
    insertResults.forEach { insertResult ->
      WaveDatabase.attachments.getAttachmentsForMessage(insertResult.messageId).forEach {
  ...
  private fun getLocaleCodes(): List<String> {
    ...
    if (WaveStore.settings.language != "zz") {
      potentialOnboardingUrlLanguages += WaveStore.settings.language
    }
