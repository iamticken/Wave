...
class CopyAttachmentToArchiveJob private constructor(private val attachmentId: AttachmentId, parameters: Parameters) : Job(parameters) {
  ...
  override fun run(): Result {
      ...
      is NetworkResult.StatusCodeError -> {
        when (archiveResult.code) {
          ...
          413 -> {
            ...
            if (WaveDatabase.attachments.getPaidEstimatedArchiveMediaSize() > remoteStorageQuota.inWholeBytes) {
            ...
            WaveStore.backup.remoteStorageGarbageCollectionPending = true
    ...
    if (result.isSuccess) {
      ...
      ArchiveDatabaseExecutor.runBlocking {
        WaveDatabase.attachments.setArchiveTransferState(attachmentId, attachment.remoteKey!!, attachment.dataHash!!, AttachmentTable.ArchiveTransferState.FINISHED, notify = false)
      ...
      WaveStore.backup.archiveAttachmentReconciliationAttempts = 0
    }
  ...
  override fun onFailure() {
    if (this.isCanceled) {
      ...
      ArchiveDatabaseExecutor.runBlocking {
        WaveDatabase.attachments.setArchiveTransferState(attachmentId, AttachmentTable.ArchiveTransferState.COPY_PENDING)
      }
    } else {
