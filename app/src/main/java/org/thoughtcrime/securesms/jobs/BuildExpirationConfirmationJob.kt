...
import org.whispersystems.waveservice.api.remoteconfig.RemoteConfigResult
...
class BuildExpirationConfirmationJob private constructor(params: Parameters) : Job(params) {
  ...
  override fun run(): Result {
    if (Util.getTimeUntilBuildExpiry(WaveStore.misc.estimatedServerTime) > 0) {
    ...
    return when (val result: NetworkResult<RemoteConfigResult> = WaveNetwork.remoteConfig.getRemoteConfig()) {
      is NetworkResult.Success -> {
        val serverTimeMs = result.result.serverEpochTimeMilliseconds
        WaveStore.misc.setLastKnownServerTime(serverTimeMs, System.currentTimeMillis())
