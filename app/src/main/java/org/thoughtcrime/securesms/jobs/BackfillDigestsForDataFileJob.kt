...
class BackfillDigestsForDataFileJob private constructor(
  ...
  override fun run(): Result {
    val (originalKey, decryptingStream) = WaveDatabase.rawDatabase.withinTransaction {
      val attachment = WaveDatabase.attachments.getMostRecentValidAttachmentUsingDataFile(dataFile)
      if (attachment == null) {
      ...
      val stream = try {
        WaveDatabase.attachments.getAttachmentStream(attachment.attachmentId, offset = 0)
      } catch (e: IOException) {
    ...
    WaveDatabase.attachments.updateRemoteKeyAndDigestByDataFile(
      dataFile = dataFile,
      key = key,
      digest = digest
    )
