...
public class ReportSpamJob extends BaseJob {
  ...
  public void onRun() throws IOException {
    ...
    Recipient threadRecipient = WaveDatabase.threads().getRecipientForThreadId(threadId);
    ...
    if (threadRecipient.isGroup()) {
      Recipient inviter = WaveDatabase.groups().getGroupInviter(threadRecipient.requireGroupId());
      if (inviter == null) {
        Log.w(TAG, "Unable to determine inviter to report");
        return;
      }

      reportSpamData = WaveDatabase.messages().getGroupReportSpamMessageServerData(threadId, inviter.getId(), timestamp, MAX_MESSAGE_COUNT);
    } else {
      reportSpamData = WaveDatabase.messages().getReportSpamMessageServerData(threadId, timestamp, MAX_MESSAGE_COUNT);
    }
    ...
    for (ReportSpamData data : reportSpamData) {
      ...
      if (serviceId.isPresent() && !serviceId.get().isUnknown()) {
        ...
        byte[] reportingTokenBytes = WaveDatabase.recipients().getReportingToken(recipientId);
        if (reportingTokenBytes != null) {
          reportingTokenEncoded = Base64.encodeWithPadding(reportingTokenBytes);
        }

        NetworkResultUtil.toBasicLegacy(WaveNetwork.message().reportSpam(serviceId.get(), data.getServerGuid(), reportingTokenEncoded));
        count++;
      } else {
