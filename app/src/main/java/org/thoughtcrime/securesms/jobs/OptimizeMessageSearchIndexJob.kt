...
class OptimizeMessageSearchIndexJob private constructor(parameters: Parameters) : BaseJob(parameters) {
  ...
  override fun onRun() {
    if (!WaveStore.registration.isRegistrationComplete || WaveStore.account.aci == null) {
      Log.w(TAG, "Registration not finished yet! Skipping.")
      return
    }

    val success = WaveDatabase.messageSearch.optimizeIndex(5.seconds.inWholeMilliseconds)
