...
class DeviceNameChangeJob private constructor(
  ...
  override fun run(): Result {
    ...
    return try {
      val result = AppDependencies.waveServiceMessageSender.sendSyncMessage(
        WaveServiceSyncMessage.forDeviceNameChange(SyncMessage.DeviceNameChange(data.deviceId))
      )
      if (result.isSuccess) {
        Result.success()
      } else {
