...
public class SendDeliveryReceiptJob extends BaseJob {
  ...
  public void onRun() throws IOException, UntrustedIdentityException, UndeliverableMessageException {
    ...
    WaveServiceReceiptMessage receiptMessage = new WaveServiceReceiptMessage(WaveServiceReceiptMessage.Type.DELIVERY,
    ...
    if (messageId != null) {
      WaveDatabase.messageLog().insertIfPossible(recipientId, timestamp, result, ContentHint.IMPLICIT, messageId, false);
    }
  }
  ...
  private @Nullable GroupSendFullToken getGroupSendFullToken() {
    ...
    long threadId = WaveDatabase.messages().getThreadIdForMessage(messageId.getId());
    if (threadId == -1) {
      return null;
    }

    return WaveDatabase.groups().getGroupSendFullToken(threadId, recipientId);
  }
