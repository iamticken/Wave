...
public class ResendMessageJob extends BaseJob {
  ...
  protected void onRun() throws Exception {
    if (WaveStore.internal().getDelayResends()) {
    ...
    if (distributionId != null) {
      if (groupId != null) {
        ...
        Optional<GroupRecord> groupRecord = WaveDatabase.groups().getGroupByDistributionId(distributionId);
        ...
        createGroupSendToken = () -> WaveDatabase.groups().getGroupSendFullToken(groupId, recipientId);
      } else {
        Log.d(TAG, "GroupId is not present. Assuming this is a message for a distribution list.");
        DistributionListRecord listRecord = WaveDatabase.distributionLists().getListByDistributionId(distributionId);
    ...
    try {
      ...
      AppDependencies.getProtocolStore().aci().sessions().archiveSessions(recipientId, WaveServiceAddress.DEFAULT_DEVICE_ID);
      AppDependencies.getProtocolStore().aci().sessions().archiveSiblingSessions(recipient.requireServiceId().toProtocolAddress(WaveServiceAddress.DEFAULT_DEVICE_ID));
      AppDependencies.getProtocolStore().pni().sessions().archiveSessions(recipientId, WaveServiceAddress.DEFAULT_DEVICE_ID);
      AppDependencies.getProtocolStore().pni().sessions().archiveSiblingSessions(recipient.requireServiceId().toProtocolAddress(WaveServiceAddress.DEFAULT_DEVICE_ID));
    ...
    if (result.isSuccess() && distributionId != null) {
      List<WaveProtocolAddress> addresses = result.getSuccess()
                                                    .getDevices()
                                                    .stream()
