...
public class RetrieveProfileAvatarJob extends BaseJob {
  ...
  public static void enqueueForceUpdate(Recipient recipient) {
    WaveExecutors.BOUNDED.execute(() -> AppDependencies.getJobManager().add(new RetrieveProfileAvatarJob(recipient, recipient.resolve().getProfileAvatar(), true)));
  }

  public static void enqueueUnblurredAvatar(Recipient recipient) {
    WaveExecutors.BOUNDED.execute(() -> AppDependencies.getJobManager().add(new RetrieveProfileAvatarJob(recipient, recipient.resolve().getProfileAvatar(), true, true)));
  }
