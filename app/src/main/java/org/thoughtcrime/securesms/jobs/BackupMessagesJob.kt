...
import org.wave.libwave.net.SvrBStoreResponse
import org.wave.libwave.zkgroup.VerificationFailedException
import org.wave.protos.resumableuploads.ResumableUpload
...
class BackupMessagesJob private constructor(
  ...
  companion object {
    ...
    private fun isBackupAllowed(): Boolean {
      return when {
        ...
        WaveStore.account.isLinkedDevice -> {
  ...
  override fun run(): Result {
    ...
    if (WaveStore.backup.backupSecretRestoreRequired) {
      ...
      if (forwardSecrecyMetadata != null) {
        when (val result = WaveNetwork.svrB.restore(auth, WaveStore.backup.messageBackupKey, forwardSecrecyMetadata)) {
      ...
      WaveStore.backup.backupSecretRestoreRequired = false
    }

    val backupSecretData = WaveStore.backup.nextBackupSecretData ?: run {
      Log.i(TAG, "First SVRB backup! Creating new backup chain.", true)
      val secretData = WaveNetwork.svrB.createNewBackupChain(auth, WaveStore.backup.messageBackupKey)
      WaveStore.backup.nextBackupSecretData = secretData
      secretData
    }

    val svrBMetadata: SvrBStoreResponse = when (val result = WaveNetwork.svrB.store(auth, WaveStore.backup.messageBackupKey, backupSecretData)) {
      ...
      SvrBApi.StoreResult.InvalidDataError -> {
        ...
        WaveStore.backup.nextBackupSecretData = null
        return Result.retry(defaultBackoff())
      }
    ...
    val createKeyResult = WaveDatabase.attachments.createRemoteKeyForAttachmentsThatNeedArchiveUpload()
    ...
    WaveDatabase.attachments.clearIncrementalMacsForAttachmentsThatNeedArchiveUpload().takeIf { it > 0 }?.let { count -> Log.w(TAG, "Needed to clear $count incrementalMacs.", true) }
    ...
    WaveStore.backup.lastBackupProtoVersion = BackupRepository.VERSION
    ...
    val backupSpec: ResumableMessagesBackupUploadSpec = resumableMessagesBackupUploadSpec ?: when (val result = BackupRepository.getResumableMessagesBackupUploadSpec(tempBackupFile.length())) {
      ...
      is NetworkResult.StatusCodeError -> {
        when (result.code) {
          413 -> {
            Log.i(TAG, "Backup file is too large! Size: ${tempBackupFile.length()} bytes. Current threshold: ${WaveStore.backup.messageCuttoffDuration}", result.getCause(), true)
            ...
            if (WaveStore.backup.messageCuttoffDuration == null) {
              Log.i(TAG, "Setting message cuttoff duration to $TOO_LARGE_MESSAGE_CUTTOFF_DURATION", true)
              WaveStore.backup.messageCuttoffDuration = TOO_LARGE_MESSAGE_CUTTOFF_DURATION
              return Result.retry(defaultBackoff())
            } else {
              return Result.failure()
            }
    ...
    FileInputStream(tempBackupFile).use { fileStream ->
      val uploadResult = WaveNetwork.archive.uploadBackupFile(
      ...
      when (uploadResult) {
        is NetworkResult.Success -> {
          ...
          WaveStore.backup.hasBackupBeenUploaded = true
        }
    ...
    WaveStore.backup.nextBackupSecretData = svrBMetadata.nextBackupSecretData

    WaveStore.backup.lastBackupProtoSize = tempBackupFile.length()
    if (!tempBackupFile.delete()) {
      Log.e(TAG, "Failed to delete temp backup file", true)
    }

    WaveStore.backup.lastBackupTime = System.currentTimeMillis()
    ...
    if (WaveStore.backup.backsUpMedia && WaveDatabase.attachments.doAnyAttachmentsNeedArchiveUpload()) {
      Log.i(TAG, "Enqueuing attachment backfill job.", true)
      AppDependencies.jobManager.add(ArchiveAttachmentBackfillJob())
    } else {
      Log.i(TAG, "No attachments need to be uploaded, we can finish. Tier: ${WaveStore.backup.backupTier}", true)
      ArchiveUploadProgress.onMessageBackupFinishedEarly()
    }

    if (WaveStore.backup.backsUpMedia && WaveDatabase.attachments.doAnyThumbnailsNeedArchiveUpload()) {
      Log.i(TAG, "Enqueuing thumbnail backfill job.", true)
      AppDependencies.jobManager.add(ArchiveThumbnailBackfillJob())
    } else {
      Log.i(TAG, "No thumbnails need to be uploaded: ${WaveStore.backup.backupTier}", true)
    }

    WaveStore.backup.messageCuttoffDuration = null
    WaveStore.backup.lastUsedMessageCutoffTime = messageCutoffTime
    if (messageCutoffTime == 0L) {
      BackupRepository.clearBackupFailure()
    }
    WaveDatabase.backupMediaSnapshots.commitPendingRows()
  ...
  private fun getOrCreateBackupFile(
    ...
    if (System.currentTimeMillis() > syncTime && syncTime > 0L && dataFile.isNotNullOrBlank()) {
      ...
      if (file.exists() && file.canRead() && elapsed < FILE_REUSE_TIMEOUT) {
        ...
        return BackupFileResult.Success(file, syncTime, messageInclusionCutoffTime = WaveStore.backup.lastUsedMessageCutoffTime)
      }
    }
    ...
    val messageInclusionCutoffTime = WaveStore.backup.messageCuttoffDuration?.let { currentTime - it.inWholeMilliseconds } ?: 0

    try {
      BackupRepository.exportForWaveBackup(
        ...
        cancellationWave = { this.isCanceled },
        ...
        if (attachmentInfoBuffer.size > ATTACHMENT_SNAPSHOT_BUFFER_SIZE) {
          WaveDatabase.backupMediaSnapshots.writePendingMediaEntries(attachmentInfoBuffer.toFullSizeMediaEntries(mediaRootBackupKey))
          WaveDatabase.backupMediaSnapshots.writePendingMediaEntries(attachmentInfoBuffer.toThumbnailMediaEntries(mediaRootBackupKey))
    ...
    when (val result = ArchiveValidator.validateWaveBackup(tempBackupFile, backupKey, forwardSecrecyToken)) {
  ...
  private fun maybePostRemoteKeyMissingNotification() {
    if (!RemoteConfig.internalUser || !WaveStore.backup.backsUpMedia) {
      return
    }
  ...
  private fun Set<ArchiveAttachmentInfo>.toThumbnailMediaEntries(mediaRootBackupKey: MediaRootBackupKey): Set<BackupMediaSnapshotTable.MediaEntry> {
    return this
      ...
      .toSet()
      .let { WaveDatabase.attachments.filterPermanentlyFailedThumbnails(it) }
  }
