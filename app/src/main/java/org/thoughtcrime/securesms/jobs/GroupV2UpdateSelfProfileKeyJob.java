...
public final class GroupV2UpdateSelfProfileKeyJob extends BaseJob {
  ...
  public static void enqueueForGroupsIfNecessary() {
    if (!WaveStore.account().isRegistered() || WaveStore.account().getAci() == null || !Recipient.self().isRegistered()) {
    ...
    long timeSinceLastCheck = System.currentTimeMillis() - WaveStore.misc().getLastGv2ProfileCheckTime();
    ...
    WaveStore.misc().setLastGv2ProfileCheckTime(System.currentTimeMillis());

    WaveExecutors.BOUNDED.execute(() -> {
      boolean foundMismatch = false;

      for (GroupId.V2 id : WaveDatabase.groups().getAllGroupV2Ids()) {
        Optional<GroupRecord> group = WaveDatabase.groups().getGroup(id);
        if (!group.isPresent()) {
