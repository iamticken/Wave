...
class Svr3MirrorJob private constructor(parameters: Parameters, private var serializedChangeSession: String?) : Job(parameters) {
  ...
  override fun run(): Result {
    ...
    SvrRepository.operationLock.withLock {
      ...
      val svr3: SecureValueRecoveryV3 = AppDependencies.waveServiceAccountManager.getSecureValueRecoveryV3(AppDependencies.libwaveNetwork)

      val session: PinChangeSession = serializedChangeSession?.let { session ->
        svr3.resumePinChangeSession(pin, WaveStore.svr.masterKey, session)
      } ?: svr3.setPin(pin, WaveStore.svr.masterKey)
      ...
      return when (val response: BackupResponse = session.execute()) {
        is BackupResponse.Success -> {
          ...
          WaveStore.svr.appendSvr3AuthTokenToList(response.authorization.asBasic())
