...
public class RefreshAttributesJob extends BaseJob {
  ...
  public void onRun() throws IOException {
    if (!WaveStore.account().isRegistered() || WaveStore.account().getE164() == null) {
    ...
    int       registrationId              = WaveStore.account().getRegistrationId();
    boolean   fetchesMessages             = !WaveStore.account().isFcmEnabled() || WaveStore.internal().isWebsocketModeForced();
    byte[]    unidentifiedAccessKey       = UnidentifiedAccess.deriveAccessKeyFrom(ProfileKeyUtil.getSelfProfileKey());
    boolean   universalUnidentifiedAccess = TextSecurePreferences.isUniversalUnidentifiedAccess(context);
    String    registrationLockV2          = null;
    SvrValues svrValues                   = WaveStore.svr();
    ...
    boolean phoneNumberDiscoverable = WaveStore.phoneNumberPrivacy().getPhoneNumberDiscoverabilityMode() == PhoneNumberDiscoverabilityMode.DISCOVERABLE;

    String deviceName = WaveStore.account().getDeviceName();
    byte[] encryptedDeviceName = (deviceName == null) ? null : DeviceNameCipher.encryptDeviceName(deviceName.getBytes(StandardCharsets.UTF_8), WaveStore.account().getAciIdentityKey());

    AccountAttributes.Capabilities capabilities = AppCapabilities.getCapabilities(svrValues.hasPin() && !svrValues.hasOptedOut());
    Log.i(TAG, "Calling setAccountAttributes() reglockV2? " + !TextUtils.isEmpty(registrationLockV2) + ", pin? " + svrValues.hasPin() + ", restoredAEP? " + WaveStore.account().restoredAccountEntropyPool() +
    ...
    NetworkResultUtil.toBasicLegacy(WaveNetwork.account().setAccountAttributes(accountAttributes));
