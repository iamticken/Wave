...
import org.wave.paging.PagedDataSource
...
class ConversationDataSource(
  ...
  private val threadRecipient: Recipient by lazy {
    WaveDatabase.threads.getRecipientForThreadId(threadId)!!
  }
  ...
  private fun getSizeInternal(): Int {
    ...
    return WaveDatabase.messages.getMessageCountForThread(threadId)
  }

  override fun load(start: Int, length: Int, totalSize: Int, cancellationWave: PagedDataSource.CancellationWave): List<ConversationElement> {
    ...
    MessageTable.mmsReaderFor(WaveDatabase.messages.getConversation(threadId, start.toLong(), length.toLong()))
      .use { reader ->
  ...
  override fun load(key: ConversationElementKey): ConversationElement? {
    ...
    var record = WaveDatabase.messages.getMessageRecordOrNull(key.id)
