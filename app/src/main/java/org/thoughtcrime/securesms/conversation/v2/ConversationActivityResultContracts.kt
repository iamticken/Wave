...
class ConversationActivityResultContracts(private val fragment: Fragment, private val callbacks: Callbacks) {
  ...
  fun launchSelectContact() {
    Permissions
      ...
      .ifNecessary()
      .withPermanentDenialDialog(fragment.getString(R.string.AttachmentManager_wave_requires_contacts_permission_in_order_to_attach_contact_information))
  ...
  fun launchCamera(recipientId: RecipientId, isReply: Boolean) {
    if (CameraXUtil.isSupported()) {
      ...
      Permissions.with(fragment)
        ...
        .withPermanentDenialDialog(fragment.getString(R.string.CameraXFragment_wave_needs_camera_access_capture_photos), null, R.string.CameraXFragment_allow_access_camera, R.string.CameraXFragment_to_capture_photos_videos, fragment.parentFragmentManager)
        .onAllGranted {
          cameraLauncher.launch(MediaSelectionInput(emptyList(), recipientId, null, isReply))
          fragment.requireActivity().overridePendingTransition(R.anim.camera_slide_from_bottom, R.anim.stationary)
        }
        .onAnyDenied { Toast.makeText(fragment.requireContext(), R.string.CameraXFragment_wave_needs_camera_access_capture_photos, Toast.LENGTH_LONG).show() }
        .execute()
    }
  }
  ...
  fun launchSelectLocation(chatColors: ChatColors) {
    if (Permissions.hasAny(fragment.requireContext(), Manifest.permission.ACCESS_FINE_LOCATION, Manifest.permission.ACCESS_COARSE_LOCATION)) {
      ...
      Permissions.with(fragment)
        ...
        .ifNecessary()
        .withRationaleDialog(fragment.getString(R.string.AttachmentManager_wave_allow_access_location), fragment.getString(R.string.AttachmentManager_wave_allow_wave_access_location), R.drawable.symbol_location_white_24)
        .withPermanentDenialDialog(fragment.getString(R.string.AttachmentManager_wave_requires_location_information_in_order_to_attach_a_location), null, R.string.AttachmentManager_wave_allow_access_location, R.string.AttachmentManager_wave_to_send_location, fragment.parentFragmentManager)
        .onAnyDenied { Toast.makeText(fragment.requireContext(), R.string.AttachmentManager_wave_needs_location_access, Toast.LENGTH_LONG).show() }
  ...
  private object MediaSelection : ActivityResultContract<MediaSelectionInput, MediaSendActivityResult?>() {
    override fun createIntent(context: Context, input: MediaSelectionInput): Intent {
      ...
      return MediaSelectionActivity.editor(context, MessageSendType.WaveMessageSendType, media, recipientId, text)
    }
  ...
  private object MediaCapture : ActivityResultContract<MediaSelectionInput, MediaSendActivityResult?>() {
    override fun createIntent(context: Context, input: MediaSelectionInput): Intent {
      ...
      return MediaSelectionActivity.camera(context, MessageSendType.WaveMessageSendType, recipientId, isReply)
    }
  ...
  private object MediaGallery : ActivityResultContract<MediaSelectionInput, MediaSendActivityResult?>() {
    override fun createIntent(context: Context, input: MediaSelectionInput): Intent {
      ...
      return MediaSelectionActivity.gallery(context, MessageSendType.WaveMessageSendType, media, recipientId, text, isReply)
    }
  ...
  private object GifSearch : ActivityResultContract<GifSearchInput, MediaSendActivityResult?>() {
    override fun createIntent(context: Context, input: GifSearchInput): Intent {
      return Intent(context, GiphyActivity::class.java).apply {
        ...
        putExtra(GiphyActivity.EXTRA_TRANSPORT, MessageSendType.WaveMessageSendType)
  ...
  private object SelectLocation : ActivityResultContract<ChatColors, SelectLocationOutput?>() {
    ...
    override fun parseResult(resultCode: Int, intent: Intent?): SelectLocationOutput? {
      return if (resultCode == Activity.RESULT_OK) {
        intent?.data?.let { uri -> SelectLocationOutput(WavePlace(PlacePickerActivity.addressFromData(intent)), uri) }
      } else {
        null
      }
    }
  ...
  private data class SelectLocationOutput(val place: WavePlace, val uri: Uri)

  interface Callbacks {
    fun onSendContacts(contacts: List<Contact>)
    fun onMediaSend(result: MediaSendActivityResult?)
    fun onContactSelect(uri: Uri?)
    fun onLocationSelected(place: WavePlace?, uri: Uri?)
