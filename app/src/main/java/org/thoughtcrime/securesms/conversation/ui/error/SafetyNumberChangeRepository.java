...
public final class SafetyNumberChangeRepository {
  ...
  public Single<TrustAndVerifyResult> trustOrVerifyChangedRecipientsAndResendRx(@NonNull List<SafetyNumberRecipient> safetyNumberRecipients, @NonNull MessageId messageId) {
    ...
    return Single.fromCallable(() -> {
      MessageRecord messageRecord = WaveDatabase.messages().getMessageRecord(messageId.getId());
  ...
  @NonNull LiveData<TrustAndVerifyResult> trustOrVerifyChangedRecipients(@NonNull List<ChangedRecipient> changedRecipients) {
    ...
    WaveExecutors.BOUNDED.execute(() -> liveData.postValue(trustOrVerifyChangedRecipientsInternal(changedRecipients)));
    return liveData;
  }
  ...
  @NonNull LiveData<TrustAndVerifyResult> trustOrVerifyChangedRecipientsAndResend(@NonNull List<ChangedRecipient> changedRecipients, @NonNull MessageRecord messageRecord) {
    ...
    MutableLiveData<TrustAndVerifyResult> liveData = new MutableLiveData<>();
    WaveExecutors.BOUNDED.execute(() -> liveData.postValue(trustOrVerifyChangedRecipientsAndResendInternal(changedRecipients, messageRecord)));
    return liveData;
  }
  ...
  private @Nullable MessageRecord getMessageRecord(Long messageId, String messageType) {
    try {
      return WaveDatabase.messages().getMessageRecord(messageId);
  ...
  private TrustAndVerifyResult trustOrVerifyChangedRecipientsInternal(@NonNull List<ChangedRecipient> changedRecipients) {
    WaveIdentityKeyStore identityStore = AppDependencies.getProtocolStore().aci().identities();
                                                                               ...
                                                                               @NonNull MessageRecord messageRecord)
  {
    ...
    try (WaveSessionLock.Lock unused = ReentrantSessionLock.INSTANCE.acquire()) {
      for (ChangedRecipient changedRecipient : changedRecipients) {
        WaveProtocolAddress mismatchAddress = changedRecipient.getRecipient().requireServiceId().toProtocolAddress(WaveServiceAddress.DEFAULT_DEVICE_ID);
        ...
        WaveIdentityKeyStore.SaveResult result = AppDependencies.getProtocolStore().aci().identities().saveIdentity(mismatchAddress, newIdentityKey, true);

        Log.d(TAG, "Saving identity result: " + result);
        if (result == WaveIdentityKeyStore.SaveResult.NO_CHANGE) {
          Log.i(TAG, "Archiving sessions explicitly as they appear to be out of sync.");
          AppDependencies.getProtocolStore().aci().sessions().archiveSessions(changedRecipient.getRecipient().getId(), WaveServiceAddress.DEFAULT_DEVICE_ID);
          AppDependencies.getProtocolStore().aci().sessions().archiveSiblingSessions(mismatchAddress);
          WaveDatabase.senderKeyShared().deleteAllFor(changedRecipient.getRecipient().getId());
        }
      }
    }
  ...
  private void processOutgoingMessageRecord(@NonNull List<ChangedRecipient> changedRecipients, @NonNull MessageRecord messageRecord) {
    ...
    for (ChangedRecipient changedRecipient : changedRecipients) {
      ...
      if (messageRecord.isMms()) {
        WaveDatabase.messages().removeMismatchedIdentity(messageRecord.getId(), id, identityKey);
