...
public class ConversationRepository {
  ...
  public @NonNull ConversationData getConversationData(long threadId, @NonNull Recipient conversationRecipient, int jumpToPosition) {
    ThreadTable.ConversationMetadata    metadata                       = WaveDatabase.threads().getConversationMetadata(threadId);
    int                                 threadSize                     = WaveDatabase.messages().getMessageCountForThread(threadId);
    ...
    if (lastSeen > 0) {
      lastSeenPosition = WaveDatabase.messages().getMessagePositionByDateReceivedTimestamp(threadId, lastSeen, false);
    }
    ...
    if (lastSeen == 0 && lastScrolled > 0) {
      lastScrolledPosition = WaveDatabase.messages().getMessagePositionByDateReceivedTimestamp(threadId, lastScrolled, true);
    }
    ...
    if (!isMessageRequestAccepted) {
      ...
      if (conversationRecipient.isGroup()) {
        Optional<GroupRecord> group = WaveDatabase.groups().getGroup(conversationRecipient.getId());
        if (group.isPresent()) {
  ...
  public void markGiftBadgeRevealed(long messageId) {
    WaveExecutors.BOUNDED_IO.execute(() -> {
      List<MessageTable.MarkedMessageInfo> markedMessageInfo = WaveDatabase.messages().setOutgoingGiftsRevealed(Collections.singletonList(messageId));
