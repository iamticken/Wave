...
public final class ContactUtil {
  ...
  public static @Nullable String getNormalizedPhoneNumber(@Nullable String number) {
    return WaveE164Util.formatAsE164(number != null ? number : "");
  }
  ...
  public static List<RecipientId> getRecipients(@NonNull Contact contact) {
    return contact
        .getPhoneNumbers()
        .stream()
        .map(phone -> WaveE164Util.formatAsE164(phone.getNumber()))
        .filter(number -> number != null)
        .map(phone -> WaveDatabase.recipients().getOrInsertFromE164(phone))
