...
class CallLogRepository(
  ...
  override fun getCallLinksCount(query: String?, filter: CallLogFilter): Int {
    return when (filter) {
      ...
      CallLogFilter.ALL, CallLogFilter.AD_HOC -> WaveDatabase.callLinks.getCallLinksCount(query)
    }
  }
  ...
  override fun getCallLinks(query: String?, filter: CallLogFilter, start: Int, length: Int): List<CallLogRow> {
    return when (filter) {
      ...
      CallLogFilter.ALL, CallLogFilter.AD_HOC -> WaveDatabase.callLinks.getCallLinks(query, start, length)
    }
  }
  ...
  fun markAllCallEventsRead() {
    WaveExecutors.BOUNDED_IO.execute {
      val latestCall = WaveDatabase.calls.getLatestCall() ?: return@execute
      WaveDatabase.calls.markAllCallEventsRead()
  ...
  fun deleteSelectedCallLogs(
    ...
    return Completable.fromAction {
      WaveDatabase.calls.deleteNonAdHocCallEvents(selectedCallRowIds)
  ...
  fun deleteAllCallLogsExcept(
    ...
    return Completable.fromAction {
      WaveDatabase.calls.deleteAllNonAdHocCallEventsExcept(selectedCallRowIds, missedOnly)
    }.subscribeOn(Schedulers.io())
  }
  ...
  fun deleteAllCallLogsOnOrBeforeNow(): Single<Int> {
    return Single.fromCallable {
      WaveDatabase.rawDatabase.withinTransaction {
        val latestCall = WaveDatabase.calls.getLatestCall() ?: return@withinTransaction
        WaveDatabase.calls.deleteNonAdHocCallEventsOnOrBefore(latestCall.timestamp)
        WaveDatabase.callLinks.deleteNonAdminCallLinksOnOrBefore(latestCall.timestamp)
        AppDependencies.jobManager.add(CallLogEventSendJob.forClearHistory(latestCall))
      }

      WaveDatabase.callLinks.getAllAdminCallLinksExcept(emptySet())
  ...
  fun deleteSelectedCallLinks(
    ...
    return Single.fromCallable {
      val allCallLinkIds = WaveDatabase.calls.getCallLinkRoomIdsFromCallRowIds(selectedCallRowIds) + selectedRoomIds
      WaveDatabase.callLinks.deleteNonAdminCallLinks(allCallLinkIds)
      WaveDatabase.callLinks.getAdminCallLinks(allCallLinkIds)
  ...
  fun deleteAllCallLinksExcept(
    ...
    return Single.fromCallable {
      val allCallLinkIds = WaveDatabase.calls.getCallLinkRoomIdsFromCallRowIds(selectedCallRowIds) + selectedRoomIds
      WaveDatabase.callLinks.deleteAllNonAdminCallLinksExcept(allCallLinkIds)
      WaveDatabase.callLinks.getAllAdminCallLinksExcept(allCallLinkIds)
    }.flatMap(this::deleteAndCollectResults).subscribeOn(Schedulers.io())
  }
  ...
  private fun deleteAndCollectResults(callLinksToRevoke: Set<CallLinkTable.CallLink>): Single<Int> {
    return Single.merge(
      ...
      WaveDatabase.calls.updateAdHocCallEventDeletionTimestamps()
    }.doOnDispose {
      WaveDatabase.calls.updateAdHocCallEventDeletionTimestamps()
    }
  }
