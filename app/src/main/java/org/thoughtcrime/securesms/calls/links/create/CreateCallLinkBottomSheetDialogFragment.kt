...
class CreateCallLinkBottomSheetDialogFragment : ComposeBottomSheetDialogFragment() {
  ...
  override fun SheetContent() {
    ...
    CreateCallLinkBottomSheetContent(
      ...
      onShareViaWaveClicked = this@CreateCallLinkBottomSheetDialogFragment::onShareViaWaveClicked,
  ...
  private fun onShareViaWaveClicked() {
    lifecycleDisposable += viewModel.commitCallLink().subscribeBy(onSuccess = {
      when (it) {
        is EnsureCallLinkCreatedResult.Success -> {
          startActivity(
            ShareActivity.sendSimpleText(
              requireContext(),
              getString(R.string.CreateCallLink__use_this_link_to_join_a_wave_call, CallLinks.url(viewModel.linkKeyBytes, viewModel.epochBytes))
            )
          )
        }
