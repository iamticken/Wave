...
class CallLogPeekHelper : DefaultLifecycleObserver {
  ...
  private val executor: Executor = SerialExecutor(WaveExecutors.BOUNDED_IO)
  ...
  private fun performPeeks() {
    executor.execute {
      ...
      AppDependencies.waveCallManager.isCallActive(object : ResultReceiver(null) {
      ...
      items.forEach {
        when (it.identifier.peekEntryType) {
          PeekEntryType.CALL_LINK -> AppDependencies.waveCallManager.peekCallLinkCall(it.recipientId)
          PeekEntryType.GROUP_CALL -> AppDependencies.waveCallManager.peekGroupCall(it.recipientId)
        }
      }
