...
object CallQuality {
  ...
  private fun handleCallSummary(callSummary: CallSummary, callType: CallType) {
    if (isCallQualitySurveyRequired(callSummary)) {
      WaveStore.callQuality.surveyRequest = SubmitCallQualitySurveyRequest.Builder()
      ...
      WaveStore.callQuality.surveyRequest = null
    }
  }

  fun consumeQualityRequest(): SubmitCallQualitySurveyRequest? {
    val request = WaveStore.callQuality.surveyRequest
    WaveStore.callQuality.surveyRequest = null
  ...
  private fun isCallQualitySurveyRequired(callSummary: CallSummary): Boolean {
    ...
    val lastFailure = WaveStore.callQuality.lastFailureReportTime ?: 0.milliseconds
    val failureDelta = now - lastFailure

    if (!isSuccess && (failureDelta < 1.days)) {
      WaveStore.callQuality.lastFailureReportTime = now
      return true
    }

    if (isSuccess) {
      val lastSurveyPromptTime = WaveStore.callQuality.lastSurveyPromptTime ?: 0.milliseconds
  ...
  private fun isFeatureEnabled(): Boolean {
    return (RemoteConfig.callQualitySurvey || WaveStore.internal.callQualitySurveys) && WaveStore.callQuality.isQualitySurveyEnabled
  }
