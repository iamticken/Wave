...
object GroupArchiveImporter {
  fun import(group: ArchiveGroup): RecipientId {
    ...
    val recipientId = WaveDatabase.writableDatabase.insert(RecipientTable.TABLE_NAME, null, values)
    val restoredId = WaveDatabase.groups.create(masterKey, decryptedState, groupSendEndorsements = null)
    if (restoredId != null) {
      WaveDatabase.groups.setShowAsStoryState(restoredId, group.storySendMode.toLocal())
    }
...
private fun Group.GroupSnapshot.toLocal(operations: GroupsV2Operations.GroupOperations): DecryptedGroup {
  val selfAciBytes = WaveStore.account.aci?.toByteString()
