...
object RecipientArchiveProcessor {
  ...
  fun export(db: WaveDatabase, waveStore: WaveStore, exportState: ExportState, selfAci: ServiceId.ACI, emitter: BackupFrameEmitter) {
    val releaseChannelId = waveStore.releaseChannelValues.releaseChannelRecipientId
  ...
  fun import(recipient: ArchiveRecipient, importState: ImportState) {
    val newId: RecipientId? = when {
      ...
      recipient.releaseNotes != null -> WaveDatabase.recipients.restoreReleaseNotes()
      recipient.callLink != null -> CallLinkArchiveImporter.import(recipient.callLink)
      recipient.self != null -> {
        WaveDatabase.writableDatabase
