...
typealias ArchiveResult = org.wave.core.util.Result<Unit, LocalArchiver.FailureCause>
...
object LocalArchiver {
   ...
   */
  fun export(snapshotFileSystem: SnapshotFileSystem, filesFileSystem: FilesFileSystem, stopwatch: Stopwatch, cancellationWave: () -> Boolean = { false }): ArchiveResult {
    ...
    try {
      ...
      BackupRepository.exportForLocalBackup(mainStream, LocalExportProgressListener(), cancellationWave) { attachment, source ->
        if (cancellationWave()) {
