...
import org.wave.core.util.ParallelEventTimer
...
import org.wave.core.util.emptyIfNull
...
class ChatItemArchiveExporter(
  ...
  private fun fetchExtraMessageData(db: WaveDatabase, messageIds: Set<Long>): ExtraMessageData {
    val executor = WaveExecutors.BOUNDED
...
private fun BackupMessageRecord.toRemoteGroupUpdate(): ChatUpdateMessage? {
  ...
  body?.let { body ->
    return try {
      ...
      ChatUpdateMessage(
        groupChange = GroupsV2UpdateMessageConverter.translateDecryptedChange(selfIds = WaveStore.account.getServiceIds(), context)
      )
    } catch (e: IOException) {
...
private fun BackupMessageRecord.toRemotePaymentNotificationUpdate(db: WaveDatabase): PaymentNotification {
...
private fun BackupMessageRecord.getBodyText(attachments: List<DatabaseAttachment>?): Pair<String, DatabaseAttachment?> {
  val longTextAttachment = attachments?.firstOrNull { it.contentType == "text/x-wave-plain" }
