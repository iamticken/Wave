...
object ChatFolderArchiveProcessor {
  ...
  fun export(db: WaveDatabase, exportState: ExportState, emitter: BackupFrameEmitter) {
    val folders = db
      .chatFoldersTable
  ...
  fun import(chatFolder: ChatFolderProto, importState: ImportState) {
    ...
    includedChatsQueries.forEach {
      WaveDatabase.writableDatabase.execSQL(it.where, it.whereArgs)
    }
...
private fun List<Long>.toMembershipInsertQueries(chatFolderId: Long, importState: ImportState, membershipType: MembershipType): List<SqlUtil.Query> {
  val values = this
    ...
    .map { recipientId -> importState.recipientIdToLocalThreadId[recipientId] ?: WaveDatabase.threads.getOrCreateThreadIdFor(recipientId, importState.recipientIdToIsGroup[recipientId] == true) }
    .map { threadId ->
      contentValuesOf(
