...
object BackupAlertDelegate {
    ...
    lifecycle.coroutineScope.launch {
      lifecycle.repeatOnLifecycle(Lifecycle.State.RESUMED) {
        if (BackupRepository.shouldDisplayBackupFailedSheet()) {
          ...
          BackupAlertBottomSheet.create(BackupAlert.CouldNotCompleteBackup(daysSinceLastBackup = WaveStore.backup.daysSinceLastBackup)).show(fragmentManager, FRAGMENT_TAG)
  ...
  private suspend fun displayBackupDownloadNotifier(fragmentManager: FragmentManager) {
    val downloadYourBackupToday = withContext(WaveDispatchers.IO) { BackupRepository.getDownloadYourBackupData() }
