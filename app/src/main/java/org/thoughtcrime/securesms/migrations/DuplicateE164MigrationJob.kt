...
internal class DuplicateE164MigrationJob(
    ...
    for ((_, entries) in dupes) {
      ...
      WaveDatabase.rawDatabase.withinTransaction {
        ...
        for (entry in resolved.drop(1)) {
          ...
          WaveDatabase.recipients.mergeForMigration(first.id, entry.id)
        }
      }
    }
  ...
  private fun attemptToGetRidOfE164(entries: List<E164Entry>): List<E164Entry> {
    ...
    for (invalidPrefix in entries) {
      if (WaveDatabase.recipients.removeE164IfAnotherIdentifierIsPresent(invalidPrefix.id)) {
      ...
      if (WaveDatabase.recipients.deleteRecipientIfItHasNoMessages(invalidPrefix.id)) {
