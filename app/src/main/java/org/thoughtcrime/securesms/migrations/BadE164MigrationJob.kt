...
internal class BadE164MigrationJob(
  ...
  override fun performMigration() {
    ...
    val invalidWithOtherIdentifiersCount = WaveDatabase.recipients.removeInvalidE164sFromRecipientsWithOtherIdentifiers()
    ...
    val invalidWithNoMessagesCount = WaveDatabase.recipients.deleteRecipientsWithInvalidE164sAndNoMessages()
    Log.d(TAG, "Deleted $invalidWithNoMessagesCount recipients with invalid e164's and no messages.")
    stopwatch.split("no-msg")

    val invalidEntries = WaveDatabase.recipients.getRecipientsWithInvalidE164s()
    ...
    for (entry in invalidEntries) {
      ...
      val formattedNumber = WaveE164Util.formatAsE164(entry.e164)
      ...
      WaveDatabase.recipients.updateE164(entry.id, entry.e164)
