...
public class LegacyMigrationJob extends MigrationJob {
  ...
  void performMigration() throws RetryLaterException {
    ...
    if (lastSeenVersion < SQLCIPHER && masterSecret != null) {
      WaveDatabase.onApplicationLevelUpgrade(context, masterSecret, lastSeenVersion, (progress, total) -> {
...
//      }
    ...
    if (lastSeenVersion < COLOR_MIGRATION) {
      ...
      WaveDatabase.recipients().updateSystemContactColors();
    ...
    if (lastSeenVersion < SIGNALING_KEY_DEPRECATION) {
      Log.i(TAG, "Scheduling a RefreshAttributesJob to remove the waveing key remotely.");
      AppDependencies.getJobManager().add(new RefreshAttributesJob());
    }
  }
  ...
  private void schedulePendingIncomingParts(Context context) {
    final AttachmentTable          attachmentDb       = WaveDatabase.attachments();
    final MessageTable             mmsDb              = WaveDatabase.messages();
    final List<DatabaseAttachment> pendingAttachments = WaveDatabase.attachments().getPendingAttachments();
