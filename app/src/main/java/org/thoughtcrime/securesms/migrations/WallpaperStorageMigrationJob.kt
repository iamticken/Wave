...
internal class WallpaperStorageMigrationJob(parameters: Parameters = Parameters.Builder().build()) : MigrationJob(parameters) {
  ...
  override fun performMigration() {
    ...
    val currentDefaultWallpaperUri = WaveStore.wallpaper.currentRawWallpaper?.file_?.uri
    ...
    for (filename in wallpaperFileNames) {
      ...
      val inputStream = try {
        ...
        val updatedUserCount = WaveDatabase.recipients.migrateWallpaperUri(
          legacyUri = legacyUri,
          newUri = null
        )
      ...
      val wallpaperAttachmentId = WaveDatabase.attachments.insertWallpaper(inputStream)
      ...
      if (currentDefaultWallpaperUri == legacyUri.toString()) {
        ...
        WaveStore.wallpaper.setRawWallpaperForMigration(Wallpaper(file_ = Wallpaper.File(uri = newUri.toString())))
      }
