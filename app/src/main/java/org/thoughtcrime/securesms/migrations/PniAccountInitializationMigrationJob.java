...
public class PniAccountInitializationMigrationJob extends MigrationJob {
  ...
  public void performMigration() throws IOException {
    ...
    PNI pni = WaveStore.account().getPni();

    if (pni == null || WaveStore.account().getAci() == null || !Recipient.self().isRegistered()) {
    ...
    WaveServiceAccountDataStore protocolStore  = AppDependencies.getProtocolStore().pni();
    PreKeyMetadataStore           metadataStore  = WaveStore.account().pniPreKeys();
    ...
    if (!metadataStore.isSignedPreKeyRegistered()) {
      ...
      NetworkResultUtil.toPreKeysLegacy(WaveNetwork.keys().setPreKeys(new PreKeyUpload(ServiceIdType.PNI, signedPreKey, oneTimePreKeys, null, null)));
