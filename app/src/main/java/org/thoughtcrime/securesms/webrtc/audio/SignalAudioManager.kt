...
import org.thoughtcrime.securesms.audio.WaveBluetoothManager
import org.thoughtcrime.securesms.dependencies.AppDependencies
import org.thoughtcrime.securesms.recipients.RecipientId
import org.thoughtcrime.securesms.util.safeUnregisterReceiver
import org.whispersystems.waveservice.api.util.Preconditions

private val TAG = Log.tag(WaveAudioManager::class.java)

sealed class WaveAudioManager(protected val context: Context, protected val eventListener: EventListener?) {

  private var commandAndControlThread = WaveExecutors.getAndStartHandlerThread("call-audio", ThreadUtil.PRIORITY_IMPORTANT_BACKGROUND_THREAD)
  protected val handler = WaveAudioHandler(commandAndControlThread.looper)
  ...
  companion object {
    @JvmStatic
    fun create(context: Context, eventListener: EventListener?): WaveAudioManager {
      return if (Build.VERSION.SDK_INT >= 31) {
        FullWaveAudioManagerApi31(context, eventListener)
      } else {
        FullWaveAudioManager(context, eventListener)
      }
    }
  }
 ...
 */
class FullWaveAudioManager(context: Context, eventListener: EventListener?) : WaveAudioManager(context, eventListener), AudioDeviceUpdatedListener {
  private val waveBluetoothManager = WaveBluetoothManager(context, this, handler)
  ...
  private var previousBluetoothState: WaveBluetoothManager.State? = null
  ...
  override fun onAudioDeviceUpdated() {
    ...
    Log.i(
      ...
      "updateAudioDeviceState(): " +
        ...
        "bt: ${waveBluetoothManager.state} " +
        "available: $audioDevices " +
        "selected: $selectedAudioDevice " +
        "userSelected: $userSelectedAudioDevice"
    )

    if (waveBluetoothManager.state.shouldUpdate()) {
      waveBluetoothManager.updateDevice()
    }

    val newAudioDevices = mutableSetOf(AudioDevice.SPEAKER_PHONE)

    if (waveBluetoothManager.state.hasDevice()) {
    ...
    if (waveBluetoothManager.state == WaveBluetoothManager.State.UNAVAILABLE && userSelectedAudioDevice == AudioDevice.BLUETOOTH) {
    ...
    val needBluetoothAudioStart = waveBluetoothManager.state == WaveBluetoothManager.State.AVAILABLE &&
    ...
    val needBluetoothAudioStop = (waveBluetoothManager.state == WaveBluetoothManager.State.CONNECTED || waveBluetoothManager.state == WaveBluetoothManager.State.CONNECTING) &&
      (userSelectedAudioDevice != AudioDevice.NONE && userSelectedAudioDevice != AudioDevice.BLUETOOTH)

    if (waveBluetoothManager.state.hasDevice()) {
      Log.i(TAG, "Need bluetooth audio: state: ${waveBluetoothManager.state} start: $needBluetoothAudioStart stop: $needBluetoothAudioStop")
    }

    if (needBluetoothAudioStop) {
      waveBluetoothManager.stopScoAudio()
      waveBluetoothManager.updateDevice()
    }

    if (!autoSwitchToBluetooth && waveBluetoothManager.state == WaveBluetoothManager.State.UNAVAILABLE) {
      autoSwitchToBluetooth = true
    }

    if (needBluetoothAudioStart && !needBluetoothAudioStop) {
      if (!waveBluetoothManager.startScoAudio()) {
        audioDevices.remove(AudioDevice.BLUETOOTH)
        audioDeviceSetUpdated = true
      }
    }

    if (autoSwitchToBluetooth && waveBluetoothManager.state == WaveBluetoothManager.State.CONNECTED) {
      userSelectedAudioDevice = AudioDevice.BLUETOOTH
      autoSwitchToBluetooth = false
    }

    if (previousBluetoothState != null && previousBluetoothState != WaveBluetoothManager.State.PERMISSION_DENIED && waveBluetoothManager.state == WaveBluetoothManager.State.PERMISSION_DENIED) {
      eventListener?.onBluetoothPermissionDenied()
    }
    previousBluetoothState = waveBluetoothManager.state
