...
public class PartAuthority {
      ...
      throws IOException
  {
    ...
    try {
      switch (match) {
      case PART_ROW:          return WaveDatabase.attachments().getAttachmentStream(new PartUriParser(uri).getPartId(), 0);
      case STICKER_ROW:       return WaveDatabase.stickers().getStickerStream(ContentUris.parseId(uri));
      ...
      case THUMBNAIL_ROW:     return WaveDatabase.attachments().getAttachmentThumbnailStream(new PartUriParser(uri).getPartId(), 0);
  ...
  public static @Nullable String getAttachmentFileName(@NonNull Context context, @NonNull Uri uri) {
    ...
    switch (match) {
    case PART_ROW:
      Attachment attachment = WaveDatabase.attachments().getAttachment(new PartUriParser(uri).getPartId());
  ...
  public static @Nullable AttachmentTable.TransformProperties getAttachmentTransformProperties(@NonNull Uri uri) {
    ...
    switch (match) {
      case PART_ROW:
        return WaveDatabase.attachments().getTransformProperties(new PartUriParser(uri).getPartId());
      default:
        return null;
    }
  }
