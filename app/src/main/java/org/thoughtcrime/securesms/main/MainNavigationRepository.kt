...
object MainNavigationRepository {
  ...
  fun getNumberOfUnreadMessages(): Flow<Long> {
    return RxDatabaseObserver.conversationList.map { WaveDatabase.threads.getUnreadMessageCount() }.asFlow()
  }
  ...
  fun getHasFailedOutgoingStories(): Flow<Boolean> {
    return RxDatabaseObserver.conversationList.map { WaveDatabase.messages.hasFailedOutgoingStory() }.asFlow()
  }

  fun getNumberOfUnseenCalls(): Flow<Long> {
    return RxDatabaseObserver.conversationList.map { WaveDatabase.calls.getUnreadMissedCallCount() }.asFlow()
  }
}
