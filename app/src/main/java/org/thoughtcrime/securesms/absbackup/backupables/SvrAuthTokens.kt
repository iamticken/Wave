...
object SvrAuthTokens : AndroidBackupItem {
  ...
  override fun getDataForBackup(): ByteArray {
    val proto = SvrAuthToken(svr2Tokens = WaveStore.svr.svr2AuthTokens)
    return proto.encode()
  }

  override fun restoreData(data: ByteArray) {
    if (WaveStore.svr.svr2AuthTokens.isNotEmpty()) {
      return
    }
    ...
    try {
      ...
      WaveStore.svr.putSvr2AuthTokens(proto.svr2Tokens)
    } catch (e: IOException) {
