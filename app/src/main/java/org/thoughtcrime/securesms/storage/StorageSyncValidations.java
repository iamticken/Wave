...
public final class StorageSyncValidations {
  ...
  public static void validate(@NonNull StorageSyncHelper.WriteOperationResult result,
                              @NonNull WaveStorageManifest previousManifest,
  ...
  public static void validateForcePush(@NonNull WaveStorageManifest manifest, @NonNull List<WaveStorageRecord> inserts, @NonNull Recipient self) {
    validateManifestAndInserts(manifest, inserts, self);
  }

  private static void validateManifestAndInserts(@NonNull WaveStorageManifest manifest, @NonNull List<WaveStorageRecord> inserts, @NonNull Recipient self) {
    int accountCount = 0;
    ...
    Set<StorageId>  insertSet = new HashSet<>(Stream.of(inserts).map(WaveStorageRecord::getId).toList());
