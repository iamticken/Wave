...
class DataAndStorageSettingsViewModel(
  ...
  fun setCallDataMode(callDataMode: CallDataMode) {
    WaveStore.settings.callDataMode = callDataMode
    AppDependencies.waveCallManager.dataModeUpdate()
    getStateAndCopyStorageUsage()
  }

  fun setSentMediaQuality(sentMediaQuality: SentMediaQuality) {
    WaveStore.settings.sentMediaQuality = sentMediaQuality
    getStateAndCopyStorageUsage()
  }
  ...
  private fun getState() = DataAndStorageSettingsState(
    ...
    callDataMode = WaveStore.settings.callDataMode,
    isProxyEnabled = WaveStore.proxy.isProxyEnabled,
    sentMediaQuality = WaveStore.settings.sentMediaQuality
  )
