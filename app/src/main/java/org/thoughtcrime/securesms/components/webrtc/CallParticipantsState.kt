...
data class CallParticipantsState(
  ...
  val listParticipants: List<CallParticipant>
    get() {
      ...
      if (foldableState.isFlat && !WaveStore.internal.newCallingUi) {
  ...
  fun getPreJoinGroupDescription(context: Context): String? {
    ...
    return if (remoteParticipants.isEmpty) {
      describeGroupMembers(
        context = context,
        noParticipants = if (isCallLink) R.string.WebRtcCallView__wave_call_link else null,
        oneParticipant = if (ringGroup) R.string.WebRtcCallView__wave_will_ring_s else R.string.WebRtcCallView__s_will_be_notified,
        twoParticipants = if (ringGroup) R.string.WebRtcCallView__wave_will_ring_s_and_s else R.string.WebRtcCallView__s_and_s_will_be_notified,
        multipleParticipants = if (ringGroup) R.plurals.WebRtcCallView__wave_will_ring_s_s_and_d_others else R.plurals.WebRtcCallView__s_s_and_d_others_will_be_notified,
