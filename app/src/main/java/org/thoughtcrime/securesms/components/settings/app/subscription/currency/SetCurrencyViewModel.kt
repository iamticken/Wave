...
class SetCurrencyViewModel(
  ...
  private val store = Store(
    SetCurrencyState(
      selectedCurrencyCode = if (inAppPaymentType.recurring) {
        WaveStore.inAppPayments.getRecurringDonationCurrency().currencyCode
      } else {
        WaveStore.inAppPayments.getOneTimeCurrency().currencyCode
      },
  ...
  fun setSelectedCurrency(selectedCurrencyCode: String) {
    ...
    if (!inAppPaymentType.recurring) {
      WaveStore.inAppPayments.setOneTimeCurrency(Currency.getInstance(selectedCurrencyCode))
    } else {
