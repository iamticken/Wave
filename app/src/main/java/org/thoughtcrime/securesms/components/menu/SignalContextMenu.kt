 ...
 */
class WaveContextMenu private constructor(
  val anchor: View,
  ...
  LayoutInflater.from(anchor.context).inflate(R.layout.wave_context_menu, null),
  ...
  private val contextMenuList = ContextMenuList(
    recyclerView = contentView.findViewById(R.id.wave_context_menu_list),
    onItemClick = { dismiss() }
  )

  init {
    setBackgroundDrawable(ContextCompat.getDrawable(context, R.drawable.wave_context_menu_background))
  ...
  private fun show(): WaveContextMenu {
  ...
  class Builder(
    ...
    fun show(items: List<ActionItem>): WaveContextMenu {
