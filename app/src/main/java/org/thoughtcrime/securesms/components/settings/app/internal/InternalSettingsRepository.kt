...
class InternalSettingsRepository(context: Context) {
  ...
  fun enqueueSubscriptionRedemption() {
    WaveExecutors.BOUNDED.execute {
      val latest = WaveDatabase.inAppPayments.getByLatestEndOfPeriod(InAppPaymentType.RECURRING_DONATION)
      if (latest != null) {
  ...
  fun addSampleReleaseNote() {
    WaveExecutors.UNBOUNDED.execute {
      ...
      val recipientId = WaveStore.releaseChannel.releaseChannelRecipientId!!
      val threadId = WaveDatabase.threads.getOrCreateThreadIdFor(Recipient.resolved(recipientId))
      ...
      val insertResult: MessageTable.InsertResult? = ReleaseChannel.insertReleaseChannelMessage(
        ...
        media = "/static/release-notes/wave.png",
        mediaWidth = 1800,
        mediaHeight = 720
      )

      WaveDatabase.messages.insertBoostRequestMessage(recipientId, threadId)
