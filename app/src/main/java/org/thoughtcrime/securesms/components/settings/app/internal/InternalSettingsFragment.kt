...
import org.wave.core.util.AppUtil
...
class InternalSettingsFragment : DSLSettingsFragment(R.string.preferences__internal_preferences) {
  ...
  override fun onPause() {
    ...
    if (firstVisiblePosition != null) {
      WaveStore.internal.lastScrollPosition = firstVisiblePosition
    }
  }

  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    super.onViewCreated(view, savedInstanceState)
    scrollToPosition = WaveStore.internal.lastScrollPosition
    ...
    return configure {
        ...
        onClick = {
          MaterialAlertDialogBuilder(requireContext())
            ...
            .setPositiveButton(android.R.string.ok) { _, _ ->
              WaveDatabase.signedPreKeys.debugDeleteAll()
              WaveDatabase.oneTimePreKeys.debugDeleteAll()
              WaveDatabase.kyberPreKeys.debugDeleteAll()
      ...
      if (InAppPaymentsRepository.getSubscriber(InAppPaymentSubscriberRecord.Type.DONATION) != null) {
        ...
        clickPref(
          ...
          onClick = {
            WaveStore.inAppPayments.setLastEndOfPeriod(0L)
      ...
      if (state.hasPendingOneTimeDonation) {
        clickPref(
          ...
          onClick = {
            WaveStore.inAppPayments.setPendingOneTimeDonation(null)
          }
        )
      } else {
        clickPref(
      ...
      clickPref(
        ...
        onClick = {
          WaveStore.releaseChannel.highestVersionNoteReceived = max(WaveStore.releaseChannel.highestVersionNoteReceived - 10, 0)
        }
      )
      ...
      clickPref(
        ...
        onClick = {
          WaveDatabase.remoteMegaphones.debugRemoveAll()
      ...
      clickPref(
        ...
        onClick = {
          WaveStore.releaseChannel.previousManifestMd5 = ByteArray(0)
      ...
      clickPref(
        ...
        isEnabled = true,
        onClick = {
          WaveStore.story.userHasBeenNotifiedAboutStories = false
        }
      )
      ...
      clickPref(
        ...
        isEnabled = true,
        onClick = {
          WaveStore.story.userHasSeenFirstNavView = false
        }
      )
        ...
        onClick = {
          MaterialAlertDialogBuilder(requireContext())
            ...
            .setPositiveButton(android.R.string.ok) { _, _ ->
              ...
              WaveStore.account.username = random
              WaveDatabase.recipients.setUsername(Recipient.self().id, random)
          ...
          MaterialAlertDialogBuilder(requireContext())
            ...
            .setPositiveButton(android.R.string.ok) { _, _ ->
              WaveStore.account.usernameLink = UsernameLinkComponents(
                ...
                serverId = WaveStore.account.usernameLink?.serverId ?: UUID.randomUUID()
              )
              StorageSyncHelper.scheduleSyncForDataChange()
              Toast.makeText(context, "Done", Toast.LENGTH_SHORT).show()
            }
      ...
      clickPref(
        ...
        onClick = {
          WaveStore.uiHints.resetNeverDisplayPullToRefreshCount()
        }
      )
  ...
  private fun onUnregisterClicked() {
    MaterialAlertDialogBuilder(requireContext())
      .setTitle("Unregister?")
      .setMessage("Are you sure? You'll have to re-register to use Wave again -- no promises that the process will go smoothly.")
      .setPositiveButton(android.R.string.ok) { _, _ ->
  ...
  private fun clearAllServiceIds() {
    MaterialAlertDialogBuilder(requireContext())
      ...
      .setPositiveButton(android.R.string.ok) { _, _ ->
        WaveDatabase.recipients.debugClearServiceIds()
  ...
  private fun clearAllProfileKeys() {
    MaterialAlertDialogBuilder(requireContext())
      ...
      .setMessage("Are you sure? Never do this on a non-test device.")
      .setPositiveButton(android.R.string.ok) { _, _ ->
        WaveDatabase.recipients.debugClearProfileData()
  ...
  private fun logPreKeyIds() {
    SimpleTask.run({
      val oneTimePreKeys = WaveDatabase.rawDatabase
  ...
  private fun promptUserForSentTimestamp() {
    ...
    MaterialAlertDialogBuilder(requireContext())
      ...
      .setPositiveButton(android.R.string.ok) { _, _ ->
        ...
        val messages = WaveDatabase.messages.getMessagesBySentTimestamp(number)
        if (messages.isEmpty()) {
        ...
        val startingPosition = WaveDatabase.messages.getMessagePositionInConversation(message.threadId, message.dateReceived)
