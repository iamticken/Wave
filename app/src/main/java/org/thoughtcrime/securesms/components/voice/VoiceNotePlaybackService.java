...
public class VoiceNotePlaybackService extends MediaSessionService {
    ...
    ContextCompat.getMainExecutor(getApplicationContext()).execute(() -> {
      ...
      for (int i = player.getMediaItemCount() - 1; i >= 0; i--) {
        ...
        try {
          ...
          DatabaseAttachment attachment = WaveDatabase.attachments().getAttachment(partId);
   ...
   * If that doesn't work, it returns null, waveing the {@link MediaSession} cannot be built on this device.
  ...
  private void sendViewedReceiptForCurrentWindowIndex() {
        ...
        player.getCurrentMediaItemIndex() != C.INDEX_UNSET)
    {
      ...
      WaveExecutors.BOUNDED.execute(() -> {
        ...
        MessageTable messageDatabase = WaveDatabase.messages();
  ...
  /**
   * Receiver to stop playback and kill the notification if user locks wave via screen lock.
