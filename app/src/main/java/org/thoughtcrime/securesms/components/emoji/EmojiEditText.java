...
public class EmojiEditText extends AppCompatEditText {
  ...
  public EmojiEditText(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    if (!isInEditMode() && !WaveStore.settings().isPreferSystemEmoji()) {
  ...
  public boolean onTextContextMenuItem(int id) {
    if (id == android.R.id.paste) {
      ...
      if (clipData != null) {
        ...
        if (TextUtils.equals(Util.COPY_LABEL, label) && shouldPersistWaveStylingWhenPasting()) {
  ...
  protected boolean shouldPersistWaveStylingWhenPasting() {
    return false;
  }
}
