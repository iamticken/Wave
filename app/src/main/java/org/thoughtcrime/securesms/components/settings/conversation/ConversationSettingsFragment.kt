...
import org.wave.core.util.getParcelableArrayListExtraCompat
...
class ConversationSettingsFragment : DSLSettingsFragment(
  ...
  private val alertTint by lazy { ContextCompat.getColor(requireContext(), R.color.wave_alert_primary) }
  private val alertDisabledTint by lazy { ContextCompat.getColor(requireContext(), R.color.wave_alert_primary_50) }
  ...
  override fun getMaterial3OnScrollHelper(toolbar: Toolbar?): Material3OnScrollHelper {
    return object : Material3OnScrollHelper(activity = requireActivity(), views = listOf(toolbar!!), lifecycleOwner = viewLifecycleOwner) {
      override val inactiveColorSet = ColorSet(
        toolbarColorRes = R.color.wave_colorBackground_0,
        statusBarColorRes = R.color.wave_colorBackground
      )
    }
  }
    ...
    return configure {
          ...
          onAddToStoryClick = {
            if (state.recipient.isPushV2Group && state.requireGroupSettingsState().isAnnouncementGroup && !state.requireGroupSettingsState().isSelfAdmin) {
              ...
              Permissions.with(this@ConversationSettingsFragment)
                ...
                .withPermanentDenialDialog(getString(R.string.CameraXFragment_wave_needs_camera_access_capture_photos), null, R.string.CameraXFragment_allow_access_camera, R.string.CameraXFragment_to_capture_photos_videos, getParentFragmentManager())
                .onAllGranted { addToGroupStoryDelegate.addToStory(state.recipient.id) }
                .onAnyDenied { Toast.makeText(requireContext(), R.string.CameraXFragment_wave_needs_camera_access_capture_photos, Toast.LENGTH_LONG).show() }
                .execute()
            }
          },
          onVideoClick = {
      ...
      if (state.recipient.isReleaseNotes) {
        ...
        clickPref(
          ...
          title = DSLSettingsText.from(R.string.preferences__donate_to_wave),
      ...
      if (state.canModifyBlockedState) {
        ...
        if (!state.recipient.isReleaseNotes) {
          val reportSpamTint = if (state.isDeprecatedOrUnregistered) R.color.wave_alert_primary_50 else R.color.wave_alert_primary
          clickPref(
