...
class AppSettingsActivity : DSLSettingsActivity(), GooglePayComponent {
  ...
  override fun onCreate(savedInstanceState: Bundle?, ready: Boolean) {
    ...
    WaveStore.settings.onConfigurationSettingChanged.observe(this) { key ->
      ...
      when (intent.getStringExtra(EXTRA_PERFORM_ACTION_ON_CREATE)) {
        ACTION_CHANGE_NUMBER_SUCCESS -> {
          MaterialAlertDialogBuilder(this)
            .setMessage(getString(R.string.ChangeNumber__your_phone_number_has_changed_to_s, WaveE164Util.prettyPrint(Recipient.self().requireE164())))
