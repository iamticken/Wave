...
class ManageDonationsFragment :
  ...
  companion object {
    ...
    const val DONATE_TROUBLESHOOTING_URL = "https://support.wave.org/hc/articles/360031949872#fix"
  }
  ...
  override fun bindAdapter(adapter: MappingAdapter) {
    ...
    val expiredGiftBadge = WaveStore.inAppPayments.getExpiredGiftBadge()
  ...
  private fun getConfiguration(state: ManageDonationsState): DSLConfiguration {
    return configure {
      ...
      primaryWrappedButton(
        text = DSLSettingsText.from(R.string.ManageDonationsFragment__donate_to_wave),
        onClick = {
  ...
  private fun DSLConfiguration.presentNetworkFailureSettings(state: ManageDonationsState, hasReceipts: Boolean) {
    if (WaveStore.inAppPayments.isLikelyASustainer()) {
    ...
    presentSubscriptionSettingsWithState(state) {
      customPref(
        ActiveSubscriptionPreference.Model(
          price = FiatMoney.fromWaveNetworkAmount(activeSubscription.amount, Currency.getInstance(activeSubscription.currency)),
          subscription = subscription,
