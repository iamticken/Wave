...
class NotificationProfilesRepository {
  private val database: NotificationProfileTables = WaveDatabase.notificationProfiles
  ...
  fun manuallyToggleProfile(profileId: Long, schedule: NotificationProfileSchedule, now: Long = System.currentTimeMillis()): Completable {
    return Completable.fromAction {
      ...
      if (profileId == activeProfile?.id) {
        ...
        WaveStore.notificationProfile.manuallyDisabledAt = now
        WaveStore.notificationProfile.lastProfilePopup = 0
        WaveStore.notificationProfile.lastProfilePopupTime = 0
      } else {
        val inScheduledWindow = schedule.isCurrentlyActive(now)
        WaveStore.notificationProfile.manuallyEnabledProfile = profileId
        WaveStore.notificationProfile.manuallyEnabledUntil = if (inScheduledWindow) schedule.endDateTime(now.toLocalDateTime()).toMillis() else Long.MAX_VALUE
        WaveStore.notificationProfile.manuallyDisabledAt = now
      }
    }
      .doOnComplete {
  ...
  fun manuallyEnableProfileForDuration(profileId: Long, enableUntil: Long, now: Long = System.currentTimeMillis()): Completable {
    return Completable.fromAction {
      WaveStore.notificationProfile.manuallyEnabledProfile = profileId
      WaveStore.notificationProfile.manuallyEnabledUntil = enableUntil
      WaveStore.notificationProfile.manuallyDisabledAt = now
    }
      .doOnComplete {
        scheduleManualOverrideSync()
  ...
  fun manuallyEnableProfileForSchedule(profileId: Long, schedule: NotificationProfileSchedule, now: Long = System.currentTimeMillis()): Completable {
    return Completable.fromAction {
      val inScheduledWindow = schedule.isCurrentlyActive(now)
      WaveStore.notificationProfile.manuallyEnabledProfile = if (inScheduledWindow) profileId else 0
      WaveStore.notificationProfile.manuallyEnabledUntil = if (inScheduledWindow) schedule.endDateTime(now.toLocalDateTime()).toMillis() else Long.MAX_VALUE
      WaveStore.notificationProfile.manuallyDisabledAt = if (inScheduledWindow) now else 0
    }
      .doOnComplete {
        scheduleManualOverrideSync()
        AppDependencies.databaseObserver.notifyNotificationProfileObservers()
  ...
  fun scheduleNotificationProfileSync(profileId: Long) {
    WaveDatabase.notificationProfiles.markNeedsSync(profileId)
