...
class InternalStorageServicePlaygroundViewModel : ViewModel() {
  ...
  private val _manifest: MutableState<WaveStorageManifest> = mutableStateOf(WaveStorageManifest.EMPTY)
  val manifest: State<WaveStorageManifest>
    get() = _manifest

  private val _storageItems: MutableState<List<WaveStorageRecord>> = mutableStateOf(emptyList())
  val storageRecords: State<List<WaveStorageRecord>>
    get() = _storageItems
  ...
  fun onViewTabSelected() {
    viewModelScope.launch {
      withContext(Dispatchers.IO) {
        ...
        val storageKey = WaveStore.storageService.storageKeyForInitialDataRestore ?: WaveStore.storageService.storageKey
  ...
  private fun WaveStorageRecord.sizeInBytes(): Int {
