...
class AdvancedPrivacySettingsViewModel(
  ...
  fun setCensorshipCircumventionEnabled(enabled: Boolean) {
    WaveStore.settings.setCensorshipCircumventionEnabled(enabled)
  ...
  private fun getState(): AdvancedPrivacySettingsState {
    ...
    return AdvancedPrivacySettingsState(
      isPushEnabled = WaveStore.account.isRegistered,
  ...
  private fun getCensorshipCircumventionState(): CensorshipCircumventionState {
    val countryCode: Int = WaveE164Util.getLocalCountryCode()
    val isCountryCodeCensoredByDefault: Boolean = AppDependencies.waveServiceNetworkAccess.isCountryCodeCensoredByDefault(countryCode)
    val enabledState: SettingsValues.CensorshipCircumventionEnabled = WaveStore.settings.censorshipCircumventionEnabled
    ...
    return when {
      WaveStore.internal.allowChangingCensorshipSetting -> {
  ...
  private fun getCensorshipCircumventionEnabled(state: CensorshipCircumventionState): Boolean {
    return when (state) {
      ...
      CensorshipCircumventionState.AVAILABLE_AUTOMATICALLY_ENABLED -> {
        ...
        WaveStore.settings.censorshipCircumventionEnabled == SettingsValues.CensorshipCircumventionEnabled.ENABLED
      }
    }
  }
