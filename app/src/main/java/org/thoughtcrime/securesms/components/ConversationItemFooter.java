  ...
  private void init(@Nullable AttributeSet attrs) {
    ...
    if (typedArray != null) {
      ...
      if (isOutgoing) {
        ...
        playbackSpeedToggleTextView.setTextColor(getResources().getColor(R.color.wave_text_secondary));
    ...
    if (messageRecord.getExpiresIn() > 0 && !messageRecord.isPending()) {
      ...
      if (messageRecord.getExpireStarted() > 0) {
        ...
        WaveExecutors.BOUNDED.execute(() -> {
          ...
          WaveDatabase.messages().markExpireStarted(id, now);
  ...
  private void presentDeliveryStatus(@NonNull MessageRecord messageRecord) {
    ...
    if (previousMessageId == newMessageId && deliveryStatusView.isPending() && !messageRecord.isPending()) {
      if (messageRecord.getToRecipient().isGroup()) {
        WaveLocalMetrics.GroupMessageSend.onUiUpdated(messageRecord.getId());
      } else {
        WaveLocalMetrics.IndividualMessageSend.onUiUpdated(messageRecord.getId());
      }
    }
