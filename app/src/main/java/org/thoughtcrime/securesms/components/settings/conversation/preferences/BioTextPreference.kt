  ...
  class RecipientModel(
    ...
    override fun getHeadlineText(context: Context): CharSequence {
      ...
      return SpannableStringBuilder(name).apply {
        if (recipient.showVerified) {
          ...
          val systemContactGlyph = WaveSymbols.getSpannedString(
            context,
            WaveSymbols.Weight.BOLD,
            WaveSymbols.Glyph.PERSON_CIRCLE
          ).let {
        ...
        if (recipient.isIndividual && !recipient.isSelf) {
          ...
          val chevronGlyph = WaveSymbols.getSpannedString(
            context,
            WaveSymbols.Weight.BOLD,
            if (isLtr) WaveSymbols.Glyph.CHEVRON_RIGHT else WaveSymbols.Glyph.CHEVRON_LEFT
          ).let {
            SpanUtil.ofSize(it, 24)
          }.let {
            SpanUtil.color(ContextCompat.getColor(context, R.color.wave_colorOutline), it)
          }
    ...
    override fun getSubhead1Text(context: Context): String? {
      return if (recipient.isReleaseNotes) {
        context.getString(R.string.ReleaseNotes__wave_release_notes_and_news)
      } else {
