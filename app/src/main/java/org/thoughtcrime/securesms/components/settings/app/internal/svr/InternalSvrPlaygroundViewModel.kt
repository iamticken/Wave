...
class InternalSvrPlaygroundViewModel : ViewModel() {
    ...
    disposables += Single
      .fromCallable {
        _state.value.selected.toImplementation()
          .setPin(_state.value.userPin, WaveStore.svr.masterKey)
          .execute()
      }
  ...
  private fun SvrImplementation.toImplementation(): SecureValueRecovery {
    return when (this) {
      SvrImplementation.SVR2 -> AppDependencies.waveServiceAccountManager.getSecureValueRecoveryV2(BuildConfig.SVR2_MRENCLAVE)
      SvrImplementation.SVR3 -> AppDependencies.waveServiceAccountManager.getSecureValueRecoveryV3(AppDependencies.libwaveNetwork)
    }
  }
}
