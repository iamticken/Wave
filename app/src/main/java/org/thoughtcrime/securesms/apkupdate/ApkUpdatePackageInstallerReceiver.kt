...
class ApkUpdatePackageInstallerReceiver : BroadcastReceiver() {
  ...
  companion object {
    ...
    const val EXTRA_USER_INITIATED = "wave.user_initiated"
    const val EXTRA_DOWNLOAD_ID = "wave.download_id"
  }
  ...
  override fun onReceive(context: Context, intent: Intent?) {
    ...
    when (statusCode) {
      PackageInstaller.STATUS_SUCCESS -> {
        if (WaveStore.apkUpdate.lastApkUploadTime != WaveStore.apkUpdate.pendingApkUploadTime) {
          Log.i(TAG, "Update installed successfully! Updating our lastApkUploadTime to ${WaveStore.apkUpdate.pendingApkUploadTime}")
          WaveStore.apkUpdate.lastApkUploadTime = WaveStore.apkUpdate.pendingApkUploadTime
