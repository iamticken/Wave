...
import org.wave.libwave.protocol.util.ByteUtil;
...
public class BackupUtil {
  ...
  public static void disableBackups(@NonNull Context context) {
    ...
    WaveStore.settings().setBackupEnabled(false);
    BackupUtil.deleteAllBackups();

    if (BackupUtil.isUserSelectionRequired(context)) {
      Uri backupLocationUri = WaveStore.settings().getWaveBackupDirectory();

      if (backupLocationUri == null) {
        return;
      }

      WaveStore.settings().clearWaveBackupDirectory();
