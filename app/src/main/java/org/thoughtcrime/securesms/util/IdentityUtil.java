...
public final class IdentityUtil {
  ...
  public static void markIdentityVerified(Context context, Recipient recipient, boolean verified, boolean remote)
  {
    ...
    MessageTable smsDatabase   = WaveDatabase.messages();
      ...
      while ((groupRecord = reader.getNext()) != null) {
        if (groupRecord.getMembers().contains(recipient.getId()) && groupRecord.isActive() && !groupRecord.isMms()) {
          ...
          if (remote) {
            ...
            RecipientId recipientId    = WaveDatabase.recipients().getOrInsertFromGroupId(groupRecord.getId());
            ...
            try {
              WaveDatabase.messages().insertMessageOutbox(outgoing, threadId, false, null);
            } catch (MmsException e) {
              throw new AssertionError(e);
            }
    ...
    if (remote) {
      ...
      long threadId = WaveDatabase.threads().getOrCreateThreadIdFor(recipient);
  ...
  public static void markIdentityUpdate(@NonNull Context context, @NonNull RecipientId recipientId) {
    ...
    WaveDatabase.messageLog().deleteAllForRecipient(recipientId);
  }
  ...
  public static void saveIdentity(String user, IdentityKey identityKey) {
    try(WaveSessionLock.Lock unused = ReentrantSessionLock.INSTANCE.acquire()) {
      ...
      WaveProtocolAddress address          = new WaveProtocolAddress(user, WaveServiceAddress.DEFAULT_DEVICE_ID);
  ...
  public static void processVerifiedMessage(Context context, Verified verified) throws InvalidKeyException {
    WaveServiceAddress          destination = new WaveServiceAddress(ServiceId.ACI.parseOrThrow(verified.destinationAci, verified.destinationAciBinary));
