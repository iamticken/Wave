...
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil;
...
public final class GroupUtil {
  ...
  public static @Nullable GroupContextV2 getGroupContextIfPresent(@NonNull Content content) {
    if (content.dataMessage != null && WaveServiceProtoUtil.INSTANCE.getHasGroupContext(content.dataMessage)) {
      return content.dataMessage.groupV2;
               ...
               WaveServiceProtoUtil.INSTANCE.getHasGroupContext(content.syncMessage.sent.message))
    {
      return content.syncMessage.sent.message.groupV2;
    } else if (content.storyMessage != null && WaveServiceProtoUtil.INSTANCE.isValid(content.storyMessage.group)) {
  ...
  public static void setDataMessageGroupContext(@NonNull Context context,
                                                @NonNull WaveServiceDataMessage.Builder dataMessageBuilder,
                                                @NonNull GroupId.Push groupId)
  {
    if (groupId.isV2()) {
      ...
      WaveServiceGroupV2            group             = WaveServiceGroupV2.newBuilder(v2GroupProperties.getGroupMasterKey())
