...
object ExpirationTimerUtil {
  ...
  fun setExpirationTimer(recipientId: RecipientId, expirationTimeSeconds: Int): Int {
    ...
    return if (selfCapable && recipientCapable) {
      WaveDatabase.recipients.setExpireMessagesAndIncrementVersion(recipientId, expirationTimeSeconds)
    } else {
      WaveDatabase.recipients.setExpireMessagesWithoutIncrementingVersion(recipientId, expirationTimeSeconds)
      1
    }
  }
}
