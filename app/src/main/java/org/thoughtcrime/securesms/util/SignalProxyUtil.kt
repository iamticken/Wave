...
import org.wave.core.util.logging.Log.w
...
object WaveProxyUtil {
  private val TAG = tag(WaveProxyUtil::class.java)

  private const val PROXY_LINK_HOST = "wave.tube"
  ...
  fun startListeningToWebsocket() {
    if (WaveStore.proxy.isProxyEnabled && AppDependencies.authWebSocket.state.firstOrError().blockingGet().isFailure) {
   ...
   */
  @JvmStatic
  fun enableProxy(proxy: WaveProxy) {
    WaveStore.proxy.enableProxy(proxy)
  ...
  @JvmStatic
  fun disableProxy() {
    WaveStore.proxy.disableProxy()
    ConscryptWave.setUseEngineSocketByDefault(false)
    AppDependencies.resetNetwork()
    startListeningToWebsocket()
  }
  ...
  fun testWebsocketConnection(timeout: Long): Boolean {
    ...
    if (WaveStore.account.e164 == null) {
  ...
  private fun testWebsocketConnectionUnregistered(timeout: Long): Boolean {
    ...
    val accountManager = AccountManagerFactory.getInstance()
      .createUnauthenticated(AppDependencies.application, "", WaveServiceAddress.DEFAULT_DEVICE_ID, "")
