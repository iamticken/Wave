...
public final class PaymentItem implements MappingModel<PaymentItem> {
  ...
  public @NonNull CharSequence getDate(@NonNull Context context) {
    ...
    if (payment.getState() == State.FAILED) {
      return SpanUtil.color(ContextCompat.getColor(context, R.color.wave_alert_primary), context.getString(R.string.PaymentsHomeFragment__payment_failed));
    }
  ...
  public @ColorRes int getAmountColor() {
    if (isInProgress()) {
      return R.color.wave_text_primary_disabled;
    } else if (payment.getState() == State.FAILED) {
      return R.color.wave_text_secondary;
      ...
      return R.color.wave_text_primary;
    }
  }
