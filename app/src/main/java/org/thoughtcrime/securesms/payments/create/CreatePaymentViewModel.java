...
public class CreatePaymentViewModel extends ViewModel {
  ...
  private CreatePaymentViewModel(@NonNull PayeeParcelable payee, @Nullable CharSequence note) {
    this.payee            = payee;
    this.spendableBalance = Transformations.map(WaveStore.payments().liveMobileCoinBalance(), Balance::getTransferableAmount);
    ...
    this.enclaveFailure   = LiveDataUtil.mapDistinct(WaveStore.payments().enclaveFailure(), isFailure -> isFailure);
    ...
    LiveData<Optional<CurrencyExchange.ExchangeRate>> liveExchangeRate = LiveDataUtil.mapAsync(WaveStore.payments().liveCurrentCurrency(),
                                                                                               currency -> {
                                                                                                 try {
  ...
  private @NonNull InputState updateAmount(@NonNull Context context, @NonNull InputState inputState, @NonNull AmountKeyboardGlyph glyph) {
    switch (inputState.getInputTarget()) {
      case FIAT_MONEY:
        return updateFiatAmount(context, inputState, glyph, WaveStore.payments().currentCurrency());
      case MONEY:
