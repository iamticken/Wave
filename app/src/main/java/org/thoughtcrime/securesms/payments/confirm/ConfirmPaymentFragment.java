...
public class ConfirmPaymentFragment extends BottomSheetDialogFragment {
  ...
  public void onCreate(@Nullable Bundle savedInstanceState) {
    setStyle(DialogFragment.STYLE_NORMAL, R.style.Wave_DayNight_BottomSheet_Rounded);
  ...
  private static CharSequence mono(Context context, CharSequence address) {
    ...
    spannable.setSpan(new TextAppearanceSpan(context, R.style.TextAppearance_Wave_Mono),
                      0,
                      address.length(),
  ...
  private boolean isPaymentLockEnabled(Context context) {
    return WaveStore.payments().isPaymentLockEnabled() && ServiceUtil.getKeyguardManager(context).isKeyguardSecure();
  }
  ...
  private class Callbacks implements ConfirmPaymentAdapter.Callbacks {
    ...
    public Unit showConfirmDeviceCredentialIntent() {
      activityResultLauncher.launch(getString(R.string.BiometricDeviceAuthentication__wave));
