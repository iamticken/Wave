...
final class ConfirmPaymentViewModel extends ViewModel {
                          ...
                          @NonNull ConfirmPaymentRepository confirmPaymentRepository)
  {
    ...
    this.store.update(WaveStore.payments().liveMobileCoinBalance(), (balance, state) -> state.updateBalance(balance.getFullAmount()));
    ...
    LiveData<ConfirmPaymentState.Status> timeoutWave  = Transformations.switchMap(statusLiveData,
                                                                                    s -> {
    ...
    this.store.update(timeoutWave, this::handleTimeout);
  }
