...
public final class PaymentDetailsFragment extends LoggingFragment {
               ...
               .observe(getViewLifecycleOwner(),
                        state -> {
                          ...
                          if (state.getPayment().getState() == State.FAILED) {
                            amount.setTextColor(ContextCompat.getColor(requireContext(), R.color.wave_text_primary_disabled));
  ...
  private @NonNull CharSequence describeStatus(@NonNull PaymentTable.PaymentTransaction payment) {
    switch (payment.getState()) {
      ...
      case FAILED:
        return SpanUtil.color(ContextCompat.getColor(requireContext(), R.color.wave_alert_primary), getResources().getString(R.string.PaymentsDetailsFragment__payment_failed));
      default:
        throw new AssertionError();
    }
  }
