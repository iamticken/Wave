...
class MediaReviewFragment : Fragment(R.layout.v2_media_review_fragment), ScheduleMessageTimePickerBottomSheet.ScheduleCallback, VideoThumbnailsRangeSelectorView.RangeDragListener {
  ...
  private var sentMediaQuality: SentMediaQuality = WaveStore.settings.sentMediaQuality
  ...
  private fun presentSendButton(enabled: Boolean, sendType: MessageSendType, recipient: Recipient?) {
    val sendButtonBackgroundTint = when {
      ...
      sendType.usesWaveTransport -> ContextCompat.getColor(requireContext(), R.color.wave_colorOnSecondaryContainer)
    ...
    val sendButtonForegroundTint = when {
      !enabled -> ContextCompat.getColor(requireContext(), R.color.wave_colorSecondaryContainer)
      recipient != null -> ContextCompat.getColor(requireContext(), R.color.wave_colorOnCustom)
      else -> ContextCompat.getColor(requireContext(), R.color.wave_colorSecondaryContainer)
    }
