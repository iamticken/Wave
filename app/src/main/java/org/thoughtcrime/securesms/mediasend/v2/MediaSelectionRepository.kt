...
class MediaSelectionRepository(context: Context) {
  ...
  fun send(
    ...
    return Maybe.create { emitter ->
      ...
      val storyType: StoryType = if (singleRecipient?.isDistributionList == true) {
        WaveDatabase.distributionLists.getStoryType(singleRecipient.requireDistributionListId())
  ...
  private fun scheduleMessages(
    ...
    for (recipientId in recipientIds) {
      ...
      val thread = WaveDatabase.threads.getOrCreateThreadIdFor(recipient)
  ...
  private fun sendMessages(
    ...
    for (contact in contacts) {
      ...
      if (isStory && !recipient.isMyStory) {
        WaveStore.story.setLatestStorySend(StorySend.newSend(recipient))
      }

      val storyType: StoryType = when {
        recipient.isDistributionList -> WaveDatabase.distributionLists.getStoryType(recipient.requireDistributionListId())
