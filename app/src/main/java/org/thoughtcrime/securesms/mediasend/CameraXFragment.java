...
import org.wave.qr.QrProcessor;
  ...
  private void requestPermissions(boolean includeAudio) {
    if (includeAudio) {
      Permissions.with(this)
                 ...
                 .onSomeDenied(deniedPermissions -> {
                   if (deniedPermissions.contains(Manifest.permission.CAMERA)) {
                     Toast.makeText(requireContext(), R.string.CameraXFragment_wave_needs_camera_access_capture_photos, Toast.LENGTH_LONG).show();
                   }
                 })
                 .execute();
    } else {
      Permissions.with(this)
                 ...
                 .withPermanentDenialDialog(getString(R.string.CameraXFragment_wave_needs_camera_access_capture_photos), null, R.string.CameraXFragment_allow_access_camera, R.string.CameraXFragment_to_capture_photos, getParentFragmentManager())
                 .execute();
    }
  }
  ...
  private void displayVideoRecordingTooltipIfNecessary(CameraButtonView captureButton) {
    if (shouldDisplayVideoRecordingTooltip()) {
      ...
      TooltipPopup.forTarget(captureButton)
                  ...
                  .setTextColor(ContextCompat.getColor(requireContext(), R.color.wave_text_toolbar_title))
