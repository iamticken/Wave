...
public final class PartProvider extends BaseContentProvider {
  ...
  public ParcelFileDescriptor openFile(@NonNull Uri uri, @NonNull String mode) throws FileNotFoundException {
    ...
    if (WaveDatabase.getInstance() == null) {
      Log.w(TAG, "WaveDatabase unavailable");
      return null;
    }
  ...
  public String getType(@NonNull Uri uri) {
    ...
    if (uriMatcher.match(uri) == SINGLE_ROW) {
      ...
      DatabaseAttachment attachment    = WaveDatabase.attachments().getAttachment(partUriParser.getPartId());
  ...
  public Cursor query(@NonNull Uri url, @Nullable String[] projection, String selection, String[] selectionArgs, String sortOrder) {
    ...
    if (uriMatcher.match(url) == SINGLE_ROW) {
      ...
      DatabaseAttachment attachment = WaveDatabase.attachments().getAttachment(partUri.getPartId());
  ...
  private ParcelFileDescriptor getParcelStreamForAttachment(AttachmentId attachmentId) throws IOException {
    ...
    try (InputStream in = WaveDatabase.attachments().getAttachmentStream(attachmentId, 0)) {
    ...
    InputStream  in  = WaveDatabase.attachments().getAttachmentStream(attachmentId, 0);
  ...
  private ParcelFileDescriptor getParcelStreamProxyForAttachment(AttachmentId attachmentId) throws IOException {
    ...
    HandlerThread  thread         = WaveExecutors.getAndStartHandlerThread("storageservice-proxy", ThreadUtil.PRIORITY_IMPORTANT_BACKGROUND_THREAD);
    ...
    ParcelFileDescriptor parcelFileDescriptor = storageManager.openProxyFileDescriptor(ParcelFileDescriptor.MODE_READ_ONLY,
                                                                                       new ProxyCallback(WaveDatabase.attachments(), attachmentId, thread),
                                                                                       handler);
