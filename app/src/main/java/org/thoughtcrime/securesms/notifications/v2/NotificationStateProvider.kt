...
object NotificationStateProvider {
  ...
  fun constructNotificationState(stickyThreads: Map<ConversationId, DefaultMessageNotifier.StickyThread>, notificationProfile: NotificationProfile?): NotificationState {
    ...
    WaveDatabase.messages.getMessagesForNotificationState(stickyThreads.values).use { unreadMessages ->
      ...
      MessageTable.mmsReaderFor(unreadMessages).use { reader ->
        ...
        while (record != null) {
          val threadRecipient: Recipient? = WaveDatabase.threads.getRecipientForThreadId(record.threadId)
          if (threadRecipient != null) {
            ...
            val parentRecord = conversationId.groupStoryId?.let {
              try {
                WaveDatabase.messages.getMessageRecord(it)
            ...
            val hasSelfRepliedToGroupStory = conversationId.groupStoryId?.let {
              WaveDatabase.messages.hasGroupReplyOrReactionInStory(it)
            }

            if (record is MmsMessageRecord) {
              val attachments = WaveDatabase.attachments.getAttachmentsForMessage(record.id)
              if (attachments.isNotEmpty()) {
                record = record.withAttachments(attachments)
              }
              val poll = WaveDatabase.polls.getPoll(record.id)
              if (poll != null) {
            ...
            messages += NotificationMessage(
              ...
              reactions = if (hasUnreadReactions) WaveDatabase.reactions.getReactions(MessageId(record.id)) else emptyList(),
              pollVotes = if (hasUnreadVotes) WaveDatabase.polls.getAllVotes(record.id) else emptyList(),
