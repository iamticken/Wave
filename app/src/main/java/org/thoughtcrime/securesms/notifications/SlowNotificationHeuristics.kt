...
object SlowNotificationHeuristics {
  ...
  fun shouldPromptUserForDelayedNotificationLogs(): Boolean {
    if (!LocaleRemoteConfig.isDelayedNotificationPromptEnabled() || WaveStore.uiHints.hasDeclinedToShareNotificationLogs()) {
      return false
    }
    if (System.currentTimeMillis() - WaveStore.uiHints.lastNotificationLogsPrompt < TimeUnit.DAYS.toMillis(7)) {
      return false
    }
  ...
  fun shouldPromptBatterySaver(): Boolean {
    ...
    if (!remoteEnabled || WaveStore.uiHints.hasDismissedBatterySaverPrompt()) {
      return false
    }

    if (System.currentTimeMillis() - WaveStore.uiHints.lastBatterySaverPrompt < TimeUnit.DAYS.toMillis(7)) {
      return false
    }
  ...
  fun isHavingDelayedNotifications(): Boolean {
    if (!WaveStore.settings.isMessageNotificationsEnabled ||
  ...
  fun shouldShowDeviceSpecificDialog(): Boolean {
    return LocaleRemoteConfig.isDeviceSpecificNotificationEnabled() && WaveStore.uiHints.lastSupportVersionSeen < DeviceSpecificNotificationConfig.currentConfig.version
  }
  ...
  private fun hasRepeatedFailedServiceStarts(metrics: List<LocalMetricsDatabase.EventMetrics>, minimumEventAgeMs: Long, minimumEventCount: Int, failurePercentage: Float): Boolean {
    if (!haveEnoughData(WaveLocalMetrics.FcmServiceStartSuccess.NAME, minimumEventAgeMs) && !haveEnoughData(WaveLocalMetrics.FcmServiceStartFailure.NAME, minimumEventAgeMs)) {
      Log.d(TAG, "insufficient data for service starts")
      return false
    }

    val successes = metrics.filter { it.name == WaveLocalMetrics.FcmServiceStartSuccess.NAME }
    val failures = metrics.filter { it.name == WaveLocalMetrics.FcmServiceStartFailure.NAME }
  ...
  private fun isFailingToDrainQueue(metrics: List<LocalMetricsDatabase.EventMetrics>, minimumEventAgeMs: Long, failureThreshold: Int): Boolean {
    if (!haveEnoughData(WaveLocalMetrics.PushWebsocketFetch.SUCCESS_EVENT, minimumEventAgeMs) && !haveEnoughData(WaveLocalMetrics.PushWebsocketFetch.TIMEOUT_EVENT, minimumEventAgeMs)) {
      Log.d(TAG, "insufficient data for failed queue drains")
      return false
    }
    val failures = metrics.filter { it.name == WaveLocalMetrics.PushWebsocketFetch.TIMEOUT_EVENT }
  ...
  private fun hasLongMessageLatency(metrics: List<LocalMetricsDatabase.EventMetrics>, minimumEventAgeMs: Long, messageThreshold: Int, percentiles: Map<Int, Long>): Boolean {
    if (!haveEnoughData(WaveLocalMetrics.MessageLatency.NAME_HIGH, minimumEventAgeMs)) {
      Log.d(TAG, "insufficient data for message latency")
      return false
    }
    val eventCount = metrics.find { it.name == WaveLocalMetrics.MessageLatency.NAME_HIGH }?.count ?: 0
    ...
    for ((percentage, threshold) in percentiles.entries) {
      val averageLatency = db.eventPercent(WaveLocalMetrics.MessageLatency.NAME_HIGH, percentage.coerceAtMost(100).coerceAtLeast(0))
