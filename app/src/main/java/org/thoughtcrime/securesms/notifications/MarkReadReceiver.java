...
public class MarkReadReceiver extends BroadcastReceiver {
    ...
    if (threads != null) {
      ...
      WaveExecutors.BOUNDED.execute(() -> {
        ...
        for (ConversationId thread : threads) {
          ...
          List<MarkedMessageInfo> messageIds = WaveDatabase.threads().setRead(thread);
  ...
  public static void processCallEvents(@NonNull List<ConversationId> threads, long timestamp) {
    List<RecipientId> peers = WaveDatabase.threads().getRecipientIdsForThreadIds(threads.stream()
                                                                                          .filter(it -> it.getGroupStoryId() == null)
                                                                                          .map(ConversationId::getThreadId)
                                                                                          .collect(java.util.stream.Collectors.toList()));

    for (RecipientId peer : peers) {
      CallTable.Call lastCallInThread = WaveDatabase.calls().markAllCallEventsWithPeerBeforeTimestampRead(peer, timestamp);
  ...
  private static void scheduleDeletion(@NonNull List<ExpirationInfo> expirationInfo) {
    if (expirationInfo.size() > 0) {
      ...
      WaveDatabase.messages().markExpireStarted(Stream.of(expirationInfo).map(info -> new kotlin.Pair<>(info.getId(), now)).toList());
