...
import org.wave.core.util.PendingIntentFlags.mutable
...
sealed class NotificationBuilder(protected val context: Context) {
  ...
  private val privacy: NotificationPrivacyPreference = WaveStore.settings.messageNotificationsPrivacy
  ...
  fun addReplyActions(conversation: NotificationConversation) {
    if (privacy.isDisplayMessage && isNotLocked && !conversation.recipient.isPushV1Group && RecipientUtil.isMessageRequestAccepted(context, conversation.recipient)) {
      if (conversation.recipient.isPushV2Group) {
        val group: Optional<GroupRecord> = WaveDatabase.groups.getGroup(conversation.recipient.requireGroupId())
  ...
  fun setLights() {
    val ledColor: String = WaveStore.settings.messageLedColor

    if (ledColor != "none") {
      var blinkPattern = WaveStore.settings.messageLedBlinkPattern
  ...
  private class NotificationBuilderCompat(context: Context) : NotificationBuilder(context) {
    ...
    override fun setAlarms(recipient: Recipient?) {
      ...
      val defaultRingtone: Uri = WaveStore.settings.messageNotificationSound
      val defaultVibrate: Boolean = WaveStore.settings.isMessageVibrateEnabled
...
private fun ReplyMethod.toLongDescription(): Int {
  return when (this) {
    ...
    ReplyMethod.SecureMessage -> R.string.MessageNotifier_wave_message
  }
}
