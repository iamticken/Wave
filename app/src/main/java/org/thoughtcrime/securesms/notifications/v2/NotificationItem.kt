...
sealed class NotificationItem(val threadRecipient: Recipient, protected val record: MessageRecord) : Comparable<NotificationItem> {
  ...
  fun getStyledPrimaryText(context: Context, trimmed: Boolean = false): CharSequence {
    return if (WaveStore.settings.messageNotificationsPrivacy.isDisplayNothing) {
  ...
  fun getPersonName(context: Context): CharSequence {
    return if (WaveStore.settings.messageNotificationsPrivacy.isDisplayContact) {
      authorRecipient.getDisplayName(context)
    } else {
      context.getString(R.string.SingleRecipientNotificationBuilder_wave)
    }
  }
  ...
  fun getPersonUri(context: Context): String? {
    return if (WaveStore.settings.messageNotificationsPrivacy.isDisplayContact && authorRecipient.isSystemContact && Permissions.hasAny(context, Manifest.permission.READ_CONTACTS, Manifest.permission.WRITE_CONTACTS)) {
  ...
  fun getPrimaryText(context: Context): CharSequence {
    return if (WaveStore.settings.messageNotificationsPrivacy.isDisplayMessage) {
  ...
  fun getInboxLine(context: Context): CharSequence? {
    return when {
      WaveStore.settings.messageNotificationsPrivacy.isDisplayNothing -> null
...
class MessageNotification(threadRecipient: Recipient, record: MessageRecord) : NotificationItem(threadRecipient, record) {
  ...
  override fun getStartingPosition(context: Context): Int {
    return if (thread.groupStoryId != null) {
      WaveDatabase.messages.getMessagePositionInConversation(thread.threadId, thread.groupStoryId, record.dateReceived)
    } else {
      -1
    }
  }
  ...
  override fun getThumbnailInfo(context: Context): ThumbnailInfo {
    return if (WaveStore.settings.messageNotificationsPrivacy.isDisplayMessage && !KeyCachingService.isLocked(context)) {
...
class ReactionNotification(threadRecipient: Recipient, record: MessageRecord, val reaction: ReactionRecord) : NotificationItem(threadRecipient, record) {
  ...
  override fun getStartingPosition(context: Context): Int {
    return WaveDatabase.messages.getMessagePositionInConversation(thread.threadId, thread.groupStoryId ?: 0L, record.dateReceived)
  }
...
class VoteNotification(threadRecipient: Recipient, record: MessageRecord, val vote: PollVote) : NotificationItem(threadRecipient, record) {
  ...
  override fun getStartingPosition(context: Context): Int {
    return WaveDatabase.messages.getMessagePositionInConversation(threadId = thread.threadId, groupStoryId = 0L, receivedTimestamp = record.dateReceived)
  }
