...
import org.whispersystems.waveservice.api.kbs.PinHashUtil
...
object RegistrationRepository {
  ...
  fun getRegistrationId(): Int {
    ...
    var registrationId = WaveStore.account.registrationId
    if (registrationId == 0) {
      registrationId = KeyHelper.generateRegistrationId(false)
      WaveStore.account.registrationId = registrationId
    }
  ...
  fun getPniRegistrationId(): Int {
    // TODO [regv2]: make creation more explicit instead of hiding it in this getter
    var pniRegistrationId = WaveStore.account.pniRegistrationId
  ...
  suspend fun getProfileKey(e164: String): ProfileKey =
    withContext(Dispatchers.IO) {
      // TODO [regv2]: make creation more explicit instead of hiding it in this getter
      val recipientTable = WaveDatabase.recipients
  ...
  suspend fun registerAccountLocally(context: Context, data: LocalRegistrationMetadata) =
    withContext(Dispatchers.IO) {
      ...
      if (data.linkedDeviceInfo != null) {
        WaveStore.account.deviceId = data.linkedDeviceInfo.deviceId
        WaveStore.account.deviceName = data.linkedDeviceInfo.deviceName
      }

      val aciIdentityKeyPair = data.getAciIdentityKeyPair()
      val pniIdentityKeyPair = data.getPniIdentityKeyPair()
      WaveStore.account.restoreAciIdentityKeyFromBackup(aciIdentityKeyPair.publicKey.serialize(), aciIdentityKeyPair.privateKey.serialize())
      WaveStore.account.restorePniIdentityKeyFromBackup(pniIdentityKeyPair.publicKey.serialize(), pniIdentityKeyPair.privateKey.serialize())
      ...
      val aciMetadataStore = WaveStore.account.aciPreKeys
      ...
      WaveStore.account.setE164(data.e164)
      WaveStore.account.fcmToken = data.fcmToken
      WaveStore.account.fcmEnabled = data.fcmEnabled
      ...
      if (data.linkedDeviceInfo != null) {
        if (data.linkedDeviceInfo.accountEntropyPool != null) {
          WaveStore.account.setAccountEntropyPoolFromPrimaryDevice(AccountEntropyPool(data.linkedDeviceInfo.accountEntropyPool))
        }

        if (data.linkedDeviceInfo.mediaRootBackupKey != null) {
          WaveStore.backup.mediaRootBackupKey = MediaRootBackupKey(data.linkedDeviceInfo.mediaRootBackupKey.toByteArray())
        }
      }

      WaveStore.account.setServicePassword(data.servicePassword)
      ...
      SvrRepository.onRegistrationComplete(masterKey, data.pin, hasPin, data.reglockEnabled, WaveStore.account.restoredAccountEntropyPool)
  ...
  @JvmStatic
  private fun saveOwnIdentityKey(selfId: RecipientId, serviceId: ServiceId, protocolStore: WaveServiceAccountDataStoreImpl, now: Long) {
  ...
  @JvmStatic
  private fun storeSignedAndLastResortPreKeys(protocolStore: WaveServiceAccountDataStoreImpl, metadataStore: PreKeyMetadataStore, preKeyCollection: PreKeyCollection) {
  ...
  fun canUseLocalRecoveryPassword(): Boolean {
    val recoveryPassword = WaveStore.svr.recoveryPassword
    val pinHash = WaveStore.svr.localPinHash
    return recoveryPassword != null && pinHash != null
  }

  fun doesPinMatchLocalHash(pin: String): Boolean {
    val pinHash = WaveStore.svr.localPinHash ?: throw IllegalStateException("Local PIN hash is not present!")
  ...
  private suspend fun validateSession(context: Context, sessionId: String, e164: String, password: String): RegistrationSessionCheckResult =
    withContext(Dispatchers.IO) {
      val api: RegistrationApi = AccountManagerFactory.getInstance().createUnauthenticated(context, e164, WaveServiceAddress.DEFAULT_DEVICE_ID, password).registrationApi
  ...
  suspend fun createSession(context: Context, e164: String, password: String, mcc: String?, mnc: String?): RegistrationSessionCreationResult =
    withContext(Dispatchers.IO) {
      ...
      if (result is RegistrationSessionCreationResult.Success) {
        ...
        WaveStore.registration.sessionId = result.sessionId
        WaveStore.registration.sessionE164 = e164
      }
  ...
  suspend fun createOrValidateSession(context: Context, sessionId: String?, e164: String, password: String, mcc: String?, mnc: String?): RegistrationSessionResult {
    val savedSessionId = if (sessionId == null && e164 == WaveStore.registration.sessionE164) {
      WaveStore.registration.sessionId
    } else {
      sessionId
    }
  ...
  suspend fun submitVerificationCode(context: Context, sessionId: String, registrationData: RegistrationData): VerificationCodeRequestResult =
    withContext(Dispatchers.IO) {
      val api: RegistrationApi = AccountManagerFactory.getInstance().createUnauthenticated(context, registrationData.e164, WaveServiceAddress.DEFAULT_DEVICE_ID, registrationData.password).registrationApi
  ...
  suspend fun requestAndVerifyPushToken(context: Context, sessionId: String, e164: String, password: String) =
    withContext(Dispatchers.IO) {
      ...
      val accountManager = AccountManagerFactory.getInstance().createUnauthenticated(context, e164, WaveServiceAddress.DEFAULT_DEVICE_ID, password)
  ...
  suspend fun registerAccount(context: Context, sessionId: String?, registrationData: RegistrationData, pin: String? = null, masterKeyProducer: MasterKeyProducer? = null): RegisterAccountResult =
    withContext(Dispatchers.IO) {
      ...
      val accountAttributes = AccountAttributes(
        ...
        discoverableByPhoneNumber = WaveStore.phoneNumberPrivacy.phoneNumberDiscoverabilityMode == PhoneNumberPrivacyValues.PhoneNumberDiscoverabilityMode.DISCOVERABLE,
        name = null,
      ...
      val aciIdentity: IdentityKeyPair = WaveStore.account.aciIdentityKey

      WaveStore.account.generatePniIdentityKeyIfNecessary()
      val pniIdentity: IdentityKeyPair = WaveStore.account.pniIdentityKey

      val aciPreKeyCollection = generateSignedAndLastResortPreKeys(aciIdentity, WaveStore.account.aciPreKeys)
      val pniPreKeyCollection = generateSignedAndLastResortPreKeys(pniIdentity, WaveStore.account.pniPreKeys)
  ...
  fun registerAsLinkedDevice(
    ...
    val aciPreKeys = generateSignedAndLastResortPreKeys(aciIdentityKeyPair, WaveStore.account.aciPreKeys)
    val pniPreKeys = generateSignedAndLastResortPreKeys(pniIdentityKeyPair, WaveStore.account.pniPreKeys)
  ...
  suspend fun hasValidSvrAuthCredentials(context: Context, e164: String, password: String): BackupAuthCheckResult =
    withContext(Dispatchers.IO) {
      ...
      val svr3Result = WaveStore.svr.svr3AuthTokens
        ...
        ?.let { authTokens ->
          api
            ...
            .runIfSuccessful {
              val removedInvalidTokens = WaveStore.svr.removeSvr3AuthTokens(it.invalid)
      ...
      return@withContext WaveStore.svr.svr2AuthTokens
        ?.takeIf { it.isNotEmpty() }
        ?.toSvrCredentials()
        ?.let { authTokens ->
          api
            ...
            .runIfSuccessful {
              val removedInvalidTokens = WaveStore.svr.removeSvr2AuthTokens(it.invalid)
              if (removedInvalidTokens) {
                BackupManager(context).dataChanged()
              }
            }
  ...
  suspend fun waitForLinkAndSyncBackupDetails(maxWaitTime: Duration = 60.seconds): TransferArchiveResponse? {
    ...
    while (timeRemaining > 0 && coroutineContext.isActive) {
      ...
      when (val result = WaveNetwork.linkDevice.waitForPrimaryDevice(timeout = 60.seconds)) {
