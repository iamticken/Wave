...
public final class IncomingGroupCallActionProcessor extends DeviceAwareActionProcessor {
                                                                  ...
                                                                  @NonNull CallManager.RingUpdate ringUpdate)
  {
    ...
    if (WaveDatabase.calls().isRingCancelled(ringId, remotePeerGroup.getId()) && !updateForCurrentRingId) {
      try {
    ...
    boolean isCallNotificationsEnabled = WaveStore.settings().isCallNotificationsEnabled() && NotificationChannels.getInstance().areNotificationsEnabled();
    if (shouldDisturbUserWithCall && isCallNotificationsEnabled) {
      ...
      if (ringtone == null) {
        ringtone = WaveStore.settings().getCallRingtone();
      }

      webRtcInteractor.startIncomingRinger(ringtone, vibrateState == RecipientTable.VibrateState.ENABLED || (vibrateState == RecipientTable.VibrateState.DEFAULT && WaveStore.settings().isCallVibrateEnabled()));
    }
  ...
  protected @NonNull WebRtcServiceState handleAcceptCall(@NonNull WebRtcServiceState currentState, boolean answerWithVideo) {
    ...
    GroupCall groupCall = webRtcInteractor.getCallManager().createGroupCall(groupId,
                                                                            WaveStore.internal().getGroupCallingServer(),
                                                                            new byte[0],
