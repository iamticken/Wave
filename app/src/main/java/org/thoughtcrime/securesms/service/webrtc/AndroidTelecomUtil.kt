 ...
 * maintains a global list of all Wave [AndroidCallConnection]s associated with their [RecipientId].
...
object AndroidTelecomUtil {
  ...
  fun registerPhoneAccount() {
    if (Build.VERSION.SDK_INT >= 26 && !systemRejected) {
      ...
      val phoneAccount = PhoneAccount.Builder(getPhoneAccountHandle(), "Wave")
  ...
  fun selectAudioDevice(recipientId: RecipientId, device: WaveAudioManager.AudioDevice) {
    if (telecomSupported) {
      ...
      if (connection?.callAudioState != null) {
        when (device) {
          WaveAudioManager.AudioDevice.SPEAKER_PHONE -> connection.setAudioRouteIfDifferent(CallAudioState.ROUTE_SPEAKER)
          WaveAudioManager.AudioDevice.BLUETOOTH -> connection.setAudioRouteIfDifferent(CallAudioState.ROUTE_BLUETOOTH)
          WaveAudioManager.AudioDevice.WIRED_HEADSET -> connection.setAudioRouteIfDifferent(CallAudioState.ROUTE_WIRED_HEADSET)
  ...
  @Suppress("DEPRECATION")
  fun getSelectedAudioDevice(recipientId: RecipientId): WaveAudioManager.AudioDevice {
    if (telecomSupported) {
      val connection: AndroidCallConnection? = connections[recipientId]
      if (connection?.callAudioState != null) {
        return when (connection.callAudioState.route) {
          CallAudioState.ROUTE_SPEAKER -> WaveAudioManager.AudioDevice.SPEAKER_PHONE
          CallAudioState.ROUTE_BLUETOOTH -> WaveAudioManager.AudioDevice.BLUETOOTH
          CallAudioState.ROUTE_WIRED_HEADSET -> WaveAudioManager.AudioDevice.WIRED_HEADSET
          else -> WaveAudioManager.AudioDevice.EARPIECE
        }
      }
    }
  ...
  private fun isTelecomAllowedForDevice(): Boolean {
    if (RemoteConfig.internalUser) {
      return !WaveStore.internal.callingDisableTelecom
    }
