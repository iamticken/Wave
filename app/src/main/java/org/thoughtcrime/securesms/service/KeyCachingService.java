...
public class KeyCachingService extends Service {
  ...
  public static synchronized boolean isLocked(Context context) {
    boolean locked = masterSecret == null && (!WaveStore.settings().getPassphraseDisabled() || WaveStore.settings().getScreenLockEnabled());

    if (locked) {
      Log.d(TAG, "Locked! PasswordDisabled: " + WaveStore.settings().getPassphraseDisabled() + ", ScreenLock: " + WaveStore.settings().getScreenLockEnabled());
    }
  ...
  public static synchronized @Nullable MasterSecret getMasterSecret(Context context) {
    if (masterSecret == null && (WaveStore.settings().getPassphraseDisabled() && !WaveStore.settings().getScreenLockEnabled())) {
      try {
  ...
  public static void onAppForegrounded(@NonNull Context context) {
    if (WaveStore.settings().getScreenLockEnabled() || !WaveStore.settings().getPassphraseDisabled()) {
  ...
  public void onCreate() {
    ...
    if (WaveStore.settings().getPassphraseDisabled() && !WaveStore.settings().getScreenLockEnabled()) {
      try {
  ...
  private void handleDisableService() {
    if (WaveStore.settings().getPassphraseDisabled() &&
        !WaveStore.settings().getScreenLockEnabled())
    {
      stopForeground(true);
    }
  }
  ...
  private static void startTimeoutIfAppropriate(@NonNull Context context) {
    ...
    boolean timeoutEnabled   = WaveStore.settings().getPassphraseTimeoutEnabled();
    boolean passLockActive   = timeoutEnabled && !WaveStore.settings().getPassphraseDisabled();

    long    screenTimeout       = WaveStore.settings().getScreenLockTimeout();
    boolean screenLockActive    = WaveStore.settings().getScreenLockEnabled();
    ...
    if (immediateScreenLock) {
      ...
      WaveStore.settings().setScreenLockTimeout(60);
    ...
    if (!appVisible && secretSet && (passLockActive || screenLockActive)) {
      if (immediateScreenLock) {
        ...
        long passphraseTimeoutMinutes = WaveStore.settings().getPassphraseTimeout();
        long screenLockTimeoutSeconds = WaveStore.settings().getScreenLockTimeout();

        long timeoutMillis;

        if (!WaveStore.settings().getPassphraseDisabled()) timeoutMillis = TimeUnit.MINUTES.toMillis(passphraseTimeoutMinutes);
  ...
  private void foregroundService() {
    ...
    builder.setContentText(getString(R.string.KeyCachingService_wave_passphrase_cached));
    builder.setSmallIcon(R.drawable.ic_notification_unlocked);
    builder.setColor(ContextCompat.getColor(this, R.color.wave_light_colorSecondary));
    builder.setWhen(0);
