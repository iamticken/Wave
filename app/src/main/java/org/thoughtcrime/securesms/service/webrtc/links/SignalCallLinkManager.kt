...
import org.wave.libwave.zkgroup.calllinks.CreateCallLinkCredential
import org.wave.libwave.zkgroup.calllinks.CreateCallLinkCredentialPresentation
import org.wave.libwave.zkgroup.calllinks.CreateCallLinkCredentialRequestContext
...
class WaveCallLinkManager(
  ...
  private fun requestCreateCallLinkCredentialPresentation(
    ...
    when (val result: NetworkResult<CreateCallLinkCredentialResponse> = WaveNetwork.calling.createCallLinkCredential(request)) {
  ...
  fun createCallLink(
    ...
    return Single.create { emitter ->
      ...
      callManager.createCallLink(
        WaveStore.internal.groupCallingServer,
  ...
  companion object {
    ...
    private val TAG = Log.tag(WaveCallLinkManager::class.java)

    private fun CallLinkState.toAppState(): WaveCallLinkState {
      return WaveCallLinkState(
        name = name,
