...
import org.thoughtcrime.securesms.crypto.storage.WaveIdentityKeyStore.SaveResult;
 ...
 * So this class represents the core store, and we can create multiple {@link WaveIdentityKeyStore} that use this same instance, changing only what each of
 * those reports as their own identity key.
 */
public class WaveBaseIdentityKeyStore {

  private static final String TAG = Log.tag(WaveBaseIdentityKeyStore.class);
  ...
  public WaveBaseIdentityKeyStore(@NonNull Context context) {
    this(context, WaveDatabase.identities());
  }

  WaveBaseIdentityKeyStore(@NonNull Context context, @NonNull IdentityTable identityDatabase) {
    this.context = context;
    this.cache   = new Cache(identityDatabase);
  }

  public int getLocalRegistrationId() {
    return WaveStore.account().getRegistrationId();
  }

  public IdentityKeyStore.IdentityChange saveIdentity(WaveProtocolAddress address, IdentityKey identityKey) {
  ...
  public @NonNull SaveResult saveIdentity(WaveProtocolAddress address, IdentityKey identityKey, boolean nonBlockingApproval) {
    try (WaveSessionLock.Lock unused = ReentrantSessionLock.INSTANCE.acquire()) {
      ...
      RecipientId         recipientId      = RecipientId.from(ServiceId.fromLibWave(address.getServiceId()));
      ...
      if (identityKeyChanged && Recipient.self().getId().equals(recipientId) && Objects.equals(WaveStore.account().getAci(), ServiceId.parseOrNull(address.getName()))) {
        ...
        WaveDatabase.senderKeyShared().deleteAllFor(recipientId);
  ...
  public boolean isTrustedIdentity(WaveProtocolAddress address, IdentityKey identityKey, IdentityKeyStore.Direction direction) {
    boolean isSelf = address.getName().equals(WaveStore.account().requireAci().toString()) ||
                     address.getName().equals(WaveStore.account().requirePni().toString()) ||
                     address.getName().equals(WaveStore.account().getE164());

    if (isSelf) {
      return identityKey.equals(WaveStore.account().getAciIdentityKey().getPublicKey());
    }
  ...
  private static final class Cache {
    ...
    private void withWriteLock(Runnable runnable) {
      SQLiteDatabase db = WaveDatabase.getRawDatabase();
      db.beginTransaction();
      try {
        synchronized (this) {
          runnable.run();
