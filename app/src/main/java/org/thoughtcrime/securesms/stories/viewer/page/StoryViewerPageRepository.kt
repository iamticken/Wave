...
import org.wave.core.util.BreakIteratorCompat
...
open class StoryViewerPageRepository(context: Context, private val storyViewStateCache: StoryViewStateCache) {
    ...
    return Observable.create { emitter ->
      ...
      fun refresh() {
        val stories = if (recipient.isMyStory) {
          WaveDatabase.messages.getAllOutgoingStories(false, 100)
        } else if (isOutgoingOnly) {
          WaveDatabase.messages.getOutgoingStoriesTo(recipientId)
        } else {
          WaveDatabase.messages.getAllStoriesFor(recipientId, 100)
        }
  ...
  private fun getStoryPostFromRecord(recipientId: RecipientId, originalRecord: MessageRecord): Observable<StoryPost> {
    return Observable.create { emitter ->
      fun refresh(record: MessageRecord) {
        val recipient = Recipient.resolved(recipientId)
        val viewedCount = WaveDatabase.groupReceipts.getGroupReceiptInfo(record.id).filter { it.status == GroupReceiptTable.STATUS_VIEWED }.size
        val story = StoryPost(
          id = record.id,
          ...
          replyCount = WaveDatabase.messages.getNumberOfStoryReplies(record.id),
      ...
      val messageUpdateObserver = DatabaseObserver.MessageObserver {
        if (it.id == recordId) {
          try {
            val messageRecord = WaveDatabase.messages.getMessageRecord(recordId)
      ...
      val conversationObserver = DatabaseObserver.Observer {
        try {
          refresh(WaveDatabase.messages.getMessageRecord(recordId))
        } catch (e: NoSuchMessageException) {
  ...
  fun hideStory(recipientId: RecipientId): Completable {
    return Completable.fromAction {
      WaveDatabase.recipients.setHideStory(recipientId, true)
    }.subscribeOn(Schedulers.io())
  }

  fun unhideStory(recipientId: RecipientId): Completable {
    return Completable.fromAction {
      WaveDatabase.recipients.setHideStory(recipientId, false)
    }.subscribeOn(Schedulers.io())
  }

  fun markViewed(storyPost: StoryPost) {
    if (!storyPost.conversationMessage.messageRecord.isOutgoing) {
      WaveExecutors.SERIAL.execute {
        val markedMessageInfo = WaveDatabase.messages.setIncomingMessageViewed(storyPost.id)
        if (markedMessageInfo != null) {
          AppDependencies.databaseObserver.notifyConversationListListeners()

          if (storyPost.sender.isReleaseNotes) {
            WaveStore.story.userHasViewedOnboardingStory = true
            ...
            WaveDatabase.recipients.updateLastStoryViewTimestamp(recipientId)
