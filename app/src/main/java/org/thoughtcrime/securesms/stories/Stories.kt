...
object Stories {
  ...
  fun isFeatureEnabled(): Boolean {
    return !WaveStore.story.isFeatureDisabled
  }
  ...
  fun getRecipientsToSendTo(messageId: Long, sentTimestamp: Long, allowsReplies: Boolean): SendData {
    val recipientIds: List<RecipientId> = WaveDatabase.storySends.getRecipientsToSendTo(messageId, sentTimestamp, allowsReplies)
  ...
  fun onStorySettingsChanged(distributionListId: DistributionListId) {
    val recipientId = WaveDatabase.distributionLists.getRecipientId(distributionListId) ?: error("Cannot find recipient id for distribution list.")
    onStorySettingsChanged(recipientId)
  }

  @WorkerThread
  fun onStorySettingsChanged(storyRecipientId: RecipientId) {
    WaveDatabase.recipients.markNeedsSync(storyRecipientId)
  ...
  fun enqueueNextStoriesForDownload(recipientId: RecipientId, force: Boolean = false, limit: Int) {
    ...
    WaveDatabase.messages.getUnreadStories(recipientId, limit).use { reader ->
      reader.forEach {
  ...
  fun enqueueAttachmentsFromStoryForDownloadSync(record: MmsMessageRecord, ignoreAutoDownloadConstraints: Boolean) {
    WaveDatabase.attachments.getAttachmentsForMessage(record.id).filterNot { it.isSticker }.forEach {
