...
class MyStorySettingsViewModel @JvmOverloads constructor(private val repository: MyStorySettingsRepository = MyStorySettingsRepository()) : ViewModel() {
  private val store = Store(MyStorySettingsState(hasUserPerformedManualSelection = WaveStore.story.userHasBeenNotifiedAboutStories))
  ...
  fun refresh() {
    ...
    disposables += repository.getAllWaveConnectionsCount()
      .subscribe { allWaveConnectionsCount -> store.update { it.copy(allWaveConnectionsCount = allWaveConnectionsCount) } }
  }
  ...
  fun setMyStoryPrivacyMode(privacyMode: DistributionListPrivacyMode): Completable {
    ...
    WaveStore.story.userHasBeenNotifiedAboutStories = true
