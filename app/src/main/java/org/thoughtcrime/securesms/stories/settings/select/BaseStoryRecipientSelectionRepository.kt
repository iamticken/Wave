...
class BaseStoryRecipientSelectionRepository {
  ...
  fun updateDistributionListMembership(distributionListRecord: DistributionListRecord, recipients: Set<RecipientId>) {
    WaveExecutors.BOUNDED.execute {
      val currentRecipients = WaveDatabase.distributionLists.getRawMembers(distributionListRecord.id, distributionListRecord.privacyMode).toSet()
      val oldNotNew = currentRecipients - recipients
      val newNotOld = recipients - currentRecipients

      oldNotNew.forEach {
        WaveDatabase.distributionLists.removeMemberFromList(distributionListRecord.id, distributionListRecord.privacyMode, it)
      }

      newNotOld.forEach {
        WaveDatabase.distributionLists.addMemberToList(distributionListRecord.id, distributionListRecord.privacyMode, it)
      }
  ...
  fun getAllWaveContacts(): Single<Set<RecipientId>> {
    return Single.fromCallable {
      WaveDatabase.recipients.getWaveContacts(RecipientTable.IncludeSelfMode.Exclude).use {
