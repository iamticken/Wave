...
import org.wave.core.ui.view.AlertDialogResult
import org.wave.core.ui.view.awaitResult
...
class AttachmentSaver(private val host: Host) {
  ...
  private suspend fun checkIsSaveWarningAccepted(attachmentCount: Int): SaveToStorageWarningResult {
    if (WaveStore.uiHints.hasDismissedSaveStorageWarning()) {
  ...
  private suspend fun saveToStorage(attachments: Set<SaveAttachment>): SaveAttachmentsResult {
    ...
    return try {
      ...
      withContext(WaveDispatchers.Main) {
  ...
  data class FragmentHost(private val fragment: Fragment) : Host {
    ...
    override suspend fun showSaveToStorageWarning(attachmentCount: Int): SaveToStorageWarningResult = withContext(WaveDispatchers.Main) {
      ...
      if (result == AlertDialogResult.POSITIVE) {
        ...
        if (dontShowAgainCheckbox.isChecked) {
          WaveStore.uiHints.markDismissedSaveStorageWarning()
        }
    ...
    override suspend fun requestWriteExternalStoragePermission(): RequestPermissionResult = withContext(WaveDispatchers.Main) {
      suspendCoroutine { continuation ->
        Permissions.with(fragment)
          .request(Manifest.permission.WRITE_EXTERNAL_STORAGE)
          .ifNecessary()
          .withPermanentDenialDialog(fragment.getString(R.string.AttachmentSaver__wave_needs_the_storage_permission_in_order_to_write_to_external_storage_but_it_has_been_permanently_denied))
          .onAnyDenied {
