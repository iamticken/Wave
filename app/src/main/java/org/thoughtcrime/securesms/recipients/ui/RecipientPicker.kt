...
import org.wave.core.ui.compose.Fragments
...
private fun ContactSelectionListFragment.setUpCallbacks(
  ...
  if (callbacks.newConversation != null) {
    fragment.setNewConversationCallback(object : ContactSelectionListFragment.NewConversationCallback {
      override fun onInvite() = callbacks.newConversation.onInviteToWave()
  ...
  if (callbacks.newCall != null) {
    fragment.setNewCallCallback { callbacks.newCall.onInviteToWave() }
  } else {
...
private suspend fun showItemContextMenu(
  ...
  val actions = buildList {
    val messageItem = ActionItem(
      ...
      tintRes = R.color.wave_colorOnSurface,
    ...
    if (!recipient.isSelf) {
      val blockItem = ActionItem(
        ...
        tintRes = R.color.wave_colorError,
  ...
  WaveContextMenu.Builder(anchorView, anchorView.getRootView() as ViewGroup)
    .preferredVerticalPosition(WaveContextMenu.VerticalPosition.BELOW)
    .preferredHorizontalPosition(WaveContextMenu.HorizontalPosition.START)
