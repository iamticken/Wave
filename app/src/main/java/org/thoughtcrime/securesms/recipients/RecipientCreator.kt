...
object RecipientCreator {
  ...
  fun forIndividual(context: Context, record: RecipientRecord): Recipient {
    val isSelf = record.e164 != null && record.e164 == WaveStore.account.e164 || record.aci != null && record.aci == WaveStore.account.aci
    val isReleaseChannel = record.id == WaveStore.releaseChannel.releaseChannelRecipientId
    var registeredState = record.registered

    if (isSelf) {
      registeredState = if (WaveStore.account.isRegistered && !TextSecurePreferences.isUnauthorizedReceived(context)) {
  ...
  private fun getGroupRecipientDetails(record: RecipientRecord): Recipient {
    val groupRecord = WaveDatabase.groups.getGroup(record.id)
  ...
  private fun getDistributionListRecipientDetails(record: RecipientRecord): Recipient {
    val groupRecord = WaveDatabase.distributionLists.getList(record.distributionListId!!)
  ...
  private fun getCallLinkRecipientDetails(record: RecipientRecord): Recipient {
    val callLink = WaveDatabase.callLinks.getCallLinkByRoomId(record.callLinkRoomId!!)
