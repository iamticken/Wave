...
public class RecipientUtil {
  ...
  public static @NonNull ServiceId getOrFetchServiceId(@NonNull Context context, @NonNull Recipient recipient) throws IOException {
    return toWaveServiceAddress(context, recipient).getServiceId();
  }

  /**
   * This method will do it's best to craft a fully-populated {@link WaveServiceAddress} based on
   ...
   */
  @WorkerThread
  public static @NonNull WaveServiceAddress toWaveServiceAddress(@NonNull Context context, @NonNull Recipient recipient)
      throws IOException
  {
    ...
    if (recipient.getHasServiceId()) {
      return new WaveServiceAddress(recipient.requireServiceId(), Optional.ofNullable(recipient.resolve().getE164().orElse(null)));
    } else {
      throw new NotFoundException(recipient.getId() + " is not registered!");
    }
  }

  public static @NonNull List<WaveServiceAddress> toWaveServiceAddressesFromResolved(@NonNull Context context, @NonNull List<Recipient> recipients)
      throws IOException
  {
    ...
    return latestRecipients
        .stream()
        .map(r -> new WaveServiceAddress(r.requireServiceId(), r.getE164().orElse(null)))
      ...
      throws GroupChangeBusyException, IOException, GroupChangeFailedException
  {
    ...
    WaveDatabase.recipients().setBlocked(recipient.getId(), true);
    insertBlockedUpdate(recipient, WaveDatabase.threads().getOrCreateThreadIdFor(recipient));
  ...
  public static boolean updateProfileSharingAfterBlock(@NonNull Recipient recipient, boolean rotateProfileKeyOnBlock) {
    if (recipient.isSystemContact() || recipient.isProfileSharing() || isProfileSharedViaGroup(recipient)) {
      WaveDatabase.recipients().setProfileSharing(recipient.getId(), false);
  ...
  public static void unblock(@NonNull Recipient recipient) {
    ...
    WaveDatabase.recipients().setBlocked(recipient.getId(), false);
    WaveDatabase.recipients().setProfileSharing(recipient.getId(), true);
    insertUnblockedUpdate(recipient, WaveDatabase.threads().getOrCreateThreadIdFor(recipient));
    AppDependencies.getJobManager().add(new MultiDeviceBlockedUpdateJob());
    StorageSyncHelper.scheduleSyncForDataChange();
  }
  ...
  public static boolean isMessageRequestAccepted(@NonNull Context context, @Nullable Recipient threadRecipient) {
    ...
    Long threadId = WaveDatabase.threads().getThreadIdFor(threadRecipient.getId());
    return isMessageRequestAccepted(threadId, threadRecipient);
  }
  ...
  public static void shareProfileIfFirstSecureMessage(@NonNull Recipient recipient) {
    ...
    long    threadId     = WaveDatabase.threads().getThreadIdIfExistsFor(recipient.getId());
    boolean firstMessage = WaveDatabase.messages().getOutgoingSecureMessageCount(threadId) == 0;
  ...
  public static @Nullable Integer setAndSendUniversalExpireTimerIfNecessary(@NonNull Context context, @NonNull Recipient recipient, long threadId) {
    int defaultTimer = WaveStore.settings().getUniversalExpireTimer();
    if (defaultTimer == 0 || recipient.isGroup() || recipient.isDistributionList() || recipient.getExpiresInSeconds() != 0 || !recipient.isRegistered()) {
      return null;
    }

    if (threadId == -1 || WaveDatabase.messages().canSetUniversalTimer(threadId)) {
      int expireTimerVersion = WaveDatabase.recipients().setExpireMessagesAndIncrementVersion(recipient.getId(), defaultTimer);
      ...
      MessageSender.send(context, outgoingMessage, WaveDatabase.threads().getOrCreateThreadIdFor(recipient), MessageSender.SendType.SIGNAL, null, null);
  ...
  public static boolean hasSentMessageInThread(@Nullable Long threadId) {
    return threadId != null && WaveDatabase.messages().getOutgoingSecureMessageCount(threadId) != 0;
  }
  ...
  private static boolean noSecureMessagesAndNoCallsInThread(@Nullable Long threadId) {
    ...
    return WaveDatabase.messages().getSecureMessageCount(threadId) == 0 &&
           !WaveDatabase.threads().hasReceivedAnyCallsSince(threadId, 0);
  }

  @WorkerThread
  private static boolean isProfileSharedViaGroup(@NonNull Recipient recipient) {
    return Stream.of(WaveDatabase.groups().getPushGroupsContainingMember(recipient.getId()))
