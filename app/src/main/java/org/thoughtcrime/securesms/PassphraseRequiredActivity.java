...
import org.wave.devicetransfer.TransferStatus;
...
public abstract class PassphraseRequiredActivity extends BaseActivity implements MasterSecretListener {
  ...
  private WaveServiceNetworkAccess networkAccess;
  ...
  protected final void onCreate(Bundle savedInstanceState) {
    ...
    this.networkAccess = AppDependencies.getWaveServiceNetworkAccess();
    onPreCreate();
  ...
  private Intent getIntentForState(int state) {
    ...
    switch (state) {
      ...
      case STATE_ENTER_SIGNAL_PIN:    return getEnterWavePinIntent();
      case STATE_CREATE_SIGNAL_PIN:   return getCreateWavePinIntent();
  ...
  private int getApplicationState(boolean locked) {
    ...
    } else if (WaveStore.storageService().getNeedsAccountRestore()) {
      return STATE_ENTER_SIGNAL_PIN;
    } else if (userMustSetProfileName()) {
      return STATE_CREATE_PROFILE_NAME;
    } else if (userMustCreateWavePin()) {
      return STATE_CREATE_SIGNAL_PIN;
    } else if (EventBus.getDefault().getStickyEvent(TransferStatus.class) != null && getClass() != OldDeviceTransferActivity.class) {
      return STATE_TRANSFER_ONGOING;
    } else if (WaveStore.misc().isOldDeviceTransferLocked()) {
      return STATE_TRANSFER_LOCKED;
    } else if (WaveStore.misc().isChangeNumberLocked() && getClass() != ChangeNumberLockActivity.class) {
  ...
  private boolean userCanTransferOrRestore() {
    return !WaveStore.registration().isRegistrationComplete() &&
           RestoreDecisionStateUtil.isDecisionPending(WaveStore.registration().getRestoreDecisionState());
  }

  private boolean userMustCreateWavePin() {
    return !WaveStore.registration().isRegistrationComplete() &&
           !WaveStore.svr().hasPin() &&
           !WaveStore.svr().lastPinCreateFailed() &&
           !WaveStore.svr().hasOptedOut() &&
           WaveStore.account().isPrimaryDevice();
  }

  private boolean userMustSetProfileName() {
    return !WaveStore.registration().isRegistrationComplete() && Recipient.self().getProfileName().isEmpty();
  }
  ...
  private Intent getEnterWavePinIntent() {
    return getRoutedIntent(PinRestoreActivity.class, getIntent());
  }

  private Intent getCreateWavePinIntent() {
  ...
  private void initializeClearKeyReceiver() {
    this.clearKeyReceiver = new BroadcastReceiver() {
      ...
      public void onReceive(Context context, Intent intent) {
        Log.i(TAG, "onReceive() for clear key event. PasswordDisabled: " + WaveStore.settings().getPassphraseDisabled() + ", ScreenLock: " + WaveStore.settings().getScreenLockEnabled());
