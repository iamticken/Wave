...
public class UnknownStorageIdTable extends DatabaseTable {
  ...
  public UnknownStorageIdTable(Context context, WaveDatabase databaseHelper) {
  ...
  public List<StorageId> getAllUnknownIds() {
    ...
    try (Cursor cursor = databaseHelper.getWaveReadableDatabase().query(TABLE_NAME, null, null, null, null, null, null)) {
  ...
  public List<StorageId> getAllWithTypes(List<Integer> types) {
    ...
    try (Cursor cursor = databaseHelper.getWaveReadableDatabase().query(TABLE_NAME, null, query.getWhere(), query.getWhereArgs(), null, null, null)) {
      while (cursor != null && cursor.moveToNext()) {
        String keyEncoded = CursorUtil.requireString(cursor, STORAGE_ID);
        int    type       = CursorUtil.requireInt(cursor, TYPE);
        try {
  ...
  public @Nullable WaveStorageRecord getById(@NonNull byte[] rawId) {
    String   query = STORAGE_ID + " = ?";
    String[] args  = new String[] { Base64.encodeWithPadding(rawId) };

    try (Cursor cursor = databaseHelper.getWaveReadableDatabase().query(TABLE_NAME, null, query, args, null, null, null)) {
      if (cursor != null && cursor.moveToFirst()) {
        int type = CursorUtil.requireInt(cursor, TYPE);
        return WaveStorageRecord.forUnknown(StorageId.forType(rawId, type));
      } else {
        return null;
      }
    }
  ...
  public void insert(@NonNull Collection<WaveStorageRecord> inserts) {
    SQLiteDatabase db = databaseHelper.getWaveWritableDatabase();

    Preconditions.checkArgument(db.inTransaction(), "Must be in a transaction!");

    for (WaveStorageRecord insert : inserts) {
  ...
  public void deleteAll() {
    databaseHelper.getWaveWritableDatabase().delete(TABLE_NAME, null, null);
  }
}
