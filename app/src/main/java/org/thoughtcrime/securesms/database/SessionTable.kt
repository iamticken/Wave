...
class SessionTable(context: Context, databaseHelper: WaveDatabase) : DatabaseTable(context, databaseHelper) {
  companion object {
  ...
  fun store(serviceId: ServiceId, address: WaveProtocolAddress, record: SessionRecord) {
  ...
  fun load(serviceId: ServiceId, address: WaveProtocolAddress): SessionRecord? {
  ...
  fun load(serviceId: ServiceId, addresses: List<WaveProtocolAddress>): List<SessionRecord?> {
    ...
    val sessions: HashMap<WaveProtocolAddress, SessionRecord?> = LinkedHashMap(addresses.size)
      ...
      readableDatabase.query(TABLE_NAME, projection, combinedQuery.where, combinedQuery.whereArgs, null, null, null).use { cursor ->
        while (cursor.moveToNext()) {
          ...
          try {
            ...
            sessions[WaveProtocolAddress(address, device)] = record
          } catch (e: IOException) {
            Log.w(TAG, e)
          }
        }
  ...
  fun getSubDevices(serviceId: ServiceId, addressName: String): List<Int> {
    ...
    val selection = "$ACCOUNT_ID = ? AND $ADDRESS = ? AND $DEVICE != ?"
    val args = SqlUtil.buildArgs(serviceId, addressName, WaveServiceAddress.DEFAULT_DEVICE_ID)
  ...
  fun delete(serviceId: ServiceId, address: WaveProtocolAddress) {
