...
import org.wave.core.util.CursorExtensionsKt;
import org.wave.core.util.CursorUtil;
import org.wave.core.util.SQLiteDatabaseExtensionsKt;
...
public final class PaymentTable extends DatabaseTable implements RecipientIdDatabaseReference {
  ...
  private final MutableLiveData<Object> changeWave;

  PaymentTable(@NonNull Context context, @NonNull WaveDatabase databaseHelper) {
    super(context, databaseHelper);

    this.changeWave = new MutableLiveData<>(new Object());
  }
      ...
      throws PublicKeyConflictException, SerializationException
  {
    ...
    SQLiteDatabase database = databaseHelper.getWaveWritableDatabase();
  ...
  public @NonNull List<PaymentTransaction> getAll() {
    SQLiteDatabase           database = databaseHelper.getWaveReadableDatabase();
  ...
  public @NonNull List<PaymentTransaction> getUnseenPayments() {
    SQLiteDatabase           db      = databaseHelper.getWaveReadableDatabase();
  ...
  public @NonNull LiveData<List<PaymentTransaction>> getAllLive() {
    return LiveDataUtil.mapAsync(changeWave, change -> getAll());
  }
  ...
  private void notifyChanged() {
    changeWave.postValue(new Object());
  ...
  private void notifyUuidChanged(@Nullable UUID uuid) {
    if (uuid != null) {
      ...
      MessageId messageId = WaveDatabase.messages().getPaymentMessage(uuid);
      if (messageId != null) {
