...
import org.wave.core.util.SqlUtil.buildCustomCollectionQuery
import org.wave.core.util.SqlUtil.buildSingleCollectionQuery
import org.wave.core.util.SqlUtil.buildTrueUpdateQuery
import org.wave.core.util.SqlUtil.getNextAutoIncrementId
...
import org.wave.core.util.readToSingleLongOrNull
...
import org.wave.core.util.requireLongOrNull
...
import org.wave.core.util.toSingleLine
...
import org.thoughtcrime.securesms.database.WaveDatabase.Companion.attachments
import org.thoughtcrime.securesms.database.WaveDatabase.Companion.calls
import org.thoughtcrime.securesms.database.WaveDatabase.Companion.distributionLists
import org.thoughtcrime.securesms.database.WaveDatabase.Companion.groupReceipts
import org.thoughtcrime.securesms.database.WaveDatabase.Companion.groups
import org.thoughtcrime.securesms.database.WaveDatabase.Companion.mentions
import org.thoughtcrime.securesms.database.WaveDatabase.Companion.messages
import org.thoughtcrime.securesms.database.WaveDatabase.Companion.polls
import org.thoughtcrime.securesms.database.WaveDatabase.Companion.reactions
import org.thoughtcrime.securesms.database.WaveDatabase.Companion.recipients
import org.thoughtcrime.securesms.database.WaveDatabase.Companion.storySends
...
open class MessageTable(context: Context?, databaseHelper: WaveDatabase) : DatabaseTable(context, databaseHelper), MessageTypes, RecipientIdDatabaseReference, ThreadIdDatabaseReference {
  ...
  fun updateGroupCall(
    ...
    writableDatabase.withinTransaction { db ->
      ...
      val updateDetail = GroupCallUpdateDetailsUtil.parse(message.body)
      val containsSelf = joinedUuids.contains(WaveStore.account.requireAci().rawUuid)
  ...
  fun updatePreviousGroupCall(
    ...
    writableDatabase.withinTransaction { db ->
      ...
      MmsReader(cursor).use { reader ->
        ...
        val containsSelf = peekJoinedUuids.contains(WaveStore.account.requireAci().rawUuid)
  ...
  fun insertProfileNameChangeMessages(recipient: Recipient, newProfileName: String, previousProfileName: String) {
    writableDatabase.withinTransaction { db ->
      ...
      groupRecords.filter { it.isV2Group }.forEach {
        WaveDatabase.nameCollisions.handleGroupNameCollisions(it.id.requireV2(), setOf(recipient.id))
      }
    }
  }
  ...
  fun insertLearnedProfileNameChangeMessage(recipient: Recipient, e164: String?, username: String?) {
    ...
    val threadId: Long? = WaveDatabase.threads.getThreadIdFor(recipient.id)
  ...
  private fun insertGroupV1MigrationMembershipChanges(recipientId: RecipientId, threadId: Long, membershipChange: GroupMigrationMembershipChange) {
    ...
    databaseHelper.waveWritableDatabase.insert(TABLE_NAME, null, values)
  }
  ...
  fun ensureMigration() {
    databaseHelper.waveWritableDatabase
  }
  ...
  fun markOnboardingStoryRead() {
    val recipientId = WaveStore.releaseChannel.releaseChannelRecipientId ?: return
  ...
  fun setNetworkFailures(messageId: Long, failures: Set<NetworkFailure?>?) {
    try {
      setDocument(databaseHelper.waveWritableDatabase, messageId, NETWORK_FAILURES, NetworkFailureSet(failures))
    } catch (e: IOException) {
      Log.w(TAG, e)
    }
  }
  ...
  private fun rawQueryWithAttachments(where: String, arguments: Array<String>?, reverse: Boolean = false, limit: Long = 0, orderBy: String = ""): Cursor {
    val database = databaseHelper.waveReadableDatabase
    var rawQueryString = """
      SELECT 
    ...
    return mmsReaderFor(cursor).use { reader ->
      reader.mapNotNull {
        if (!it.isMms) {
          ...
          WaveDatabase.payments.updateMessageWithPayment(it) as MmsMessageRecord
        } else {
  ...
  private fun markAsRemoteDeleteInternal(messageId: Long) {
    ...
    writableDatabase.withinTransaction { db ->
      ...
      WaveDatabase.messageLog.deleteAllRelatedToMessage(messageId)
  ...
  fun insertMessageOutbox(
    ...
    contentValues.put(FROM_DEVICE_ID, WaveStore.account.deviceId)
  ...
  private fun insertMediaMessage(
    ...
    val (messageId, insertedAttachments) = writableDatabase.withinTransaction { db ->
      ...
      WaveDatabase.mentions.insert(threadId, messageId, mentions)

      val insertedAttachments = WaveDatabase.attachments.insertAttachmentsForMessage(messageId, allAttachments, quoteAttachments)
  ...
  fun getAllRateLimitedMessageIds(): Set<Long> {
    val db = databaseHelper.waveReadableDatabase
  ...
  fun deleteMessagesInThread(threadIds: Collection<Long>, extraWhere: String = ""): Int {
    ...
    writableDatabase.withinTransaction { db ->
      WaveDatabase.messageSearch.dropAfterMessageDeleteTrigger()
      WaveDatabase.messageLog.dropAfterMessageDeleteTrigger()
      ...
      WaveDatabase.messageSearch.restoreAfterMessageDeleteTrigger()
      WaveDatabase.messageLog.restoreAfterMessageDeleteTrigger()
    }
  ...
  fun deleteMessages(messagesToDelete: List<SyncMessageId>): List<SyncMessageId> {
    ...
    threads
      .forEach { threadId ->
        WaveDatabase.threads.update(threadId, unarchive = false)
        notifyConversationListeners(threadId)
      }
  ...
  fun setMismatchedIdentities(messageId: Long, mismatches: Set<IdentityKeyMismatch?>) {
    try {
      setDocument(
        database = databaseHelper.waveWritableDatabase,
        messageId = messageId,
        column = MISMATCHED_IDENTITIES,
  ...
  private fun getReleaseChannelThreadId(hasSeenReleaseChannelStories: Boolean): Long {
    ...
    val releaseChannelRecipientId = WaveStore.releaseChannel.releaseChannelRecipientId ?: return -1L
