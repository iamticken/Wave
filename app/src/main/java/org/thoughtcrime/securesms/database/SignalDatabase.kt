...
import java.io.File
import org.thoughtcrime.securesms.database.SQLiteDatabase as WaveSQLiteDatabase

open class WaveDatabase(private val context: Application, databaseSecret: DatabaseSecret, attachmentSecret: AttachmentSecret, name: String = DATABASE_NAME) :
  SQLiteOpenHelper(
    context,
    name,
    databaseSecret.asString(),
    null,
    WaveDatabaseMigrations.DATABASE_VERSION,
    0,
    SqlCipherErrorHandler(context, name),
    SqlCipherDatabaseHook(),
    true
  ),
  WaveDatabaseOpenHelper {
  ...
  override fun onCreate(db: net.zetetic.database.sqlcipher.SQLiteDatabase) {
    val waveDb = WaveSQLiteDatabase(db)
    onCreateTablesIndexesAndTriggers(waveDb)

    // Requires FTS5
    executeStatements(waveDb, SearchTable.CREATE_TABLE)
    executeStatements(waveDb, SearchTable.CREATE_TRIGGERS)
  ...
  @VisibleForTesting
  fun onCreateTablesIndexesAndTriggers(db: WaveSQLiteDatabase) {
  ...
  override fun onUpgrade(db: net.zetetic.database.sqlcipher.SQLiteDatabase, oldVersion: Int, newVersion: Int) {
    ...
    try {
      ...
      WaveDatabaseMigrations.migrate(context, WaveSQLiteDatabase(db), oldVersion, newVersion)
    } finally {
      db.setForeignKeyConstraintsEnabled(true)
    ...
    WaveDatabaseMigrations.migratePostTransaction(context, oldVersion)
    Log.i(TAG, "Upgrade complete. Took " + (System.currentTimeMillis() - startTime) + " ms.")
  }

  override val readableDatabase: net.zetetic.database.sqlcipher.SQLiteDatabase
    get() = throw UnsupportedOperationException("Call getWaveReadableDatabase() instead!")

  override val writableDatabase: net.zetetic.database.sqlcipher.SQLiteDatabase
    get() = throw UnsupportedOperationException("Call getWaveWritableDatabase() instead!")
  ...
  open val waveReadableDatabase: WaveSQLiteDatabase
    get() = WaveSQLiteDatabase(super.readableDatabase)

  open val waveWritableDatabase: WaveSQLiteDatabase
    get() = WaveSQLiteDatabase(super.writableDatabase)
  ...
  open fun markCurrent(db: net.zetetic.database.sqlcipher.SQLiteDatabase) {
    db.version = WaveDatabaseMigrations.DATABASE_VERSION
  }
  ...
  companion object {
    private val TAG = Log.tag(WaveDatabase::class.java)
    const val DATABASE_NAME = "wave.db"

    @JvmStatic
    @Volatile
    var instance: WaveDatabase? = null
      private set

    @JvmStatic
    fun init(application: Application, databaseSecret: DatabaseSecret, attachmentSecret: AttachmentSecret) {
      if (instance == null) {
        synchronized(WaveDatabase::class.java) {
          if (instance == null) {
            instance = WaveDatabase(application, databaseSecret, attachmentSecret)
          }
        }
      }
    }
    ...
    @JvmStatic
    @VisibleForTesting
    fun setWaveDatabaseInstanceForTesting(waveDatabase: WaveDatabase) {
      this.instance = waveDatabase
    }
    ...
    @JvmStatic
    val readableDatabase: WaveSQLiteDatabase
      get() = instance!!.waveReadableDatabase

    @JvmStatic
    val writableDatabase: WaveSQLiteDatabase
      get() = instance!!.waveWritableDatabase
    ...
    fun runPostSuccessfulTransaction(dedupeKey: String, task: Runnable) {
      instance!!.waveWritableDatabase.runPostSuccessfulTransaction(dedupeKey, task)
    }

    @JvmStatic
    fun runPostSuccessfulTransaction(task: Runnable) {
      instance!!.waveWritableDatabase.runPostSuccessfulTransaction(task)
    }
    ...
    @JvmStatic
    fun <T> runInTransaction(block: (WaveSQLiteDatabase) -> T): T {
      return instance!!.waveWritableDatabase.withinTransaction {
        block(it)
      }
    }
