 ...
 */
class MessageSendLogTables constructor(context: Context?, databaseHelper: WaveDatabase?) : DatabaseTable(context, databaseHelper), RecipientIdDatabaseReference {
  ...
  fun addRecipientToExistingEntryIfPossible(payloadId: Long, recipientId: RecipientId, sentTimestamp: Long, sendMessageResult: SendMessageResult, contentHint: ContentHint, messageId: MessageId, urgent: Boolean): Long {
    ...
    if (sendMessageResult.isSuccess && sendMessageResult.success.content.isPresent) {
      val db = databaseHelper.waveWritableDatabase
  ...
  fun deleteAll() {
    databaseHelper.waveWritableDatabase.delete(MslPayloadTable.TABLE_NAME, null, null)
  }
  ...
  fun dropAfterMessageDeleteTrigger() {
    check(WaveDatabase.inTransaction)
