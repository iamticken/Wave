 ...
 */
class ChatFolderTables(context: Context?, databaseHelper: WaveDatabase?) : DatabaseTable(context, databaseHelper), ThreadIdDatabaseReference {
  ...
  fun getUnreadCountAndEmptyAndMutedStatusForFolders(folders: List<ChatFolderRecord>): HashMap<Long, Triple<Int, Boolean, Boolean>> {
    ...
    folders.map { folder ->
      val unreadCount = WaveDatabase.threads.getUnreadCountByChatFolder(folder)
      val isEmpty = !WaveDatabase.threads.hasChatInFolder(folder)
      val isMuted = !WaveDatabase.threads.hasUnmutedChatsInFolder(folder)
   ...
   */
  fun insertChatFolderFromStorageSync(record: WaveChatFolderRecord) {
   ...
   */
  fun updateChatFolderFromStorageSync(record: WaveChatFolderRecord) {
  ...
  /**
   * Parses a remote chat folder [WaveChatFolderRecord] into a local folder [ChatFolderRecord]
   */
  private fun remoteChatFolderRecordToLocal(record: WaveChatFolderRecord): ChatFolderRecord {
    ...
    return ChatFolderRecord(
      ...
      includedChats = record.proto.includedRecipients
        ...
        .map { recipient -> WaveDatabase.threads.getOrCreateThreadIdFor(recipient) },
