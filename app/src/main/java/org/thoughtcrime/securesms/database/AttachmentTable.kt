...
import org.wave.core.util.groupBy
import org.wave.core.util.isNull
...
import org.wave.core.util.requireNonNullBlob
import org.wave.core.util.requireNonNullString
import org.wave.core.util.requireObject
...
class AttachmentTable(
  ...
  fun deleteAbandonedAttachmentFiles(): Int {
    ...
    filesInDb += WaveDatabase.stickers.getAllStickerFiles()
  ...
  fun getMediaObjectsThatCantBeFound(objects: Set<ArchivedMediaObject>): Set<ArchivedMediaObject> {
    ...
    readableDatabase
      ...
      .forEach { cursor ->
        ...
        val mediaIdThumbnail = MediaName.fromPlaintextHashAndRemoteKeyForThumbnail(plaintextHash, remoteKey).toMediaId(WaveStore.backup.mediaRootBackupKey).encode()
    ...
    run {
      readableDatabase
        ...
        .forEach { cursor ->
          val remoteKey = Base64.decode(cursor.requireNonNullString(REMOTE_KEY))
          val plaintextHash = Base64.decode(cursor.requireNonNullString(DATA_HASH_END))
          val mediaId = MediaName.fromPlaintextHashAndRemoteKey(plaintextHash, remoteKey).toMediaId(WaveStore.backup.mediaRootBackupKey).value.toByteString()
          val mediaIdThumbnail = MediaName.fromPlaintextHashAndRemoteKeyForThumbnail(plaintextHash, remoteKey).toMediaId(WaveStore.backup.mediaRootBackupKey).value.toByteString()
  ...
  fun debugGetAttachmentStats(): DebugAttachmentStats {
    ...
    val lastSnapshotFullSizeCount = WaveDatabase.backupMediaSnapshots.debugGetFullSizeAttachmentCountForMostRecentSnapshot()
    val lastSnapshotThumbnailCount = WaveDatabase.backupMediaSnapshots.debugGetThumbnailAttachmentCountForMostRecentSnapshot()
