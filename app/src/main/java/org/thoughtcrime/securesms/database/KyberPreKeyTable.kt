...
/**
 * A table for storing data related to [org.thoughtcrime.securesms.crypto.storage.WaveKyberPreKeyStore].
 */
class KyberPreKeyTable(context: Context, databaseHelper: WaveDatabase) : DatabaseTable(context, databaseHelper) {
  companion object {
  ...
  fun handleMarkKyberPreKeyUsed(serviceId: ServiceId, kyberPreKeyId: Int, signedPreKeyId: Int, baseKey: ECPublicKey) {
    writableDatabase.withinTransaction { db ->
      ...
      if (lastResortRowId < 0) {
        ...
        WaveDatabase.lastResortKeyTuples.insert(
