...
import org.wave.core.util.areForeignKeyConstraintsEnabled
import org.wave.core.util.logging.Log
import org.thoughtcrime.securesms.database.WaveDatabase
import org.thoughtcrime.securesms.database.helpers.migration.WaveDatabaseMigration
...
/**
 * Contains all of the database migrations for [WaveDatabase]. Broken into a separate file for cleanliness.
 */
object WaveDatabaseMigrations {

  val TAG: String = Log.tag(WaveDatabaseMigrations.javaClass)

  private val migrations: List<Pair<Int, WaveDatabaseMigration>> = listOf(
  ...
  @JvmStatic
  fun migrate(context: Application, db: WaveSqliteDatabase, oldVersion: Int, newVersion: Int) {
    ...
    for (migrationData in eligibleMigrations) {
      ...
      if (ftsException != null) {
        ...
        WaveDatabase.messageSearch.fullyResetTables(db)
        throw ftsException
      }
