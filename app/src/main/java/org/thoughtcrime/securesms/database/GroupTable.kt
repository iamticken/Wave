...
import org.wave.core.util.SqlUtil.appendArg
import org.wave.core.util.SqlUtil.buildArgs
import org.wave.core.util.SqlUtil.buildCaseInsensitiveGlobPattern
import org.wave.core.util.SqlUtil.buildCollectionQuery
...
import org.wave.core.util.readToMap
...
import org.whispersystems.waveservice.api.groupsv2.findMemberByAci
import org.whispersystems.waveservice.api.groupsv2.findPendingByServiceId
import org.whispersystems.waveservice.api.groupsv2.findRequestingByAci
import org.whispersystems.waveservice.api.groupsv2.toAciList
import org.whispersystems.waveservice.api.groupsv2.toAciListWithUnknowns
...
class GroupTable(context: Context?, databaseHelper: WaveDatabase?) :
  ...
  class MembershipTable(context: Context, databaseHelper: WaveDatabase) : DatabaseTable(context, databaseHelper) {
    companion object {
  ...
  fun queryGroupsByMemberName(inputQuery: String): Cursor {
    ...
    return databaseHelper.waveReadableDatabase.query(statement, subquery.whereArgs)
  }
  ...
  fun queryGroupsByTitle(inputQuery: String, includeInactive: Boolean, excludeV1: Boolean, excludeMms: Boolean): Reader {
    ...
    val cursor = databaseHelper.waveReadableDatabase.query(statement, query.whereArgs)
  ...
  private fun queryGroupsByRecency(groupQuery: GroupQuery): Reader {
    ...
    return Reader(databaseHelper.waveReadableDatabase.rawQuery(sql, query.whereArgs))
  }
  ...
  @CheckReturnValue
  fun create(groupId: GroupId.V1, title: String?, members: Collection<RecipientId>, avatar: WaveServiceAttachmentPointer?): Boolean {
  ...
  private fun create(
    ...
    avatar: WaveServiceAttachmentPointer?,
    groupMasterKey: GroupMasterKey?,
    ...
    if (avatar != null) {
      values.put(AVATAR_ID, (avatar.remoteId as WaveServiceAttachmentRemoteId.V2).cdnId)
    ...
    if (groupId.isV2) {
      WaveDatabase.nameCollisions.handleGroupNameCollisions(groupId.requireV2(), members.toSet())
    }

    return true
  }

  fun update(groupId: GroupId.V1, title: String?, avatar: WaveServiceAttachmentPointer?) {
    val contentValues = ContentValues().apply {
      ...
      if (avatar != null) {
        put(AVATAR_ID, (avatar.remoteId as WaveServiceAttachmentRemoteId.V2).cdnId)
  ...
  fun update(groupId: GroupId.V2, decryptedGroup: DecryptedGroup, receivedGroupSendEndorsements: ReceivedGroupSendEndorsements?) {
    ...
    if (groupId.isV2 && addedMembers.isNotEmpty()) {
      WaveDatabase.nameCollisions.handleGroupNameCollisions(groupId.requireV2(), addedMembers.toSet())
    }
  }
  ...
  fun getGroupSendFullToken(threadId: Long, recipientId: RecipientId): GroupSendFullToken? {
    val threadRecipient = WaveDatabase.threads.getRecipientForThreadId(threadId)
  ...
  fun getGroupSendFullToken(groupId: GroupId.V2, recipientId: RecipientId): GroupSendFullToken? {
    val groupRecord = WaveDatabase.groups.getGroup(groupId).orElse(null) ?: return null
    val endorsement = WaveDatabase.groups.getGroupSendEndorsement(groupId, recipientId) ?: return null
  ...
  class V2GroupProperties(val groupMasterKey: GroupMasterKey, val groupRevision: Int, private val decryptedGroupBytes: ByteArray) {
    ...
    fun getMemberRecipientIds(memberSet: MemberSet): List<RecipientId> {
      ...
      val selfAci = WaveStore.account.requireAci()
  ...
  private fun gv2GroupActive(decryptedGroup: DecryptedGroup): Boolean {
    val aci = WaveStore.account.requireAci()
