...
class ChatColorsTable(context: Context, databaseHelper: WaveDatabase) : DatabaseTable(context, databaseHelper) {
  ...
  private fun insertChatColors(chatColors: ChatColors): ChatColors {
    ...
    val db: SQLiteDatabase = databaseHelper.waveWritableDatabase
  ...
  private fun updateChatColors(chatColors: ChatColors): ChatColors {
    ...
    if (WaveStore.chatColors.chatColors?.id == chatColors.id) {
      WaveStore.chatColors.chatColors = chatColors
    }

    WaveDatabase.recipients.onUpdatedChatColors(chatColors)
    notifyListeners()
  ...
  fun deleteChatColors(chatColors: ChatColors) {
    ...
    WaveDatabase.recipients.onDeletedChatColors(chatColors)
    notifyListeners()
  }
