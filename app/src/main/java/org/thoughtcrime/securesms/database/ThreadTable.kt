...
import org.wave.core.util.readToSingleIntOrNull
...
import org.thoughtcrime.securesms.database.WaveDatabase.Companion.drafts
...
import org.thoughtcrime.securesms.database.WaveDatabase.Companion.messageLog
...
import org.whispersystems.waveservice.api.storage.toWaveServiceAddress
...
class ThreadTable(context: Context, databaseHelper: WaveDatabase) : DatabaseTable(context, databaseHelper) {
  ...
  private fun allowedToUnarchive(threadId: Long): Boolean {
    if (!WaveStore.settings.shouldKeepMutedChatsArchived()) {
      return true
    }
  ...
  fun trimAllThreads(length: Int, trimBeforeDate: Long) {
    ...
    val syncThreadTrimDeletes = WaveStore.settings.shouldSyncThreadTrimDeletes()
  ...
  private fun trimThreadInternal(
    ...
    if (finalTrimBeforeDate != NO_TRIM_BEFORE_DATE_SET) {
      ...
      if (deletes > 0) {
        ...
        WaveDatabase.calls.updateCallEventDeletionTimestamps()
  ...
  fun getRecentConversationList(limit: Int, includeInactiveGroups: Boolean, individualsOnly: Boolean, groupsOnly: Boolean, hideV1Groups: Boolean, hideSms: Boolean, hideSelf: Boolean): Cursor {
    ...
    if (WaveStore.releaseChannel.releaseChannelRecipientId != null) {
      where += " AND $TABLE_NAME.$RECIPIENT_ID != ${WaveStore.releaseChannel.releaseChannelRecipientId!!.toLong()}"
    }
  ...
  fun deleteAllConversations() {
    writableDatabase.withinTransaction { db ->
      ...
      WaveDatabase.calls.deleteAllCalls()
      synchronized(threadIdCache) {
        threadIdCache.clear()
      }
    }
  ...
  fun applyStorageSyncUpdate(recipientId: RecipientId, record: WaveContactRecord) {
    applyStorageSyncUpdate(recipientId, record.proto.archived, record.proto.markedUnread, isGroup = false)
  }

  fun applyStorageSyncUpdate(recipientId: RecipientId, record: WaveGroupV1Record) {
    applyStorageSyncUpdate(recipientId, record.proto.archived, record.proto.markedUnread, isGroup = true)
  }

  fun applyStorageSyncUpdate(recipientId: RecipientId, record: WaveGroupV2Record) {
    applyStorageSyncUpdate(recipientId, record.proto.archived, record.proto.markedUnread, isGroup = true)
  }

  fun applyStorageSyncUpdate(recipientId: RecipientId, record: WaveAccountRecord) {
    writableDatabase.withinTransaction { db ->
      ...
      for (pinned: AccountRecord.PinnedConversation in record.proto.pinnedConversations) {
        val pinnedRecipient: Recipient? = if (pinned.contact != null) {
          if (ServiceId.parseOrNull(pinned.contact!!.serviceId, pinned.contact!!.serviceIdBinary) != null) {
            Recipient.externalPush(pinned.contact!!.toWaveServiceAddress())
          } else {
  ...
  private fun hasMoreRecentDraft(threadId: Long, timestamp: Long): Boolean {
    val drafts: DraftTable.Drafts = WaveDatabase.drafts.getDrafts(threadId)
  ...
  fun merge(primaryRecipientId: RecipientId, secondaryRecipientId: RecipientId): MergeResult {
    check(databaseHelper.waveWritableDatabase.inTransaction()) { "Must be in a transaction!" }
