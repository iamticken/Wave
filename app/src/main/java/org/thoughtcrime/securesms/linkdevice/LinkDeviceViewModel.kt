...
class LinkDeviceViewModel : ViewModel() {
  ...
  private fun checkForNewDevice(devices: List<Device>) {
    val newLinkedDeviceId = WaveStore.misc.newLinkedDeviceId
    val newLinkedDeviceCreatedAt = WaveStore.misc.newLinkedDeviceCreatedTime
    ...
    if (hasNewLinkedDevice) {
      ...
      WaveStore.misc.newLinkedDeviceId = 0
      WaveStore.misc.newLinkedDeviceCreatedTime = 0
    }
  ...
  fun markQrEducationSheetSeen() {
    WaveStore.uiHints.markHasSeenLinkDeviceQrEducationSheet()
    _state.update {
      it.copy(
  ...
  private fun addDeviceWithSync(linkUri: Uri) {
    ...
    val uploadResult = LinkDeviceRepository.createAndUploadArchive(
      ...
      cancellationWave = { _state.value.shouldCancelArchiveUpload }
    )
