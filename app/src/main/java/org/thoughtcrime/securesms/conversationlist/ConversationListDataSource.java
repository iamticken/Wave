...
import org.thoughtcrime.securesms.util.WaveTrace;
...
abstract class ConversationListDataSource implements PagedDataSource<Long, Conversation> {
  ...
  @Override
  public @NonNull List<Conversation> load(int start, int length, int totalSize, @NonNull CancellationWave cancellationWave) {
    WaveTrace.beginSection("ConversationListDataSource#load");
    ...
    try (ConversationReader reader = new ConversationReader(getCursor(start, length))) {
      ThreadRecord record;
      while ((record = reader.getNext()) != null && !cancellationWave.isCanceled()) {
