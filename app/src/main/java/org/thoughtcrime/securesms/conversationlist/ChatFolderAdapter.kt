    ...
    override fun bind(model: ChatFolderMappingModel) {
      ...
      if (model.isSelected) {
        itemView.backgroundTintList = if (callbacks.isScrolled()) {
          ColorStateList.valueOf(ContextCompat.getColor(itemView.context, R.color.wave_colorBackground))
        } else {
          ColorStateList.valueOf(ContextCompat.getColor(itemView.context, R.color.wave_colorSurface2))
        }
        name.setTextColor(ContextCompat.getColor(itemView.context, R.color.wave_colorOnSurface))
      } else {
        itemView.backgroundTintList = ColorStateList.valueOf(ContextCompat.getColor(itemView.context, R.color.transparent))
        name.setTextColor(ContextCompat.getColor(itemView.context, R.color.wave_colorOnSurfaceVariant))
      }
    }
