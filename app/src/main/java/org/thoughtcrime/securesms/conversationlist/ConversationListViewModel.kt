...
import org.wave.paging.ProxyPagingController
...
sealed class ConversationListViewModel(
  ...
  init {
    ...
    conversationListDataSource = saveableStateFlowable
      ...
      .map { (filterRequest, folder) ->
        ConversationListDataSource.create(
          ...
          isArchived,
          WaveStore.uiHints.canDisplayPullToFilterTip() && filterRequest.source === ConversationFilterSource.OVERFLOW
        )
      }
      .replay(1)
      .refCount()
    ...
    val pinnedCount = RxDatabaseObserver
      .conversationList
      .map { WaveDatabase.threads.getPinnedConversationListCount(ConversationFilter.OFF) }
      .distinctUntilChanged()
    ...
    hasNoConversations = store
      ...
      .map { (filterRequest, conversations) ->
        if (conversations.isNotEmpty()) {
          ...
          val archivedCount = WaveDatabase.threads.getArchivedConversationListCount(filterRequest.filter)
          val unarchivedCount = WaveDatabase.threads.getUnarchivedConversationListCount(filterRequest.filter)
  ...
  fun onUpdateMute(chatFolder: ChatFolderRecord, until: Long) {
    viewModelScope.launch(Dispatchers.IO) {
      val ids = WaveDatabase.threads.getRecipientIdsByChatFolder(chatFolder)
  ...
  fun markChatFolderRead(chatFolder: ChatFolderRecord) {
    viewModelScope.launch(Dispatchers.IO) {
      val ids = WaveDatabase.threads.getThreadIdsByChatFolder(chatFolder)
      val messageIds = WaveDatabase.threads.setRead(ids)
  ...
  fun removeChatFromFolder(threadId: Long) {
    viewModelScope.launch(Dispatchers.IO) {
      WaveDatabase.chatFolders.removeFromFolder(currentFolder.id, threadId)
  ...
  fun addToFolder(folderId: Long, threadIds: List<Long>) {
    viewModelScope.launch(Dispatchers.IO) {
      ...
      WaveDatabase.chatFolders.addToFolder(folderId, threadIdsNotIncluded)
