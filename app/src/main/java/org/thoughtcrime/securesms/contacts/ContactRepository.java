...
public class ContactRepository {
  ...
  private static final List<Pair<String, ValueMapper>> SEARCH_CURSOR_MAPPERS = new ArrayList<Pair<String, ValueMapper>>() {{
    ...
    add(new Pair<>(NUMBER_COLUMN, cursor -> {
      ...
      if (phone != null) {
        phone = WaveE164Util.prettyPrint(phone);
      }
  ...
  @WorkerThread
  public @NonNull Cursor queryWaveContacts(@NonNull String query) {
    return queryWaveContacts(new RecipientTable.ContactSearchQuery(query, new RecipientTable.IncludeSelfMode.IncludeWithRemap(noteToSelfTitle), ContactSearchSortOrder.NATURAL));
  }

  @WorkerThread
  public @NonNull Cursor queryWaveContacts(@NonNull RecipientTable.ContactSearchQuery contactSearchQuery) {
    Cursor cursor = TextUtils.isEmpty(contactSearchQuery.getQuery()) ? recipientTable.getWaveContacts(contactSearchQuery.getIncludeSelfMode())
                                                                     : recipientTable.queryWaveContacts(contactSearchQuery);
