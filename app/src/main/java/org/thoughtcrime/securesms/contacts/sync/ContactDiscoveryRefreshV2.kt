...
import org.wave.contacts.SystemContactsRepository
...
import org.whispersystems.waveservice.api.cds.CdsiV2Service
...
object ContactDiscoveryRefreshV2 {
  ...
  fun refreshAll(context: Context, timeoutMs: Long? = null): ContactDiscovery.RefreshResult {
    val recipientE164s: Set<String> = WaveDatabase.recipients.getAllE164s().sanitize()
    ...
    return refreshInternal(
      ...
      inputPreviousE164s = WaveDatabase.cds.getAllE164s(),
  ...
  fun lookupE164(e164: String): ContactDiscovery.LookupResult? {
    val result = WaveNetwork.cdsApi.getRegisteredUsers(
      previousE164s = emptySet(),
      newE164s = setOf(e164),
      serviceIds = WaveDatabase.recipients.getAllServiceIdProfileKeyPairs(),
      token = Optional.empty(),
      timeoutMs = 10_000,
      libwaveNetwork = AppDependencies.libwaveNetwork
    ) {
      ...
      is NetworkResult.StatusCodeError -> {
        when (val e = result.exception) {
          is CdsiResourceExhaustedException -> {
            ...
            WaveStore.misc.cdsBlockedUtil = System.currentTimeMillis() + e.retryAfterSeconds.seconds.inWholeMilliseconds
            throw e
          }
    ...
    return response.results[e164]?.let { item ->
      val id = WaveDatabase.recipients.processIndividualCdsLookup(e164 = e164, aci = item.aci.orElse(null), pni = item.pni)
  ...
  private fun refreshInternal(
    ...
    val previousE164s: Set<String> = if (WaveStore.misc.cdsToken != null && !isPartialRefresh) inputPreviousE164s else emptySet()
    ...
    if (newE164s.size > RemoteConfig.cdsHardLimit) {
      ...
      WaveStore.misc.markCdsPermanentlyBlocked()
      throw IOException("New contacts over the CDS hard limit!")
    }

    val token: ByteArray? = if (previousE164s.isNotEmpty() && !isPartialRefresh) WaveStore.misc.cdsToken else null
    ...
    val result = WaveNetwork.cdsApi.getRegisteredUsers(
      ...
      if (!isPartialRefresh) {
        WaveStore.misc.cdsToken = tokenToSave
        WaveDatabase.cds.updateAfterFullCdsQuery(previousE164s + newE164s, allE164s + newE164s)
        Log.d(TAG, "Token saved!")
      } else {
        WaveDatabase.cds.updateAfterPartialCdsQuery(newE164s)
      ...
      is NetworkResult.StatusCodeError -> {
        when (val e = result.exception) {
          ...
          is CdsiInvalidTokenException -> {
            ...
            WaveStore.misc.cdsToken = null
            WaveDatabase.cds.clearAll()
            throw e
          }
    ...
    if (!isPartialRefresh && WaveStore.misc.isCdsBlocked) {
      Log.i(TAG, "Successfully made a request while blocked -- clearing blocked state.")
      WaveStore.misc.clearCdsBlocked()
    }
    ...
    WaveDatabase.recipients.rewritePhoneNumbers(fuzzyOutput.rewrites)
    stopwatch.split("rewrite-e164")

    registeredIds += WaveDatabase.recipients.bulkProcessCdsResult(fuzzyOutput.numbers)
    rewrites += fuzzyOutput.rewrites
    stopwatch.split("process-result")

    val existingIds: Set<RecipientId> = WaveDatabase.recipients.getAllPossiblyRegisteredByE164(recipientE164s + rewrites.values)
    ...
    WaveDatabase.recipients.updatePhoneNumberDiscoverability(registeredIds, missingFromCds)

    WaveDatabase.recipients.bulkUpdatedRegisteredStatus(registeredIds, inactiveIds)
  ...
  private fun hasCommunicatedWith(recipient: Recipient): Boolean {
    val localAci = WaveStore.account.requireAci()
    return WaveDatabase.threads.hasActiveThread(recipient.id) || (recipient.hasServiceId && WaveDatabase.sessions.hasSessionFor(localAci, recipient.requireServiceId().toString()))
  }
  ...
  private fun Set<String>.toE164s(): Set<String> {
    return this.mapNotNull { WaveE164Util.formatAsE164(it) }.toSet()
  }
