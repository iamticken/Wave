...
import org.wave.contacts.SystemContactsRepository.ContactIterator
import org.wave.contacts.SystemContactsRepository.ContactPhoneDetails
...
object ContactDiscovery {
  ...
  fun refreshAll(context: Context, notifyOfNewUsers: Boolean) {
    if (TextUtils.isEmpty(WaveStore.account.e164)) {
    ...
    if (!WaveStore.registration.isRegistrationComplete) {
      if (WaveStore.account.isRegistered && WaveStore.svr.lastPinCreateFailed()) {
  ...
  private fun phoneNumberFormatter(): (String) -> String? {
    return { WaveE164Util.formatNonShortCodeAsE164(it) }
  }
  ...
  private fun refreshRecipients(
    ...
    val preExistingRegisteredIds: Set<RecipientId> = WaveDatabase.recipients.getRegistered()
    ...
    if (hasContactsPermissions(context)) {
      ...
      if (TextSecurePreferences.hasSuccessfullyRetrievedDirectory(context) && notifyOfNewUsers) {
        val systemContacts: Set<RecipientId> = WaveDatabase.recipients.getSystemContacts().toSet()
  ...
  private fun notifyNewUsers(context: Context, newUserIds: Collection<RecipientId>) {
    if (!WaveStore.settings.isNotifyWhenContactJoinsWave) return
    ...
    Recipient.resolvedList(newUserIds)
      ...
      .map {
        ...
        WaveDatabase.messages.insertMessageInbox(message)
      }
      .filter { it.isPresent }
      .map { it.get() }
      .forEach { result ->
  ...
  private fun syncRecipientsWithSystemContacts(
    ...
    val localNumber: String = WaveStore.account.e164 ?: ""
    val handle = WaveDatabase.recipients.beginBulkSystemContactUpdate(clearInfoForMissingContacts)
    try {
    ...
    if (NotificationChannels.supported()) {
      WaveDatabase.recipients.getRecipientsWithNotificationChannels().use { reader ->
  ...
  private fun hasSession(id: RecipientId): Boolean {
    ...
    val protocolAddress = Recipient.resolved(id).requireServiceId().toProtocolAddress(WaveServiceAddress.DEFAULT_DEVICE_ID)
