...
import org.wave.libwave.zkgroup.internal.ByteArray;
...
public final class GroupsV2AuthorizationWaveStoreCache implements GroupsV2Authorization.ValueCache {

  private static final String TAG = Log.tag(GroupsV2AuthorizationWaveStoreCache.class);
  ...
  public static GroupsV2AuthorizationWaveStoreCache createAciCache(@NonNull KeyValueStore store) {
    ...
    return new GroupsV2AuthorizationWaveStoreCache(store, ACI_PNI_PREFIX + ":" + ACI_PNI_VERSION);
  }

  private GroupsV2AuthorizationWaveStoreCache(@NonNull KeyValueStore store, @NonNull String key) {
    this.store           = store;
    this.key             = key;
