...
import org.wave.core.util.ByteSerializer;
import org.wave.core.util.StringSerializer;
import org.thoughtcrime.securesms.database.model.databaseprotos.WaveStoreList;
...
public abstract class WaveStoreValues {

  private final KeyValueStore store;

  WaveStoreValues(@NonNull KeyValueStore store) {
    this.store = store;
  }
  ...
  <T> List<T> getList(@NonNull String key, @NonNull StringSerializer<T> serializer) {
    ...
    try {
      WaveStoreList waveStoreList = WaveStoreList.ADAPTER.decode(blob);

      return waveStoreList.contents
                            .stream()
  ...
  <T> void putList(@NonNull String key, @NonNull List<T> values, @NonNull StringSerializer<T> serializer) {
    putBlob(key, new WaveStoreList.Builder()
