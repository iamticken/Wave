 ...
 */
class InAppPaymentValues internal constructor(store: KeyValueStore) : WaveStoreValues(store) {
  ...
  companion object {
     ...
     * Needed to show donation pending sheet after returning to Wave.
     */
  ...
  fun getRecurringDonationCurrency(): Currency {
    ...
    val currency: Currency? = if (currencyCode == null) {
      ...
      if (localeCurrency == null) {
        val e164: String? = WaveStore.account.e164
        if (e164 == null) {
          null
        } else {
  ...
  fun updateLocalStateForManualCancellation(subscriberType: InAppPaymentSubscriberRecord.Type) {
    subscriberType.lock.withLock {
      ...
      if (subscriber != null) {
        WaveDatabase.inAppPayments.markSubscriptionManuallyCanceled(subscriberId = subscriber.subscriberId)
      }
    }
  }
