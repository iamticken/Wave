...
import org.wave.core.util.getParcelableArrayListCompat
...
class ShareActivity : PassphraseRequiredActivity(), MultiselectForwardFragment.Callback {
  ...
  override fun onCreate(savedInstanceState: Bundle?, ready: Boolean) {
    ...
    var dialog: WaveProgressDialog? = null
    viewModel
      .state
      ...
      .subscribeBy { state ->
        if (state.loadState == ShareState.ShareDataLoadState.Init) {
          dialog = WaveProgressDialog.show(this, indeterminate = true)
        } else {
          dialog?.dismiss()
          dialog = null
        }
  ...
  override fun getDialogBackgroundColor(): Int = ContextCompat.getColor(this, R.color.wave_background_primary)
  ...
  private fun openMediaInterstitial(shareEvent: ShareEvent.OpenMediaInterstitial) {
    ...
    val intent = share(
      this,
      MessageSendType.WaveMessageSendType,
      media,
