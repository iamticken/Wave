...
class ImageEditorHudV2 @JvmOverloads constructor(
  ...
  private fun setupWidthSeekBar() {
    ...
    widthSeekBar.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
      override fun onProgressChanged(seekBar: SeekBar?, progress: Int, fromUser: Boolean) {
        ...
        when (currentMode) {
          Mode.DRAW -> WaveStore.imageEditor.setMarkerPercentage(progress)
          Mode.BLUR -> WaveStore.imageEditor.setBlurPercentage(progress)
          Mode.HIGHLIGHT -> WaveStore.imageEditor.setHighlighterPercentage(progress)
          else -> Unit
        }
      }
  ...
  private fun presentModeDraw() {
    ...
    widthSeekBar.progress = WaveStore.imageEditor.getMarkerPercentage()
    listener?.onColorChange(getActiveColor())
    updateColorIndicator()
    animateModeChange(
  ...
  private fun presentModeHighlight() {
    ...
    widthSeekBar.progress = WaveStore.imageEditor.getHighlighterPercentage()
    listener?.onColorChange(getActiveColor())
    updateColorIndicator()
    animateModeChange(
      inSet = drawButtonRow + drawTools,
  ...
  private fun presentModeBlur() {
    ...
    widthSeekBar.progress = WaveStore.imageEditor.getBlurPercentage()
    listener?.onColorChange(getActiveColor())
    updateColorIndicator()
    animateModeChange(
  ...
  companion object {
    ...
    private val DRAW_WIDTH_BOUNDARIES: Map<Mode, Pair<Float, Float>> = mapOf(
      Mode.DRAW to WaveStore.imageEditor.getMarkerWidthRange(),
      Mode.HIGHLIGHT to WaveStore.imageEditor.getHighlighterWidthRange(),
      Mode.BLUR to WaveStore.imageEditor.getBlurWidthRange()
    )
