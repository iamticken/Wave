...
public class EditProxyViewModel extends ViewModel {
  ...
  public EditProxyViewModel() {
    ...
    this.pipeState = WaveStore.account().getE164() == null ? Flowable.empty()
  ...
  void onToggleProxy(boolean enabled, String text) {
    if (enabled) {
      WaveProxy currentProxy = WaveStore.proxy().getProxy();

      if (currentProxy != null && !Util.isEmpty(currentProxy.getHost())) {
        WaveProxyUtil.enableProxy(currentProxy);
      }
      uiState.onNext(UiState.ALL_ENABLED);
    } else if (Util.isEmpty(text)) {
        WaveProxyUtil.disableAndClearProxy();
        uiState.onNext(UiState.ALL_DISABLED);
    } else {
        WaveProxyUtil.disableProxy();
        uiState.onNext(UiState.ALL_DISABLED);
    }
  }

  public void onSaveClicked(@NonNull String host) {
    String trueHost = WaveProxyUtil.convertUserEnteredAddressToHost(host);

    saveState.onNext(SaveState.IN_PROGRESS);

    WaveExecutors.BOUNDED.execute(() -> {
      WaveProxyUtil.enableProxy(new WaveProxy(trueHost, 443));

      boolean success = WaveProxyUtil.testWebsocketConnection(TimeUnit.SECONDS.toMillis(10));
