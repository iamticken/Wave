...
public class BackupsPreferenceFragment extends Fragment {
  ...
  private void setBackupStatus() {
    if (WaveStore.settings().isBackupEnabled()) {
  ...
  private void setBackupFolderName() {
    ...
    if (BackupUtil.canUserAccessBackupDirectory(requireContext())) {
      ...
      {
        Uri backupUri = Objects.requireNonNull(WaveStore.settings().getWaveBackupDirectory());

        folder.setVisibility(View.VISIBLE);
        folderName.setText(StorageUtil.getDisplayPath(requireContext(), backupUri));
      } else if (StorageUtil.canWriteInWaveStorageDir()) {
        try {
          folder.setVisibility(View.VISIBLE);
  ...
  private void onToggleClickedLegacy() {
    Permissions.with(this)
               ...
               .withPermanentDenialDialog(getString(R.string.BackupsPreferenceFragment_wave_requires_external_storage_permission_in_order_to_create_backups))
               .execute();
  }
  ...
  private void pickTime() {
    ...
    final MaterialTimePicker timePickerFragment = new MaterialTimePicker.Builder()
        ...
        .setHour(WaveStore.settings().getBackupHour())
        .setMinute(WaveStore.settings().getBackupMinute())
    ...
    timePickerFragment.addOnPositiveButtonClickListener(v -> {
      ...
      WaveStore.settings().setBackupSchedule(hour, minute);
      updateTimeLabel();
  ...
  private void updateTimeLabel() {
    final int backupHour   = WaveStore.settings().getBackupHour();
    final int backupMinute = WaveStore.settings().getBackupMinute();
  ...
  private void updateToggle() {
    boolean userUnregistered          = TextSecurePreferences.isUnauthorizedReceived(AppDependencies.getApplication()) || !WaveStore.account().isRegistered();
    boolean clientDeprecated          = WaveStore.misc().isClientDeprecated();
    boolean legacyLocalBackupsEnabled = WaveStore.settings().isBackupEnabled() && BackupUtil.canUserAccessBackupDirectory(AppDependencies.getApplication());
