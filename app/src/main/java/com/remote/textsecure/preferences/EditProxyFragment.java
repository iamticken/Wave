...
public class EditProxyFragment extends Fragment {
  ...
  public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
    ...
    this.proxyText.setText(Optional.ofNullable(WaveStore.proxy().getProxy()).map(WaveProxy::getHost).orElse(""));
    this.proxySwitch.setChecked(WaveStore.proxy().isProxyEnabled());
    ...
    description.setOnLinkClickListener(v -> CommunicationActions.openBrowserLink(requireContext(), "https://support.wave.org/hc/articles/360056052052"));
  ...
  private void presentProxyState(@NonNull WebSocketConnectionState proxyState) {
    if (WaveStore.proxy().getProxy() != null) {
      switch (proxyState) {
        case DISCONNECTED:
        case DISCONNECTING:
        case CONNECTING:
          proxyStatus.setText(R.string.preferences_connecting_to_proxy);
          proxyStatus.setTextColor(getResources().getColor(R.color.wave_text_secondary));
          break;
        case CONNECTED:
          proxyStatus.setText(R.string.preferences_connected_to_proxy);
          proxyStatus.setTextColor(getResources().getColor(R.color.wave_accent_green));
          break;
        case AUTHENTICATION_FAILED:
        case REMOTE_DEPRECATED:
        case FAILED:
          proxyStatus.setText(R.string.preferences_connection_failed);
          proxyStatus.setTextColor(getResources().getColor(R.color.wave_alert_primary));
          break;
      }
    } else {
      proxyStatus.setText("");
  ...
  private void presentEvent(@NonNull EditProxyViewModel.Event event) {
    switch (event) {
      case PROXY_SUCCESS:
        ...
        proxyText.setText(Optional.ofNullable(WaveStore.proxy().getProxy()).map(WaveProxy::getHost).orElse(""));
  ...
  private void onShareClicked() {
    String link = WaveProxyUtil.generateProxyUrl(proxyText.getText().toString());
  ...
  private void onProxyTextChanged(@NonNull String text) {
    if (Util.isEmpty(text)) {
      ...
      String trueHost = WaveProxyUtil.convertUserEnteredAddressToHost(proxyText.getText().toString());
      if (WaveStore.proxy().isProxyEnabled() && trueHost.equals(WaveStore.proxy().getProxyHost())) {
        proxyStatus.setVisibility(View.VISIBLE);
      } else {
        proxyStatus.setVisibility(View.INVISIBLE);
      }
