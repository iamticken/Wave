...
object JumboEmoji {
  ...
  private val executor = WaveExecutors.newCachedSingleThreadExecutor("jumbo-emoji", ThreadUtil.PRIORITY_IMPORTANT_BACKGROUND_THREAD)
  ...
  fun updateCurrentVersion(context: Context) {
    WaveExecutors.BOUNDED.execute {
      ...
      if (EmojiFiles.getLatestEmojiData(context, version)?.format != null) {
        ...
        ThreadUtil.runOnMain { downloadedJumbos.addAll(WaveStore.emoji.getJumboEmojiSheets(version.version)) }
      }
    }
  }
  ...
  fun loadJumboEmoji(context: Context, drawInfo: EmojiDrawInfo): LoadResult {
    ...
    val newTask = ListenableFutureTask<Bitmap> {
      ...
      if (uuid == null) {
        ...
        WaveStore.emoji.addJumboEmojiSheet(version.version, drawInfo.jumboSheet)
      }
