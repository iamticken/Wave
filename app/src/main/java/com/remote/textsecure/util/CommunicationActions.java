...
import org.wave.core.util.concurrent.JvmRxExtensions;
...
public class CommunicationActions {
  ...
  private static void startVoiceCall(@NonNull CallContext callContext, @NonNull Recipient recipient, @NonNull OnUserAlreadyInAnotherCall onUserAlreadyInAnotherCall) {
    ...
    if (recipient.isRegistered()) {
      AppDependencies.getWaveCallManager().isCallActive(new ResultReceiver(new Handler(Looper.getMainLooper())) {
        @Override
                                       ...
                                       @Nullable TaskStackBuilder backStack)
  {
    new AsyncTask<Void, Void, Long>() {
      ...
      protected Long doInBackground(Void... voids) {
        return WaveDatabase.threads().getOrCreateThreadIdFor(recipient);
      }
   ...
   * If the url is a malformed group link, it will assume Wave needs to update.
  ...
  public static boolean handlePotentialProxyLinkUrl(@NonNull FragmentActivity activity, @NonNull String potentialProxyLinkUrl) {
    String proxy = WaveProxyUtil.parseHostFromProxyDeepLink(potentialProxyLinkUrl);
  ...
  /**
   * If the url is a wave.me link it will handle it.
   */
  public static void handlePotentialWaveMeUrl(@NonNull FragmentActivity activity, @NonNull String potentialUrl) {
    String                 e164     = WaveMeUtil.parseE164FromLink(potentialUrl);
  ...
  private static void startVideoCall(@NonNull CallContext callContext, @NonNull CallLinkRootKey rootKey, @Nullable CallLinkEpoch epoch, @NonNull OnUserAlreadyInAnotherCall onUserAlreadyInAnotherCall) {
    SimpleTask.run(() -> {
      ...
      CallLinkTable.CallLink callLink = WaveDatabase.callLinks().getOrCreateCallLinkByRootKey(rootKey, epoch);

      if (callLink.getState().hasBeenRevoked()) {
        return Optional.<Recipient>empty();
      }

      return WaveDatabase.recipients().getByCallLinkRoomId(roomId).map(Recipient::resolved);
    }, callLinkRecipient -> {
  ...
  private static void startAudioCallInternal(@NonNull CallContext callContext, @NonNull Recipient recipient) {
    callContext.getPermissionsBuilder()
               ...
               .ifNecessary()
               .withRationaleDialog(callContext.getContext().getString(R.string.ConversationActivity_allow_access_microphone), callContext.getContext().getString(R.string.ConversationActivity__to_call_wave_needs_access_to_your_microphone), R.drawable.symbol_phone_24)
               .withPermanentDenialDialog(callContext.getContext().getString(R.string.ConversationActivity__to_call_wave_needs_access_to_your_microphone), null, R.string.ConversationActivity_allow_access_microphone, R.string.ConversationActivity__to_start_call, callContext.getFragmentManager())
               .onAnyDenied(() -> Toast.makeText(callContext.getContext(), R.string.ConversationActivity_wave_needs_microphone_access_voice_call, Toast.LENGTH_LONG).show())
               .onAllGranted(() -> {
                 AppDependencies.getWaveCallManager().startOutgoingAudioCall(recipient);
  ...
  private static void startVideoCallInternal(@NonNull CallContext callContext, @NonNull Recipient recipient, boolean fromCallLink) {
    AppDependencies.getWaveCallManager().startPreJoinCall(recipient);
    ...
    SimpleTask.run(() -> {
      ...
      if (!recipient.isRegistered() || !recipient.getHasServiceId()) {
        try {
          ...
          Log.w(TAG, "[handlePotentialWaveMeUrl] Failed to refresh directory for new contact.");
        }
      }
      ...
      if (recipient != null && recipient.isRegistered() && recipient.getHasServiceId()) {
        ...
        new MaterialAlertDialogBuilder(activity)
            .setMessage(activity.getString(R.string.RecipientLookup_error__s_is_not_a_wave_user, e164))
            .setPositiveButton(android.R.string.ok, null)
            .show();
      }
    });
