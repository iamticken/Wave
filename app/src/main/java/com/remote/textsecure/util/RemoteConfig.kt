...
import org.wave.core.util.gibiBytes
import org.wave.core.util.kibiBytes
...
object RemoteConfig {
  ...
  @JvmStatic
  val libwaveConfigs: Map<String, String>
    get() {
      if (!initialized) {
        Log.w(TAG, "Tried to read libwaveConfigs before initialization. Initializing now.")
        initLock.withLock {
          if (!initialized) {
            init()
          }
      ...
      return computeLibwaveConfigs(REMOTE_VALUES)
    }
  ...
  fun init() {
    initLock.withLock {
      val current = parseStoredConfig(WaveStore.remoteConfig.currentConfig)
      val pending = parseStoredConfig(WaveStore.remoteConfig.pendingConfig)
      val changes = computeChanges(current, pending)

      WaveStore.remoteConfig.currentConfig = mapToJson(pending)
  ...
  fun refreshIfNecessary() {
    val timeSinceLastFetch = System.currentTimeMillis() - WaveStore.remoteConfig.lastFetchTime
  ...
  fun refreshSync() {
    val result = NetworkResultUtil.toBasicLegacy(WaveNetwork.remoteConfig.getRemoteConfig())
  ...
  fun update(config: Map<String, Any?>) {
    ...
    val disk = parseStoredConfig(WaveStore.remoteConfig.pendingConfig)
    ...
    WaveStore.remoteConfig.pendingConfig = mapToJson(result.disk)
    REMOTE_VALUES.clear()
  ...
  val debugDiskValues: Map<String, Any>
    get() = TreeMap(parseStoredConfig(WaveStore.remoteConfig.currentConfig))

  /** Only for rendering debug info.  */
  @JvmStatic
  @get:Synchronized
  val debugPendingDiskValues: Map<String, Any>
    get() = TreeMap(parseStoredConfig(WaveStore.remoteConfig.pendingConfig))
  ...
  fun updateInternal(
    ...
    allKeys
      .filter { remoteCapable.contains(it) || it.startsWith("android.libwave.") }
      .forEach { key: String ->
    ...
    // Libwave does not support sticky flags and is okay with any flag that is not known to the server being
    // forgotten about.
    val libwaveConfigsKnownToServer = remote.keys.filter { it.startsWith("android.libwave.") }.toSet()

    allKeys
      .filterNot { key ->
        remoteCapable.contains(key) || libwaveConfigsKnownToServer.contains(key)
      }
  ...
  private fun computeLibwaveConfigs(config: Map<String, Any?>): Map<String, String> {
    return config
      .filterKeys { it.startsWith("android.libwave.") }
      .mapNotNull { (key, value) ->
        val newKey = key.removePrefix("android.libwave.")
        when (value) {
          ...
          // Libwave expects that disabled values are simply absent from the
          ...
          is Boolean -> if (value) newKey to "true" else null
            ...
            Log.w(TAG, "[libwave] Unexpected type for $newKey! Was a $type")
  ...
  @JvmStatic
  @get:JvmName("libwaveEnforceMinTlsVersion")
  val libwaveEnforceMinTlsVersion by remoteBoolean(
    key = "android.libwaveEnforceMinTlsVersion",
    defaultValue = false,
    hotSwappable = false
  )
