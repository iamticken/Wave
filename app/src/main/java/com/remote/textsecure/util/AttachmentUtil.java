...
public class AttachmentUtil {
  ...
  public static @Nullable MessageRecord deleteAttachment(@NonNull DatabaseAttachment attachment) {
    ...
    int          attachmentCount = WaveDatabase.attachments()
                                                 .getAttachmentsForMessage(mmsId)
                                                 .size();

    MessageRecord deletedMessageRecord = null;
    if (attachmentCount <= 1) {
      deletedMessageRecord = WaveDatabase.messages().getMessageRecordOrNull(mmsId);
      WaveDatabase.messages().deleteMessage(mmsId);
    } else {
      WaveDatabase.attachments().deleteAttachment(attachmentId);
      MultiDeviceDeleteSyncJob.enqueueAttachmentDelete(WaveDatabase.messages().getMessageRecordOrNull(mmsId), attachment);
    }
  ...
  private static boolean isFromTrustedConversation(@NonNull Context context, @NonNull DatabaseAttachment attachment) {
    try {
      MessageRecord message = WaveDatabase.messages().getMessageRecord(attachment.mmsId);

      Recipient fromRecipient = message.getFromRecipient();
      Recipient toRecipient   = WaveDatabase.threads().getRecipientForThreadId(message.getThreadId());
