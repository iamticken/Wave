...
class MediaGalleryFragment : Fragment(R.layout.v2_media_gallery_fragment) {
  ...
  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    ...
    galleryItemsWithSelection.observe(viewLifecycleOwner) {
      if (StorageUtil.canOnlyReadSelectedMediaStore() && it.isEmpty()) {
        ...
        binding.mediaGalleryMissingPermissions.visible = true
        binding.mediaGalleryManageContainer.visible = false
        binding.mediaGalleryPermissionText.text = getString(R.string.AttachmentKeyboard_Wave_needs_permission_to_show_your_photos_and_videos)
  ...
  private fun requestRequiredPermissions() {
    Permissions.with(this)
      ...
      .ifNecessary()
      .onAnyResult { refreshMediaGallery() }
      .withPermanentDenialDialog(getString(R.string.AttachmentManager_wave_requires_the_external_storage_permission_in_order_to_attach_photos_videos_or_audio), null, R.string.AttachmentManager_wave_allow_storage, R.string.AttachmentManager_wave_to_show_photos, true, parentFragmentManager)
      .onSomeDenied {
        val deniedPermission = PermissionCompat.getRequiredPermissionsForDenial()
        if (it.containsAll(deniedPermission.toList())) {
          Toast.makeText(requireContext(), R.string.AttachmentManager_wave_needs_storage_access, Toast.LENGTH_LONG).show()
        }
      }
      .execute()
  }
