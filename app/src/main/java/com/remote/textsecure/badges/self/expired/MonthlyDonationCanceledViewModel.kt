...
class MonthlyDonationCanceledViewModel(
  ...
  private fun initializeFromWaveStore() {
    internalState.value = MonthlyDonationCanceledState(
      loadState = MonthlyDonationCanceledState.LoadState.READY,
      badge = WaveStore.inAppPayments.getExpiredBadge(),
      errorMessage = getErrorMessage(WaveStore.inAppPayments.getUnexpectedSubscriptionCancelationChargeFailure())
    )
  }
