...
object ApkUpdateInstaller {
  ...
  fun installOrPromptForInstall(context: Context, downloadId: Long, userInitiated: Boolean) {
    if (downloadId != WaveStore.apkUpdate.downloadId) {
      Log.w(TAG, "DownloadId doesn't match the one we're waiting for (current: $downloadId, expected: ${WaveStore.apkUpdate.downloadId})! We likely have newer data. Ignoring.")
      ApkUpdateNotifications.dismissInstallPrompt(context)
      AppDependencies.jobManager.add(ApkUpdateJob())
      return
    }

    val digest = WaveStore.apkUpdate.digest
    if (digest == null) {
    ...
    if (!userInitiated && !shouldAutoUpdate()) {
      Log.w(TAG, "Not user-initiated and not eligible for auto-update. Prompting. (API=${Build.VERSION.SDK_INT}, Foreground=${AppForegroundObserver.isForegrounded()}, AutoUpdate=${WaveStore.apkUpdate.autoUpdate})")
  ...
  private fun shouldAutoUpdate(): Boolean {
    ...
    return Environment.IS_NIGHTLY && Build.VERSION.SDK_INT >= 31 && WaveStore.apkUpdate.autoUpdate && !AppForegroundObserver.isForegrounded()
  }
}
