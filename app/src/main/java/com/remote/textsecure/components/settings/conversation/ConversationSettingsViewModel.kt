...
sealed class ConversationSettingsViewModel(
  ...
  protected val store = Store(
    ConversationSettingsState(
      specificSettingsState = specificSettingsState,
      isDeprecatedOrUnregistered = WaveStore.misc.isClientDeprecated || TextSecurePreferences.isUnauthorizedReceived(AppDependencies.application)
    )
  )
  ...
  init {
    ...
    val sharedMedia: LiveData<List<MediaTable.MediaRecord>> = LiveDataUtil.mapAsync(WaveExecutors.BOUNDED, updater) { tId ->
      ...
      store.update(recipientAndIsActive) { (recipient, isActive), state ->
        state.copy(
          ...
          buttonStripState = ButtonStripPreference.State(
            ...
            isSearchAvailable = callMessageIds.isEmpty(),
            isAddToStoryAvailable = recipient.isPushV2Group && !recipient.isBlocked && isActive && !WaveStore.story.isFeatureDisabled
          ),
