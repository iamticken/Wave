...
import org.wave.core.ui.compose.Rows.TextAndLabel
...
fun Screen(
  ...
  storageRecords: List<WaveStorageRecord>,
  storageInsights: StorageInsights,
  oneOffEvent: OneOffEvent,
...
fun ToolScreen() {
  Column(
    ...
    ActionRow("Set initial master key", "Initializes it to something random. Will cause a decryption failure.") {
      WaveStore.svr.masterKeyForInitialDataRestore = MasterKey(Util.getSecretBytes(32))
    }
...
@Composable
private fun ManifestRow(manifest: WaveStorageManifest) {
  Column {
...
@Composable
private fun StorageRecordRow(record: WaveStorageRecord) {
  Row(modifier = Modifier.fillMaxWidth()) {
    when {
...
fun ScreenPreview() {
  Previews.Preview {
    Screen(
      manifest = WaveStorageManifest.EMPTY,
