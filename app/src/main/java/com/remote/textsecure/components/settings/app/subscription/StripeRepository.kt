...
import org.wave.donations.PaymentSource
import org.wave.donations.PaymentSourceType
import org.wave.donations.StripeApi
import org.wave.donations.StripeIntentAccessor
import org.wave.donations.json.StripeIntentStatus
...
import org.whispersystems.waveservice.api.subscriptions.StripeClientSecret
 ...
 * 1. Generate and send a SubscriberId, which is a 32 byte ID representing this user, to Wave Service, which creates a Stripe Customer
 ...
 * 1. Set the default PaymentMethod for the customer, using the PaymentMethod id, via the Wave service
 *
...
object StripeRepository : StripeApi.PaymentIntentFetcher, StripeApi.SetupIntentHelper {
  ...
  override fun fetchPaymentIntent(price: FiatMoney, level: Long, sourceType: PaymentSourceType.Stripe): StripeIntentAccessor {
    Log.d(TAG, "Fetching payment intent from Wave service for $price... (Locale.US minimum precision: ${price.minimumUnitPrecisionString})")
  ...
  override fun fetchSetupIntent(inAppPaymentType: InAppPaymentType, sourceType: PaymentSourceType.Stripe): StripeIntentAccessor {
    ...
    Log.d(TAG, "Got setup intent from Wave service!")
  ...
  /**
   * Sets the default payment method for the given subscriber type via Wave Service
  ...
  fun setDefaultPaymentMethod(
    ...
    Log.d(TAG, "Setting default payment method via Wave service...")
    ...
    Log.d(TAG, "Set default payment method via Wave service!")
    Log.d(TAG, "Storing the subscription payment source type locally.")
    WaveDatabase.inAppPaymentSubscribers.setPaymentMethod(subscriber.subscriberId, paymentSourceType.toPaymentMethodType())
  }
