    ...
    if (result.action == InAppPaymentProcessorAction.CANCEL_SUBSCRIPTION) {
      ...
      fragment.lifecycleScope.launch {
        val inAppPayment = withContext(WaveDispatchers.IO) {
          WaveDatabase.inAppPayments.getById(result.inAppPaymentId!!)!!
        }
  ...
  class ErrorHandler : DefaultLifecycleObserver {
    ...
    fun attach(
      ...
      disposables += inAppPaymentIdSource
        ...
        .doOnNext {
          WaveDatabase.inAppPayments.update(it.copy(notified = true))
        }
      ...
      disposables += inAppPaymentIdSource
        ...
        .concatMapSingle { (id, err) -> Single.fromCallable { WaveDatabase.inAppPayments.getById(id)!! to err } }
        .subscribeOn(Schedulers.io())
        .observeOn(AndroidSchedulers.mainThread())
