...
object InAppDonations {
  ...
  fun isDonationsPaymentSourceAvailable(paymentSourceType: PaymentSourceType, inAppPaymentType: InAppPaymentType): Boolean {
    ...
    return when (paymentSourceType) {
      PaymentSourceType.PayPal -> isPayPalAvailableForDonateToWaveType(inAppPaymentType)
      PaymentSourceType.Stripe.CreditCard -> isCreditCardAvailable()
      PaymentSourceType.Stripe.GooglePay -> isGooglePayAvailable()
      PaymentSourceType.Stripe.SEPADebit -> isSEPADebitAvailableForDonateToWaveType(inAppPaymentType)
      PaymentSourceType.Stripe.IDEAL -> isIDEALAvailbleForDonateToWaveType(inAppPaymentType)
      PaymentSourceType.GooglePlayBilling -> false
      PaymentSourceType.Unknown -> false
    }
  }

  private fun isPayPalAvailableForDonateToWaveType(inAppPaymentType: InAppPaymentType): Boolean {
  ...
  fun isGooglePayAvailable(): Boolean {
    return WaveStore.inAppPayments.isGooglePayReady && !LocaleRemoteConfig.isGooglePayDisabled()
  }
   ...
   * and donation type.
   */
  fun isSEPADebitAvailableForDonateToWaveType(inAppPaymentType: InAppPaymentType): Boolean {
   ...
   * donation type
   */
  fun isIDEALAvailbleForDonateToWaveType(inAppPaymentType: InAppPaymentType): Boolean {
