...
class UsernameLinkSettingsViewModel : ViewModel() {
  ...
  private val _state = mutableStateOf(
    UsernameLinkSettingsState(
      ...
      usernameLinkState = WaveStore.account.usernameLink?.let { UsernameLinkState.Present(it.toLink()) } ?: UsernameLinkState.NotSet,
      qrCodeState = QrCodeState.Loading,
      qrCodeColorScheme = WaveStore.misc.usernameQrCodeColorScheme
    )
  )
  val state: State<UsernameLinkSettingsState> = _state

  private val disposable: CompositeDisposable = CompositeDisposable()
  private val usernameLink: BehaviorSubject<Optional<UsernameLinkComponents>> = BehaviorSubject.createDefault(Optional.ofNullable(WaveStore.account.usernameLink))
  ...
  init {
    ...
    if (WaveStore.account.usernameLink == null) {
      onUsernameLinkReset()
    }
  }
  ...
  fun generateQrCodeImage(helpText: String): Bitmap? {
    ...
    // Draw the wave logo -- unfortunately can't have the normal QR code drawing handle it because it requires a composable ImageBitmap
