...
class DebugLogsPromptDialogFragment : FixedRoundedCornerBottomSheetDialogFragment() {
  ...
  companion object {
    ...
    fun show(activity: AppCompatActivity, purpose: Purpose) {
      ...
      if (NetworkUtil.isConnected(activity) && activity.supportFragmentManager.findFragmentByTag(BottomSheetUtil.STANDARD_BOTTOM_SHEET_FRAGMENT_TAG) == null) {
        ...
        when (purpose) {
          Purpose.NOTIFICATIONS -> WaveStore.uiHints.lastNotificationLogsPrompt = System.currentTimeMillis()
          Purpose.CRASH -> WaveStore.uiHints.lastCrashPrompt = System.currentTimeMillis()
          Purpose.CONNECTIVITY_WARNING -> WaveStore.misc.lastConnectivityWarningTime = System.currentTimeMillis()
        }
      }
    }
  }
  ...
  override val themeResId: Int = R.style.Widget_Wave_FixedRoundedCorners_Messages
  ...
  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    ...
    binding.submit.setOnClickListener {
      val progressDialog = WaveProgressDialog.show(requireContext())
    ...
    binding.decline.setOnClickListener {
      if (purpose == Purpose.NOTIFICATIONS) {
        WaveStore.uiHints.markDeclinedShareNotificationLogs()
      }
  ...
  private fun submitLogs(debugLog: String, purpose: Purpose) {
    CommunicationActions.openEmail(
      ...
      getString(R.string.DebugLogsPromptDialogFragment__wave_android_support_request),
  ...
  private fun getEmailBody(debugLog: String?, purpose: Purpose): String {
    ...
    return SupportEmailUtil.generateSupportEmailBody(
      requireContext(),
      R.string.DebugLogsPromptDialogFragment__wave_android_support_request,
      " - $category",
      "\n\n",
      suffix.toString()
    )
