...
import org.wave.core.ui.compose.BottomSheets
...
private fun getDrawableResourceForDeviceType(deviceType: WaveAudioManager.AudioDevice): Int {
  return when (deviceType) {
    WaveAudioManager.AudioDevice.WIRED_HEADSET -> R.drawable.symbol_headphones_outline_24
    WaveAudioManager.AudioDevice.EARPIECE -> R.drawable.symbol_phone_speaker_outline_24
    WaveAudioManager.AudioDevice.BLUETOOTH -> R.drawable.symbol_speaker_bluetooth_fill_white_24
    WaveAudioManager.AudioDevice.SPEAKER_PHONE, WaveAudioManager.AudioDevice.NONE -> R.drawable.symbol_speaker_outline_24
  }
}

private fun getDescriptionStringResourceForDeviceType(deviceType: WaveAudioManager.AudioDevice): Int {
  return when (deviceType) {
    WaveAudioManager.AudioDevice.WIRED_HEADSET -> R.string.WebRtcAudioOutputBottomSheet__headset_icon_content_description
    WaveAudioManager.AudioDevice.EARPIECE -> R.string.WebRtcAudioOutputBottomSheet__earpiece_icon_content_description
    WaveAudioManager.AudioDevice.BLUETOOTH -> R.string.WebRtcAudioOutputBottomSheet__bluetooth_icon_content_description
    WaveAudioManager.AudioDevice.SPEAKER_PHONE, WaveAudioManager.AudioDevice.NONE -> R.string.WebRtcAudioOutputBottomSheet__speaker_icon_content_description
  }
}

data class AudioOutputOption(val friendlyName: String, val deviceType: WaveAudioManager.AudioDevice, val deviceId: Int)

@Preview
@Composable
private fun SampleOutputBottomSheet() {
  val outputs: ImmutableList<AudioOutputOption> = listOf(
    AudioOutputOption("Earpiece", WaveAudioManager.AudioDevice.EARPIECE, 0),
    AudioOutputOption("Speaker", WaveAudioManager.AudioDevice.SPEAKER_PHONE, 1),
    AudioOutputOption("BT Headset", WaveAudioManager.AudioDevice.BLUETOOTH, 2),
    AudioOutputOption("Wired Headset", WaveAudioManager.AudioDevice.WIRED_HEADSET, 3)
  ).toImmutableList()
