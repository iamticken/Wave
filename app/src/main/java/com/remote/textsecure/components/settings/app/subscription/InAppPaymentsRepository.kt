...
import org.wave.donations.StripeFailureCode
import org.wave.libwave.zkgroup.InvalidInputException
import org.wave.libwave.zkgroup.VerificationFailedException
import org.wave.libwave.zkgroup.receipts.ReceiptCredentialRequestContext
import org.wave.libwave.zkgroup.receipts.ReceiptSerial
...
import org.whispersystems.waveservice.internal.push.exceptions.InAppPaymentProcessorError
...
object InAppPaymentsRepository {
  ...
  fun updateBackupInAppPaymentWithCancelation(activeSubscription: ActiveSubscription) {
    if (activeSubscription.isCanceled || activeSubscription.willCancelAtPeriodEnd()) {
      ...
      val latestPayment = WaveDatabase.inAppPayments.getLatestBySubscriberId(subscriber.subscriberId) ?: return
      if (latestPayment.state == InAppPaymentTable.State.END && latestPayment.data.cancellation == null) {
        synchronized(subscriber.type.lock) {
          val payment = WaveDatabase.inAppPayments.getLatestBySubscriberId(subscriber.subscriberId) ?: return
          ...
          WaveDatabase.inAppPayments.update(
            payment.copy(
  ...
  fun updateInAppPayment(inAppPayment: InAppPaymentTable.InAppPayment): Completable {
    return Completable.fromAction {
      WaveDatabase.inAppPayments.update(inAppPayment)
  ...
  fun observeLatestBackupPayment(): Flow<InAppPaymentTable.InAppPayment> {
    return callbackFlow {
      fun refresh() {
        val latest = WaveDatabase.inAppPayments.getLatestInAppPaymentByType(InAppPaymentType.RECURRING_BACKUP)
        if (latest != null) {
  ...
  fun handlePipelineError(
    ...
    val inAppPayment = WaveDatabase.inAppPayments.getById(inAppPaymentId)!!
  ...
  fun requireInAppPayment(inAppPaymentId: InAppPaymentTable.InAppPaymentId): Single<InAppPaymentTable.InAppPayment> {
    return Single.fromCallable {
      WaveDatabase.inAppPayments.getById(inAppPaymentId) ?: throw Exception("Not found.")
    }.subscribeOn(Schedulers.io())
  }
  ...
  fun observeUpdates(inAppPaymentId: InAppPaymentTable.InAppPaymentId): Flowable<InAppPaymentTable.InAppPayment> {
    return Flowable.create({ emitter ->
      ...
      WaveDatabase.inAppPayments.getById(inAppPaymentId)?.also {
  ...
  fun resolveLock(inAppPaymentId: InAppPaymentTable.InAppPaymentId): Lock {
    val payment = WaveDatabase.inAppPayments.getById(inAppPaymentId) ?: error("Not found")
   ...
   * back on information in WaveStore. This information is utilized in some error presentation as well as in subscription
   * updates.
   */
  @WorkerThread
  fun getLatestPaymentMethodType(subscriberType: InAppPaymentSubscriberRecord.Type): InAppPaymentData.PaymentMethodType {
    ...
    return if (paymentMethodType != InAppPaymentData.PaymentMethodType.UNKNOWN) {
      ...
      WaveStore.inAppPayments.getSubscriptionPaymentSourceType().toPaymentMethodType()
    } else {
  ...
  fun isUserManuallyCancelled(subscriberType: InAppPaymentSubscriberRecord.Type): Boolean {
    return if (subscriberType == InAppPaymentSubscriberRecord.Type.DONATION) {
      ...
      WaveStore.inAppPayments.isBackupSubscriptionManuallyCancelled()
    }
  }
  ...
  fun getFallbackLastEndOfPeriod(subscriberType: InAppPaymentSubscriberRecord.Type): Duration {
    return if (subscriberType == InAppPaymentSubscriberRecord.Type.DONATION) {
      WaveStore.inAppPayments.getLastEndOfPeriod().seconds
    } else {
      0.seconds
    }
  }
  ...
  fun setShouldCancelSubscriptionBeforeNextSubscribeAttempt(subscriberType: InAppPaymentSubscriberRecord.Type, subscriberId: SubscriberId?, shouldCancel: Boolean) {
    if (subscriberType == InAppPaymentSubscriberRecord.Type.DONATION) {
      WaveStore.inAppPayments.shouldCancelSubscriptionBeforeNextSubscribeAttempt = shouldCancel
    }
    ...
    WaveDatabase.inAppPaymentSubscribers.setRequiresCancel(
   ...
   * type. This method will first check the database, and then fall back on the deprecated WaveStore value. This method
  ...
  fun getShouldCancelSubscriptionBeforeNextSubscribeAttempt(subscriberType: InAppPaymentSubscriberRecord.Type): Boolean {
    ...
    val localState = latestSubscriber?.requiresCancel ?: if (subscriberType == InAppPaymentSubscriberRecord.Type.DONATION) {
      WaveStore.inAppPayments.shouldCancelSubscriptionBeforeNextSubscribeAttempt
    } else {
      false
    }
  ...
  fun getRecurringDonationSubscriber(currency: Currency): InAppPaymentSubscriberRecord? {
    val subscriber = WaveDatabase.inAppPaymentSubscribers.getByCurrencyCode(currency.currencyCode)

    return subscriber ?: WaveStore.inAppPayments.getSubscriber(currency)
  }
  ...
  fun getSubscriber(type: InAppPaymentSubscriberRecord.Type): InAppPaymentSubscriberRecord? {
    if (type == InAppPaymentSubscriberRecord.Type.BACKUP) {
      return WaveDatabase.inAppPaymentSubscribers.getBackupsSubscriber()
    }

    val currency = WaveStore.inAppPayments.getRecurringDonationCurrency()
  ...
  fun hasPendingDonation(): Boolean {
    return WaveDatabase.inAppPayments.hasPendingDonation()
  }
  ...
  fun observeInAppPaymentRedemption(type: InAppPaymentType): Observable<DonationRedemptionJobStatus> {
    val fromDatabase: Observable<DonationRedemptionJobStatus> = Observable.create { emitter ->
      val observer = InAppPaymentObserver {
        val latestInAppPayment = WaveDatabase.inAppPayments.getLatestInAppPaymentByType(type)
