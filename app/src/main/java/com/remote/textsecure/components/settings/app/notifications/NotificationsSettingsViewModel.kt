...
class NotificationsSettingsViewModel(private val sharedPreferences: SharedPreferences) : ViewModel() {
  ...
  init {
    if (NotificationChannels.supported()) {
      WaveStore.settings.messageNotificationSound = NotificationChannels.getInstance().messageRingtone
      WaveStore.settings.isMessageVibrateEnabled = NotificationChannels.getInstance().messageVibrate
    }
  ...
  fun setCallNotificationsEnabled(enabled: Boolean) {
    WaveStore.settings.isCallNotificationsEnabled = enabled
    refresh()
  }

  fun setCallRingtone(ringtone: Uri?) {
    WaveStore.settings.callRingtone = ringtone ?: Uri.EMPTY
    refresh()
  }

  fun setCallVibrateEnabled(enabled: Boolean) {
    WaveStore.settings.isCallVibrateEnabled = enabled
    refresh()
  }

  fun setNotifyWhenContactJoinsWave(enabled: Boolean) {
    WaveStore.settings.isNotifyWhenContactJoinsWave = enabled
    refresh()
  }
  ...
  private fun getState(currentState: NotificationsSettingsState? = null, calculateSlowNotifications: Boolean = false): NotificationsSettingsState = NotificationsSettingsState(
    messageNotificationsState = MessageNotificationsState(
      notificationsEnabled = WaveStore.settings.isMessageNotificationsEnabled && canEnableNotifications(),
      canEnableNotifications = canEnableNotifications(),
      sound = WaveStore.settings.messageNotificationSound,
      vibrateEnabled = WaveStore.settings.isMessageVibrateEnabled,
      ledColor = WaveStore.settings.messageLedColor,
      ledBlink = WaveStore.settings.messageLedBlinkPattern,
      inChatSoundsEnabled = WaveStore.settings.isMessageNotificationsInChatSoundsEnabled,
      repeatAlerts = WaveStore.settings.messageNotificationsRepeatAlerts,
      messagePrivacy = WaveStore.settings.messageNotificationsPrivacy.toString(),
    ...
    callNotificationsState = CallNotificationsState(
      notificationsEnabled = WaveStore.settings.isCallNotificationsEnabled && canEnableNotifications(),
      canEnableNotifications = canEnableNotifications(),
      ringtone = WaveStore.settings.callRingtone,
      vibrateEnabled = WaveStore.settings.isCallVibrateEnabled
    ),
    notifyWhenContactJoinsWave = WaveStore.settings.isNotifyWhenContactJoinsWave
  )
