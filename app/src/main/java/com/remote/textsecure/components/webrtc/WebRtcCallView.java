...
public class WebRtcCallView extends InsetAwareConstraintLayout {
    ...
    if (state.getGroupCallState().isNotIdle()) {
      if (state.getCallState() == WebRtcViewModel.State.CALL_PRE_JOIN) {
        if (state.isCallLink()) {
          ...
          warningTextView.setText(WaveStore.phoneNumberPrivacy().isPhoneNumberSharingEnabled() ? R.string.WebRtcCallView__anyone_who_joins_pnp_enabled : R.string.WebRtcCallView__anyone_who_joins_pnp_disabled);
  ...
  public void setWebRtcControls(@NonNull WebRtcControls webRtcControls) {
    ...
    if (webRtcControls.displayIncomingCallButtons()) {
      ...
      incomingRingStatus.setText(webRtcControls.displayAnswerWithoutVideo() ? R.string.WebRtcCallView__wave_video_call: R.string.WebRtcCallView__wave_call);
