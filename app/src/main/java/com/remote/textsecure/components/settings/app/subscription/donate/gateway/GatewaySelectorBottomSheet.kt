...
class GatewaySelectorBottomSheet : DSLSettingsBottomSheetFragment() {
  ...
  private fun DSLConfiguration.renderCreditCardButton(state: GatewaySelectorState.Ready) {
    if (state.isCreditCardAvailable) {
      ...
      primaryButton(
        ...
        icon = DSLSettingsIcon.from(R.drawable.credit_card, R.color.wave_colorOnCustom),
        disableOnClick = true,
        onClick = {
  ...
  companion object {
    ...
    private fun DSLConfiguration.presentMonthlyText(context: Context, inAppPayment: InAppPaymentTable.InAppPayment) {
      noPadTextPref(
        title = DSLSettingsText.from(
          context.getString(R.string.GatewaySelectorBottomSheet__donate_s_month_to_wave, FiatMoneyUtil.format(context.resources, inAppPayment.data.amount!!.toFiatMoney())),
      ...
      noPadTextPref(
        title = DSLSettingsText.from(
          ...
          DSLSettingsText.ColorModifier(ContextCompat.getColor(context, R.color.wave_colorOnSurfaceVariant))
        )
      )
    }
    ...
    private fun DSLConfiguration.presentOneTimeText(context: Context, inAppPayment: InAppPaymentTable.InAppPayment) {
      noPadTextPref(
        title = DSLSettingsText.from(
          context.getString(R.string.GatewaySelectorBottomSheet__donate_s_to_wave, FiatMoneyUtil.format(context.resources, inAppPayment.data.amount!!.toFiatMoney())),
          DSLSettingsText.CenterModifier,
          DSLSettingsText.TitleLargeModifier
        )
      )
