  ...
  private fun setPinRemindersEnabled(enabled: Boolean) {
    if (!enabled) {
      ...
      turnOffButton.setOnClickListener {
        ...
        val correct = PinHashUtil.verifyLocalPinHash(WaveStore.svr.localPinHash!!, pin)
        if (correct) {
          WaveStore.pin.setPinRemindersEnabled(false)
          viewModel.refreshState()
          dialog.dismiss()
        } else {
      ...
      WaveStore.pin.setPinRemindersEnabled(true)
      viewModel.refreshState()
    }
  }
...
object AccountSettingsTestTags {
  ...
  const val ROW_UPDATE_SIGNAL = "row-update-wave"
...
fun AccountSettingsScreen(
  ...
  Scaffolds.Settings(
    ...
    LazyColumn(
      ...
      item {
        Texts.SectionHeader(
          text = stringResource(R.string.preferences_app_protection__wave_pin)
        )
      }
      ...
      item {
        Rows.ToggleRow(
          ...
          label = stringResource(R.string.AccountSettingsFragment__require_your_wave_pin),
      ...
      if (!state.isNotDeprecatedOrUnregistered()) {
        if (state.clientDeprecated) {
          item {
            Rows.TextRow(
              text = stringResource(R.string.preferences_account_update_wave),
          ...
          Rows.TextRow(
            text = {
              Text(
                ...
                color = colorResource(R.color.wave_alert_primary)
              )
            },
            onClick = {
              displayDialog = true
      ...
      item {
        @ColorRes val textColor = if (state.isNotDeprecatedOrUnregistered()) {
          R.color.wave_alert_primary
        } else {
          R.color.wave_alert_primary_50
        }
