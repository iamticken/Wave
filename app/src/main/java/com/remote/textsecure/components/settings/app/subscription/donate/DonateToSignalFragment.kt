...
import org.wave.core.util.concurrent.LifecycleDisposable
import org.wave.core.util.dp
import org.wave.core.util.getParcelableCompat
import org.wave.core.util.getSerializableCompat
...
import org.thoughtcrime.securesms.databinding.DonateToWaveFragmentBinding
 ...
 */
class DonateToWaveFragment :
  DSLSettingsFragment(
    layoutId = R.layout.donate_to_wave_fragment
  ),
  ThanksForYourSupportBottomSheetDialogFragment.Callback,
  InAppPaymentCheckoutDelegate.Callback {

  companion object {
    private val TAG = Log.tag(DonateToWaveFragment::class.java)
  }
  ...
  private val args: DonateToWaveFragmentArgs by navArgs()
  private val viewModel: DonateToWaveViewModel by viewModels(factoryProducer = {
    DonateToWaveViewModel.Factory(args.startType)
  })

  private val disposables = LifecycleDisposable()
  private val binding by ViewBinderDelegate(DonateToWaveFragmentBinding::bind)

  private val supportTechSummary: CharSequence by lazy {
    SpannableStringBuilder(SpanUtil.color(ContextCompat.getColor(requireContext(), R.color.wave_colorOnSurfaceVariant), requireContext().getString(R.string.DonateToWaveFragment__private_messaging)))
      .append(" ")
      .append(
        SpanUtil.readMore(requireContext(), ContextCompat.getColor(requireContext(), R.color.wave_colorPrimary)) {
          findNavController().safeNavigate(DonateToWaveFragmentDirections.actionDonateToWaveFragmentToSubscribeLearnMoreBottomSheetDialog())
        }
      )
  }
  ...
  override fun getMaterial3OnScrollHelper(toolbar: Toolbar?): Material3OnScrollHelper {
    return object : Material3OnScrollHelper(activity = requireActivity(), views = listOf(toolbar!!), lifecycleOwner = viewLifecycleOwner) {
      override val activeColorSet: ColorSet = ColorSet(R.color.transparent, R.color.wave_colorBackground)
      override val inactiveColorSet: ColorSet = ColorSet(R.color.transparent, R.color.wave_colorBackground)
    }
  }
  ...
  override fun bindAdapter(adapter: MappingAdapter) {
    ...
    disposables += viewModel.actions.subscribe { action ->
      when (action) {
        is DonateToWaveAction.DisplayCurrencySelectionDialog -> {
          val navAction = DonateToWaveFragmentDirections.actionDonateToWaveFragmentToSetCurrencyFragment(
        ...
        is DonateToWaveAction.DisplayGatewaySelectorDialog -> {
          Log.d(TAG, "Presenting gateway selector for ${action.inAppPayment.id}")
          val navAction = DonateToWaveFragmentDirections.actionDonateToWaveFragmentToGatewaySelectorBottomSheetDialog(action.inAppPayment.id)

          findNavController().safeNavigate(navAction)
        }

        is DonateToWaveAction.CancelSubscription -> {
          val navAction = DonateToWaveFragmentDirections.actionDonateToWaveFragmentToStripePaymentInProgressFragment(
        ...
        is DonateToWaveAction.UpdateSubscription -> {
          if (action.inAppPayment.data.paymentMethodType == InAppPaymentData.PaymentMethodType.PAYPAL) {
            val navAction = DonateToWaveFragmentDirections.actionDonateToWaveFragmentToPaypalPaymentInProgressFragment(
  ...
  private fun getConfiguration(state: DonateToWaveState): DSLConfiguration {
    return configure {
      space(36.dp)
      ...
      noPadTextPref(
        title = DSLSettingsText.from(
          R.string.DonateToWaveFragment__privacy_over_profit,
      ...
      if (state.inAppPaymentType == InAppPaymentType.RECURRING_DONATION && (state.monthlyDonationState.isSubscriptionActive || state.monthlyDonationState.isSubscriptionInProgress)) {
        ...
        primaryButton(
          text = DSLSettingsText.from(R.string.DonateToWaveFragment__continue),
  ...
  private fun showDonationPendingDialog(state: DonateToWaveState) {
    val message = if (state.inAppPaymentType == InAppPaymentType.ONE_TIME_DONATION) {
      if (state.oneTimeDonationState.isOneTimeDonationLongRunning) {
        R.string.DonateToWaveFragment__bank_transfers_usually_take_1_business_day_to_process_onetime
      } else if (state.oneTimeDonationState.isNonVerifiedIdeal) {
        R.string.DonateToWaveFragment__your_ideal_payment_is_still_processing
      } else {
        R.string.DonateToWaveFragment__your_payment_is_still_being_processed_onetime
      }
    } else {
      if (state.monthlyDonationState.activeSubscription?.paymentMethod == ActiveSubscription.PaymentMethod.SEPA_DEBIT) {
        R.string.DonateToWaveFragment__bank_transfers_usually_take_1_business_day_to_process_monthly
      } else if (state.monthlyDonationState.nonVerifiedMonthlyDonation != null) {
        R.string.DonateToWaveFragment__your_ideal_payment_is_still_processing
      } else {
        R.string.DonateToWaveFragment__your_payment_is_still_being_processed_monthly
      }
    }

    MaterialAlertDialogBuilder(requireContext())
      .setTitle(R.string.DonateToWaveFragment__you_have_a_donation_pending)
      .setMessage(message)
      .setPositiveButton(android.R.string.ok, null)
      .show()
  }

  private fun DSLConfiguration.displayOneTimeSelection(areFieldsEnabled: Boolean, state: DonateToWaveState.OneTimeDonationState) {
    when (state.donationStage) {
      DonateToWaveState.DonationStage.INIT -> customPref(Boost.LoadingModel())
      DonateToWaveState.DonationStage.FAILURE -> customPref(NetworkFailure.Model { viewModel.retryOneTimeDonationState() })
      DonateToWaveState.DonationStage.READY -> {
        customPref(
  ...
  private fun DSLConfiguration.displayMonthlySelection(areFieldsEnabled: Boolean, state: DonateToWaveState.MonthlyDonationState) {
    when (state.donationStage) {
      DonateToWaveState.DonationStage.INIT -> customPref(Subscription.LoaderModel())
      DonateToWaveState.DonationStage.FAILURE -> customPref(NetworkFailure.Model { viewModel.retryMonthlyDonationState() })
      else -> {
        state.subscriptions.forEach { subscription ->
          ...
          val activePrice = state.activeSubscription?.let { sub ->
            FiatMoney.fromWaveNetworkAmount(sub.amount, Currency.getInstance(sub.currency))
          }
  ...
  private fun showSepaEuroMaximumDialog(sepaEuroMaximum: FiatMoney) {
    ...
    MaterialAlertDialogBuilder(requireContext())
      .setTitle(R.string.DonateToWave__donation_amount_too_high)
      .setMessage(getString(R.string.DonateToWaveFragment__you_can_send_up_to_s_via_bank_transfer, max))
      .setPositiveButton(android.R.string.ok, null)
      .show()
  }
  ...
  override fun navigateToStripePaymentInProgress(inAppPayment: InAppPaymentTable.InAppPayment) {
    findNavController().safeNavigate(
      DonateToWaveFragmentDirections.actionDonateToWaveFragmentToStripePaymentInProgressFragment(
  ...
  override fun navigateToPayPalPaymentInProgress(inAppPayment: InAppPaymentTable.InAppPayment) {
    findNavController().safeNavigate(
      DonateToWaveFragmentDirections.actionDonateToWaveFragmentToPaypalPaymentInProgressFragment(
        InAppPaymentProcessorAction.PROCESS_NEW_IN_APP_PAYMENT,
        inAppPayment.id
      )
    )
  ...
  override fun navigateToCreditCardForm(inAppPayment: InAppPaymentTable.InAppPayment) {
    findNavController().safeNavigate(DonateToWaveFragmentDirections.actionDonateToWaveFragmentToCreditCardFragment(inAppPayment.id))
  }

  override fun navigateToIdealDetailsFragment(inAppPayment: InAppPaymentTable.InAppPayment) {
    findNavController().safeNavigate(DonateToWaveFragmentDirections.actionDonateToWaveFragmentToIdealTransferDetailsFragment(inAppPayment.id))
  }

  override fun navigateToBankTransferMandate(inAppPayment: InAppPaymentTable.InAppPayment) {
    findNavController().safeNavigate(DonateToWaveFragmentDirections.actionDonateToWaveFragmentToBankTransferMandateFragment(inAppPayment.id))
  }

  override fun onPaymentComplete(inAppPayment: InAppPaymentTable.InAppPayment) {
    findNavController().safeNavigate(DonateToWaveFragmentDirections.actionDonateToWaveFragmentToThanksForYourSupportBottomSheetDialog(Badges.fromDatabaseBadge(inAppPayment.data.badge!!)))
  }
  ...
  override fun navigateToDonationPending(inAppPayment: InAppPaymentTable.InAppPayment) {
    findNavController().safeNavigate(DonateToWaveFragmentDirections.actionDonateToWaveFragmentToDonationPendingBottomSheet(inAppPayment.id))
  }
