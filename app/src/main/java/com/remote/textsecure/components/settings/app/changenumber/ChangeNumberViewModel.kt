...
class ChangeNumberViewModel : ViewModel() {
  ...
  private val serialContext = WaveExecutors.SERIAL.asCoroutineDispatcher()
  private val smsRetrieverReceiver: SmsRetrieverReceiver = SmsRetrieverReceiver(AppDependencies.application)

  private val initialLocalNumber = WaveStore.account.e164
  private val password = WaveStore.account.servicePassword!!
  ...
  init {
    try {
      val countryCode: Int = PhoneNumberUtil.getInstance()
        .parse(WaveStore.account.e164!!, null)
        .countryCode
  ...
  fun reattemptChangeLocalNumber(onSuccess: () -> Unit, onError: (Throwable) -> Unit) {
    require(WaveStore.misc.pendingChangeNumberMetadata != null)
    val metadata = WaveStore.misc.pendingChangeNumberMetadata!!
    ...
    viewModelScope.launch(Dispatchers.IO) {
      try {
        ...
        val remoteMatchesLocal = remoteE164 == WaveStore.account.requireE164()
  ...
  private suspend fun changeNumberWithRecoveryPassword(): ChangeLocalNumberOutcome {
    ...
    WaveStore.svr.recoveryPassword?.let { recoveryPassword ->
  ...
  private fun handleSuccessfulChangedRemoteNumber(e164: String, pni: ServiceId.PNI, changeNumberOutcome: ChangeNumberOutcome): ChangeLocalNumberOutcome {
    ...
    val changeLocalNumberResult: ChangeLocalNumberOutcome = try {
      ...
      WaveStore.misc.clearPendingChangeNumberMetadata()
  ...
  private suspend fun getRegistrationData(context: Context): RegistrationData {
    ...
    val recoveryPassword = if (currentState.sessionId == null) WaveStore.svr.recoveryPassword else null
