...
import org.wave.core.util.isInMultiWindowModeCompat
...
import org.thoughtcrime.securesms.webrtc.audio.WaveAudioManager.ChosenAudioDeviceIdentifier
import org.whispersystems.waveservice.api.messages.calls.HangupMessage
...
class WebRtcCallActivity : BaseActivity(), SafetyNumberChangeDialog.Callback, ReactWithAnyEmojiBottomSheetDialogFragment.Callback, RecaptchaProofBottomSheetFragment.Callback {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    windowInfoTrackerCallbackAdapter.addWindowLayoutInfoListener(this, WaveExecutors.BOUNDED, windowLayoutInfoConsumer)

    requestNewSizesThrottle = ThrottledDebouncer(1.seconds.inWholeMilliseconds)

    initializePendingParticipantFragmentListener()

    if (!WaveStore.internal.newCallingUi) {
      WindowUtil.setNavigationBarColor(this, ContextCompat.getColor(this, R.color.wave_dark_colorSurface))
    }
  ...
  override fun onStart() {
    ...
    ephemeralStateDisposable = AppDependencies.waveCallManager
      .ephemeralStates()
  ...
  override fun onStop() {
    ...
    if (!isChangingConfigurations && !isInMultiWindowModeCompat()) {
      AppDependencies.waveCallManager.setEnableVideo(false)
    }
    ...
    if (!viewModel.isCallStarting && !isChangingConfigurations) {
      ...
      if (state.callState.isPreJoinOrNetworkUnavailable || state.callState.isIncomingOrHandledElsewhere) {
        AppDependencies.waveCallManager.cancelPreJoin()
      } else if (state.callState.inOngoingCall && isInPipMode()) {
        AppDependencies.waveCallManager.relaunchPipOnForeground()
      }
    }
  }
  ...
  override fun onSendAnywayAfterSafetyNumberChange(changedRecipients: MutableList<RecipientId>) {
    ...
    if (state.groupCallState.isConnected) {
      AppDependencies.waveCallManager.groupApproveSafetyChange(changedRecipients)
    } else {
  ...
  override fun onReactWithAnyEmojiSelected(emoji: String) {
    AppDependencies.waveCallManager.react(emoji)
    callScreen.dismissCallOverflowPopup()
  }

  override fun onProofCompleted() {
    AppDependencies.waveCallManager.resendMediaKeys()
  }
  ...
  private fun initializeViewModel() {
    ...
    if (orientation == Orientation.PORTRAIT_BOTTOM_EDGE && !WaveStore.internal.newCallingUi) {
      WindowUtil.setNavigationBarColor(this, ContextCompat.getColor(this, R.color.wave_dark_colorSurface2))
      WindowUtil.clearTranslucentNavigationBar(window)
    }

    AppDependencies.waveCallManager.orientationChanged(true, orientation.degrees)

    viewModel.setIsLandscapeEnabled(true)
    viewModel.setIsInPipMode(isInPipMode())

    lifecycleScope.launch {
      launch(WaveDispatchers.Unconfined) {
    ...
    rootView().viewTreeObserver.addOnGlobalLayoutListener {
      ...
      if (state.needsNewRequestSizes()) {
        requestNewSizesThrottle.publish { AppDependencies.waveCallManager.updateRenderedResolutions() }
      }
    }
          ...
          MaterialAlertDialogBuilder(this)
            ...
            .setPositiveButton(R.string.WebRtcCallActivity__approve_all) { _, _ ->
              for (id in recipientIds) {
                AppDependencies.waveCallManager.setCallLinkJoinRequestAccepted(id)
              }
            }
            .show()
        }
        ...
        PendingParticipantsBottomSheet.Action.DENY_ALL -> {
          MaterialAlertDialogBuilder(this)
            ...
            .setPositiveButton(R.string.WebRtcCallActivity__deny_all) { _, _ ->
              for (id in recipientIds) {
                AppDependencies.waveCallManager.setCallLinkJoinRequestRejected(id)
              }
            }
            .show()
        }
  ...
  private fun handleSetAudioHandset() {
    AppDependencies.waveCallManager.selectAudioDevice(ChosenAudioDeviceIdentifier(WaveAudioManager.AudioDevice.EARPIECE))
  }

  private fun handleSetAudioSpeaker() {
    AppDependencies.waveCallManager.selectAudioDevice(ChosenAudioDeviceIdentifier(WaveAudioManager.AudioDevice.SPEAKER_PHONE))
  }

  private fun handleSetAudioBluetooth() {
    AppDependencies.waveCallManager.selectAudioDevice(ChosenAudioDeviceIdentifier(WaveAudioManager.AudioDevice.BLUETOOTH))
  }

  private fun handleSetAudioWiredHeadset() {
    AppDependencies.waveCallManager.selectAudioDevice(ChosenAudioDeviceIdentifier(WaveAudioManager.AudioDevice.WIRED_HEADSET))
  }

  private fun handleSetMuteAudio(enabled: Boolean) {
    AppDependencies.waveCallManager.setMuteAudio(enabled)
  }
  ...
  private fun handleSetMuteVideo(muted: Boolean) {
    ...
    if (recipient != Recipient.UNKNOWN) {
      askCameraPermissions {
        AppDependencies.waveCallManager.setEnableVideo(!muted)
      }
    }
  }

  private fun handleFlipCamera() {
    AppDependencies.waveCallManager.flipCamera()
  }

  private fun handleAnswerWithAudio() {
    askAudioPermissions {
      callScreen.setStatus(getString(R.string.RedPhone_answering))
      AppDependencies.waveCallManager.acceptCall(false)
    }
  }
  ...
  private fun handleAnswerWithVideo() {
    val onGranted: () -> Unit = {
      callScreen.setStatus(getString(R.string.RedPhone_answering))
      AppDependencies.waveCallManager.acceptCall(true)
      handleSetMuteVideo(false)
    }
  ...
  private fun handleDenyCall() {
    val recipient = viewModel.recipient.get()
    if (recipient != Recipient.UNKNOWN) {
      AppDependencies.waveCallManager.denyCall()
  ...
  private fun handleEndCall() {
    ...
    AppDependencies.waveCallManager.localHangup()
  }
  ...
  private fun handleCallBusy() {
    ...
    delayedFinish(WaveCallManager.BUSY_TONE_LENGTH.toLong())
  }
  ...
  private fun handleCallPreJoin(event: WebRtcViewModel) {
    if (event.groupState.isNotIdle) {
      ...
      if (event.ringGroup && event.areRemoteDevicesInCall()) {
        AppDependencies.waveCallManager.setRingGroup(false)
      }
    }
  }
  ...
  private fun handleInCallStatus(inCallStatus: InCallStatus) {
    when (inCallStatus) {
      is InCallStatus.ElapsedTime -> {
        ...
        callScreen.setStatus(getString(R.string.WebRtcCallActivity__wave_s, formatter.toString()))
      }
  ...
  private fun updateGroupMembersForGroupCall() {
    AppDependencies.waveCallManager.requestUpdateGroupMembers()
  }
  ...
  private fun handleGroupMemberCountChange(count: Int) {
    ...
    AppDependencies.waveCallManager.setRingGroup(canRing)
  }
  ...
  private fun startCall(isVideoCall: Boolean) {
    ...
    if (isVideoCall) {
      AppDependencies.waveCallManager.startOutgoingVideoCall(viewModel.recipient.get())
    } else {
      AppDependencies.waveCallManager.startOutgoingAudioCall(viewModel.recipient.get())
    }
  ...
  private inner class ControlsListener : CallScreenControlsListener {
    ...
    override fun onAudioOutputChanged31(audioOutput: WebRtcAudioDevice) {
      ...
      AppDependencies.waveCallManager.selectAudioDevice(ChosenAudioDeviceIdentifier(audioOutput.deviceId!!))
    }
    ...
    override fun onRingGroupChanged(ringGroup: Boolean, ringingAllowed: Boolean) {
      if (ringingAllowed) {
        AppDependencies.waveCallManager.setRingGroup(ringGroup)
  ...
  private inner class PendingParticipantsViewListener : PendingParticipantsListener {
    ...
    override fun onAllowPendingRecipient(pendingRecipient: Recipient) {
      AppDependencies.waveCallManager.setCallLinkJoinRequestAccepted(pendingRecipient.id)
    }

    override fun onRejectPendingRecipient(pendingRecipient: Recipient) {
      AppDependencies.waveCallManager.setCallLinkJoinRequestRejected(pendingRecipient.id)
    }
