...
class NotificationProfileDetailsFragment : DSLSettingsFragment() {
  ...
  private fun getConfiguration(state: NotificationProfileDetailsViewModel.State.Valid): DSLConfiguration {
    ...
    return configure {
      ...
      dividerPref()
      clickPref(
        title = DSLSettingsText.from(R.string.NotificationProfileDetails__delete_profile, ContextCompat.getColor(requireContext(), R.color.wave_alert_primary)),
        icon = DSLSettingsIcon.from(R.drawable.symbol_trash_24, R.color.wave_alert_primary),
        onClick = {
          MaterialAlertDialogBuilder(requireContext())
            ...
            .setPositiveButton(
              SpanUtil.color(
                ContextCompat.getColor(requireContext(), R.color.wave_alert_primary),
