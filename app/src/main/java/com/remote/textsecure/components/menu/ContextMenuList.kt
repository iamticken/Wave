...
class ContextMenuList(recyclerView: RecyclerView, onItemClick: () -> Unit) {
  ...
  private val mappingAdapter = MappingAdapter().apply {
    registerFactory(DisplayItem::class.java, LayoutFactory({ ItemViewHolder(it, onItemClick) }, R.layout.wave_context_menu_item))
  }
  ...
  private class ItemViewHolder(
    ...
    val icon: ImageView = itemView.findViewById(R.id.wave_context_menu_item_icon)
    val title: TextView = itemView.findViewById(R.id.wave_context_menu_item_title)
    ...
    override fun bind(model: DisplayItem) {
      ...
      when (model.displayType) {
        DisplayType.TOP -> itemView.setBackgroundResource(R.drawable.wave_context_menu_item_background_top)
        DisplayType.BOTTOM -> itemView.setBackgroundResource(R.drawable.wave_context_menu_item_background_bottom)
        DisplayType.MIDDLE -> itemView.setBackgroundResource(R.drawable.wave_context_menu_item_background_middle)
        DisplayType.ONLY -> itemView.setBackgroundResource(R.drawable.wave_context_menu_item_background_only)
      }
    }
  }
}
