...
import org.whispersystems.waveservice.api.subscriptions.IdempotencyKey
import org.whispersystems.waveservice.api.subscriptions.SubscriberId
import org.whispersystems.waveservice.internal.EmptyResponse
...
object RecurringInAppPaymentRepository {
  ...
  fun getActiveSubscriptionSync(type: InAppPaymentSubscriberRecord.Type): NetworkResult<ActiveSubscription> {
    if (type == InAppPaymentSubscriberRecord.Type.BACKUP && WaveStore.backup.backupTierInternalOverride == MessageBackupTier.PAID) {
    ...
    response.result.ifPresent { result ->
      if (result.isActive && result.activeSubscription.endOfCurrentPeriod > WaveStore.inAppPayments.getLastEndOfPeriod()) {
  ...
  /**
   * Ensures that the given [InAppPaymentSubscriberRecord.Type] has a [SubscriberId] that has been sent to the Wave Service.
  ...
  fun ensureSubscriberIdSync(subscriberType: InAppPaymentSubscriberRecord.Type, isRotation: Boolean = false, iapSubscriptionId: IAPSubscriptionId? = null) {
    Log.d(TAG, "Ensuring SubscriberId for type $subscriberType exists on Wave service {isRotation?$isRotation}...", true)
    ...
    Log.d(TAG, "Successfully set SubscriberId exists on Wave service.", true)

    InAppPaymentsRepository.setSubscriber(
      InAppPaymentSubscriberRecord(
        subscriberId = subscriberId,
        currency = if (subscriberType == InAppPaymentSubscriberRecord.Type.DONATION) {
          WaveStore.inAppPayments.getRecurringDonationCurrency()
        } else {
          null
        },
        type = subscriberType,
  ...
  /**
   * Cancels the active subscription via the Wave service.
   */
  @WorkerThread
  fun cancelActiveSubscriptionSync(subscriberType: InAppPaymentSubscriberRecord.Type) {
    ...
    WaveStore.inAppPayments.updateLocalStateForManualCancellation(subscriberType)
  ...
  fun setSubscriptionLevelSync(inAppPayment: InAppPaymentTable.InAppPayment): InAppPaymentTable.InAppPayment {
    ...
    getOrCreateLevelUpdateOperation(TAG, subscriptionLevel).use { operation ->
      ...
      if (response.status == 200 || response.status == 204) {
        ...
        WaveStore.inAppPayments.updateLocalStateForLocalSubscribe(subscriberType)
        ...
        if (response.applicationError.isPresent) {
          ...
          WaveStore.inAppPayments.clearLevelOperations()
        } else {
    ...
    val freshPayment = WaveDatabase.inAppPayments.getById(inAppPayment.id)!!
  ...
  fun getOrCreateLevelUpdateOperation(tag: String, subscriptionLevel: String): LevelUpdateOperation {
    ...
    val levelUpdateOperation = WaveStore.inAppPayments.getLevelOperation(subscriptionLevel)
    return if (levelUpdateOperation == null) {
      ...
      WaveStore.inAppPayments.setLevelOperation(newOperation)
