 ...
 *
 * Overflow items are rendered in a [WaveContextMenu].
 */
class WaveBottomActionBar(context: Context, attributeSet: AttributeSet?) : LinearLayout(context, attributeSet) {
  ...
  init {
    ...
    setBackgroundResource(R.drawable.wave_bottom_action_bar_background)
    elevation = 20f
  }
  ...
  private fun present(items: List<ActionItem>) {
    ...
    renderableItems.forEach { item ->
      val view: View = LayoutInflater.from(context).inflate(R.layout.wave_bottom_action_bar_item, this, false)
      addView(view)
      bindItem(view, item)
    }
    ...
    if (overflowItems.isNotEmpty()) {
      ...
      bindItem(
        ...
        ActionItem(
          ...
          title = context.getString(R.string.WaveBottomActionBar_more),
          action = {
            WaveContextMenu.Builder(view, parent as ViewGroup)
              .preferredHorizontalPosition(WaveContextMenu.HorizontalPosition.END)
  ...
  private fun bindItem(view: View, item: ActionItem) {
    val icon: ImageView = view.findViewById(R.id.wave_bottom_action_bar_item_icon)
    val title: TextView = view.findViewById(R.id.wave_bottom_action_bar_item_title)
...
fun WaveBottomActionBar(
  ...
  AnimatedVisibility(
    ...
    AndroidView(
      factory = { context ->
        WaveBottomActionBar(context, null)
          .apply {
            elevation = 0f
            setItems(items)
          }
