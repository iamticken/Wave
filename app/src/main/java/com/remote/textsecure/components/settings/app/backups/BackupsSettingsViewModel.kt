...
class BackupsSettingsViewModel : ViewModel() {
  ...
  init {
      ...
      repo.backupState.collect { enabledState ->
        ...
        internalStateFlow.update {
          it.copy(
            ...
            lastBackupAt = WaveStore.backup.lastBackupTime.milliseconds,
            showBackupTierInternalOverride = Environment.IS_STAGING,
            backupTierInternalOverride = WaveStore.backup.backupTierInternalOverride
          )
        }
      }
    }
    ...
    viewModelScope.launch(Dispatchers.IO) {
      WaveStore.backup.lastBackupTimeFlow
  ...
  fun onBackupTierInternalOverrideChanged(tier: MessageBackupTier?) {
    WaveStore.backup.backupTierInternalOverride = tier
    WaveStore.backup.deletionState = DeletionState.NONE
    viewModelScope.launch(WaveDispatchers.Default) {
