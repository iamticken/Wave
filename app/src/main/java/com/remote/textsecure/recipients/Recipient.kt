...
import org.whispersystems.waveservice.api.util.OptionalUtil
...
class Recipient(
  ...
  /** Whether the recipient represents a group. It could be a Wave group or MMS group. */
  val isGroup: Boolean
  ...
  /** Whether the recipient represents a Wave group. */
  ...
  /** Whether the recipient represents a V1 Wave group. These types of groups were deprecated in 2020. */
  ...
  /** Whether the recipient represents a V2 Wave group. */
  ...
  val contactPhoto: ContactPhoto?
    ...
    } else if (systemContactPhoto != null && WaveStore.settings.isPreferSystemContactPhotos) {
  ...
  /** The [RegisteredState] of this recipient. Wave groups/lists are always registered. */
  ...
  val wallpaper: ChatWallpaper?
    get() {
      return if (wallpaperValue != null) {
        ...
        WaveStore.wallpaper.getWallpaper()
      }
    }
  ...
  val hasWallpaper: Boolean
    get() = wallpaperValue != null || WaveStore.wallpaper.hasWallpaperSet()
  ...
  val chatColors: ChatColors
    get() {
      return if (chatColorsValue != null && chatColorsValue.id !is Auto) {
        ...
        val global = WaveStore.chatColors.chatColors
   ...
   * This value respects the local user's [WaveStore.inAppPayments.getDisplayBadgesOnProfile()] preference.
   */
  val featuredBadge: Badge? get() {
    return if (isSelf && !WaveStore.inAppPayments.getDisplayBadgesOnProfile()) {
      null
    } else {
      badges.firstOrNull()
    }
  ...
  fun getGroupName(context: Context): String? {
    return if (groupIdValue != null && Util.isEmpty(groupName)) {
      ...
      context.getString(R.string.Recipient_wave_call)
    } else {
      groupName
    }
  }
  ...
  private fun getNameFromLocalData(context: Context): String? {
    ...
    if (name.isBlank() && e164Value.isNotNullOrBlank()) {
      name = WaveE164Util.prettyPrint(e164Value)
    }
  ...
  companion object {
    ...
    fun distributionList(distributionListId: DistributionListId): Recipient {
      val id = WaveDatabase.recipients.getOrInsertFromDistributionListId(distributionListId)
      return resolved(id)
    }
    ...
    fun externalUsername(serviceId: ServiceId, username: String): Recipient {
      ...
      WaveDatabase.recipients.setUsername(recipient.id, username)
      return recipient
    }

    /**
     * Returns a fully-populated [Recipient] based off of a [WaveServiceAddress], creating one in the database if necessary.
     */
    @JvmStatic
    @WorkerThread
    fun externalPush(waveServiceAddress: WaveServiceAddress): Recipient {
      return externalPush(waveServiceAddress.serviceId, waveServiceAddress.number.orElse(null))
    }
    ...
    private fun externalPush(serviceId: ServiceId, e164: String?): Recipient {
      ...
      val recipientId = RecipientId.from(WaveServiceAddress(serviceId, e164))
      ...
      if (!resolved.isRegistered) {
        ...
        WaveDatabase.recipients.markRegistered(recipientId, serviceId)
      }
    ...
    fun trustedPush(aci: ACI, pni: PNI?, e164: String?): Recipient {
      ...
      val recipientId = WaveDatabase.recipients.getAndPossiblyMergePnpVerified(aci, pni, e164)
      val resolved = resolved(recipientId)
    ...
    fun externalContact(identifier: String): Recipient? {
      val id: RecipientId = if (UuidUtil.isUuid(identifier)) {
        ...
        WaveDatabase.recipients.getOrInsertFromEmail(identifier)
      } else {
        val e164 = WaveE164Util.formatAsE164(identifier) ?: return null
        WaveDatabase.recipients.getOrInsertFromE164(e164)
      }
    ...
    fun externalGroupExact(groupId: GroupId): Recipient {
      return resolved(WaveDatabase.recipients.getOrInsertFromGroupId(groupId))
    }
    ...
    fun externalPossiblyMigratedGroup(groupId: GroupId): Recipient {
      ...
      return try {
        ...
        resolved(WaveDatabase.recipients.getOrInsertFromPossiblyMigratedGroupId(groupId))
      }
    }
     ...
     *
     * If the identifier is a UUID of a Wave user, prefer using
    ...
    fun external(identifier: String): Recipient? {
      ...
      val id: RecipientId = if (serviceId != null) {
        ...
        WaveDatabase.recipients.getOrInsertFromGroupId(GroupId.parseOrThrow(identifier))
      } else if (NumberUtil.isValidEmail(identifier)) {
        WaveDatabase.recipients.getOrInsertFromEmail(identifier)
        ...
        val e164: String = WaveE164Util.formatAsE164(identifier) ?: return null
        WaveDatabase.recipients.getOrInsertFromE164(e164)
      }
