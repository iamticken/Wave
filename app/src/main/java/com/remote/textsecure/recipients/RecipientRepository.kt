...
object RecipientRepository {
  ...
  /**
   * Validates whether the provided [PhoneNumber] is a registered Wave user. Checks locally first, then queries the CDSI directory if needed.
   */
  ...
  /**
   * Validates whether the provided [RecipientId]s are registered Wave users.
   */
  suspend fun lookup(recipientIds: List<RecipientId>): IdLookupResult {
    ...
    return if (unregisteredRecipients.isNotEmpty()) {
      Log.w(TAG, "Found ${unregisteredRecipients.size} non-Wave users: ${unregisteredRecipients.joinToString(", ") { it.id.toString() }}")
  ...
  fun lookupNewE164(inputE164: String): PhoneLookupResult {
    val e164 = WaveE164Util.formatAsE164(inputE164)

    if (e164 == null || !NumberUtil.isVisuallyValidNumber(e164)) {
      return PhoneLookupResult.InvalidPhone(inputE164)
    }

    val matchingFullRecipientId = WaveDatabase.recipients.getByE164IfRegisteredAndDiscoverable(e164)
