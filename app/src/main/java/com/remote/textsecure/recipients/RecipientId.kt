...
class RecipientId private constructor(private val id: Long) : Parcelable, Comparable<RecipientId>, DatabaseId {
  ...
  companion object {
    ...
    @JvmStatic
    fun from(address: WaveServiceAddress): RecipientId = from(address.serviceId, address.number.orNull())
    ...
    fun from(groupId: GroupId): RecipientId {
      ...
      if (recipientId == null) {
        ...
        recipientId = WaveDatabase.recipients.getOrInsertFromPossiblyMigratedGroupId(groupId)
        if (groupId.isV2) {
    ...
    fun from(serviceId: ServiceId?, e164: String?): RecipientId {
      ...
      if (recipientId == null) {
        recipientId = WaveDatabase.recipients.getAndPossiblyMerge(serviceId, e164)
