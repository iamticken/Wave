...
final class RecipientDialogViewModel extends ViewModel {
                                   ...
                                   @NonNull RecipientDialogRepository recipientDialogRepository)
  {
    ...
    this.isDeprecatedOrUnregistered = WaveStore.misc().isClientDeprecated() || TextSecurePreferences.isUnauthorizedReceived(context);
  ...
  public void onResetBlurAvatar(@NonNull Recipient recipient) {
    WaveExecutors.BOUNDED.execute(() -> WaveDatabase.recipients().manuallyUpdateShowAvatar(recipient.getId(), false));
  }

  public void refreshGroupId(@Nullable GroupId groupId) {
    if (groupId != null) {
      WaveExecutors.BOUNDED.execute(() -> {
        RecipientId groupRecipientId = WaveDatabase.groups().getGroup(groupId).get().getRecipientId();
