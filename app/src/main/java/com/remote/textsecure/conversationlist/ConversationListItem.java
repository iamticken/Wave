...
public final class ConversationListItem extends ConstraintLayout implements BindableConversationListItem, Unbindable {
  ...
  protected void onFinishInflate() {
    ...
    this.searchStyleFactory      = () -> new CharacterStyle[] { new ForegroundColorSpan(ContextCompat.getColor(getContext(), R.color.wave_colorOnSurface)), SpanUtil.getBoldSpan() };
                         ...
                         long activeThreadId)
  {
    ...
    if (appendSystemContactIcon && recipient.get().isSystemContact() && !recipient.get().getShowVerified()) {
      ...
      drawable.setTint(ContextCompat.getColor(getContext(), R.color.wave_colorOnSurface));
    ...
    if (thread.getDate() > 0) {
      ...
      dateView.setTextColor(thread.isRead() ? ContextCompat.getColor(getContext(), R.color.wave_text_secondary)
                                            : ContextCompat.getColor(getContext(), R.color.wave_text_primary));
  ...
  {
    int defaultTint = ContextCompat.getColor(context, R.color.wave_text_secondary);
    ...
    if (!thread.isMessageRequestAccepted()) {
      ...
      return emphasisAdded(recipientToStringAsync(thread.getRecipient().getId(), r -> new SpannableString(context.getString(R.string.ThreadRecord_s_is_on_wave, r.getDisplayName(context)))));
      ...
      if (thread.getRecipient().getE164().isPresent()) {
        return emphasisAdded(context, context.getString(R.string.ThreadRecord_s_belongs_to_s, WaveE164Util.prettyPrint(thread.getRecipient().requireE164()), thread.getRecipient().getDisplayName(context)), defaultTint);
      } else {
