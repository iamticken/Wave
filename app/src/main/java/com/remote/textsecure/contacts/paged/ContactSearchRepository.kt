...
class ContactSearchRepository {
  ...
  private fun canSelectRecipient(recipientId: RecipientId): Boolean {
    ...
    return if (recipient.isPushV2Group) {
      val record = WaveDatabase.groups.getGroup(recipient.requireGroupId())
  ...
  fun markDisplayAsStory(recipientIds: Collection<RecipientId>): Completable {
    return Completable.fromAction {
      WaveDatabase.groups.setShowAsStoryState(recipientIds, GroupTable.ShowAsStoryState.ALWAYS)
      WaveDatabase.recipients.markNeedsSync(recipientIds)
