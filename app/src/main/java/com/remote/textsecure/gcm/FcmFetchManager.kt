...
object FcmFetchManager {
  ...
  private val EXECUTOR = SerialMonoLifoExecutor(WaveExecutors.UNBOUNDED)

  private val KEEP_ALIVE_TOKEN = "FcmFetch"

  val WEBSOCKET_DRAIN_TIMEOUT: Long
    get() {
      return if (AppDependencies.waveServiceNetworkAccess.isCensored()) {
  ...
  fun startBackgroundService(context: Context) {
    ...
    WaveLocalMetrics.FcmServiceStartSuccess.onFcmStarted()
  }
  ...
  fun startForegroundService(context: Context) {
    ...
    if (FcmFetchForegroundService.startServiceIfNecessary(context)) {
      ...
      WaveLocalMetrics.FcmServiceStartFailure.onFcmFailedToStart()
    }
  }
  ...
  private fun fetch(context: Context) {
    ...
    val metricId = WaveLocalMetrics.PushWebsocketFetch.startFetch()
    val success = retrieveMessages(context)
    if (success) {
      WaveLocalMetrics.PushWebsocketFetch.onDrained(metricId)
      cancelMayHaveMessagesNotification(context)
    } else {
      WaveLocalMetrics.PushWebsocketFetch.onTimedOut(metricId)
