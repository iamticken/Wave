...
class RestoreOptimizedMediaJob private constructor(parameters: Parameters) : Job(parameters) {
  ...
  companion object {
    ...
    fun enqueueIfNecessary() {
      if (WaveStore.backup.backsUpMedia && !WaveStore.backup.optimizeStorage) {
  ...
  override fun run(): Result {
    if (WaveStore.backup.optimizeStorage && !WaveStore.backup.userManuallySkippedMediaRestore) {
    ...
    val restorableAttachments = WaveDatabase.attachments.getRestorableOptimizedAttachments()
