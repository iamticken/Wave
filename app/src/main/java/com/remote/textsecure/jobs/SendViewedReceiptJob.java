...
import org.wave.core.util.ListUtil;
...
public class SendViewedReceiptJob extends BaseJob {
  ...
  public void onRun() throws IOException, UntrustedIdentityException {
    ...
    boolean canSendStoryReceipts    = WaveStore.story().getViewedReceiptsEnabled();

    List<MessageId> foundMessageIds       = new LinkedList<>();
    List<Long>      messageSentTimestamps = new LinkedList<>();
    List<StoryType> storyTypes            = WaveDatabase.messages().getStoryTypes(this.messageIds);
    ...
    WaveServiceAddress        remoteAddress  = RecipientUtil.toWaveServiceAddress(context, recipient);
    WaveServiceReceiptMessage receiptMessage = new WaveServiceReceiptMessage(WaveServiceReceiptMessage.Type.VIEWED,
    ...
    SendMessageResult result = messageSender.sendReceipt(remoteAddress,
                                                         SealedSenderAccessUtil.getSealedSenderAccessFor(recipient,
                                                                                                         () -> WaveDatabase.groups().getGroupSendFullToken(threadId, recipientId)),
                                                         receiptMessage,
                                                         recipient.getNeedsPniSignature());

    if (Util.hasItems(foundMessageIds)) {
      WaveDatabase.messageLog().insertIfPossible(recipientId, timestamp, result, ContentHint.IMPLICIT, foundMessageIds, false);
    }
  }
