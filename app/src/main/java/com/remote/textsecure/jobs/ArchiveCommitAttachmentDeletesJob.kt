...
class ArchiveCommitAttachmentDeletesJob private constructor(parameters: Parameters) : Job(parameters) {
  ...
  companion object {
     ...
     */
    fun deleteMediaObjectsFromCdn(tag: String, attachmentsToDelete: Set<ArchivedMediaObject>, backoffGenerator: () -> Long, cancellationWave: () -> Boolean): Result? {
  ...
  override fun run(): Result {
    ...
    var mediaObjects = WaveDatabase.backupMediaSnapshots.getPageOfOldMediaObjects(REMOTE_DELETE_BATCH_SIZE)
    ...
    while (mediaObjects.isNotEmpty()) {
      ...
      WaveDatabase.backupMediaSnapshots.deleteOldMediaObjects(mediaObjects)

      mediaObjects = WaveDatabase.backupMediaSnapshots.getPageOfOldMediaObjects(REMOTE_DELETE_BATCH_SIZE)
    }
