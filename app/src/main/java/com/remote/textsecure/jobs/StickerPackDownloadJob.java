...
public class StickerPackDownloadJob extends BaseJob {
  ...
  protected void onRun() throws IOException, InvalidMessageException {
    if (isReferencePack && !WaveDatabase.attachments().containsStickerPackId(packId) && !BlessedPacks.contains(packId)) {
      Log.w(TAG, "There are no attachments with the requested packId present for this reference pack. Skipping.");
      return;
    }

    if (isReferencePack && WaveDatabase.stickers().isPackAvailableAsReference(packId)) {
    ...
    StickerTable stickerDatabase = WaveDatabase.stickers();
  ...
  public void onFailure() {
    ...
    WaveDatabase.stickers().uninstallPack(packId);
    WaveDatabase.stickers().deleteOrphanedPacks();
  }
