...
import org.wave.core.util.readLength
...
import org.whispersystems.waveservice.api.archive.ArchiveMediaUploadFormStatusCodes
...
class UploadAttachmentToArchiveJob private constructor(
  ...
  override fun onAdded() {
    val transferStatus = WaveDatabase.attachments.getArchiveTransferState(attachmentId) ?: return
    ...
    if (transferStatus == AttachmentTable.ArchiveTransferState.NONE) {
      ...
      ArchiveDatabaseExecutor.runBlocking {
        WaveDatabase.attachments.setArchiveTransferStateUnlessPermanentFailure(attachmentId, AttachmentTable.ArchiveTransferState.UPLOAD_IN_PROGRESS)
      }
    }
  }
  ...
  override fun run(): Result {
    ...
    val attachment: DatabaseAttachment? = WaveDatabase.attachments.getAttachment(attachmentId)
    ...
    if (WaveDatabase.messages.isStory(attachment.mmsId)) {
    ...
    if (WaveDatabase.messages.isViewOnce(attachment.mmsId)) {
    ...
    if (WaveDatabase.messages.willMessageExpireBeforeCutoff(attachment.mmsId)) {
    ...
    progressServiceController.use {
      val uploadResult: AttachmentUploadResult = attachmentStream.use { managedAttachmentStream ->
        when (val result = WaveNetwork.attachments.uploadAttachmentV4(managedAttachmentStream)) {
          ...
          is NetworkResult.NetworkError -> {
            ...
            if (result.exception.cause is ProtocolException) {
              ...
              val actualLength = WaveDatabase.attachments.getAttachmentStream(attachmentId, 0)
                .use { it.readLength() }
              if (actualLength != attachment.size) {
                ...
                ArchiveDatabaseExecutor.runBlocking {
                  WaveDatabase.attachments.updateAttachmentLength(attachmentId, actualLength)
                }
              } else {
      ...
      ArchiveDatabaseExecutor.runBlocking {
        WaveDatabase.attachments.finalizeAttachmentAfterUpload(attachment.attachmentId, uploadResult)
      }
    }
  ...
  override fun onFailure() {
    if (this.isCanceled) {
      ...
      ArchiveDatabaseExecutor.runBlocking {
        WaveDatabase.attachments.setArchiveTransferStateFailure(attachmentId, AttachmentTable.ArchiveTransferState.NONE)
      }
    } else {
      ...
      ArchiveDatabaseExecutor.runBlocking {
        WaveDatabase.attachments.setArchiveTransferStateFailure(attachmentId, AttachmentTable.ArchiveTransferState.TEMPORARY_FAILURE)
      }
    }
  }
  ...
  private fun fetchResumableUploadSpec(key: ByteArray, iv: ByteArray): Pair<ResumableUpload?, Result?> {
    val uploadSpec = BackupRepository
      ...
      .then { form -> WaveNetwork.attachments.getResumableUploadSpec(key, iv, form) }
  ...
  private fun setArchiveTransferStateWithDelayedNotification(attachmentId: AttachmentId, transferState: AttachmentTable.ArchiveTransferState) {
    WaveDatabase.attachments.setArchiveTransferState(attachmentId, transferState, notify = false)
