...
import org.whispersystems.waveservice.api.crypto.InvalidCiphertextException
import org.whispersystems.waveservice.api.crypto.ProfileCipher
import org.whispersystems.waveservice.api.profiles.ProfileRepository
import org.whispersystems.waveservice.api.profiles.ProfileRepository.IdProfilePair
import org.whispersystems.waveservice.api.profiles.ProfileRepository.ProfileFetchRequest
import org.whispersystems.waveservice.api.profiles.ProfileRepository.ProfileFetchResult
import org.whispersystems.waveservice.api.profiles.ProfileRepository.WaveServiceProfileWithCredential
import org.whispersystems.waveservice.api.profiles.WaveServiceProfile
import org.whispersystems.waveservice.api.util.ExpiringProfileCredentialUtil
...
class RetrieveProfileJob private constructor(parameters: Parameters, private val recipientIds: MutableSet<RecipientId>, private val skipDebounce: Boolean) : BaseJob(parameters) {
  ...
  public override fun onRun() {
    ...
    val response: ProfileFetchResult<RecipientId> = runBlocking {
      withContext(Dispatchers.IO) {
        ProfileRepository(WaveNetwork.profile).fetchProfiles(requests)
      }
    }
    stopwatch.split("responses")

    val localRecords = WaveDatabase.recipients.getExistingRecords(fetchingRecipientIds)
    ...
    WaveDatabase.recipients.markProfilesFetched(successIds, System.currentTimeMillis())
    ...
    if (newlyRegisteredIds.isNotEmpty()) {
      ...
      WaveDatabase.recipients.bulkUpdatedRegisteredStatus(newlyRegisteredIds, emptySet())
    }
    if (response.unregistered.isNotEmpty()) {
      ...
      for (recipientId in response.unregistered) {
        WaveDatabase.recipients.markUnregistered(recipientId)
      }
    }
    ...
    if (response.verificationFailures.isNotEmpty()) {
      ...
      for (recipientId in response.verificationFailures) {
        WaveDatabase.recipients.clearProfileKeyData(recipientId)
      }
    }
  ...
  private fun isUpdated(localRecipientRecord: RecipientRecord, remoteProfile: WaveServiceProfile, remoteExpiringProfileKeyCredential: ExpiringProfileKeyCredential?): Boolean {
    if (localRecipientRecord.waveProfileAvatar != remoteProfile.avatar) {
      return true
    }
    ...
    if (localRecipientRecord.waveProfileName != newProfileName) {
      return true
    }
  ...
  private fun process(recipient: Recipient, profileAndCredential: WaveServiceProfileWithCredential) {
  ...
  private fun setProfileBadges(recipient: Recipient, serviceBadges: List<WaveServiceProfile.Badge>?) {
    ...
    WaveDatabase.recipients.setBadges(recipient.id, badges)
  }
  ...
  private fun setExpiringProfileKeyCredential(
    ...
    WaveDatabase.recipients.setProfileKeyCredential(recipient.id, recipientProfileKey, credential)
  }
  ...
  private fun setUnidentifiedAccessMode(recipient: Recipient, unidentifiedAccessVerifier: String?, unrestrictedUnidentifiedAccess: Boolean) {
    ...
    if (recipient.sealedSenderAccessMode !== newMode) {
      ...
      WaveDatabase.recipients.setSealedSenderAccessMode(recipient.id, newMode)
    }
  }
  ...
  private fun setProfileName(recipient: Recipient, profileName: String?): Boolean {
    try {
      ...
      if (localProfileName.isEmpty &&
        ...
        val username = WaveDatabase.recipients.getUsername(recipient.id)
        val e164 = if (username == null) WaveDatabase.recipients.getE164sForIds(listOf(recipient.id)).firstOrNull() else null

        if (username != null || e164 != null) {
          Log.i(TAG, "Learned profile name for first time, inserting event")
          WaveDatabase.messages.insertLearnedProfileNameChangeMessage(recipient, e164, username)
        } else {
      ...
      if (remoteProfileName != localProfileName) {
        ...
        WaveDatabase.recipients.setProfileName(recipient.id, remoteProfileName)
        ...
        if (writeChangeEvent) {
          ...
          WaveDatabase.messages.insertProfileNameChangeMessages(recipient, remoteDisplayName, localDisplayName)
        } else {
        ...
        if (recipient.isIndividual &&
          ...
          val threadId = WaveDatabase.threads.getThreadIdFor(recipient.id)
          if (threadId != null && !RecipientUtil.isMessageRequestAccepted(threadId, recipient)) {
            WaveDatabase.nameCollisions.handleIndividualNameCollision(recipient.id)
          }
        }
  ...
  private fun setProfileAbout(recipient: Recipient, encryptedAbout: String?, encryptedEmoji: String?) {
    try {
      ...
      WaveDatabase.recipients.setAbout(recipient.id, plaintextAbout, plaintextEmoji)
    } catch (e: InvalidCiphertextException) {
      Log.w(TAG, e)
    } catch (e: IOException) {
      Log.w(TAG, e)
  ...
  private fun clearUsername(recipient: Recipient) {
    WaveDatabase.recipients.setUsername(recipient.id, null)
  }

  private fun setProfileCapabilities(recipient: Recipient, capabilities: WaveServiceProfile.Capabilities?) {
    if (capabilities == null) {
      return
    }

    WaveDatabase.recipients.setCapabilities(recipient.id, capabilities)
  }
  ...
  private fun setPhoneNumberSharingMode(recipient: Recipient, phoneNumberSharing: String?) {
    ...
    try {
      ...
      if (recipient.phoneNumberSharing !== remotePhoneNumberSharing) {
        ...
        WaveDatabase.recipients.setPhoneNumberSharing(recipient.id, remotePhoneNumberSharing)
      }
    } catch (e: InvalidCiphertextException) {
  ...
  private fun setProfileAvatar(recipient: Recipient, profileAvatar: String?) {
    ...
    if (profileAvatar != recipient.profileAvatar) {
      WaveDatabase.runPostSuccessfulTransaction(DEDUPE_KEY_RETRIEVE_AVATAR + recipient.id) {
  ...
  companion object {
    ...
    fun forRecipients(recipientIds: Set<RecipientId>, skipDebounce: Boolean = false): List<Job> {
      ...
      for (recipientId in recipientIds) {
        ...
        when {
          ...
          recipient.isGroup -> combined += WaveDatabase.groups.getGroupMemberIds(recipient.requireGroupId(), GroupTable.MemberSet.FULL_MEMBERS_EXCLUDING_SELF)
    ...
    fun enqueueRoutineFetchIfNecessary() {
      if (!WaveStore.registration.isRegistrationComplete || !WaveStore.account.isRegistered || WaveStore.account.aci == null) {
        Log.i(TAG, "Registration not complete. Skipping.")
        return
      }

      val timeSinceRefresh = System.currentTimeMillis() - WaveStore.misc.lastProfileRefreshTime
      ...
      WaveExecutors.BOUNDED.execute {
        ...
        val ids: List<RecipientId> = WaveDatabase.recipients.getRecipientsForRoutineProfileFetch(
        ...
        WaveStore.misc.lastProfileRefreshTime = System.currentTimeMillis()
      }
    }
  }
}
