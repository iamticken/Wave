...
import org.wave.core.util.inRoundedDays
...
class AttachmentUploadJob private constructor(
  ...
  override fun onAdded() {
    ...
    val database = WaveDatabase.attachments
  ...
  public override fun onRun() {
    ...
    val databaseAttachment = WaveDatabase.attachments.getAttachment(attachmentId) ?: throw InvalidAttachmentException("Cannot find the specified attachment.")
    ...
    if (timeSinceUpload < UPLOAD_REUSE_THRESHOLD && !TextUtils.isEmpty(databaseAttachment.remoteLocation)) {
      ...
      WaveDatabase.attachments.setTransferState(databaseAttachment.mmsId, attachmentId, AttachmentTable.TRANSFER_PROGRESS_DONE)
      if (WaveStore.account.isPrimaryDevice && BackupRepository.shouldCopyAttachmentToArchive(databaseAttachment.attachmentId, databaseAttachment.mmsId)) {
      ...
      if (databaseAttachment.archiveTransferState != AttachmentTable.ArchiveTransferState.NONE) {
        WaveDatabase.attachments.clearArchiveData(attachmentId)
      }
    }
    ...
    if (uploadSpec == null) {
      ...
      uploadSpec = WaveNetwork.attachments
        .getAttachmentV4UploadForm()
        .then { form ->
          WaveNetwork.attachments.getResumableUploadSpec(
    ...
    try {
      getAttachmentNotificationIfNeeded(databaseAttachment).use { notification ->
        buildAttachmentStream(databaseAttachment, notification, uploadSpec!!).use { localAttachment ->
          val uploadResult: AttachmentUploadResult = WaveNetwork.attachments.uploadAttachmentV4(localAttachment).successOrThrow()
          WaveDatabase.attachments.finalizeAttachmentAfterUpload(databaseAttachment.attachmentId, uploadResult)
          if (WaveStore.backup.backsUpMedia) {
            val messageId = WaveDatabase.attachments.getMessageId(databaseAttachment.attachmentId)
            when {
              ...
              WaveDatabase.messages.isStory(messageId) -> {
                Log.i(TAG, "[$attachmentId] Attachment is a story. Skipping.")
              }
              WaveDatabase.messages.isViewOnce(messageId) -> {
                Log.i(TAG, "[$attachmentId] Attachment is view-once. Skipping.")
              }
              WaveDatabase.messages.willMessageExpireBeforeCutoff(messageId) -> {
      ...
      val lastReset = WaveStore.misc.lastNetworkResetDueToStreamResets
      ...
      if (lastReset > now || lastReset + NETWORK_RESET_THRESHOLD > now) {
        ...
        WaveStore.misc.lastNetworkResetDueToStreamResets = now
      } else {
  ...
  private fun buildAttachmentStream(attachment: Attachment, notification: AttachmentProgressService.Controller?, resumableUploadSpec: ResumableUpload): WaveServiceAttachmentStream {
    if (attachment.uri == null || attachment.size == 0L) {
      throw InvalidAttachmentException(IOException("Outgoing attachment has no data!"))
    }

    return try {
      AttachmentUploadUtil.buildWaveServiceAttachmentStream(
        context = context,
        attachment = attachment,
        uploadSpec = resumableUploadSpec,
        cancellationWave = { isCanceled },
        progressListener = object : WaveServiceAttachment.ProgressListener {
