...
class ArchiveThumbnailUploadJob private constructor(
  ...
  override fun onAdded() {
    val transferStatus = WaveDatabase.attachments.getArchiveThumbnailTransferState(attachmentId) ?: return

    if (transferStatus == AttachmentTable.ArchiveTransferState.NONE) {
      ArchiveDatabaseExecutor.runBlocking {
        WaveDatabase.attachments.setArchiveThumbnailTransferState(attachmentId, AttachmentTable.ArchiveTransferState.UPLOAD_IN_PROGRESS)
      }
    }
  }
  ...
  override fun run(): Result {
    ...
    if (!MediaUtil.isImageOrVideoType(attachment.contentType)) {
      ...
      ArchiveDatabaseExecutor.runBlocking {
        WaveDatabase.attachments.setArchiveThumbnailTransferState(attachmentId, AttachmentTable.ArchiveTransferState.NONE)
      }
      return Result.success()
    }
    ...
    if (thumbnailResult == null) {
      ...
      ArchiveDatabaseExecutor.runBlocking {
        WaveDatabase.attachments.setArchiveThumbnailTransferState(attachmentId, AttachmentTable.ArchiveTransferState.PERMANENT_FAILURE)
      }
      return Result.success()
    }
    ...
    if (isCanceled) {
      ArchiveDatabaseExecutor.runBlocking {
        WaveDatabase.attachments.setArchiveThumbnailTransferState(attachmentId, AttachmentTable.ArchiveTransferState.TEMPORARY_FAILURE)
      }
      return Result.failure()
    }
    ...
    val attachmentPointer = try {
      buildWaveServiceAttachmentStream(thumbnailResult, resumableUpload).use { stream ->
        val pointer = AppDependencies.waveServiceMessageSender.uploadAttachment(stream)
    ...
    return when (val result = BackupRepository.copyThumbnailToArchive(attachmentPointer, attachment)) {
      is NetworkResult.Success -> {
        ...
        ArchiveDatabaseExecutor.runBlocking {
          WaveDatabase.attachments.finalizeAttachmentThumbnailAfterUpload(
          ...
          WaveDatabase.attachments.setArchiveThumbnailTransferState(attachmentId, AttachmentTable.ArchiveTransferState.FINISHED)
        }
        Result.success()
      }
  ...
  override fun onFailure() {
    if (this.isCanceled) {
      ...
      ArchiveDatabaseExecutor.runBlocking {
        WaveDatabase.attachments.setArchiveThumbnailTransferStateFailure(attachmentId, AttachmentTable.ArchiveTransferState.TEMPORARY_FAILURE)
      }
    }
  }
  ...
  private fun buildWaveServiceAttachmentStream(result: ImageCompressionUtil.Result, uploadSpec: ResumableUpload): WaveServiceAttachmentStream {
