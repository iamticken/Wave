...
class RefreshCallLinkDetailsJob private constructor(
  ...
  override fun onRun() {
    val manager: WaveCallLinkManager = AppDependencies.waveCallManager.callLinkManager
    ...
    when (val result = manager.readCallLink(credentials).safeBlockingGet()) {
      is ReadCallLinkResult.Success -> {
        WaveDatabase.callLinks.updateCallLinkState(credentials.roomId, result.callLinkState)
      }
      else -> Unit
    }
  }
