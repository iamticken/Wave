...
/**
 * Creates the Release Channel (Wave) recipient.
 */
class CreateReleaseChannelJob private constructor(parameters: Parameters) : BaseJob(parameters) {
  ...
  override fun onRun() {
    ...
    if (WaveStore.releaseChannel.releaseChannelRecipientId != null) {
      Log.i(TAG, "Already created Release Channel recipient ${WaveStore.releaseChannel.releaseChannelRecipientId}")

      val recipient = Recipient.resolved(WaveStore.releaseChannel.releaseChannelRecipientId!!)
      if (recipient.profileAvatar.isNullOrEmpty() || !WaveStore.releaseChannel.hasUpdatedAvatar) {
        WaveStore.releaseChannel.hasUpdatedAvatar = true
      ...
      WaveStore.releaseChannel.setReleaseChannelRecipientId(releaseChannelId)
      WaveStore.releaseChannel.hasUpdatedAvatar = true

      recipients.setProfileName(releaseChannelId, ProfileName.asGiven("Wave"))
  ...
  private fun setAvatar(id: RecipientId) {
    ...
    AvatarRenderer.renderAvatar(
      ...
      onAvatarRendered = { media ->
        ...
        WaveDatabase.recipients.setProfileAvatar(id, "local")
        latch.countDown()
      },
      onRenderFailed = { t ->
        Log.w(TAG, t)
