...
public final class SenderKeyDistributionSendJob extends BaseJob {
  ...
  protected void onRun() throws Exception {
    ...
    if (threadRecipient.isPushV2Group()) {
      ...
      createGroupSendFullToken = () -> WaveDatabase.groups().getGroupSendFullToken(groupId, targetRecipientId);
    } else if (threadRecipient.isDistributionList()) {
      groupId        = null;
      distributionId = WaveDatabase.distributionLists().getDistributionId(threadRecipientId);
    } else {
    ...
    if (groupId != null && !WaveDatabase.groups().isCurrentMember(groupId, targetRecipientId)) {
      Log.w(TAG, targetRecipientId + " is no longer a member of " + groupId + "! Not sending.");
      return;
    } else if (groupId == null) {
      DistributionListRecord listRecord = WaveDatabase.distributionLists().getList(threadRecipientId);
    ...
    List<WaveServiceAddress>   address       = Collections.singletonList(RecipientUtil.toWaveServiceAddress(context, targetRecipient));
