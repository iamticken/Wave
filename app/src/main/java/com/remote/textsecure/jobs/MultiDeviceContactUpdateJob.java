...
import org.whispersystems.waveservice.api.messages.multidevice.ContactsMessage;
import org.whispersystems.waveservice.api.messages.multidevice.DeviceContact;
import org.whispersystems.waveservice.api.messages.multidevice.DeviceContactAvatar;
import org.whispersystems.waveservice.api.messages.multidevice.DeviceContactsOutputStream;
...
import org.whispersystems.waveservice.api.util.InvalidNumberException;
...
public class MultiDeviceContactUpdateJob extends BaseJob {
      ...
      throws IOException, UntrustedIdentityException, NetworkException
  {
    ...
    try {
      ...
      Map<RecipientId, Integer> inboxPositions  = WaveDatabase.threads().getInboxPositions();
      Set<RecipientId>          archived        = WaveDatabase.threads().getArchivedRecipients();
      ...
      sendUpdate(AppDependencies.getWaveServiceMessageSender(),
      ...
      throws IOException, UntrustedIdentityException, NetworkException
  {
    ...
    try {
      DeviceContactsOutputStream out            = new DeviceContactsOutputStream(writeDetails.outputStream, RemoteConfig.useBinaryId(), BuildConfig.USE_STRING_ID);
      List<Recipient>            recipients     = WaveDatabase.recipients().getRecipientsForMultiDeviceSync();
      Map<RecipientId, Integer>  inboxPositions = WaveDatabase.threads().getInboxPositions();
      Set<RecipientId>           archived       = WaveDatabase.threads().getArchivedRecipients();
      ...
      if (profileKey != null) {
        out.write(new DeviceContact(Optional.of(WaveStore.account().getAci()),
                                    Optional.of(WaveStore.account().getE164()),
                                    Optional.empty(),
                                    Optional.empty(),
  ...
  private void sendUpdate(WaveServiceMessageSender messageSender, InputStream stream, long length, boolean complete)
      throws UntrustedIdentityException, NetworkException
  {
    if (length > 0) {
      try {
        WaveServiceAttachmentStream.Builder attachmentStream = WaveServiceAttachment.newStreamBuilder()
                                                                                        .withStream(stream)
                                                                                        .withContentType("application/octet-stream")
                                                                                        .withLength(length)
                                                                                        .withResumableUploadSpec(messageSender.getResumableUploadSpec());

        messageSender.sendSyncMessage(WaveServiceSyncMessage.forContacts(new ContactsMessage(attachmentStream.build(), complete))
        );
      ...
      throws InvalidNumberException, IOException
  {
    ...
    WaveServiceAddress destination = RecipientUtil.toWaveServiceAddress(context, recipient);
