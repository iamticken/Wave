...
import org.whispersystems.waveservice.api.storage.StorageServiceRepository.ManifestResult
...
class StorageAccountRestoreJob private constructor(parameters: Parameters) : BaseJob(parameters) {
  ...
  override fun onRun() {
    val storageServiceKey = WaveStore.storageService.storageKeyForInitialDataRestore?.let {
      Log.i(TAG, "Using temporary storage key.")
      it
    } ?: run {
      Log.i(TAG, "Using normal storage key.")
      WaveStore.storageService.storageKey
    }

    val repository = StorageServiceRepository(WaveNetwork.storageService)

    Log.i(TAG, "Retrieving manifest...")
    val manifest: WaveStorageManifest? = when (val result = repository.getStorageManifest(storageServiceKey)) {
    ...
    WaveStore.storageService.manifest = WaveStorageManifest.EMPTY
    ...
    val records: List<WaveStorageRecord> = when (val result = repository.readStorageRecords(storageServiceKey, manifest.recordIkm, listOf(accountId.get()))) {
    ...
    val accountRecord = WaveAccountRecord(record.id, record.proto.account!!)
    ...
    if (WaveStore.account.username != null) {
