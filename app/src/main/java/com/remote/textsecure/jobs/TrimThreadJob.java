...
public class TrimThreadJob extends BaseJob {
  ...
  public static void enqueueAsync(long threadId) {
    if (WaveStore.settings().getKeepMessagesDuration() != KeepMessagesDuration.FOREVER || WaveStore.settings().isTrimByLengthEnabled()) {
      WaveExecutors.BOUNDED.execute(() -> AppDependencies.getJobManager().add(new TrimThreadJob(threadId)));
    }
  }
  ...
  public void onRun() {
    ...
    int trimLength = WaveStore.settings().isTrimByLengthEnabled() ? WaveStore.settings().getThreadTrimLength()
    ...
    WaveDatabase.threads().trimThread(threadId, WaveStore.settings().shouldSyncThreadTrimDeletes(), trimLength, trimBeforeDate, false);
  }
