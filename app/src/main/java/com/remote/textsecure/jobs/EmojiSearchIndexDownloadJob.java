...
public final class EmojiSearchIndexDownloadJob extends BaseJob {
  ...
  public static void scheduleIfNecessary() {
    long    timeSinceCheck = System.currentTimeMillis() - WaveStore.emoji().getLastSearchIndexCheck();
    boolean needsCheck     = false;

    if (WaveStore.emoji().hasSearchIndex() && !WaveDatabase.emojiSearch().hasSearchIndexData()) {
      Log.w(TAG, "Emoji search data missing with metadata, clearing version to redownload.");
      WaveStore.emoji().clearSearchIndexVersion();
      scheduleImmediately();
      return;
    }

    if (WaveStore.emoji().hasSearchIndex()) {
  ...
  protected void onRun() throws Exception {
    ...
    if (manifest.getVersion() == WaveStore.emoji().getSearchVersion() &&
        remoteLanguage.equals(WaveStore.emoji().getSearchLanguage()))
    {
      Log.i(TAG, "Already using the latest version of " + manifest.getVersion() + " with the correct language " + remoteLanguage);
      WaveStore.emoji().setLastSearchIndexCheck(System.currentTimeMillis());
      return;
    }
    ...
    WaveDatabase.emojiSearch().setSearchIndex(localizedSearchIndex, englishSearchIndex);
    WaveStore.emoji().onSearchIndexUpdated(manifest.getVersion(), remoteLanguage);
    WaveStore.emoji().setLastSearchIndexCheck(System.currentTimeMillis());
