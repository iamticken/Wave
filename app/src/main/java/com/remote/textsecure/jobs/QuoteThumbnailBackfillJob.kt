...
class QuoteThumbnailBackfillJob private constructor(parameters: Parameters) : Job(parameters) {
  ...
  private fun peformSingleBackfill(): Boolean {
    val attachment = WaveDatabase.attachments.getNextQuoteAttachmentForThumbnailProcessing()
    ...
    val thumbnail = WaveDatabase.attachments.generateQuoteThumbnail(DecryptableUri(attachment.uri), attachment.contentType, quiet = true)
    if (thumbnail != null) {
      WaveDatabase.attachments.migrationFinalizeQuoteWithData(attachment.dataFile, thumbnail, attachment.contentType)
    } else {
      Log.w(TAG, "Failed to generate thumbnail for attachment: ${attachment.id}. Clearing data.")
      WaveDatabase.attachments.finalizeQuoteWithNoData(attachment.dataFile)
    }
