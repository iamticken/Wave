...
class BackupRestoreMediaJob private constructor(parameters: Parameters) : BaseJob(parameters) {
  ...
  override fun onRun() {
    ...
    do {
      ...
      val last30DaysAttachments = WaveDatabase.attachments.getLast30DaysOfRestorableAttachments(batchSize)
      val remainingSize = batchSize - last30DaysAttachments.size

      val remaining = if (remainingSize > 0) {
        WaveDatabase.attachments.getOlderRestorableAttachments(batchSize = remainingSize)
      } else {
        listOf()
      }
      ...
      val messageMap = WaveDatabase.messages.getMessages(messageIds).associate { it.id to (it as MmsMessageRecord) }
      ...
      for (attachment in attachmentBatch) {
        ...
        if (isWallpaper || shouldRestoreFullSize(message!!, restoreTime, WaveStore.backup.optimizeStorage)) {
      ...
      WaveDatabase.rawDatabase.withinTransaction {
        ...
        WaveDatabase.attachments.setThumbnailRestoreState(notRestorable, AttachmentTable.ThumbnailRestoreState.PERMANENT_FAILURE)
        WaveDatabase.attachments.setRestoreTransferState(notRestorable, AttachmentTable.TRANSFER_PROGRESS_FAILED)

        // Set thumbnail only attachments as offloaded
        WaveDatabase.attachments.setRestoreTransferState(restoreThumbnailOnlyAttachmentsIds, AttachmentTable.TRANSFER_RESTORE_OFFLOADED)
      }
