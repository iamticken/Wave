...
public class FcmRefreshJob extends BaseJob {
  ...
  public void onRun() throws Exception {
    if (!WaveStore.account().isFcmEnabled()) return;
    ...
    if (result != ConnectionResult.SUCCESS) {
      ...
      if (token.isPresent()) {
        String oldToken = WaveStore.account().getFcmToken();
        ...
        NetworkResultUtil.toBasicLegacy(WaveNetwork.account().setFcmToken(token.get()));
        WaveStore.account().setFcmToken(token.get());
      } else {
  ...
  private void notifyFcmFailure() {
    ...
    builder.setContentTitle(context.getString(R.string.GcmRefreshJob_Permanent_Wave_communication_failure));
    builder.setContentText(context.getString(R.string.GcmRefreshJob_Wave_was_unable_to_register_with_Google_Play_Services));
    builder.setTicker(context.getString(R.string.GcmRefreshJob_Permanent_Wave_communication_failure));
