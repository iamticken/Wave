...
class BackupDeleteJob private constructor(
  ...
  override fun run(): Result {
    ...
    if (WaveStore.backup.deletionState.isIdle()) {
      Log.w(TAG, "Invalid state ${WaveStore.backup.deletionState}. Exiting without local cleanup.")
      return Result.failure()
    }
  ...
  private fun doRun(): Result {
    ...
    val clearLocalStateResult = if (WaveStore.backup.deletionState == DeletionState.CLEAR_LOCAL_STATE) {
      val results = listOf(
        deleteLocalState(),
    ...
    if (isMediaRestoreRequired()) {
      ...
      WaveStore.backup.deletionState = DeletionState.AWAITING_MEDIA_DOWNLOAD
    ...
    WaveStore.backup.deletionState = DeletionState.DELETE_BACKUPS
    ...
    if (result.isSuccess) {
      ...
      BackupRepository.resetInitializedStateAndAuthCredentials()
      WaveStore.backup.deletionState = DeletionState.COMPLETE
    }
  ...
  override fun onFailure() {
    ...
    } else if (WaveStore.backup.deletionState == DeletionState.AWAITING_MEDIA_DOWNLOAD) {
      Log.w(TAG, "BackupDeleteFailure occurred while awaiting media download, ignoring.")
    } else {
      WaveStore.backup.deletionState = DeletionState.FAILED
    }
  }
  ...
  private fun isMediaRestoreRequired(): Boolean {
    ...
    val requiresMediaRestore = WaveDatabase.attachments.getRemainingRestorableAttachmentSize() > 0L
    val hasOffloadedMedia = WaveDatabase.attachments.getOptimizedMediaAttachmentSize() > 0L

    if ((requiresMediaRestore || hasOffloadedMedia) && !WaveStore.backup.userManuallySkippedMediaRestore) {
  ...
  private fun <T> handleNetworkError(networkResult: NetworkResult<T>): Result {
    ...
    if (networkResult.getCause() is org.wave.libwave.zkgroup.VerificationFailedException) {
