...
class InAppPaymentRedemptionJob private constructor(
  ...
  override fun onAdded() {
    if (jobData.giftMessageId != null) {
      ...
      WaveDatabase.messages.markGiftRedemptionStarted(jobData.giftMessageId)
    }
  }
  ...
  override fun onFailure() {
    if (jobData.giftMessageId != null) {
      ...
      WaveDatabase.messages.markGiftRedemptionFailed(jobData.giftMessageId)
    }

    if (jobData.inAppPaymentId != null) {
      Log.w(TAG, "A permanent failure occurred.")

      val inAppPayment = WaveDatabase.inAppPayments.getById(InAppPaymentTable.InAppPaymentId(jobData.inAppPaymentId))
  ...
  private fun onRunForGiftMessageId(messageId: Long) {
    val giftBadge = WaveDatabase.messages.getMessageRecord(messageId)
    ...
    val serviceResponse = AppDependencies
      ...
      .redeemDonationReceipt(
        ...
        WaveStore.inAppPayments.getDisplayBadgesOnProfile(),
        jobData.makePrimary
      )
    ...
    WaveDatabase.messages.markGiftRedemptionCompleted(messageId)
  }
