...
public class ReactionSendJob extends BaseJob {
  ...
  {
    MessageRecord message = WaveDatabase.messages().getMessageRecord(messageId.getId());

    Recipient conversationRecipient = WaveDatabase.threads().getRecipientForThreadId(message.getThreadId());
  ...
  protected void onRun() throws Exception {
    ...
    ReactionTable reactionTable = WaveDatabase.reactions();
    MessageRecord message       = WaveDatabase.messages().getMessageRecord(messageId.getId());

    Recipient targetAuthor        = message.getFromRecipient();
    long      targetSentTimestamp = message.getDateSent();

    if (targetAuthor.getId().equals(WaveStore.releaseChannel().getReleaseChannelRecipientId())) {
      return;
    }
  ...
  {
    WaveServiceDataMessage.Builder dataMessageBuilder = WaveServiceDataMessage.newBuilder()
    ...
    WaveServiceDataMessage dataMessage         = dataMessageBuilder.build();
    ...
    if (includesSelf) {
      results.add(AppDependencies.getWaveServiceMessageSender().sendSyncMessage(dataMessage));
    }
  ...
  private static WaveServiceDataMessage.Reaction buildReaction(@NonNull Context context,
                                                                 @NonNull ReactionRecord reaction,
                                                                 boolean remove,
      ...
      throws IOException
  {
    return new WaveServiceDataMessage.Reaction(reaction.getEmoji(),
                                                 remove,
