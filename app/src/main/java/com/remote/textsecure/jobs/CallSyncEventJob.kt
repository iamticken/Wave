...
class CallSyncEventJob private constructor(
  ...
  private fun processEvent(callSyncEvent: CallSyncEventJobRecord): CallSyncEventJobRecord? {
    val call = WaveDatabase.calls.getCallById(callSyncEvent.callId, callSyncEvent.deserializeRecipientId())
    if (call == null) {
    ...
    return try {
      AppDependencies.waveServiceMessageSender.sendSyncMessage(WaveServiceSyncMessage.forCallEvent(syncMessage))
      null
    } catch (e: Exception) {
