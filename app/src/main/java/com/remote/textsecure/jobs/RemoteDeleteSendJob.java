...
public class RemoteDeleteSendJob extends BaseJob {
  ...
  {
    MessageRecord message = WaveDatabase.messages().getMessageRecord(messageId);
    ...
    if (conversationRecipient.isDistributionList()) {
      recipients = WaveDatabase.storySends().getRemoteDeleteRecipients(message.getId(), message.getTimestamp());
  ...
  protected void onRun() throws Exception {
    ...
    if (totalSkips.size() > 0 && message.getToRecipient().isGroup()) {
      WaveDatabase.groupReceipts().setSkipped(totalSkips, messageId);
    }
      ...
      throws IOException, UntrustedIdentityException
  {
    WaveServiceDataMessage.Builder dataMessageBuilder = WaveServiceDataMessage.newBuilder()
                                                                                  ...
                                                                                  .withRemoteDelete(new WaveServiceDataMessage.RemoteDelete(targetSentTimestamp));
    ...
    if (conversationRecipient.isSelf()) {
      AppDependencies.getWaveServiceMessageSender().sendSyncMessage(dataMessage);
    }
