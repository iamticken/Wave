...
public class IndividualSendJob extends PushSendJob {
  ...
  public static void enqueue(@NonNull Context context, @NonNull JobManager jobManager, long messageId, @NonNull Recipient recipient, boolean isScheduledSend) {
    try {
      OutgoingMessage message = WaveDatabase.messages().getOutgoingMessage(messageId);
  ...
  {
    WaveLocalMetrics.IndividualMessageSend.onJobStarted(messageId);
    ...
    try {
      ...
      if (recipient.isSelf()) {
        WaveDatabase.messages().incrementDeliveryReceiptCount(message.getSentTimeMillis(), recipient.getId(), System.currentTimeMillis());
        WaveDatabase.messages().incrementReadReceiptCount(message.getSentTimeMillis(), recipient.getId(), System.currentTimeMillis());
        WaveDatabase.messages().incrementViewedReceiptCount(message.getSentTimeMillis(), recipient.getId(), System.currentTimeMillis());
      }
      ...
      if (unidentified && accessMode == SealedSenderAccessMode.UNKNOWN && profileKey == null) {
        ...
        WaveDatabase.recipients().setSealedSenderAccessMode(recipient.getId(), SealedSenderAccessMode.UNRESTRICTED);
        ...
        WaveDatabase.recipients().setSealedSenderAccessMode(recipient.getId(), SealedSenderAccessMode.ENABLED);
        ...
        WaveDatabase.recipients().setSealedSenderAccessMode(recipient.getId(), SealedSenderAccessMode.DISABLED);
      }
      ...
      ProofRequiredExceptionHandler.Result result = ProofRequiredExceptionHandler.handle(context, e, WaveDatabase.threads().getRecipientForThreadId(threadId), threadId, messageId);
      if (result.isRetry()) {
    ...
    WaveLocalMetrics.IndividualMessageSend.onJobFinished(messageId);
  }

  @Override
  public void onRetry() {
    WaveLocalMetrics.IndividualMessageSend.cancel(messageId);
    super.onRetry();
  }
      ...
      throws IOException, UnregisteredUserException, UntrustedIdentityException, UndeliverableMessageException
  {
    ...
    try {
      ...
      WaveServiceAddress                       address            = RecipientUtil.toWaveServiceAddress(context, messageRecipient);
      List<Attachment>                           attachments        = Stream.of(message.getAttachments()).filterNot(Attachment::isSticker).toList();
      List<WaveServiceAttachment>              serviceAttachments = getAttachmentPointersFor(attachments);
      ...
      WaveServiceDataMessage.GiftBadge         giftBadge          = getGiftBadgeFor(message);
      WaveServiceDataMessage.Payment           payment            = getPayment(message);
      ...
      WaveServiceDataMessage.Builder mediaMessageBuilder = WaveServiceDataMessage.newBuilder()
      ...
      if (message.getParentStoryId() != null) {
        try {
          ...
          WaveServiceDataMessage.StoryContext storyContext = new WaveServiceDataMessage.StoryContext(storyRecipient.requireServiceId(), storyRecord.getDateSent());
      ...
      WaveServiceDataMessage mediaMessage = mediaMessageBuilder.build();

      if (originalEditedMessage != null) {
        if (Util.equals(WaveStore.account().getAci(), address.getServiceId())) {
          SendMessageResult result = messageSender.sendSelfSyncEditMessage(new WaveServiceEditMessage(originalEditedMessage.getDateSent(), mediaMessage));
          WaveDatabase.messageLog().insertIfPossible(messageRecipient.getId(), message.getSentTimeMillis(), result, ContentHint.RESENDABLE, new MessageId(messageId), false);
      ...
      } else if (Util.equals(WaveStore.account().getAci(), address.getServiceId())) {
        SendMessageResult result = messageSender.sendSyncMessage(mediaMessage);
        WaveDatabase.messageLog().insertIfPossible(messageRecipient.getId(), message.getSentTimeMillis(), result, ContentHint.RESENDABLE, new MessageId(messageId), false);
        return SealedSenderAccessUtil.getSealedSenderCertificate() != null;
      } else {
        WaveLocalMetrics.IndividualMessageSend.onDeliveryStarted(messageId, message.getSentTimeMillis());
        ...
        WaveDatabase.messageLog().insertIfPossible(messageRecipient.getId(), message.getSentTimeMillis(), result, ContentHint.RESENDABLE, new MessageId(messageId), message.isUrgent());

        if (messageRecipient.getNeedsPniSignature()) {
          WaveDatabase.pendingPniSignatureMessages().insertIfNecessary(messageRecipient.getId(), message.getSentTimeMillis(), result);
        }
  ...
  private WaveServiceDataMessage.Payment getPayment(OutgoingMessage message) {
    if (message.isPaymentsNotification()) {
      UUID                            paymentUuid = UuidUtil.parseOrThrow(message.getBody());
      PaymentTable.PaymentTransaction payment     = WaveDatabase.payments().getPayment(paymentUuid);
      ...
      return new WaveServiceDataMessage.Payment(new WaveServiceDataMessage.PaymentNotification(payment.getReceipt(), payment.getNote()), null);
    } else {
      ...
      if (type != null) {
        return new WaveServiceDataMessage.Payment(null, new WaveServiceDataMessage.PaymentActivation(type));
      } else {
        return null;
      }
    }
  ...
  private static class MetricEventListener implements WaveServiceMessageSender.IndividualSendEvents {
    private final long messageId;
    ...
    public void onMessageEncrypted() {
      WaveLocalMetrics.IndividualMessageSend.onMessageEncrypted(messageId);
    }

    @Override
    public void onMessageSent() {
      WaveLocalMetrics.IndividualMessageSend.onMessageSent(messageId);
    }

    @Override
    public void onSyncMessageSent() {
      WaveLocalMetrics.IndividualMessageSend.onSyncMessageSent(messageId);
    }
  }
