...
class RetrieveRemoteAnnouncementsJob private constructor(private val force: Boolean, parameters: Parameters) : BaseJob(parameters) {
  ...
  companion object {
    ...
    fun enqueue(force: Boolean = false) {
      ...
      if (!force && System.currentTimeMillis() < WaveStore.releaseChannel.nextScheduledCheck) {
  ...
  override fun onRun() {
    ...
    val values = WaveStore.releaseChannel
  ...
  private fun updateReleaseNotes(announcements: List<ReleaseNote>) {
    ...
    if (!values.hasMetConversationRequirement) {
      if ((WaveDatabase.threads.getArchivedConversationListCount(ConversationFilter.OFF) + WaveDatabase.threads.getUnarchivedConversationListCount(ConversationFilter.OFF)) < 6) {
    ...
    val threadId = WaveDatabase.threads.getOrCreateThreadIdFor(Recipient.resolved(values.releaseChannelRecipientId!!))
    ...
    resolvedNotes
      ...
      .forEach { note ->
        ...
        if (insertResult != null) {
          ...
          WaveDatabase.attachments.getAttachmentsForMessage(insertResult.messageId)
    ...
    if (addedNewNotes) {
      ThreadUtil.sleep(5)
      WaveDatabase.messages.insertBoostRequestMessage(values.releaseChannelRecipientId!!, threadId)
    }
  ...
  private fun updateMegaphones(megaphones: List<RemoteMegaphone>) {
    ...
    val existingMegaphones: Map<String, RemoteMegaphoneRecord> = WaveDatabase.remoteMegaphones.getAll().associateBy { it.uuid }
    ...
    resolvedMegaphones
      ...
      .forEach { megaphone ->
        ...
        if (existingMegaphones.contains(uuid)) {
          WaveDatabase.remoteMegaphones.update(
            uuid = uuid,
          ...
          WaveDatabase.remoteMegaphones.insert(record)
    ...
    if (megaphonesToDelete.isNotEmpty()) {
      ...
      for ((uuid, megaphone) in megaphonesToDelete) {
        WaveDatabase.remoteMegaphones.clear(uuid)
  ...
  private fun String.getLocaleUrls(): List<String> {
    ...
    if (WaveStore.settings.language != "zz") {
      potentialNoteUrls += "$this/${WaveStore.settings.language}.json"
    }
