...
class LeaveGroupV2WorkerJob(parameters: Parameters, private val groupId: GroupId.V2) : BaseJob(parameters) {
  ...
  override fun onRun() {
    ...
    val threadId = WaveDatabase.threads.getThreadIdIfExistsFor(groupRecipient.id)
    if (threadId != -1L) {
      WaveDatabase.recipients.setProfileSharing(groupRecipient.id, enabled = false)
      WaveDatabase.threads.setEntireThreadRead(threadId)
