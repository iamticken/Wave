  ...
  override fun run(): Result {
    ...
    try {
      ...
      val archiveFileSystem = if (BackupUtil.isUserSelectionRequired(context)) {
        val backupDirectoryUri = WaveStore.settings.waveBackupDirectory
      ...
      try {
        try {
          val result = LocalArchiver.export(snapshotFileSystem, archiveFileSystem.filesFileSystem, stopwatch, cancellationWave = { isCanceled })
          Log.i(TAG, "Archive finished with result: $result")
          if (result !is org.wave.core.util.Result.Success) {
            return Result.failure()
          }
        } catch (e: Exception) {
