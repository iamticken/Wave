...
public final class PushDistributionListSendJob extends PushSendJob {
                             ...
                             @NonNull Set<RecipientId> filterRecipientIds)
  {
    try {
      ...
      if (!message.getStoryType().isTextStory()) {
        ...
        WaveDatabase.attachments().updateAttachmentCaption(storyAttachment.attachmentId, message.getBody());
      }
      ...
      throws IOException, UntrustedIdentityException, UndeliverableMessageException
  {
    try {
      ...
      if (message.getStoryType().isTextStory()) {
        storyMessage = WaveServiceStoryMessage.forTextAttachment(Recipient.self().getProfileKey(), null, StorySendUtil.deserializeBodyToStoryTextAttachment(message, this::getPreviewsFor), message.getStoryType().isStoryWithReplies(), bodyRanges);
      } else if (!attachmentPointers.isEmpty()) {
        storyMessage = WaveServiceStoryMessage.forFileAttachment(Recipient.self().getProfileKey(), null, attachmentPointers.get(0), message.getStoryType().isStoryWithReplies(), bodyRanges);
      } else {
        throw new UndeliverableMessageException("No attachment on non-text story.");
      }

      SentStorySyncManifest                   manifest           = WaveDatabase.storySends().getFullSentStorySyncManifest(messageId, message.getSentTimeMillis());
      Set<WaveServiceStoryMessageRecipient> manifestCollection = manifest != null ? manifest.toRecipientsSet() : Collections.emptySet();
