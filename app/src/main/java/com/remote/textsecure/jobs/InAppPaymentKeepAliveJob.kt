...
class InAppPaymentKeepAliveJob private constructor(
  ...
  companion object {
    ...
    fun enqueueAndTrackTimeIfNecessary() {
      ...
      val lastKeepAliveTime = WaveStore.inAppPayments.getLastKeepAliveLaunchTime().milliseconds
    ...
    fun enqueueAndTrackTime(now: Duration) {
      ...
      WaveStore.inAppPayments.setLastKeepAliveLaunchTime(now.inWholeMilliseconds)
    }
  }
  ...
  private fun doRun() {
    ...
    if (WaveDatabase.inAppPayments.hasPrePendingRecurringTransaction(type.inAppPaymentType)) {
    ...
    when (activeInAppPayment.data.redemption?.stage) {
      InAppPaymentData.RedemptionState.Stage.INIT -> {
        ...
        WaveDatabase.inAppPayments.update(payment)
  ...
  private fun clearOldPendingPaymentsIfRequired() {
    ...
    val inAppPayments = WaveDatabase.inAppPayments.getOldPendingPayments(type.inAppPaymentType)
  ...
  private fun getActiveInAppPayment(
    ...
    val current: InAppPaymentTable.InAppPayment? = WaveDatabase.inAppPayments.getByEndOfPeriod(type.inAppPaymentType, endOfCurrentPeriod)

    val inAppPayment = if (current == null) {
      val oldInAppPayment = WaveDatabase.inAppPayments.getByLatestEndOfPeriod(type.inAppPaymentType)
      ...
      if (type == InAppPaymentSubscriberRecord.Type.DONATION) {
        WaveStore.inAppPayments.setLastEndOfPeriod(endOfCurrentPeriod.inWholeSeconds)
      }
      ...
      WaveDatabase.inAppPaymentSubscribers.setPaymentMethod(subscriber.subscriberId, paymentMethodType = newInAppPaymentMethodType)

      val inAppPaymentId = WaveDatabase.inAppPayments.insert(
        ...
        inAppPaymentData = InAppPaymentData(
          ...
          badge = badge,
          amount = FiatMoney.fromWaveNetworkAmount(subscription.amount, Currency.getInstance(subscription.currency)).toFiatValue(),
          error = null,
      ...
      WaveDatabase.inAppPayments.getById(current.id)
    ...
    if (
      ...
      return WaveDatabase.inAppPayments.getById(inAppPayment.id)
    } else {
      return inAppPayment
    }
  }
