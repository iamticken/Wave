...
import org.whispersystems.waveservice.api.messages.multidevice.RequestMessage
...
import org.whispersystems.waveservice.api.storage.StorageServiceRepository.ManifestIfDifferentVersionResult
...
class StorageSyncJob private constructor(parameters: Parameters, private var localManifestOutOfDate: Boolean) : BaseJob(parameters) {
  ...
  override fun onRun() {
    if (!(WaveStore.svr.hasPin() || WaveStore.account.restoredAccountEntropyPool || WaveStore.account.restoredAccountEntropyPoolFromPrimary) && !WaveStore.svr.hasOptedOut()) {
    ...
    if (WaveStore.internal.storageServiceDisabled) {
      Log.w(TAG, "Storage service has been manually disabled. Skipping.")
      return
    }

    if (WaveStore.account.isLinkedDevice && !WaveStore.account.restoredAccountEntropyPoolFromPrimary) {
      Log.w(TAG, "Have not restored AEP from primary, skipping.")
      return
    }

    val (storageServiceKey, usingTempKey) = WaveStore.storageService.storageKeyForInitialDataRestore?.let {
      Log.i(TAG, "Using temporary storage key.")
      it to true
    } ?: run {
      WaveStore.storageService.storageKey to false
    }
    ...
    try {
      ...
      if (WaveStore.account.isMultiDevice && needsMultiDeviceSync) {
        AppDependencies.jobManager.add(MultiDeviceStorageSyncRequestJob())
      }

      WaveStore.storageService.lastSyncTime = System.currentTimeMillis()
      ...
      if (WaveStore.account.isPrimaryDevice) {
        ...
        AppDependencies.waveServiceMessageSender.sendSyncMessage(WaveServiceSyncMessage.forRequest(RequestMessage.forType(SyncMessage.Request.Type.KEYS)))
      }
    }
  }
  ...
  private fun performSync(storageServiceKey: StorageKey): Boolean {
    ...
    val db = WaveDatabase.rawDatabase
    val repository = StorageServiceRepository(WaveNetwork.storageService)

    val localManifest = WaveStore.storageService.manifest
    ...
    if (self.storageId == null) {
      ...
      WaveDatabase.recipients.updateStorageId(self.id, StorageSyncHelper.generateKey())
      self = freshSelf()
    }
    ...
    if (remoteManifest.version > localManifest.version) {
      ...
      if (idDifference.hasTypeMismatches && WaveStore.account.isPrimaryDevice) {
      ...
      if (idDifference.localOnlyIds.isNotEmpty()) {
        val updatedRecipients = WaveDatabase.recipients.removeStorageIdsFromLocalOnlyUnregisteredRecipients(idDifference.localOnlyIds)
        val updatedFolders = WaveDatabase.chatFolders.removeStorageIdsFromLocalOnlyDeletedFolders(idDifference.localOnlyIds)
        val updatedProfiles = WaveDatabase.notificationProfiles.removeStorageIdsFromLocalOnlyDeletedProfiles(idDifference.localOnlyIds)
      ...
      if (!idDifference.isEmpty) {
        ...
        try {
          ...
          val unknownInserts: List<WaveStorageRecord> = remoteOnly.unknown
          ...
          WaveDatabase.unknownStorageIds.insert(unknownInserts)
          WaveDatabase.unknownStorageIds.delete(unknownDeletes)
    ...
    if (remoteManifest !== localManifest) {
      ...
      WaveStore.storageService.manifest = remoteManifest
    }
    ...
    val remoteWriteOperation: WriteOperationResult = db.withinTransaction {
      ...
      val removedUnregistered = WaveDatabase.recipients.removeStorageIdsFromOldUnregisteredRecipients(System.currentTimeMillis())
      val removedDeletedFolders = WaveDatabase.chatFolders.removeStorageIdsFromOldDeletedFolders(System.currentTimeMillis())
      val removedDeletedProfiles = WaveDatabase.notificationProfiles.removeStorageIdsFromOldDeletedProfiles(System.currentTimeMillis())
      ...
      WriteOperationResult(
        manifest = WaveStorageManifest(
          version = remoteManifest.version + 1,
          sourceDeviceId = WaveStore.account.deviceId,
    ...
    if (!remoteWriteOperation.isEmpty) {
      ...
      WaveStore.storageService.manifest = remoteWriteOperation.manifest
    ...
    val knownUnknownIds = WaveDatabase.unknownStorageIds.getAllWithTypes(knownTypes)
    ...
    if (knownUnknownIds.isNotEmpty()) {
      ...
      db.withinTransaction {
        ...
        WaveDatabase.unknownStorageIds.deleteAllWithTypes(knownTypes)
      }
    ...
    if (needsForcePush && WaveStore.account.isPrimaryDevice) {
  ...
  private fun getAllLocalStorageIds(self: Recipient): List<StorageId> {
    return WaveDatabase.recipients.getContactStorageSyncIds() +
      listOf(StorageId.forAccount(self.storageId)) +
      WaveDatabase.chatFolders.getStorageSyncIds() +
      WaveDatabase.notificationProfiles.getStorageSyncIds() +
      WaveDatabase.unknownStorageIds.allUnknownIds
  }

  private fun buildLocalStorageRecords(context: Context, self: Recipient, ids: Collection<StorageId>): List<WaveStorageRecord> {
    if (ids.isEmpty()) {
      return emptyList()
    }

    val records: MutableList<WaveStorageRecord> = ArrayList(ids.size)
    ...
    for (id in ids) {
      ...
      when (type) {
        ManifestRecord.Identifier.Type.CONTACT, ManifestRecord.Identifier.Type.GROUPV1, ManifestRecord.Identifier.Type.GROUPV2 -> {
          val settings = WaveDatabase.recipients.getByStorageId(id.raw)
          if (settings != null) {
        ...
        ManifestRecord.Identifier.Type.STORY_DISTRIBUTION_LIST -> {
          val record = WaveDatabase.recipients.getByStorageId(id.raw)
          if (record != null) {
        ...
        ManifestRecord.Identifier.Type.CALL_LINK -> {
          val callLinkRecord = WaveDatabase.recipients.getByStorageId(id.raw)
        ...
        ManifestRecord.Identifier.Type.CHAT_FOLDER -> {
          ...
          val chatFolderRecord = WaveDatabase.chatFolders.getChatFolder(query)
        ...
        ManifestRecord.Identifier.Type.NOTIFICATION_PROFILE -> {
          ...
          val unknown = WaveDatabase.unknownStorageIds.getById(id.raw)
          if (unknown != null) {
            records.add(unknown)
          } else {
  ...
  private class StorageRecordCollection(records: Collection<WaveStorageRecord>) {
    val contacts: MutableList<WaveContactRecord> = mutableListOf()
    val gv1: MutableList<WaveGroupV1Record> = mutableListOf()
    val gv2: MutableList<WaveGroupV2Record> = mutableListOf()
    val account: MutableList<WaveAccountRecord> = mutableListOf()
    val unknown: MutableList<WaveStorageRecord> = mutableListOf()
    val storyDistributionLists: MutableList<WaveStoryDistributionListRecord> = mutableListOf()
    val callLinkRecords: MutableList<WaveCallLinkRecord> = mutableListOf()
    val chatFolderRecords: MutableList<WaveChatFolderRecord> = mutableListOf()
    val notificationProfileRecords: MutableList<WaveNotificationProfileRecord> = mutableListOf()

    init {
      for (record in records) {
        if (record.proto.contact != null) {
          contacts += record.proto.contact!!.toWaveContactRecord(record.id)
        } else if (record.proto.groupV1 != null) {
          gv1 += record.proto.groupV1!!.toWaveGroupV1Record(record.id)
        } else if (record.proto.groupV2 != null) {
          gv2 += record.proto.groupV2!!.toWaveGroupV2Record(record.id)
        } else if (record.proto.account != null) {
          account += record.proto.account!!.toWaveAccountRecord(record.id)
        } else if (record.proto.storyDistributionList != null) {
          storyDistributionLists += record.proto.storyDistributionList!!.toWaveStoryDistributionListRecord(record.id)
        } else if (record.proto.callLink != null) {
          callLinkRecords += record.proto.callLink!!.toWaveCallLinkRecord(record.id)
        } else if (record.proto.chatFolder != null) {
          chatFolderRecords += record.proto.chatFolder!!.toWaveChatFolderRecord(record.id)
        } else if (record.proto.notificationProfile != null) {
          notificationProfileRecords += record.proto.notificationProfile!!.toWaveNotificationProfileRecord(record.id)
