...
class AttachmentDownloadJob private constructor(
    ...
    fun downloadAttachmentIfNeeded(databaseAttachment: DatabaseAttachment): String? {
      return when (val transferState = databaseAttachment.transferState) {
        ...
        AttachmentTable.TRANSFER_PROGRESS_FAILED -> {
          if (WaveStore.backup.backsUpMedia && (databaseAttachment.remoteLocation == null || databaseAttachment.remoteDigest == null)) {
  ...
  override fun onAdded() {
    ...
    if (pending && (manual || AttachmentUtil.isAutoDownloadPermitted(context, attachment))) {
      ...
      WaveDatabase.attachments.setTransferState(messageId, attachmentId, AttachmentTable.TRANSFER_PROGRESS_STARTED)
    }
  }
  ...
  fun doWork() {
    ...
    if (!manual && !AttachmentUtil.isAutoDownloadPermitted(context, attachment)) {
      ...
      WaveDatabase.attachments.setTransferState(messageId, attachmentId, AttachmentTable.TRANSFER_PROGRESS_PENDING)
      return
    }

    if (WaveStore.backup.backsUpMedia && attachment.remoteLocation == null) {
    ...
    if (WaveStore.backup.backsUpMedia) {
      val isStory = WaveDatabase.messages.isStory(messageId)
      when {
  ...
  private fun retrieveAttachment(
    ...
    val attachmentFile: File = WaveDatabase.attachments.getOrCreateTransferFile(attachmentId)
    ...
    try {
      ...
      decryptingStream.use { input ->
        WaveDatabase.attachments.finalizeAttachmentAfterDownload(messageId, attachmentId, input)
      }
      ...
      if (WaveStore.backup.backsUpMedia && e.code == 404 && attachment.archiveTransferState === AttachmentTable.ArchiveTransferState.FINISHED) {
  ...
  private fun createAttachmentPointer(attachment: DatabaseAttachment): WaveServiceAttachmentPointer {
    ...
    return try {
      val remoteId = WaveServiceAttachmentRemoteId.from(attachment.remoteLocation)
  ...
  private fun markFailed(messageId: Long, attachmentId: AttachmentId) {
    WaveDatabase.attachments.setTransferProgressFailed(attachmentId, messageId)
  }

  private fun markPermanentlyFailed(messageId: Long, attachmentId: AttachmentId) {
    WaveDatabase.attachments.setTransferProgressPermanentFailure(attachmentId, messageId)
  }
