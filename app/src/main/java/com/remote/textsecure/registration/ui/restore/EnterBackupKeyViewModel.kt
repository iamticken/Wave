...
class EnterBackupKeyViewModel : ViewModel() {
  ...
  fun handleRegistrationFailure(registerAccountResult: RegisterAccountResult) {
    store.update {
      if (it.isRegistering) {
        ...
        if (incorrectKeyError && WaveStore.account.restoredAccountEntropyPool) {
          WaveStore.account.resetAccountEntropyPool()
          WaveStore.account.resetAciAndPniIdentityKeysAfterFailedRestore()
        }
  ...
  fun handleBackupTimestampNotRestored() {
    store.update {
      it.copy(
        showBackupTierNotRestoreError = if (WaveStore.backup.isBackupTimestampRestored) TierRestoreError.NOT_FOUND else TierRestoreError.NETWORK_ERROR
      )
    }
  }
