 ...
 * conjunction with {@link org.wave.devicetransfer.DeviceToDeviceTransferService}.
 */
final class NewDeviceServerTask implements ServerTask {
  ...
  public void run(@NonNull Context context, @NonNull InputStream inputStream) {
    ...
    try {
      ...
      SQLiteDatabase database = WaveDatabase.getBackupDatabase();

      String passphrase = WaveStore.account().getAccountEntropyPool().getValue();
      ...
      WaveDatabase.runPostBackupRestoreTasks(database);
      ...
      Log.w(TAG, "Failed due to the backup being from a newer version of Wave.", e);
