...
class CallEventCache(
  private val executor: Executor = WaveExecutors.newCachedSingleThreadExecutor("call-event-cache", ThreadUtil.PRIORITY_IMPORTANT_BACKGROUND_THREAD)
) {
  companion object {
    ...
    private fun readDataFromDatabase(): List<CacheRecord> {
      ...
      val events = WaveDatabase.calls.getCallsForCache(10_000).readToList { it.readCacheRecord() }
    ...
    private fun canUserBeginCall(peer: Recipient, decryptedGroup: ByteArray?): Boolean {
      return if (peer.isGroup && decryptedGroup != null) {
        ...
        return proto.isAnnouncementGroup != EnabledState.ENABLED ||
          proto.members.firstOrNull() { it.aciBytes == WaveStore.account.aci?.toByteString() }?.role == Member.Role.ADMINISTRATOR
      } else {
        true
      }
    }
