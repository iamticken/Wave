...
class CallLinkDetailsRepository(
  ...
  fun refreshCallLinkState(callLinkRoomId: CallLinkRoomId): Disposable {
    return MaybeCompat.fromCallable { WaveDatabase.callLinks.getCallLinkByRoomId(callLinkRoomId) }
      .flatMapSingle { callLinkManager.readCallLink(it.credentials!!) }
      .subscribeOn(Schedulers.io())
      .subscribeBy { result ->
        when (result) {
          is ReadCallLinkResult.Success -> WaveDatabase.callLinks.updateCallLinkState(callLinkRoomId, result.callLinkState)
  ...
  fun watchCallLinkRecipient(callLinkRoomId: CallLinkRoomId): Observable<Recipient> {
    return MaybeCompat.fromCallable { WaveDatabase.recipients.getByCallLinkRoomId(callLinkRoomId).orNull() }
