...
import org.thoughtcrime.securesms.calls.links.WaveCallRow
...
class DefaultCallLinkDetailsCallback(
  ...
  override fun onShareLinkViaWaveClicked() {
    activity.startActivity(
      ShareActivity.sendSimpleText(
        activity,
        activity.getString(R.string.CreateCallLink__use_this_link_to_join_a_wave_call, CallLinks.url(viewModel.rootKeySnapshot, viewModel.epochSnapshot))
      )
    )
  }
...
interface CallLinkDetailsCallback {
  ...
  fun onShareLinkViaWaveClicked() = Unit
...
fun CallLinkDetailsScreen(
  ...
  Scaffolds.Settings(
    ...
    LazyColumn(
      ...
      item {
        Rows.TextRow(
          text = stringResource(id = R.string.CreateCallLinkBottomSheetDialogFragment__share_link_via_wave),
          icon = ImageVector.vectorResource(id = R.drawable.symbol_forward_24),
          onClick = callback::onShareLinkViaWaveClicked
        )
      }
