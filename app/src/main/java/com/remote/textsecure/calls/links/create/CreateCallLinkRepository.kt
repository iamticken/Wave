...
/**
 * Repository for creating new call links. This will delegate to the [WaveCallLinkManager]
...
class CreateCallLinkRepository(
  ...
  fun ensureCallLinkCreated(credentials: CallLinkCredentials): Single<EnsureCallLinkCreatedResult> {
    val callLinkRecipientId = Single.fromCallable {
      WaveDatabase.recipients.getByCallLinkRoomId(credentials.roomId)
    }
          ...
          when (it) {
            is CreateCallLinkResult.Success -> {
              ...
              EnsureCallLinkCreatedResult.Success(
                Recipient.resolved(
                  WaveDatabase.recipients.getByCallLinkRoomId(it.credentials.roomId).get()
                )
              )
            }
