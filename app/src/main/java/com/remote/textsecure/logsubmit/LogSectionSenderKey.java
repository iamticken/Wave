...
import org.wave.core.util.AsciiArt;
...
public class LogSectionSenderKey implements LogSection {
  ...
  public @NonNull CharSequence getContent(@NonNull Context context) {
    ...
    if (WaveStore.account().getAci() != null){
      try (Cursor cursor = WaveDatabase.senderKeys().getAllCreatedBySelf()) {
    ...
    try (Cursor cursor = WaveDatabase.senderKeyShared().getAllSharedWithCursor()) {
      builder.append(AsciiArt.tableFor(cursor)).append("\n");
    }
