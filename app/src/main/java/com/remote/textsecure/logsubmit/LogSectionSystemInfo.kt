...
import org.wave.core.util.FontUtil.canRenderEmojiAtFontSize
...
class LogSectionSystemInfo : LogSection {
  ...
  override fun getContent(context: Context): CharSequence {
    return """
      ...
      OS Host           : ${Build.HOST}
      RecipientId       : ${if (WaveStore.registration.isRegistrationComplete) self().id else "N/A"}
      ACI               : ${getCensoredAci()}
      Device ID         : ${WaveStore.account.deviceId}
      Censored          : ${AppDependencies.waveServiceNetworkAccess.isCensored()}
      Network Status    : ${NetworkUtil.getNetworkStatus(context)}
      Play Services     : ${getPlayServicesString(context)}
      FCM               : ${WaveStore.account.fcmEnabled}
      Locale            : ${Locale.getDefault()}
      Linked Devices    : ${WaveStore.account.isMultiDevice}
      ...
      Server Time Offset: ${WaveStore.misc.lastKnownServerTimeOffset} ms (last updated: ${WaveStore.misc.lastKnownServerTimeOffsetUpdateTime})
  ...
  private fun getTimeRegistered(): String {
    if (WaveStore.account.registeredAtTimestamp <= 0) {
      return "Unknown"
    }

    val timeSince = (System.currentTimeMillis() - WaveStore.account.registeredAtTimestamp).milliseconds.toString()
    return "${WaveStore.account.registeredAtTimestamp} ($timeSince ago)"
  }
}
