...
public class BeginCallActionProcessorDelegate extends WebRtcActionProcessor {
                                                           ...
                                                           @NonNull OfferMessage.Type offerType)
  {
    if (!remotePeer.getRecipient().getHasAci()) {
      ...
      webRtcInteractor.sendCallMessage(remotePeer, WaveServiceCallMessage.forOffer(new OfferMessage(remotePeer.getCallId().longValue(),
                                                                                                      offerType,
                                                                                                      new byte[0]),
                                                                                     null));
    ...
    try {
      webRtcInteractor.getCallManager().call(remotePeer, callMediaType, WaveStore.account().getDeviceId());
