...
public final class WebRtcUtil {
  ...
  public static void enableSpeakerPhoneIfNeeded(@NonNull WebRtcInteractor webRtcInteractor, WebRtcServiceState currentState) {
    ...
    if (currentState.getLocalDeviceState().getActiveDevice() == WaveAudioManager.AudioDevice.EARPIECE ||
        currentState.getLocalDeviceState().getActiveDevice() == WaveAudioManager.AudioDevice.NONE &&
        currentState.getCallInfoState().getActivePeer() != null)
    {
      webRtcInteractor.setDefaultAudioDevice(currentState.getCallInfoState().requireActivePeer().getId(), WaveAudioManager.AudioDevice.SPEAKER_PHONE, true);
    }
  }
