...
public class IdleActionProcessor extends WebRtcActionProcessor {
                                                                  ...
                                                                  @NonNull CallManager.RingUpdate ringUpdate)
  {
    ...
    if (ringUpdate != CallManager.RingUpdate.REQUESTED) {
      WaveDatabase.calls().insertOrUpdateGroupCallFromRingState(ringId, remotePeerGroup.getId(), sender, System.currentTimeMillis(), ringUpdate);
      return currentState;
    } else if (WaveDatabase.calls().isRingCancelled(ringId, remotePeerGroup.getId())) {
      try {
    ...
    if (activeProfile != null && !(activeProfile.isRecipientAllowed(remotePeerGroup.getId()) || activeProfile.getAllowAllCalls())) {
      try {
        ...
        WaveDatabase.calls().insertOrUpdateGroupCallFromRingState(ringId, remotePeerGroup.getId(), sender, System.currentTimeMillis(), CallManager.RingUpdate.EXPIRED_REQUEST, true);
  ...
  protected @NonNull WebRtcServiceState handleReceivedGroupCallPeekForRingingCheck(@NonNull WebRtcServiceState currentState, @NonNull GroupCallRingCheckInfo info, @NonNull PeekInfo peekInfo) {
    ...
    if (WaveDatabase.calls().isRingCancelled(info.getRingId(), info.getRecipientId())) {
      try {
    ...
    if (peekInfo.getDeviceCount() == 0) {
      ...
      WaveDatabase.calls().insertOrUpdateGroupCallFromRingState(info.getRingId(), info.getRecipientId(), info.getRingerAci(), System.currentTimeMillis(), CallManager.RingUpdate.EXPIRED_REQUEST);
      return currentState;
      ...
      WaveDatabase.calls().insertOrUpdateGroupCallFromRingState(info.getRingId(), info.getRecipientId(), info.getRingerAci(), System.currentTimeMillis(), CallManager.RingUpdate.ACCEPTED_ON_ANOTHER_DEVICE);
      return currentState;
    }
