...
import org.thoughtcrime.securesms.webrtc.audio.WaveAudioManager.Companion.create
import org.thoughtcrime.securesms.webrtc.locks.LockManager
import java.util.concurrent.locks.ReentrantLock
import kotlin.concurrent.withLock

/**
 * Entry point for [WaveCallManager] and friends to interact with the Android system.
 *
...
class ActiveCallManager(
...
) : WaveAudioManager.EventListener {
  ...
  private val callManager = AppDependencies.waveCallManager
  ...
  private var waveAudioManager: WaveAudioManager? = null
  ...
  fun shutdown(fromTimeout: Boolean = false) {
    ...
    waveAudioManager?.shutdown()
    waveAudioManager = null
  ...
  fun sendAudioCommand(audioCommand: AudioManagerCommand) {
    if (waveAudioManager == null) {
      waveAudioManager = create(application, this)
    }

    Log.i(TAG, "Sending audio command [" + audioCommand.javaClass.simpleName + "] to " + waveAudioManager?.javaClass?.simpleName)
    waveAudioManager!!.handleCommand(audioCommand)
  }
  ...
  override fun onAudioDeviceChanged(activeDevice: WaveAudioManager.AudioDevice, devices: Set<WaveAudioManager.AudioDevice>) {
    ...
    private class HangUpRtcOnPstnCallAnsweredListener : PhoneStateListener() {
      ...
      override fun onCallStateChanged(state: Int, phoneNumber: String) {
        ...
        if (state == TelephonyManager.CALL_STATE_OFFHOOK) {
          hangup()
          Log.i(TAG, "Device phone call ended Wave call.")
        }
      }
  ...
  class ActiveCallServiceReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context?, intent: Intent?) {
      ...
      when (intent?.action) {
        ACTION_DENY -> AppDependencies.waveCallManager.denyCall()
        ACTION_HANGUP -> AppDependencies.waveCallManager.localHangup()
      }
    }
  }
  ...
  private class NetworkReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
      ...
      AppDependencies.waveCallManager.apply {
  ...
  private class PowerButtonReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
      if (Intent.ACTION_SCREEN_OFF == intent.action) {
        AppDependencies.waveCallManager.screenOff()
      }
    }
  }
