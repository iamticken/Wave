...
/**
 * Wave implementation for the telecom system connection. Provides an interaction point for the system to
...
class AndroidCallConnection(
  ...
  private var initialAudioRoute: WaveAudioManager.AudioDevice? = null
  ...
  override fun onCallAudioStateChanged(state: CallAudioState) {
    ...
    val activeDevice = state.route.toDevices().firstOrNull() ?: WaveAudioManager.AudioDevice.EARPIECE
    val availableDevices = state.supportedRouteMask.toDevices()

    AppDependencies.waveCallManager.onAudioDeviceChanged(activeDevice, availableDevices)

    if (needToResetAudioRoute) {
      if (initialAudioRoute == null) {
        initialAudioRoute = activeDevice
      } else if (activeDevice == WaveAudioManager.AudioDevice.SPEAKER_PHONE) {
...
private fun Int.toDevices(): Set<WaveAudioManager.AudioDevice> {
  val devices = mutableSetOf<WaveAudioManager.AudioDevice>()

  if (this and CallAudioState.ROUTE_BLUETOOTH != 0) {
    devices += WaveAudioManager.AudioDevice.BLUETOOTH
  }

  if (this and CallAudioState.ROUTE_EARPIECE != 0) {
    devices += WaveAudioManager.AudioDevice.EARPIECE
  }

  if (this and CallAudioState.ROUTE_WIRED_HEADSET != 0) {
    devices += WaveAudioManager.AudioDevice.WIRED_HEADSET
  }

  if (this and CallAudioState.ROUTE_SPEAKER != 0) {
    devices += WaveAudioManager.AudioDevice.SPEAKER_PHONE
  }
