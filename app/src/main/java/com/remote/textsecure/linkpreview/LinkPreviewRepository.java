...
import org.whispersystems.waveservice.api.messages.WaveServiceStickerManifest.StickerInfo;
...
public class LinkPreviewRepository {
  ...
  {
    if (!WaveStore.settings().isLinkPreviewsEnabled()) {
                                                               ...
                                                               @NonNull Callback callback)
  {
    WaveExecutors.UNBOUNDED.execute(() -> {
      try {
        ...
        WaveServiceStickerManifest manifest = receiver.retrieveStickerManifest(packIdBytes, packKeyBytes);
  ...
  private static RequestController fetchCallLinkPreview(@NonNull Context context,
    ...
    Disposable disposable = AppDependencies.getWaveCallManager()
                                           .getCallLinkManager()
                                                         ...
                                                         @NonNull Callback callback)
  {
    WaveExecutors.UNBOUNDED.execute(() -> {
      try {
        ...
        Optional<GroupRecord> group   = WaveDatabase.groups().getGroup(groupId);
