...
/**
 * Handles 'invite to wave' actions.
 */
object InviteActions {
  /**
   * Called to send a message to a user to invite them to Wave.
   * The invite can be sent in one of three ways:
   *
   * 1. If Wave is the user's default SMS app, we can simply append the message to the composer.
   ...
   */
  @MainThread
  fun inviteUserToWave(
    context: Context,
    launchIntent: (Intent) -> Unit
  ) {
    val inviteText = context.getString(
      R.string.ConversationActivity_lets_switch_to_wave,
    ...
    if (intent.resolveActivity(context.packageManager) != null) {
      launchIntent(Intent.createChooser(intent, context.getString(R.string.InviteActivity_invite_to_wave)))
    } else {
