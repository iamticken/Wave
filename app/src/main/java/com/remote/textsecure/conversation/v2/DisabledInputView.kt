...
class DisabledInputView @JvmOverloads constructor(
  ...
  private var inviteToWave: View? = null
  ...
  fun showAsExpiredOrUnauthorized(clientExpired: Boolean, unauthorized: Boolean) {
    expiredOrUnauthorized = show(
      ...
      bind = {
        ...
        message.setText(if (clientExpired) R.string.ExpiredBuildReminder_this_version_of_wave_has_expired else R.string.UnauthorizedReminder_this_is_likely_because_you_registered_your_phone_number_with_Wave_on_a_different_device)
        actionButton.setText(if (clientExpired) R.string.ConversationFragment__update_build else R.string.ConversationFragment__reregister_wave)
  ...
  fun showAsInviteToWave(context: Context, recipient: Recipient, threadContainsSms: Boolean) {
    inviteToWave = show(
      existingView = inviteToWave,
      ...
      bind = {
        findViewById<TextView>(R.id.export_sms_message).text = if (recipient.isMmsGroup) {
          ...
          context.getString(R.string.ConversationActivity__sms_messaging_is_no_longer_supported_in_wave_invite_s_to_to_wave_to_keep_the_conversation_here, recipient.getDisplayName(context))
        } else {
          context.getString(R.string.ConversationActivity__this_person_is_no_longer_using_wave)
        }

        findViewById<MaterialButton>(R.id.export_sms_button).apply {
          setText(R.string.ConversationActivity__invite_to_wave)
          setOnClickListener { listener?.onInviteToWave(recipient) }
  ...
  fun setWallpaperEnabled(wallpaperEnabled: Boolean) {
    color = ContextCompat.getColor(context, if (wallpaperEnabled) R.color.wallpaper_bubble_color else R.color.wave_colorBackground)
  ...
  interface Listener {
    ...
    fun onInviteToWave(recipient: Recipient)
