...
class ConversationAdapterV2(
  ...
  init {
    ...
    if (WaveStore.internal.useConversationItemV2Media) {
  ...
  inner class ThreadHeaderViewHolder(itemView: View) : MappingViewHolder<ThreadHeader>(itemView) {
    ...
    override fun bind(model: ThreadHeader) {
      ...
      if (recipient.isGroup) {
        ...
        val subtitle: String? = recipient.takeIf { it.shouldShowE164 }?.e164?.map { e164: String? -> WaveE164Util.prettyPrint(e164!!) }?.orElse(null)
