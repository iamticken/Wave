               ...
               ConversationAdapterBridge
{
  ...
  public void onBindHeaderViewHolder(StickyHeaderViewHolder viewHolder, int position, int type) {
    ...
    if (hasWallpaper && ThemeUtil.isDarkTheme(context)) {
      viewHolder.setTextColor(ContextCompat.getColor(context, R.color.wave_colorNeutralInverse));
    } else {
      viewHolder.setTextColor(ContextCompat.getColor(context, R.color.wave_colorOnSurfaceVariant));
    }
  }
