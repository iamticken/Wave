...
class MessageQuotesRepository {
  ...
  fun getMessagesInQuoteChain(application: Application, messageId: MessageId): Observable<List<ConversationMessage>> {
    return Observable.create { emitter ->
      val threadId: Long = WaveDatabase.messages.getThreadIdForMessage(messageId.id)
      if (threadId < 0) {
  ...
  private fun getMessagesInQuoteChainSync(application: Application, messageId: MessageId): List<ConversationMessage> {
    val rootMessageId: MessageId = WaveDatabase.messages.getRootOfQuoteChain(messageId)

    var originalRecord: MessageRecord? = WaveDatabase.messages.getMessageRecordOrNull(rootMessageId.id)

    if (originalRecord == null) {
      return emptyList()
    }

    var replyRecords: List<MessageRecord> = WaveDatabase.messages.getAllMessagesThatQuote(rootMessageId)

    val reactionHelper = ReactionHelper()
    val attachmentHelper = AttachmentHelper()
    val threadRecipient = requireNotNull(WaveDatabase.threads.getRecipientForThreadId(originalRecord.threadId))
    ...
    if (originalRecord.isPaymentNotification) {
      originalRecord = WaveDatabase.payments.updateMessageWithPayment(originalRecord)
    }
