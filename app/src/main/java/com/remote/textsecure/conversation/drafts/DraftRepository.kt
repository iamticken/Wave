...
class DraftRepository(
  ...
  private val threadTable: ThreadTable = WaveDatabase.threads,
  private val draftTable: DraftTable = WaveDatabase.drafts,
  private val saveDraftsExecutor: Executor = SerialMonoLifoExecutor(WaveExecutors.BOUNDED),
  ...
  private fun getShareOrDraftDataInternal(lastShareDataTimestamp: Long): Pair<ShareOrDraftData?, Drafts?>? {
    ...
    if (conversationArguments?.canInitializeFromDatabase() == true) {
      ...
      val location: WavePlace? = drafts.firstOrNull { it.type == DraftTable.Draft.LOCATION }?.let { WavePlace.deserialize(it.value) }
      if (location != null) {
  ...
  private fun loadDraftQuoteInternal(serialized: String): ConversationMessage? {
    ...
    val messageRecord: MessageRecord = WaveDatabase.messages.getMessageFor(quoteId.id, quoteId.author)?.let {
      if (it is MmsMessageRecord) {
        it.withAttachments(WaveDatabase.attachments.getAttachmentsForMessage(it.id))
      } else {
        it
      }
    } ?: return null

    val threadRecipient = requireNotNull(WaveDatabase.threads.getRecipientForThreadId(messageRecord.threadId))
  ...
  private fun loadDraftMessageEditInternal(serialized: String): ConversationMessage? {
    ...
    val messageRecord: MessageRecord = WaveDatabase.messages.getMessageRecordOrNull(messageId.id) ?: return null
    val threadRecipient: Recipient = requireNotNull(WaveDatabase.threads.getRecipientForThreadId(messageRecord.threadId))
  ...
  sealed interface ShareOrDraftData {
    ...
    data class SetLocation(val location: WavePlace, val draftText: CharSequence?) : ShareOrDraftData
