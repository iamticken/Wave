  ...
  private inner class DateHeaderViewHolder(val itemView: View) {
    ...
    fun updateForWallpaper() {
      if (hasWallpaper) {
        ...
        date.setTextColor(ContextCompat.getColor(itemView.context, R.color.wave_colorNeutralInverse))
      } else {
        date.background = null
        date.setTextColor(ContextCompat.getColor(itemView.context, R.color.wave_colorOnSurfaceVariant))
      }
    }
  }
