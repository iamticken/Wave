...
import org.wave.core.util.concurrent.MaybeCompat
...
import org.thoughtcrime.securesms.keyvalue.WaveStore.Companion.settings
...
class ConversationRepository(
  ...
  fun getConversationThreadState(threadId: Long, requestedStartPosition: Int): Single<ConversationThreadState> {
    return Single.fromCallable {
      val recipient = WaveDatabase.threads.getRecipientForThreadId(threadId)!!

      WaveLocalMetrics.ConversationOpen.onMetadataLoadStarted()
      val metadata = oldConversationRepository.getConversationData(threadId, recipient, requestedStartPosition)
      WaveLocalMetrics.ConversationOpen.onMetadataLoaded()
  ...
  fun sendPoll(threadRecipient: Recipient, poll: Poll): Completable {
    return Completable.create { emitter ->
      ...
      val threadId = WaveDatabase.threads.getOrCreateThreadIdFor(threadRecipient)
  ...
  fun endPoll(pollId: Long): Completable {
    return Completable.create { emitter ->
      val poll = WaveDatabase.polls.getPollFromId(pollId)
      val messageRecord = WaveDatabase.messages.getMessageRecord(poll!!.messageId)
      val threadRecipient = WaveDatabase.threads.getRecipientForThreadId(messageRecord.threadId)!!
      ...
      val possibleTargets: List<Recipient> = WaveDatabase.groups.getGroupMembers(groupId, GroupTable.MemberSet.FULL_MEMBERS_EXCLUDING_SELF)
        .map { it.resolve() }
        .distinctBy { it.id }
      ...
      if (sendResults.completed.isNotEmpty() || possibleTargets.isEmpty()) {
        val allocatedThreadId = WaveDatabase.threads.getOrCreateValidThreadId(threadRecipient, messageRecord.threadId, message.distributionType)
        val outgoingMessage = applyUniversalExpireTimerIfNecessary(applicationContext, threadRecipient, message, allocatedThreadId)
        val insertResult = WaveDatabase.messages.insertMessageOutbox(outgoingMessage, allocatedThreadId, false, null)
        val messageId = insertResult.messageId

        WaveDatabase.threads.update(threadId = allocatedThreadId, unarchive = true, syncThreadDelete = true)
        ...
        if (sendResults.skipped.isNotEmpty()) {
          val messageRecord = WaveDatabase.messages.getMessageRecord(messageId)
  ...
  fun sendEndPoll(group: Recipient, message: OutgoingMessage, destinations: List<Recipient>): List<SendMessageResult?> {
    ...
    val groupRecord: GroupRecord? = WaveDatabase.groups.getGroup(group.requireGroupId()).getOrNull()
    ...
    val groupMessage = builder
      ...
      .withPollTerminate(WaveServiceDataMessage.PollTerminate(message.messageExtras!!.pollTerminate!!.targetTimestamp))
      .build()
  ...
  fun getPinnedMessages(threadId: Long): List<MmsMessageRecord> {
    return WaveDatabase.messages.getPinnedMessages(threadId = threadId, orderByPinned = true)
  }
  ...
  fun setLastVisibleMessageTimestamp(threadId: Long, lastVisibleMessageTimestamp: Long) {
    WaveExecutors.BOUNDED_IO.execute { WaveDatabase.threads.setLastScrolled(threadId, lastVisibleMessageTimestamp) }
  }
  ...
  fun getQuotedMessagePosition(threadId: Long, quoteId: Long, authorId: RecipientId): Single<Int> {
    return Single.fromCallable {
      WaveDatabase.messages.getQuotedMessagePosition(threadId, quoteId, authorId)
    }.subscribeOn(Schedulers.io())
  }

  fun getMessageResultPosition(threadId: Long, receivedTimestamp: Long): Single<Int> {
    return Single.fromCallable {
      WaveDatabase.messages.getMessagePositionInConversation(threadId, receivedTimestamp)
    }.subscribeOn(Schedulers.io())
  }

  fun getNextMentionPosition(threadId: Long): Single<Int> {
    return Single.fromCallable {
      val details = WaveDatabase.messages.getOldestUnreadMentionDetails(threadId)
      if (details == null) {
        -1
      } else {
        WaveDatabase.messages.getMessagePositionInConversation(threadId, details.second, details.first)
      }
    }.subscribeOn(Schedulers.io())
  }
  ...
  fun getMessagePosition(threadId: Long, messageId: Long): Single<Int> {
    return Single.fromCallable {
      val message = WaveDatabase.messages.getMessageRecord(messageId)
      WaveDatabase.messages.getMessagePositionInConversation(threadId, message.dateReceived, message.fromRecipient.id)
    }.subscribeOn(Schedulers.io())
  }

  fun getMessagePosition(threadId: Long, dateReceived: Long, authorId: RecipientId): Single<Int> {
    return Single.fromCallable {
      WaveDatabase.messages.getMessagePositionInConversation(threadId, dateReceived, authorId)
    }.subscribeOn(Schedulers.io())
  }
  ...
  private fun getUnreadCount(threadId: Long): Int {
    return WaveDatabase.messages.getUnreadCount(threadId)
  }

  private fun getUnreadMentionsCount(threadId: Long): Int {
    return WaveDatabase.messages.getUnreadMentionCount(threadId)
  }
  ...
  fun dismissRequestReviewState(threadRecipientId: RecipientId) {
    WaveExecutors.BOUNDED_IO.execute {
      WaveDatabase.nameCollisions.markCollisionsForThreadRecipientDismissed(threadRecipientId)
    }
  }
  ...
  fun getRequestReviewState(recipient: Recipient, group: GroupRecord?, messageRequest: MessageRequestState): Single<RequestReviewState> {
    return Single.fromCallable {
      ...
      if (group == null) {
        val recipientsToReview = WaveDatabase.nameCollisions.getCollisionsForThreadRecipientId(recipient.id)
      ...
      if (group != null && group.isV2Group) {
        ...
        val duplicateRecipients: List<ReviewRecipient> = WaveDatabase.nameCollisions.getCollisionsForThreadRecipientId(group.recipientId)
  ...
  fun setConversationMuted(recipientId: RecipientId, until: Long) {
    WaveExecutors.BOUNDED_IO.execute { recipients.setMuted(recipientId, until) }
  }
  ...
  fun updateStickerLastUsedTime(stickerRecord: StickerRecord, timestamp: Duration) {
    WaveExecutors.BOUNDED_IO.execute {
      WaveDatabase.stickers.updateStickerLastUsedTime(stickerRecord.rowId, timestamp.inWholeMilliseconds)
    }
  }

  fun startExpirationTimeout(expirationInfos: List<MessageTable.ExpirationInfo>) {
    WaveDatabase.messages.markExpireStarted(expirationInfos.map { it.id to it.expireStarted })
  ...
  fun getEarliestMessageSentDate(threadId: Long): Single<Long> {
    return Single
      .fromCallable { WaveDatabase.messages.getEarliestMessageSentDate(threadId) }
      .subscribeOn(Schedulers.io())
  }
