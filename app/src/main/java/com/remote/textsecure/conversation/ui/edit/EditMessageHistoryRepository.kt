...
object EditMessageHistoryRepository {
  ...
  fun getEditHistory(messageId: Long): Observable<List<ConversationMessage>> {
    return Observable.create { emitter ->
      val threadId: Long = WaveDatabase.messages.getThreadIdForMessage(messageId)
      if (threadId < 0) {
  ...
  fun markRevisionsRead(messageId: Long) {
    WaveExecutors.BOUNDED.execute {
      MarkReadReceiver.process(WaveDatabase.messages.setAllEditMessageRevisionsRead(messageId))
    }
  }

  private fun getEditHistorySync(messageId: Long): List<ConversationMessage> {
    val context = AppDependencies.application
    val records = WaveDatabase
      .messages
    ...
    val threadRecipient: Recipient = requireNotNull(WaveDatabase.threads.getRecipientForThreadId(records[0].threadId))
