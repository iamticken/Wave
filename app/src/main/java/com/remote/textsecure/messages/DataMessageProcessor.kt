...
import org.wave.libwave.zkgroup.receipts.ReceiptCredentialPresentation
...
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.isInvalid
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.isMediaMessage
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.isPaymentActivated
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.isPaymentActivationRequest
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.isStoryReaction
...
import org.whispersystems.waveservice.api.payments.Money
...
object DataMessageProcessor {
  ...
  fun process(
    ...
    localMetrics: WaveLocalMetrics.MessageReceive?
  ) {
    ...
    if (groupId != null) {
      val unknownGroup = when (groupProcessResult) {
        ...
        else -> WaveDatabase.groups.isUnknownGroup(groupId)
      }
      if (unknownGroup) {
    ...
    if (groupId == null && senderRecipient.hiddenState == HiddenState.HIDDEN) {
      WaveDatabase.recipients.markHidden(senderRecipient.id, clearProfileKey = false, showMessageRequest = true)
    }

    if (metadata.sealedSender && messageId != null) {
      WaveExecutors.BOUNDED.execute { AppDependencies.jobManager.add(SendDeliveryReceiptJob(senderRecipient.id, message.timestamp!!, messageId)) }
      ...
      if (RecipientUtil.shouldHaveProfileKey(threadRecipient)) {
        ...
        if (groupId != null) {
          ...
          val profileSendJob = ProfileKeySendJob.create(WaveDatabase.threads.getOrCreateThreadIdFor(threadRecipient), true)
    ...
    if (insertResult != null && insertResult.threadWasNewlyCreated && !threadRecipient.isGroup && !threadRecipient.isSelf && !senderRecipient.isSystemContact) {
      val timeSinceLastSync = System.currentTimeMillis() - WaveStore.misc.lastCdsForegroundSyncTime
      if (timeSinceLastSync > RemoteConfig.cdsForegroundSyncInterval || timeSinceLastSync < 0) {
        ...
        WaveStore.misc.lastCdsForegroundSyncTime = System.currentTimeMillis()
      } else {
  ...
  private fun handleProfileKey(
    ...
    if (senderRecipient.isSelf) {
      ...
      if (WaveDatabase.recipients.setProfileKey(senderRecipient.id, messageProfileKey)) {
  ...
  fun handleUnknownGroupMessage(timestamp: Long, groupContextV2: GroupContextV2) {
    ...
    WaveDatabase.groups.fixMissingMasterKey(groupContextV2.groupMasterKey)
  }
  ...
  private fun handleInvalidMessage(
    ...
    if (insertResult != null) {
      WaveDatabase.messages.markAsInvalidMessage(insertResult.messageId)
  ...
  private fun handleEndSessionMessage(
    ...
    val insertResult: InsertResult? = WaveDatabase.messages.insertMessageInbox(incomingMessage).orNull()
  ...
  private fun handleExpirationUpdate(
    ...
    if (WaveDatabase.recipients.getExpiresInSeconds(threadRecipientId) == expiresIn.inWholeSeconds) {
    ...
    try {
      ...
      val insertResult: InsertResult? = WaveDatabase.messages.insertMessageInbox(mediaMessage, -1).orNull()

      if (expireTimerVersion != null) {
        WaveDatabase.recipients.setExpireMessages(threadRecipientId, expiresIn.inWholeSeconds.toInt(), expireTimerVersion)
      } else {
        // TODO [expireVersion] After unsupported builds expire, we can remove this branch
        WaveDatabase.recipients.setExpireMessagesWithoutIncrementingVersion(threadRecipientId, expiresIn.inWholeSeconds.toInt())
      }
  ...
  private fun handleStoryReaction(
    ...
    WaveDatabase.messages.beginTransaction()
    return try {
      ...
      try {
        val storyId = WaveDatabase.messages.getStoryId(authorRecipientId, sentTimestamp).id

        if (groupId != null) {
          parentStoryId = GroupReply(storyId)
        } else if (WaveDatabase.storySends.canReply(senderRecipientId, sentTimestamp)) {
          val story = WaveDatabase.messages.getMessageRecord(storyId) as MmsMessageRecord
          var displayText = ""
      ...
      val insertResult: InsertResult? = WaveDatabase.messages.insertMessageInbox(mediaMessage, -1).orNull()
      if (insertResult != null) {
        WaveDatabase.messages.setTransactionSuccessful()
      ...
      WaveDatabase.messages.endTransaction()
    }
  }
  ...
  fun handleReaction(
    ...
    val targetMessage = WaveDatabase.messages.getMessageFor(targetSentTimestamp, targetAuthor.id)
    ...
    val targetThread = WaveDatabase.threads.getThreadRecord(targetMessage.threadId)
    ...
    val groupRecord = WaveDatabase.groups.getGroup(targetThreadRecipientId).orNull()
    ...
    if (isRemove) {
      WaveDatabase.reactions.deleteReaction(targetMessageId, senderRecipientId)
      AppDependencies.messageNotifier.updateNotification(context)
    } else {
      val reactionRecord = ReactionRecord(emoji!!, senderRecipientId, message.timestamp!!, System.currentTimeMillis())
      WaveDatabase.reactions.addReaction(targetMessageId, reactionRecord)
  ...
  fun handleRemoteDelete(context: Context, envelope: Envelope, message: DataMessage, senderRecipientId: RecipientId, earlyMessageCacheEntry: EarlyMessageCacheEntry?): MessageId? {
    ...
    val targetMessage: MessageRecord? = WaveDatabase.messages.getMessageFor(targetSentTimestamp, senderRecipientId)
    ...
    return if (targetMessage != null && MessageConstraintsUtil.isValidRemoteDeleteReceive(targetMessage, senderRecipientId, envelope.serverTimestamp!!)) {
      WaveDatabase.messages.markAsRemoteDelete(targetMessage)
      if (targetMessage.isStory()) {
        WaveDatabase.messages.deleteRemotelyDeletedStory(targetMessage.id)
      }
  ...
  private fun handlePaymentActivation(
    ...
    try {
      ...
      return WaveDatabase.messages.insertMessageInbox(mediaMessage, -1).orNull()
    } catch (e: MmsException) {
      throw StorageFailedException(e, metadata.sourceServiceId.toString(), metadata.sourceDeviceId)
    }
    return null
  ...
  private fun handlePayment(
    ...
    try {
      WaveDatabase.payments.createIncomingPayment(
        uuid,
        senderRecipientId,
        message.timestamp!!,
    ...
    return try {
      ...
      try {
        if (selfId == storyAuthorRecipientId) {
          storyMessageId = WaveDatabase.storySends.getStoryMessageFor(senderRecipient.id, sentTimestamp)
        }

        if (storyMessageId == null) {
          storyMessageId = WaveDatabase.messages.getStoryId(storyAuthorRecipientId, sentTimestamp)
        }

        val story: MmsMessageRecord = WaveDatabase.messages.getMessageRecord(storyMessageId.id) as MmsMessageRecord
        var threadRecipient: Recipient = WaveDatabase.threads.getRecipientForThreadId(story.threadId)!!
        val groupRecord: GroupRecord? = WaveDatabase.groups.getGroup(threadRecipient.id).orNull()
        ...
        } else if (groupStory || WaveDatabase.storySends.canReply(senderRecipient.id, sentTimestamp)) {
  ...
  private fun handleGiftMessage(
    ...
    val insertResult: InsertResult? = try {
      ...
      WaveDatabase.messages.insertMessageInbox(mediaMessage, -1).orNull()
    } catch (e: MmsException) {
      throw StorageFailedException(e, metadata.sourceServiceId.toString(), metadata.sourceDeviceId)
    }
  ...
  private fun handleMediaMessage(
    ...
    try {
      ...
      insertResult = WaveDatabase.messages.insertMessageInbox(mediaMessage, -1).orNull()
      if (insertResult != null) {
        WaveDatabase.messages.setTransactionSuccessful()
      }
    } catch (e: MmsException) {
  ...
  private fun handleTextMessage(
    ...
    val insertResult: InsertResult? = WaveDatabase.messages.insertMessageInbox(textMessage).orNull()
  ...
  fun handleGroupCallUpdateMessage(
    ...
    val groupRecipientId = WaveDatabase.recipients.getOrInsertFromPossiblyMigratedGroupId(groupId)
  ...
  fun handlePollCreate(
    ...
    val groupRecord = WaveDatabase.groups.getGroup(groupId).orNull()
    ...
    val insertResult: InsertResult? = WaveDatabase.messages.insertMessageInbox(pollMessage).orNull()
    return if (insertResult != null) {
      AppDependencies.messageNotifier.updateNotification(context, ConversationId.forConversation(insertResult.threadId))
      insertResult
    } else {
  ...
  fun handlePollTerminate(
    ...
    val poll = WaveDatabase.polls.getPoll(messageId.id)
    if (poll == null) {
  ...
  fun handlePollVote(
    ...
    val targetMessage = WaveDatabase.messages.getMessageRecord(messageId.id)
    val pollId = WaveDatabase.polls.getPollId(messageId.id)
    if (pollId == null) {
      warn(envelope.timestamp!!, "[handlePollVote] Poll was not found. timestamp: $targetSentTimestamp  author: ${senderRecipient.id}")
      return null
    }

    val existingVoteCount = WaveDatabase.polls.getCurrentPollVoteCount(pollId, senderRecipient.id.toLong())
    ...
    val allOptionIds = WaveDatabase.polls.getPollOptionIds(pollId)
    ...
    if (!WaveDatabase.polls.canAllowMultipleVotes(pollId) && pollVote.optionIndexes.size > 1) {
  ...
  fun handlePinMessage(
    ...
    val targetMessage: MmsMessageRecord? = WaveDatabase.messages.getMessageFor(pinMessage.targetSentTimestamp!!, targetAuthor.id) as? MmsMessageRecord
    if (targetMessage == null) {
    ...
    val groupRecord = WaveDatabase.groups.getGroup(threadRecipient.id).orNull()
    if (groupRecord != null && !groupRecord.members.contains(senderRecipient.id)) {
    ...
    val insertResult: InsertResult? = WaveDatabase.messages.insertMessageInbox(pinnedMessage).orNull()
  ...
  fun handleUnpinMessage(
    ...
    val targetMessage: MmsMessageRecord? = WaveDatabase.messages.getMessageFor(unpinMessage.targetSentTimestamp!!, targetAuthor.id) as? MmsMessageRecord
    if (targetMessage == null) {
    ...
    val targetMessageId = targetMessage.latestRevisionId?.id ?: targetMessage.id
    WaveDatabase.messages.unpinMessage(targetMessageId, targetMessage.threadId)
  ...
  fun notifyTypingStoppedFromIncomingMessage(context: Context, senderRecipient: Recipient, threadRecipientId: RecipientId, device: Int) {
    val threadId = WaveDatabase.threads.getThreadIdIfExistsFor(threadRecipientId)
  ...
  private fun insertPlaceholder(sender: RecipientId, timestamp: Long, groupId: GroupId?): InsertResult? {
    ...
    return WaveDatabase.messages.insertMessageInbox(textMessage).orNull()
  }
  ...
  fun getValidatedQuote(context: Context, timestamp: Long, message: DataMessage, senderRecipient: Recipient, threadRecipient: Recipient): QuoteModel? {
    ...
    var quotedMessage = WaveDatabase.messages.getMessageFor(quote.id!!, authorId) as? MmsMessageRecord
    ...
    if (quotedMessage != null && isSenderValid(quotedMessage, timestamp, senderRecipient, threadRecipient) && !quotedMessage.isRemoteDelete && !quotedMessage.isUnsupported) {
      ...
      val targetMessageAttachments = WaveDatabase.attachments.getAttachmentsForMessage(quotedMessage.id)
      val mentions: List<Mention> = WaveDatabase.mentions.getMentionsForMessage(quotedMessage.id)
      ...
      if (quotedMessage.isPaymentNotification) {
        quotedMessage = WaveDatabase.payments.updateMessageWithPayment(quotedMessage) as MmsMessageRecord
      }
  ...
  private fun handlePollValidation(
    ...
    val groupRecord = WaveDatabase.groups.getGroup(targetThread.recipient.id).orNull()
    if (groupRecord == null) {
  ...
  fun getStickerAttachment(timestamp: Long, message: DataMessage): Attachment? {
    ...
    val stickerRecord: StickerRecord? = WaveDatabase.stickers.getSticker(stickerLocator.packId, stickerLocator.stickerId, false)
