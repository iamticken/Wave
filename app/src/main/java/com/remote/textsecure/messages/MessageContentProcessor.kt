...
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.hasDisallowedAnnouncementOnlyContent
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.hasGroupContext
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.hasSignedGroupChange
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.hasStarted
...
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.isValid
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.signedGroupChange
import org.thoughtcrime.securesms.messages.WaveServiceProtoUtil.toDecryptionErrorMessage
...
open class MessageContentProcessor(private val context: Context) {
  ...
  companion object {
      ...
      if (content.dataMessage != null) {
        ...
        if (content.typingMessage!!.groupId != null) {
          ...
          return if (groupRecipient.isBlocked || !groupRecipient.isActiveGroup) {
            ...
            val groupRecord = WaveDatabase.groups.getGroup(groupId)
    ...
    private fun handlePendingRetry(pending: PendingRetryReceiptModel?, timestamp: Long, destination: Recipient): Long {
      ...
      if (pending != null) {
        ...
        val threadId = WaveDatabase.threads.getThreadIdFor(destination.id)
        if (threadId != null) {
          val lastSeen = WaveDatabase.threads.getConversationMetadata(threadId).lastSeen
    ...
    fun handleGv2PreProcessing(
      ...
      val preUpdateGroupRecord = WaveDatabase.groups.getGroup(groupId)
      ...
      val groupRecord = if (groupUpdateResult.updateStatus == UpdateStatus.GROUP_CONSISTENT_OR_AHEAD) {
        ...
        WaveDatabase.groups.getGroup(groupId)
      }
   ...
   */
  @JvmOverloads
  open fun process(envelope: Envelope, content: Content, metadata: EnvelopeMetadata, serverDeliveredTimestamp: Long, processingEarlyContent: Boolean = false, localMetric: WaveLocalMetrics.MessageReceive? = null) {
    val senderRecipient = Recipient.externalPush(WaveServiceAddress(metadata.sourceServiceId, metadata.sourceE164))
  ...
  fun processException(messageState: MessageState, exceptionMetadata: ExceptionMetadata, timestamp: Long) {
    ...
    when (messageState) {
      MessageState.DECRYPTION_ERROR -> {
        ...
        val threadId: Long? = WaveDatabase.threads.getThreadIdFor(threadRecipient.id)
      ...
      MessageState.INVALID_VERSION -> {
        ...
        insertErrorMessage(context, sender, timestamp, exceptionMetadata.groupId.toOptional()) { messageId ->
          WaveDatabase.messages.markAsInvalidVersionKeyExchange(messageId)
        }
      }
      ...
      MessageState.LEGACY_MESSAGE -> {
        ...
        insertErrorMessage(context, sender, timestamp, exceptionMetadata.groupId.toOptional()) { messageId ->
          WaveDatabase.messages.markAsLegacyVersion(messageId)
        }
      }
      ...
      MessageState.UNSUPPORTED_DATA_MESSAGE -> {
        ...
        insertErrorMessage(context, sender, timestamp, exceptionMetadata.groupId.toOptional()) { messageId ->
          WaveDatabase.messages.markAsUnsupportedProtocolVersion(messageId)
        }
      }
  ...
  private fun handleMessage(
    ...
    localMetric: WaveLocalMetrics.MessageReceive?
  ) {
    ...
    when {
      ...
      content.callMessage != null -> {
        ...
        if (message.destinationDeviceId != null && message.destinationDeviceId != WaveStore.account.deviceId) {
          log(envelope.timestamp!!, "Ignoring call message that is not for this device! intended: ${message.destinationDeviceId}, this: ${WaveStore.account.deviceId}")
          return
        }
  ...
  private fun handleTypingMessage(
    ...
    val threadId: Long = if (typingMessage.groupId != null) {
      ...
      if (!WaveDatabase.groups.isCurrentMember(groupId, senderRecipient.id)) {
        warn(envelope.timestamp!!, "Seen typing indicator for non-member " + senderRecipient.id)
        return
      }

      val groupRecipient = Recipient.externalPossiblyMigratedGroup(groupId)
      WaveDatabase.threads.getOrCreateThreadIdFor(groupRecipient)
    } else {
      WaveDatabase.threads.getOrCreateThreadIdFor(senderRecipient)
    }
  ...
  private fun handleRetryReceipt(envelope: Envelope, metadata: EnvelopeMetadata, decryptionErrorMessage: DecryptionErrorMessage, senderRecipient: Recipient) {
    ...
    if (decryptionErrorMessage.deviceId != WaveStore.account.deviceId) {
    ...
    val messageLogEntry = WaveDatabase.messageLog.getLogEntry(senderRecipient.id, metadata.sourceDeviceId, sentTimestamp)
  ...
  private fun handleSenderKeyRetryReceipt(
    ...
    val threadRecipient = WaveDatabase.threads.getRecipientForThreadId(relatedMessage.threadId)
    ...
    if (threadRecipient.isGroup) {
      ...
      distributionId = WaveDatabase.groups.getOrCreateDistributionId(groupId)
    } else {
      groupId = null
      distributionId = WaveDatabase.distributionLists.getDistributionId(threadRecipient.id)
    }
    ...
    val requesterAddress = WaveProtocolAddress(requester.requireServiceId().toString(), metadata.sourceDeviceId)
    WaveDatabase.senderKeyShared.delete(distributionId, setOf(requesterAddress))
  ...
  private fun findRetryReceiptRelatedMessage(messageLogEntry: MessageLogEntry?, sentTimestamp: Long): MessageRecord? {
    return if (messageLogEntry != null && messageLogEntry.hasRelatedMessage) {
      ...
      WaveDatabase.messages.getMessageRecordOrNull(id)
    } else {
      WaveDatabase.messages.getMessageFor(sentTimestamp, Recipient.self().id)
    }
  }
