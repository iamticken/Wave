...
import org.wave.libwave.metadata.ProtocolException
...
import org.whispersystems.waveservice.api.crypto.WaveServiceCipherResult
import org.whispersystems.waveservice.api.messages.EnvelopeContentValidator
...
object MessageDecryptor {
  ...
  /**
   * Decrypts an envelope and provides a [Result]. This method has side effects, but all of them are limited to [WaveDatabase].
  ...
  fun decrypt(
    ...
    val selfAci: ACI = WaveStore.account.requireAci()
    val selfPni: PNI = WaveStore.account.requirePni()
    ...
    if (destination == selfPni && Util.anyNotNull(envelope.sourceServiceId, envelope.sourceServiceIdBinary)) {
      ...
      if (sourceServiceId != null) {
        ...
        WaveDatabase.recipients.markNeedsPniSignature(sender)
      } else {
    ...
    val localAddress = WaveServiceAddress(destination, WaveStore.account.e164)
    val cipher = WaveServiceCipher(localAddress, WaveStore.account.deviceId, bufferedStore, ReentrantSessionLock.INSTANCE, SealedSenderAccessUtil.getCertificateValidator())

    return try {
      val startTimeNanos = System.nanoTime()
      val cipherResult: WaveServiceCipherResult? = cipher.decrypt(envelope, serverDeliveredTimestamp)
      ...
      val validationResult: EnvelopeContentValidator.Result = EnvelopeContentValidator.validate(envelope, cipherResult.content, WaveStore.account.aci!!)
      ...
      if (envelope.report_spam_token != null && envelope.report_spam_token!!.size > 0) {
        ...
        WaveDatabase.recipients.setReportingToken(sender, envelope.report_spam_token!!.toByteArray())
      }
  ...
  private fun buildResultForDecryptionError(
    ...
    if (ServiceId.parseOrNull(envelope.destinationServiceId, envelope.destinationServiceIdBinary) == WaveStore.account.pni) {
      ...
      ContentHint.RESENDABLE -> {
        ...
        followUpOperations += FollowUpOperation {
          ...
          val threadId: Long? = if (groupId != null) {
            if (WaveDatabase.groups.getGroup(groupId).isAbsent()) {
            ...
            WaveDatabase.threads.getThreadIdFor(groupRecipient.id)
          } else {
            WaveDatabase.threads.getThreadIdFor(sender.id)
          }
  ...
  private fun handleSenderKeyDistributionMessage(envelope: Envelope, serviceId: ServiceId, deviceId: Int, message: SenderKeyDistributionMessage, senderKeyStore: SenderKeyStore) {
    ...
    val sender = WaveProtocolAddress(serviceId.toString(), deviceId)
    WaveGroupSessionBuilder(ReentrantSessionLock.INSTANCE, GroupSessionBuilder(senderKeyStore)).process(sender, message)
  }
  ...
  private fun handlePniSignatureMessage(envelope: Envelope, protocolStore: BufferedProtocolStore, aci: ACI, e164: String?, deviceId: Int, pniSignatureMessage: PniSignatureMessage) {
    ...
    if (WaveDatabase.recipients.isAssociated(aci, pni)) {
      Log.i(TAG, "${logPrefix(envelope, aci)}[handlePniSignatureMessage] ACI ($aci) and PNI ($pni) are already associated.")
      return
    }

    val aciAddress = WaveProtocolAddress(aci.toString(), deviceId)
    val pniAddress = WaveProtocolAddress(pni.toString(), deviceId)
    ...
    if (pniIdentity == null) {
      ...
      if (deviceId != WaveServiceAddress.DEFAULT_DEVICE_ID) {
        pniIdentity = protocolStore.getAciStore().getIdentity(WaveProtocolAddress(pni.toString(), WaveServiceAddress.DEFAULT_DEVICE_ID))
    ...
    if (pniIdentity.verifyAlternateIdentity(aciIdentity, pniSignatureMessage.signature!!.toByteArray())) {
      ...
      WaveDatabase.recipients.getAndPossiblyMergePnpVerified(aci, pni, e164)
    } else {
  ...
  private fun logPrefix(envelope: Envelope, cipherResult: WaveServiceCipherResult): String {
  ...
  private fun WaveServiceCipherResult.toErrorMetadata(): ErrorMetadata {
  ...
  sealed interface Result {
    ...
    /** The envelope used an invalid version of the Wave protocol. */
    ...
    /**
     * Indicates the that the [org.whispersystems.waveservice.internal.push.WaveServiceProtos.DataMessage.getRequiredProtocolVersion]
