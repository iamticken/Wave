...
class IncomingMessageObserver(
  ...
  companion object {
    ...
    private val censored: Boolean
      get() = AppDependencies.waveServiceNetworkAccess.isCensored()
  }
  ...
  private val networkConnectionListener = NetworkConnectionListener(
    ...
    onNetworkLost = { isNetworkUnavailable ->
      lock.withLock {
        AppDependencies.libwaveNetwork.onNetworkChange()
  ...
  init {
    ...
    if (!WaveStore.account.fcmEnabled || WaveStore.internal.isWebsocketModeForced) {
      try {
    ...
    AppForegroundObserver.addListener(object : AppForegroundObserver.Listener {
      override fun onForeground() {
        WaveExecutors.BOUNDED.execute { onAppForegrounded() }
      }

      override fun onBackground() {
        WaveExecutors.BOUNDED.execute { onAppBackgrounded() }
      }
    })
  ...
  private fun isConnectionNecessary(): Boolean {
    ...
    val registered = WaveStore.account.isRegistered
    val fcmEnabled = WaveStore.account.fcmEnabled
    val hasNetwork = NetworkConstraint.isMet(context)
    val hasProxy = WaveStore.proxy.isProxyEnabled
    val forceWebsocket = WaveStore.internal.isWebsocketModeForced
  ...
  private fun isConnectionAvailable(): Boolean {
    return WaveStore.account.isRegistered && (authWebSocket.stateSnapshot == WebSocketConnectionState.CONNECTED || (authWebSocket.shouldSendKeepAlives() && NetworkConstraint.isMet(context)))
  }
  ...
  private fun processMessage(bufferedProtocolStore: BufferedProtocolStore, envelope: Envelope, serverDeliveredTimestamp: Long): List<FollowUpOperation> {
    val localReceiveMetric = WaveLocalMetrics.MessageReceive.start()
    ...
    WaveLocalMetrics.MessageLatency.onMessageReceived(envelope.serverTimestamp!!, serverDeliveredTimestamp, envelope.urgent!!)
    when (result) {
  ...
  private fun processReceipt(envelope: Envelope) {
    ...
    WaveDatabase.messages.incrementDeliveryReceiptCount(envelope.timestamp!!, senderId, System.currentTimeMillis())
    WaveDatabase.messageLog.deleteEntryForRecipient(envelope.timestamp!!, senderId, envelope.sourceDevice!!)
  }
  ...
  private inner class MessageRetrievalThread : Thread("MessageRetrievalService"), Thread.UncaughtExceptionHandler {
    ...
    init {
      ...
      sleepTimer = if (!WaveStore.account.fcmEnabled || WaveStore.internal.isWebsocketModeForced) AlarmSleepTimer(context) else UptimeSleepTimer()

      canProcessMessages = !WaveStore.registration.restoreDecisionState.isDecisionPending
    }
    ...
    override fun run() {
      ...
      while (!terminated) {
        ...
        val webSocketDisposable = authWebSocket.state.subscribe { state: WebSocketConnectionState ->
          ...
          if (state == WebSocketConnectionState.CONNECTED) {
            WaveStore.misc.lastWebSocketConnectTime = System.currentTimeMillis()
          }
        }
          ...
          while (!terminated && (isConnectionNecessary().also { isConnectionNecessary = it } || isConnectionAvailable())) {
            ...
            try {
              if (canProcessMessages) {
                  ...
                  GroupsV2ProcessingLock.acquireGroupProcessingLock().use {
                    ReentrantSessionLock.INSTANCE.acquire().use {
                      batch.forEach { response ->
                        ...
                        val followUpOperations = WaveDatabase.runInTransaction { db ->
                ...
                attempts = 0
                WaveLocalMetrics.PushWebsocketFetch.onProcessedBatch()
  ...
  class ForegroundService : Service() {
    ...
    private fun postForegroundNotification() {
      val notification = NotificationCompat.Builder(applicationContext, NotificationChannels.getInstance().BACKGROUND)
        .setContentTitle(applicationContext.getString(R.string.MessageRetrievalService_wave))
        .setContentText(applicationContext.getString(R.string.MessageRetrievalService_background_connection_enabled))
        .setPriority(NotificationCompat.PRIORITY_MIN)
        .setWhen(0)
        .setSmallIcon(R.drawable.ic_wave_background_connection)
        .build()
