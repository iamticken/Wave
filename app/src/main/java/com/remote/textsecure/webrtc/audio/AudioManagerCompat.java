...
public abstract class AudioManagerCompat {
  ...
  public boolean isBluetoothHeadsetAvailable() {
    if (Build.VERSION.SDK_INT >= 31) {
      return audioManager.getAvailableCommunicationDevices().stream().anyMatch(it -> AudioDeviceMapping.fromPlatformType(it.getType()) == WaveAudioManager.AudioDevice.BLUETOOTH);
    } else {
  ...
  public boolean isBluetoothConnected() {
    if (Build.VERSION.SDK_INT >= 31) {
      ...
      final WaveAudioManager.AudioDevice audioDevice = AudioDeviceMapping.fromPlatformType(communicationDevice.getType());
      return WaveAudioManager.AudioDevice.BLUETOOTH == audioDevice;
    } else {
  ...
  public boolean isSpeakerphoneOn() {
    if (Build.VERSION.SDK_INT >= 31) {
      ...
      if (audioDeviceInfo == null) {
        ...
        return AudioDeviceMapping.fromPlatformType(audioDeviceInfo.getType())  == WaveAudioManager.AudioDevice.SPEAKER_PHONE;
      }
    } else {
  ...
  public void setSpeakerphoneOn(boolean on) {
    if (Build.VERSION.SDK_INT >= 31) {
      WaveAudioManager.AudioDevice audioDevice = on ? WaveAudioManager.AudioDevice.SPEAKER_PHONE : WaveAudioManager.AudioDevice.EARPIECE;
      ...
      if (candidate != null) {
        ...
        Log.w(TAG, "setSpeakerphoneOn: Failed to find candidate for WaveAudioDevice {" + audioDevice + "}. Falling back on deprecated method.");
  ...
  public @Nullable AudioDeviceInfo getConnectedBluetoothDevice() {
    return getAvailableCommunicationDevices().stream().filter(it -> AudioDeviceMapping.fromPlatformType(it.getType()) == WaveAudioManager.AudioDevice.BLUETOOTH).findAny().orElse(null);
  }
