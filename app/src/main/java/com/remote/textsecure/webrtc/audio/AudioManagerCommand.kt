...
import org.wave.core.util.readParcelableCompat
import org.wave.core.util.readSerializableCompat
import org.thoughtcrime.securesms.recipients.RecipientId
import org.thoughtcrime.securesms.util.ParcelUtil

/**
 * Commands that can be issued to [WaveAudioManager] to perform various tasks.
 *
...
sealed class AudioManagerCommand : Parcelable {
  ...
  class SetDefaultDevice(val recipientId: RecipientId?, val device: WaveAudioManager.AudioDevice, val clearUserEarpieceSelection: Boolean) : AudioManagerCommand() {
    override fun writeToParcel(parcel: Parcel, flags: Int) {
      parcel.writeParcelable(recipientId, flags)
    ...
    companion object {
      ...
      val CREATOR: Parcelable.Creator<SetDefaultDevice> = ParcelCheat { parcel ->
        SetDefaultDevice(
          ...
          device = parcel.readSerializableCompat(WaveAudioManager.AudioDevice::class.java)!!,
