...
/**
 * Record processor for [WaveGroupV1Record].
 * Handles merging and updating our local store when processing remote gv1 storage records.
 */
class GroupV1RecordProcessor(private val groupDatabase: GroupTable, private val recipientTable: RecipientTable) : DefaultStorageRecordProcessor<WaveGroupV1Record>() {
  companion object {
    private val TAG = Log.tag(GroupV1RecordProcessor::class.java)
  }

  constructor() : this(WaveDatabase.groups, WaveDatabase.recipients)
   ...
   */
  override fun isInvalid(remote: WaveGroupV1Record): Boolean {
    try {
  ...
  override fun getMatching(remote: WaveGroupV1Record, keyGenerator: StorageKeyGenerator): Optional<WaveGroupV1Record> {
    ...
    return recipientId
      ...
      .map { record: WaveStorageRecord -> record.proto.groupV1!!.toWaveGroupV1Record(record.id) }
  }

  override fun merge(remote: WaveGroupV1Record, local: WaveGroupV1Record, keyGenerator: StorageKeyGenerator): WaveGroupV1Record {
    val merged = WaveGroupV1Record.newBuilder(remote.serializedUnknowns).apply {
      id = remote.proto.id
    ...
    }.build().toWaveGroupV1Record(StorageId.forGroupV1(keyGenerator.generate()))
  ...
  override fun insertLocal(record: WaveGroupV1Record) {
    recipientTable.applyStorageSyncGroupV1Insert(record)
  }

  override fun updateLocal(update: StorageRecordUpdate<WaveGroupV1Record>) {
    recipientTable.applyStorageSyncGroupV1Update(update)
  }

  override fun compare(lhs: WaveGroupV1Record, rhs: WaveGroupV1Record): Int {
