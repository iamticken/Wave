...
/**
 * Record processor for [WaveChatFolderRecord].
 * Handles merging and updating our local store when processing remote chat folder storage records.
 */
class ChatFolderRecordProcessor : DefaultStorageRecordProcessor<WaveChatFolderRecord>() {

  companion object {
    private val TAG = Log.tag(ChatFolderRecordProcessor::class)
  }

  override fun compare(o1: WaveChatFolderRecord, o2: WaveChatFolderRecord): Int {
   ...
   */
  override fun isInvalid(remote: WaveChatFolderRecord): Boolean {
  ...
  override fun getMatching(remote: WaveChatFolderRecord, keyGenerator: StorageKeyGenerator): Optional<WaveChatFolderRecord> {
    ...
    val folder = WaveDatabase.chatFolders.getChatFolder(query)
    ...
    return if (folder?.storageServiceId != null) {
      ...
      WaveDatabase.chatFolders.applyStorageIdUpdate(folder.chatFolderId, storageId)
      ...
      Optional.empty<WaveChatFolderRecord>()
    }
  }
   ...
   */
  override fun merge(remote: WaveChatFolderRecord, local: WaveChatFolderRecord, keyGenerator: StorageKeyGenerator): WaveChatFolderRecord {
  ...
  override fun insertLocal(record: WaveChatFolderRecord) {
    WaveDatabase.chatFolders.insertChatFolderFromStorageSync(record)
  }

  override fun updateLocal(update: StorageRecordUpdate<WaveChatFolderRecord>) {
    WaveDatabase.chatFolders.updateChatFolderFromStorageSync(update.new)
  }
