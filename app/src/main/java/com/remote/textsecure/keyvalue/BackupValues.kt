...
class BackupValues(store: KeyValueStore) : WaveStoreValues(store) {
  companion object {
  ...
  val messageBackupKey: MessageBackupKey
    get() = WaveStore.account.accountEntropyPool.deriveMessageBackupKey()
  ...
  /**
   * Denotes if there was a mismatch detected between the user's Wave subscription, on-device Google Play subscription,
