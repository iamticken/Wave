...
import org.wave.ringrtc.CallManager.DataMode
...
class InternalValues internal constructor(store: KeyValueStore) : WaveStoreValues(store) {
  companion object {
  ...
  private fun <T> WaveStoreValueDelegate<T>.defaultForExternalUsers(): WaveStoreValueDelegate<T> {
    return this.withPrecondition { RemoteConfig.internalUser }
  }

  private fun WaveStoreValueDelegate<Boolean>.falseForExternalUsers(): WaveStoreValueDelegate<Boolean> {
