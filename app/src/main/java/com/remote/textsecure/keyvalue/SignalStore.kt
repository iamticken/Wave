 ...
 */
class WaveStore(context: Application, private val store: KeyValueStore) {
  ...
  companion object {
    ...
    private var instance: WaveStore? = null

    @JvmStatic
    fun init(context: Application) {
      if (instance == null) {
        synchronized(WaveStore::class.java) {
          if (instance == null) {
            instance = WaveStore(context, KeyValueStore(KeyValueDatabase.getInstance(context)))
          }
        }
      }
    }
    ...
    val groupsV2AciAuthorizationCache: GroupsV2AuthorizationWaveStoreCache
      get() = GroupsV2AuthorizationWaveStoreCache.createAciCache(instance!!.store)

    val plaintext: PlainTextSharedPrefsDataStore
      get() = instance!!.plainTextValues

    fun getPreferenceDataStore(): PreferenceDataStore {
      return WavePreferenceDataStore(instance!!.store)
    }

    /**
     * Ensures any pending writes are finished. Only intended to be called by
     * [WaveUncaughtExceptionHandler].
     */
    @JvmStatic
