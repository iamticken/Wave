 ...
 */
public final class PinValues extends WaveStoreValues {
  ...
  public void onEntrySuccess(@NonNull String pin) {
    long nextInterval = WavePinReminders.getNextInterval(getCurrentInterval());
    ...
    WaveStore.svr().setPinIfNotPresent(pin);
  }

  public void onEntrySuccessWithWrongGuess(@NonNull String pin) {
    long nextInterval = WavePinReminders.getPreviousInterval(getCurrentInterval());
  ...
  public void onEntrySkip(boolean includedFailure) {
    ...
    if (includedFailure) {
      nextInterval = WavePinReminders.getPreviousInterval(getCurrentInterval());
    } else {
  ...
  public void resetPinReminders() {
    long nextInterval = WavePinReminders.INITIAL_INTERVAL;
  ...
  public @NonNull PinKeyboardType getKeyboardType() {
    String pin = WaveStore.svr().getPin();
