...
import org.wave.libwave.messagebackup.AccountEntropyPool as LibWaveAccountEntropyPool

class AccountValues internal constructor(store: KeyValueStore, context: Context) : WaveStoreValues(store) {
  ...
  val accountEntropyPool: AccountEntropyPool
    get() {
      AEP_LOCK.withLock {
        ...
        val newAep = LibWaveAccountEntropyPool.generate()
  ...
  /** The password for communicating with the Wave service. */
  ...
  /** Whether or not the user is registered with the Wave service. */
  ...
  var deviceId: Int by integerValue(KEY_DEVICE_ID, WaveServiceAddress.DEFAULT_DEVICE_ID)

  val isPrimaryDevice: Boolean
    get() = deviceId == WaveServiceAddress.DEFAULT_DEVICE_ID
  ...
  private fun clearLocalCredentials() {
    ...
    WaveDatabase.recipients.setProfileKey(self.id, newProfileKey)
