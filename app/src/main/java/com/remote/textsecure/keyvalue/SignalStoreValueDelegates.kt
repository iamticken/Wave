...
import org.wave.core.util.LongSerializer
import kotlin.reflect.KProperty
import kotlin.time.Duration
import kotlin.time.Duration.Companion.nanoseconds

internal fun WaveStoreValues.longValue(key: String, default: Long): WaveStoreValueDelegate<Long> {
  return LongValue(key, default, this.store)
}

internal fun WaveStoreValues.booleanValue(key: String, default: Boolean): WaveStoreValueDelegate<Boolean> {
  return BooleanValue(key, default, this.store)
}

internal fun <T : String?> WaveStoreValues.stringValue(key: String, default: T): WaveStoreValueDelegate<T> {
  return StringValue(key, default, this.store)
}

internal fun WaveStoreValues.integerValue(key: String, default: Int): WaveStoreValueDelegate<Int> {
  return IntValue(key, default, this.store)
}

internal fun WaveStoreValues.floatValue(key: String, default: Float): WaveStoreValueDelegate<Float> {
  return FloatValue(key, default, this.store)
}

internal fun WaveStoreValues.blobValue(key: String, default: ByteArray): WaveStoreValueDelegate<ByteArray> {
  return BlobValue(key, default, this.store)
}

internal fun WaveStoreValues.nullableBlobValue(key: String, default: ByteArray?): WaveStoreValueDelegate<ByteArray?> {
  return NullableBlobValue(key, default, this.store)
}

internal fun <T : Any?> WaveStoreValues.enumValue(key: String, default: T, serializer: LongSerializer<T>): WaveStoreValueDelegate<T> {
  return KeyValueEnumValue(key, default, serializer, this.store)
}

internal fun <M> WaveStoreValues.protoValue(key: String, adapter: ProtoAdapter<M>): WaveStoreValueDelegate<M?> {
  return KeyValueProtoValue(key, adapter, this.store)
}

internal fun <M> WaveStoreValues.protoValue(key: String, default: M, adapter: ProtoAdapter<M>, onSet: ((M) -> Unit)? = null): WaveStoreValueDelegate<M> {
  return KeyValueProtoWithDefaultValue(key, default, adapter, this.store, onSet)
}

internal fun WaveStoreValues.durationValue(key: String, default: Duration?): WaveStoreValueDelegate<Duration?> {
  return DurationValue(key, default, this.store)
}

internal fun <T> WaveStoreValueDelegate<T>.withPrecondition(precondition: () -> Boolean): WaveStoreValueDelegate<T> {
...
internal fun <T> WaveStoreValueDelegate<T>.map(transform: (T) -> T): WaveStoreValueDelegate<T> {
 ...
 */
sealed class WaveStoreValueDelegate<T>(val store: KeyValueStore, open val default: T, private val onSet: ((T) -> Unit)? = null) {
...
private class LongValue(private val key: String, default: Long, store: KeyValueStore) : WaveStoreValueDelegate<Long>(store, default) {
...
private class BooleanValue(private val key: String, default: Boolean, store: KeyValueStore) : WaveStoreValueDelegate<Boolean>(store, default) {
...
private class StringValue<T : String?>(private val key: String, default: T, store: KeyValueStore) : WaveStoreValueDelegate<T>(store, default) {
...
private class IntValue(private val key: String, default: Int, store: KeyValueStore) : WaveStoreValueDelegate<Int>(store, default) {
...
private class FloatValue(private val key: String, default: Float, store: KeyValueStore) : WaveStoreValueDelegate<Float>(store, default) {
...
private class BlobValue(private val key: String, default: ByteArray, store: KeyValueStore) : WaveStoreValueDelegate<ByteArray>(store, default) {
...
private class NullableBlobValue(private val key: String, default: ByteArray?, store: KeyValueStore) : WaveStoreValueDelegate<ByteArray?>(store, default) {
  override fun getValue(values: KeyValueStore): ByteArray? {
    return values.getBlob(key, default)
  }
...
private class DurationValue(private val key: String, default: Duration?, store: KeyValueStore) : WaveStoreValueDelegate<Duration?>(store, default) {
  companion object {
...
private class KeyValueProtoWithDefaultValue<M>(
  ...
  store: KeyValueStore,
  onSet: ((M) -> Unit)? = null
) : WaveStoreValueDelegate<M>(store, default, onSet) {
...
private class KeyValueProtoValue<M>(
  ...
  private val adapter: ProtoAdapter<M>,
  store: KeyValueStore
) : WaveStoreValueDelegate<M?>(store, null) {
  override fun getValue(values: KeyValueStore): M? {
    return if (values.containsKey(key)) {
      adapter.decode(values.getBlob(key, null))
    } else {
...
private class KeyValueEnumValue<T>(private val key: String, default: T, private val serializer: LongSerializer<T>, store: KeyValueStore) : WaveStoreValueDelegate<T>(store, default) {
  override fun getValue(values: KeyValueStore): T {
    return if (values.containsKey(key)) {
...
private class PreconditionDelegate<T>(
  private val delegate: WaveStoreValueDelegate<T>,
  private val precondition: () -> Boolean
) : WaveStoreValueDelegate<T>(delegate.store, delegate.default) {
