...
class PaymentsValues internal constructor(store: KeyValueStore) : WaveStoreValues(store) {
    ...
    get() {
      ...
      return if (RemoteConfig.payments) {
        if (mobileCoinPaymentsEnabled()) {
          if (GeographicalRestrictions.e164Allowed(WaveStore.account.e164)) {
            PaymentsAvailability.WITHDRAW_AND_SEND
          } else {
            return PaymentsAvailability.WITHDRAW_ONLY
          }
        } else if (GeographicalRestrictions.e164Allowed(WaveStore.account.e164)) {
  ...
  fun showUpdatePinInfoCard(): Boolean {
    return if (userHasLargeBalance() &&
      ...
      WaveStore.pin.keyboardType == PinKeyboardType.NUMERIC
    ) {
  ...
  private fun determineCurrency(): Currency {
    val localE164: String = WaveStore.account.e164 ?: ""
