...
import org.wave.core.util.stream.TailerInputStream;
    ...
    if (inProgress && !hasData && hasIncrementalDigest && attachmentKey != null) {
      ...
      if (attachment.transferState == AttachmentTable.TRANSFER_RESTORE_IN_PROGRESS && attachment.archiveTransferState == AttachmentTable.ArchiveTransferState.FINISHED) {
        ...
        try {
          ...
          String mediaId   = MediaName.toMediaIdString(mediaName, WaveStore.backup().getMediaRootBackupKey());

          MediaRootBackupKey.MediaKeyMaterial mediaKeyMaterial     = WaveStore.backup().getMediaRootBackupKey().deriveMediaSecretsFromMediaId(mediaId);
