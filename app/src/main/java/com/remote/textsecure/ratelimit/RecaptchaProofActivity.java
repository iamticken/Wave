...
public class RecaptchaProofActivity extends PassphraseRequiredActivity {
  ...
  private static final String RECAPTCHA_SCHEME = "wavecaptcha://";
  ...
  private void handleToken(@NonNull String token) {
    ...
    SimpleTask.run(() -> {
      String challenge = WaveStore.rateLimit().getChallenge();
      ...
      try {
        for (int i = 0; i < 3; i++) {
          try {
            NetworkResultUtil.toBasicLegacy(WaveNetwork.rateLimitChallenge().submitCaptchaChallenge(challenge, token));
