...
import org.whispersystems.waveservice.api.payments.CurrencyConversion;
import org.whispersystems.waveservice.api.payments.CurrencyConversions;
...
public final class Payments {
  ...
  public synchronized Wallet getWallet() {
    ...
    Entropy paymentsEntropy = WaveStore.payments().getPaymentsEntropy();
  ...
  public synchronized @NonNull CurrencyExchange getCurrencyExchange(boolean refreshIfAble) throws IOException {
    if (currencyConversions == null || shouldRefresh(refreshIfAble, currencyConversions.getTimestamp())) {
      ...
      CurrencyConversions newCurrencyConversions = NetworkResultUtil.toBasicLegacy(WaveNetwork.payments().getCurrencyConversions());
