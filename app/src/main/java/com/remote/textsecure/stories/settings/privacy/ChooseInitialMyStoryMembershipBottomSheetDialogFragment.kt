...
import org.thoughtcrime.securesms.stories.settings.connections.ViewAllWaveConnectionsFragment
...
class ChooseInitialMyStoryMembershipBottomSheetDialogFragment :
  ...
  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    allRow = view.findViewById(R.id.choose_initial_my_story_all_wave_connnections_row)
    allExceptRow = view.findViewById(R.id.choose_initial_my_story_all_wave_connnections_except_row)
    onlyWitRow = view.findViewById(R.id.choose_initial_my_story_only_share_with_row)

    allRadio = view.findViewById(R.id.choose_initial_my_story_all_wave_connnections_radio)
    allExceptRadio = view.findViewById(R.id.choose_initial_my_story_all_wave_connnections_except_radio)
    onlyWitRadio = view.findViewById(R.id.choose_initial_my_story_only_share_with_radio)

    allCount = view.findViewById(R.id.choose_initial_my_story_all_wave_connnections_count)
    allExceptCount = view.findViewById(R.id.choose_initial_my_story_all_wave_connnections_except_count)
    onlyWithCount = view.findViewById(R.id.choose_initial_my_story_only_share_with_count)

    allView = view.findViewById(R.id.choose_initial_my_story_all_wave_connnections_view)
    allView.setOnClickListener {
      ViewAllWaveConnectionsFragment.Dialog.show(parentFragmentManager)
    }
    ...
    lifecycleDisposable += viewModel.state
      .subscribe { state ->
        ...
        allCount.visible = state.allWaveConnectionsCount > 0
        allCount.text = resources.getQuantityString(R.plurals.MyStorySettingsFragment__viewers, state.allWaveConnectionsCount, state.allWaveConnectionsCount)
