...
class StoriesLandingRepository(context: Context) {
  ...
  fun getStories(): Observable<List<StoriesLandingItemData>> {
    val storyRecipients: Observable<Map<Recipient, List<StoryResult>>> = RxDatabaseObserver
      ...
      .map {
        ...
        val stories = WaveDatabase.messages.getOrderedStoryRecipientsAndIds(false)
    ...
    return storyRecipients.switchMap { map ->
      val observables = map.map { (recipient, results) ->
        val messages = results
          ...
          .mapNotNull {
            try {
              WaveDatabase.messages.getMessageRecord(it.messageId)
        ...
        if (recipient.isMyStory) {
          WaveDatabase.messages.getMessages(results.map { it.messageId }).use { reader ->
    ...
    val itemDataObservable = Observable.create<StoriesLandingItemData> { emitter ->
      fun refresh(sender: Recipient) {
        ...
        val itemData = StoriesLandingItemData(
          ...
          hasReplies = messageRecords.any { WaveDatabase.messages.getNumberOfStoryReplies(it.id) > 0 },
          hasRepliesFromSelf = messageRecords.any { WaveDatabase.messages.hasSelfReplyInStory(it.id) },
  ...
  fun setHideStory(recipientId: RecipientId, hideStory: Boolean): Completable {
    return Completable.fromAction {
      WaveDatabase.recipients.setHideStory(recipientId, hideStory)
    }.subscribeOn(Schedulers.io())
  }
  ...
  fun markStoriesRead() {
    WaveExecutors.BOUNDED_IO.execute {
      val messageInfos: List<MessageTable.MarkedMessageInfo> = WaveDatabase.messages.markAllIncomingStoriesRead()
      val releaseThread: Long? = WaveStore.releaseChannel.releaseChannelRecipientId?.let { WaveDatabase.threads.getThreadIdIfExistsFor(it) }
  ...
  fun markFailedStoriesNotified() {
    WaveExecutors.BOUNDED_IO.execute {
      WaveDatabase.messages.markAllFailedStoriesNotified()
    }
  }
}
