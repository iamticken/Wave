...
class StoryGroupReplyDataSource(private val parentStoryId: Long) : PagedDataSource<MessageId, ReplyBody> {
  override fun size(): Int {
    return WaveDatabase.messages.getNumberOfStoryReplies(parentStoryId)
  }

  override fun load(start: Int, length: Int, totalSize: Int, cancellationWave: PagedDataSource.CancellationWave): MutableList<ReplyBody> {
    val results: MutableList<ReplyBody> = ArrayList(length)
    WaveDatabase.messages.getStoryReplies(parentStoryId).use { cursor ->
  ...
  override fun load(key: MessageId): ReplyBody {
    return readRowFromRecord(WaveDatabase.messages.getMessageRecord(key.id) as MmsMessageRecord)
  }
  ...
  private fun readRowFromRecord(record: MmsMessageRecord): ReplyBody {
    val threadRecipient: Recipient = requireNotNull(WaveDatabase.threads.getRecipientForThreadId(record.threadId))
    return when {
