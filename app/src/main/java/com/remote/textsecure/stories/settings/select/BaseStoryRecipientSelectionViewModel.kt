...
class BaseStoryRecipientSelectionViewModel(
  ...
  fun toggleSelectAll(): Single<Set<RecipientId>> {
    return Single.create { emitter ->
      disposable += repository.getAllWaveContacts().subscribeBy { allWaveRecipients ->
        store.update { it.copy(selection = allWaveRecipients) }
        emitter.onSuccess(allWaveRecipients)
      }
    }
  }
