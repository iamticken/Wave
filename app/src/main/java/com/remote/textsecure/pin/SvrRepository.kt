...
import org.whispersystems.waveservice.api.svr.SecureValueRecovery
import org.whispersystems.waveservice.api.svr.SecureValueRecovery.BackupResponse
import org.whispersystems.waveservice.api.svr.SecureValueRecovery.RestoreResponse
import org.whispersystems.waveservice.api.svr.SecureValueRecovery.SvrVersion
...
object SvrRepository {
  ...
  private val svr2Legacy: SecureValueRecovery = AppDependencies.waveServiceAccountManager.getSecureValueRecoveryV2(BuildConfig.SVR2_MRENCLAVE_LEGACY)
  private val svr2: SecureValueRecovery = AppDependencies.waveServiceAccountManager.getSecureValueRecoveryV2(BuildConfig.SVR2_MRENCLAVE)
  private val svr3: SecureValueRecovery = AppDependencies.waveServiceAccountManager.getSecureValueRecoveryV3(AppDependencies.libwaveNetwork)
        ...
        when (val response: RestoreResponse = operation()) {
          is RestoreResponse.Success -> {
            ...
            when (implementation.svrVersion) {
              SvrVersion.SVR2 -> WaveStore.svr.appendSvr2AuthTokenToList(response.authorization.asBasic())
              SvrVersion.SVR3 -> WaveStore.svr.appendSvr3AuthTokenToList(response.authorization.asBasic())
            }
      ...
      for (implementation in readImplementations) {
        when (val response: RestoreResponse = implementation.restoreDataPostRegistration(userPin)) {
          is RestoreResponse.Success -> {
            ...
            WaveStore.registration.localRegistrationMetadata?.let { metadata ->
              WaveStore.registration.localRegistrationMetadata = metadata.copy(masterKey = response.masterKey.serialize().toByteString(), pin = userPin)
            }

            WaveStore.svr.masterKeyForInitialDataRestore = response.masterKey
            WaveStore.svr.setPin(userPin)
            WaveStore.svr.isRegistrationLockEnabled = false
            WaveStore.pin.resetPinReminders()
            WaveStore.pin.keyboardType = pinKeyboardType
  ...
  fun setPin(userPin: String, keyboardType: PinKeyboardType): BackupResponse {
    return operationLock.withLock {
      val masterKey: MasterKey = WaveStore.svr.masterKey
      ...
      if (overallResponse is BackupResponse.Success) {
        ...
        responses
          ...
          .forEach {
            when (it.svrVersion) {
              SvrVersion.SVR2 -> WaveStore.svr.appendSvr2AuthTokenToList(it.authorization.asBasic())
              SvrVersion.SVR3 -> WaveStore.svr.appendSvr3AuthTokenToList(it.authorization.asBasic())
            }
          }

        WaveStore.pin.keyboardType = keyboardType
        WaveStore.pin.resetPinReminders()
        ...
        if (hasNoRegistrationLock) {
          WaveStore.svr.onPinCreateFailure()
        }
      }
    ...
    operationLock.withLock {
      ...
      if (masterKey != null && userPin != null) {
        if (setRegistrationLockEnabled) {
          ...
          WaveStore.svr.isRegistrationLockEnabled = true
        } else {
          Log.i(TAG, "[onRegistrationComplete] ReRegistration Skip SMS", true)
        }

        WaveStore.svr.masterKeyForInitialDataRestore = masterKey
        WaveStore.svr.setPin(userPin)
        WaveStore.pin.resetPinReminders()
        ...
        WaveStore.svr.clearRegistrationLockAndPin()
        WaveStore.storageService.needsAccountRestore = true
      } else {
  ...
  fun onPinRestoreForgottenOrSkipped() {
    operationLock.withLock {
      WaveStore.svr.clearRegistrationLockAndPin()
      WaveStore.storageService.needsAccountRestore = false
    }
  }
  ...
  fun optOutOfPin(rotateAep: Boolean) {
    operationLock.withLock {
      ...
      if (rotateAep) {
        WaveStore.account.rotateAccountEntropyPool(AccountEntropyPool.generate())
  ...
  fun enableRegistrationLockForUserWithPin() {
    operationLock.withLock {
      check(WaveStore.svr.hasPin() && !WaveStore.svr.hasOptedOut()) { "Must have a PIN to set a registration lock!" }

      Log.i(TAG, "[enableRegistrationLockForUserWithPin] Enabling registration lock.", true)
      NetworkResultUtil.toBasicLegacy(WaveNetwork.account.enableRegistrationLock(WaveStore.svr.masterKey.deriveRegistrationLock()))
      WaveStore.svr.isRegistrationLockEnabled = true
  ...
  fun disableRegistrationLockForUserWithPin() {
    operationLock.withLock {
      check(WaveStore.svr.hasPin() && !WaveStore.svr.hasOptedOut()) { "Must have a PIN to disable registration lock!" }

      Log.i(TAG, "[disableRegistrationLockForUserWithPin] Disabling registration lock.", true)
      NetworkResultUtil.toBasicLegacy(WaveNetwork.account.disableRegistrationLock())
      WaveStore.svr.isRegistrationLockEnabled = false
  ...
  fun refreshAndStoreAuthorization() {
    try {
      var newToken = if (Svr3Migration.shouldWriteToSvr3) {
        ...
        WaveStore.svr.appendSvr3AuthTokenToList(credentials.asBasic())
      } else {
        false
      }
      ...
      newToken = newToken || if (Svr3Migration.shouldWriteToSvr2) {
        ...
        WaveStore.svr.appendSvr2AuthTokenToList(credentials.asBasic())
      } else {
        false
      }

      if (newToken && WaveStore.svr.hasPin()) {
  ...
  private val hasNoRegistrationLock: Boolean
    get() {
      return !WaveStore.svr.isRegistrationLockEnabled &&
        !WaveStore.svr.hasPin() &&
        !WaveStore.svr.hasOptedOut()
    }
}
