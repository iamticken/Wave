...
import org.wave.core.util.firstOrNull
 ...
 */
class SenderKeyTable internal constructor(context: Context?, databaseHelper: WaveDatabase?) : DatabaseTable(context, databaseHelper) {
  companion object {
  ...
  fun store(address: WaveProtocolAddress, distributionId: DistributionId, record: SenderKeyRecord) {
  ...
  fun load(address: WaveProtocolAddress, distributionId: DistributionId): SenderKeyRecord? {
   ...
   */
  fun getCreatedTime(address: WaveProtocolAddress, distributionId: DistributionId): Long {
    return readableDatabase
      .select(CREATED_AT)
      .from(TABLE_NAME)
      .where("$ADDRESS = ? AND $DEVICE = ? AND $DISTRIBUTION_ID = ?", address.name, address.deviceId, distributionId)
  ...
  fun getAllCreatedBySelf(): Cursor {
    return readableDatabase
      ...
      .from(TABLE_NAME)
      .where("$ADDRESS = ?", WaveStore.account.requireAci())
