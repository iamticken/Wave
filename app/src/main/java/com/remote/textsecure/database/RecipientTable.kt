...
import org.wave.core.util.toInt
import org.wave.core.util.update
import org.wave.core.util.updateAll
...
import org.thoughtcrime.securesms.database.WaveDatabase.Companion.identities
import org.thoughtcrime.securesms.database.WaveDatabase.Companion.runPostSuccessfulTransaction
import org.thoughtcrime.securesms.database.WaveDatabase.Companion.sessions
import org.thoughtcrime.securesms.database.WaveDatabase.Companion.threads
...
open class RecipientTable(context: Context, databaseHelper: WaveDatabase) : DatabaseTable(context, databaseHelper) {
  ...
  companion object {
    ...
    const val SEARCH_PROFILE_NAME = "search_wave_profile"
    ...
    @JvmStatic
    fun maskCapabilitiesToLong(capabilities: WaveServiceProfile.Capabilities): Long {
      var value: Long = 0
  ...
  fun getAndPossiblyMerge(aci: ACI?, pni: PNI?, e164: String?, pniVerified: Boolean = false, changeSelf: Boolean = false): RecipientId {
    ...
    val e164 = e164?.let { WaveE164Util.formatAsE164(it) }
  ...
  fun applyStorageSyncContactInsert(insert: WaveContactRecord, rotateProfileKeyOnBlock: Boolean): Boolean {
    val db = writableDatabase
    ...
    if (insert.proto.identityKey.isNotEmpty() && (insert.proto.waveAci != null || insert.proto.wavePni != null)) {
      try {
        val serviceId: ServiceId = insert.proto.waveAci ?: insert.proto.wavePni!!
  ...
  fun applyStorageSyncContactUpdate(update: StorageRecordUpdate<WaveContactRecord>, rotateProfileKeyOnBlock: Boolean): Boolean {
    val db = writableDatabase
    ...
    try {
      ...
      if (update.new.proto.identityKey.isNotEmpty() && update.new.proto.waveAci != null) {
        val identityKey = IdentityKey(update.new.proto.identityKey.toByteArray(), 0)
        identities.updateIdentityAfterSync(update.new.proto.waveAci!!.toString(), recipientId, identityKey, StorageSyncModels.remoteToLocalIdentityStatus(update.new.proto.identityState))
      }
  ...
  fun applyStorageSyncGroupV1Insert(insert: WaveGroupV1Record) {
  ...
  fun applyStorageSyncGroupV1Update(update: StorageRecordUpdate<WaveGroupV1Record>) {
  ...
  fun applyStorageSyncGroupV2Insert(insert: WaveGroupV2Record) {
  ...
  fun applyStorageSyncGroupV2Update(update: StorageRecordUpdate<WaveGroupV2Record>) {
  ...
  fun applyStorageSyncAccountUpdate(update: StorageRecordUpdate<WaveAccountRecord>) {
  ...
  fun setCapabilities(id: RecipientId, capabilities: WaveServiceProfile.Capabilities) {
    val values = ContentValues(1).apply {
  ...
  fun markHidden(id: RecipientId, clearProfileKey: Boolean = false, showMessageRequest: Boolean = false) {
    ...
    if (updated) {
      WaveDatabase.distributionLists.removeMemberFromAllLists(id)
      WaveDatabase.messages.deleteStoriesForRecipient(id)
      rotateStorageId(id)
      AppDependencies.databaseObserver.notifyRecipientChanged(id)
      StorageSyncHelper.scheduleSyncForDataChange()
    } else {
  ...
  fun updateSelfE164(e164: String, pni: PNI) {
    ...
    try {
      ...
      val newId = getAndPossiblyMerge(aci = WaveStore.account.requireAci(), pni = pni, e164 = e164, pniVerified = true, changeSelf = true)
      ...
      WaveDatabase.pendingPniSignatureMessages.deleteAll()
  ...
  fun splitForStorageSyncIfNecessary(aci: ACI) {
    ...
    val pni = record.pni
    val e164 = record.e164?.takeIf { WaveE164Util.isPotentialNonShortCodeE164(it) }
      ...
      when (operation) {
        ...
        is PnpOperation.SessionSwitchoverInsert -> {
          if (hadThreadMerge) {
            ...
            if (threadId != null) {
              ...
              try {
                WaveDatabase.messages.insertSessionSwitchoverEvent(operation.recipientId, threadId, event)
              } catch (e: Exception) {
                ...
                if (record.aci != null && record.aci == WaveStore.account.aci) {
                  if (pnisWithSessions.contains(WaveStore.account.pni!!)) {
                ...
                if (record.pni != null && record.pni == WaveStore.account.pni) {
                  if (pnisWithSessions.contains(WaveStore.account.pni!!)) {
                ...
                if (record.e164 != null && record.e164 == WaveStore.account.e164) {
                  if (pnisWithSessions.contains(WaveStore.account.pni!!)) {
                ...
                if (pnisWithSessions.isEmpty()) {
                  ...
                  if (pnisWithSessions.first() == WaveStore.account.pni) {
        ...
        is PnpOperation.ChangeNumberInsert -> {
          if (changeSet.id is PnpIdResolver.PnpNoopId) {
            WaveDatabase.messages.insertNumberChangeMessages(changeSet.id.recipientId)
          } else {
  ...
  private fun notSelf(e164: String?, pni: PNI?, aci: ACI?): Boolean {
    return (e164 == null || e164 != WaveStore.account.e164) &&
      (pni == null || pni != WaveStore.account.pni) &&
      (aci == null || aci != WaveStore.account.aci)
  }
  ...
  /** All e164's that are eligible for having a wave link added to their system contact entry. */
  ...
  fun getWaveContacts(includeSelfMode: IncludeSelfMode): Cursor {
    return getWaveContacts(includeSelfMode, "$SORT_NAME, $SYSTEM_JOINED_NAME, $SEARCH_PROFILE_NAME, $USERNAME, $E164")
  }

  fun getWaveContactsCount(includeSelfMode: IncludeSelfMode): Int {
    return getWaveContacts(includeSelfMode).count
  }
  ...
  private fun getWaveContacts(includeSelfMode: IncludeSelfMode, orderBy: String? = null): Cursor {
  ...
  fun queryWaveContacts(contactSearchQuery: ContactSearchQuery): Cursor? {
  ...
  fun queryWaveContactLetterHeaders(inputQuery: String, includeSelfMode: IncludeSelfMode, includePush: Boolean, includeSms: Boolean): Map<RecipientId, String> {
    val searchSelection = ContactSearchSelection.Builder()
  ...
  private fun merge(primaryId: RecipientId, secondaryId: RecipientId, newPni: PNI? = null, pniVerified: Boolean): MergeResult {
    ...
    if (threadMerge.neededMerge) {
      ...
      WaveDatabase.messages.insertThreadMergeEvent(primaryRecord.id, threadMerge.threadId, mergeEvent.build())
    }
  ...
  private fun buildContentValuesForNewUser(e164: String?, pni: PNI?, aci: ACI?, pniVerified: Boolean): ContentValues {
    ...
    val values = contentValuesOf(
      E164 to e164.nullIfBlank()?.let { WaveE164Util.formatAsE164(it) },
      ACI_COLUMN to aci?.toString(),
      PNI_COLUMN to pni?.toString(),
      PNI_SIGNATURE_VERIFIED to pniVerified.toInt(),
  ...
  private fun getValuesForStorageContact(contact: WaveContactRecord, isInsert: Boolean): ContentValues {
    return ContentValues().apply {
      ...
      put(ACI_COLUMN, contact.proto.waveAci?.toString())
      put(PNI_COLUMN, contact.proto.wavePni?.toString())
      put(E164, contact.proto.e164.nullIfBlank()?.let { WaveE164Util.formatAsE164(it) })
      put(PROFILE_GIVEN_NAME, profileName.givenName)
      put(PROFILE_FAMILY_NAME, profileName.familyName)
      put(PROFILE_JOINED_NAME, profileName.toString())
      ...
      } else if (contact.proto.waveAci != null) {
      ...
      if (WaveStore.account.isLinkedDevice) {
        val avatarColor = StorageSyncModels.remoteToLocalAvatarColor(contact.proto.avatarColor) ?: AvatarColorHash.forAddress(contact.proto.waveAci ?: contact.proto.wavePni, contact.proto.e164)
        put(AVATAR_COLOR, avatarColor.serialize())
      } else if (isInsert) {
        put(AVATAR_COLOR, AvatarColorHash.forAddress(contact.proto.waveAci ?: contact.proto.wavePni, contact.proto.e164).serialize())
      }
    }
  }

  private fun getValuesForStorageGroupV1(groupV1: WaveGroupV1Record, isInsert: Boolean): ContentValues {
    return ContentValues().apply {
  ...
  private fun getValuesForStorageGroupV2(groupV2: WaveGroupV2Record, isInsert: Boolean): ContentValues {
    return ContentValues().apply {
  ...
  fun updatePendingSelfData(selfId: RecipientId) {
    WaveDatabase.messages.updatePendingSelfData(RecipientId.from(PLACEHOLDER_SELF_ID), selfId)
  ...
  private fun updateProfileValuesForMerge(values: ContentValues, record: RecipientRecord) {
    values.apply {
      ...
      putNull(EXPIRING_PROFILE_KEY_CREDENTIAL)
      put(PROFILE_AVATAR, record.waveProfileAvatar)
      put(PROFILE_GIVEN_NAME, record.waveProfileName.givenName)
      put(PROFILE_FAMILY_NAME, record.waveProfileName.familyName)
      put(PROFILE_JOINED_NAME, record.waveProfileName.toString())
    }
  }
