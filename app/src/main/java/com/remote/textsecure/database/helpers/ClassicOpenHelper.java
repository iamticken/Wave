...
//    DecryptingQueue.schedulePendingDecrypts(context, masterSecret);
  ...
  public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
    ...
    if (oldVersion < NO_MORE_RECIPIENTS_PLURAL) {
      ...
      while (cursor != null && cursor.moveToNext()) {
        ...
        if (addressList.length == 1) {
          ...
          String        encodedGroupId = "__wave_mms_group__!" + Hex.toStringCondensed(groupId);
    ...
    if (oldVersion < PROFILES) {
      ...
      db.execSQL("ALTER TABLE recipient_preferences ADD COLUMN wave_profile_name TEXT DEFAULT NULL");
      db.execSQL("ALTER TABLE recipient_preferences ADD COLUMN wave_profile_avatar TEXT DEFAULT NULL");
    }
