...
import org.wave.ringrtc.CallLinkState.Restrictions
 ...
 */
class CallLinkTable(context: Context, databaseHelper: WaveDatabase) : DatabaseTable(context, databaseHelper), RecipientIdDatabaseReference {
  ...
  companion object {
    ...
    private fun WaveCallLinkState.serialize(): ContentValues {
  ...
  fun insertCallLink(
    ...
    val recipientId: RecipientId = writableDatabase.withinTransaction { db ->
      val recipientId = WaveDatabase.recipients.getOrInsertFromCallLinkRoomId(callLink.roomId, storageId = storageId?.raw)
  ...
  fun insertOrUpdateCallLinkByRootKey(
    ...
    writableDatabase.withinTransaction {
      ...
      if (callLink == null) {
        ...
        if (storageId != null) {
          WaveDatabase.recipients.updateStorageId(callLink.recipientId, storageId.raw)
        }
  ...
  fun getCallLinks(query: String?, offset: Int, limit: Int): List<CallLogRow.CallLink> {
    return queryCallLinks(query, offset, limit, false).readToList {
      ...
      CallLogRow.CallLink(
        ...
        recipient = peer,
        searchQuery = query,
        callLinkPeekInfo = AppDependencies.waveCallManager.peekInfoSnapshot[peer.id]
      )
    }
  }
  ...
  fun markRevoked(
    ...
    writableDatabase.withinTransaction { db ->
      ...
      WaveDatabase.calls.updateAdHocCallEventDeletionTimestamps()
      val recipient = WaveDatabase.recipients.getByCallLinkRoomId(roomId)

      if (recipient.isPresent) {
        WaveDatabase.recipients.markNeedsSync(recipient.get())
      }
    }
  }
  ...
  fun deleteNonAdminCallLinksOnOrBefore(timestamp: Long) {
    writableDatabase.withinTransaction { db ->
      ...
      WaveDatabase.calls.updateAdHocCallEventDeletionTimestamps(skipSync = true)
    }
  }
