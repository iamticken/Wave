...
class MediaTable internal constructor(context: Context?, databaseHelper: WaveDatabase?) : DatabaseTable(context, databaseHelper) {
  ...
  companion object {
    ...
    private val ALL_MEDIA_QUERY = String.format(
      ...
      """
        ${AttachmentTable.DATA_FILE} IS NOT NULL AND
        ${AttachmentTable.CONTENT_TYPE} NOT LIKE 'text/x-wave-plain' AND
        ${MessageTable.LINK_PREVIEWS} IS NULL AND
        ${MessageTable.SCHEDULED_DATE} < 0
      """
    )
  ...
  data class MediaRecord constructor(
    ...
    companion object {
      ...
      fun from(cursor: Cursor): MediaRecord {
        val attachments = WaveDatabase.attachments.getAttachments(cursor)
