...
data class SentStorySyncManifest(
  ...
  fun toRecipientsSet(): Set<WaveServiceStoryMessageRecipient> {
    ...
    return recipients
      ...
      .map { recipient ->
        ...
        WaveServiceStoryMessageRecipient(
          WaveServiceAddress(serviceId),
  ...
  companion object {
    @WorkerThread
    @JvmStatic
    fun fromRecipientsSet(recipientsSet: Set<WaveServiceStoryMessageRecipient>): SentStorySyncManifest {
      val entries = recipientsSet.map { recipient ->
        Entry(
          recipientId = RecipientId.from(recipient.waveServiceAddress),
