...
class RemappedRecords {
  ...
  void deleteThread(long oldId) {
    ...
    WaveDatabase.remappedRecords().deleteThreadMapping(oldId);
  }
  ...
  void addRecipient(@NonNull RecipientId oldId, @NonNull RecipientId newId) {
    ...
    WaveDatabase.remappedRecords().addRecipientMapping(oldId, newId);
  }
  ...
  void addThread(long oldId, long newId) {
    ...
    WaveDatabase.remappedRecords().addThreadMapping(oldId, newId);
  }
  ...
  private void ensureRecipientMapIsPopulated() {
    if (recipientMap == null) {
      recipientMap = WaveDatabase.remappedRecords().getAllRecipientMappings();
    }
  }

  private void ensureThreadMapIsPopulated() {
    if (threadMap == null) {
      threadMap = WaveDatabase.remappedRecords().getAllThreadMappings();
    }
  }

  private void ensureInTransaction() {
    if (!WaveDatabase.inTransaction()) {
