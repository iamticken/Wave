 ...
 */
class PendingPniSignatureMessageTable(context: Context, databaseHelper: WaveDatabase) : DatabaseTable(context, databaseHelper), RecipientIdDatabaseReference {
    ...
    pendingPniSignaturesCacheLock.writeLock().withLock {
      writableDatabase.withinTransaction { db ->
        ...
        if (!stillPending) {
          ...
          WaveDatabase.recipients.clearNeedsPniSignature(recipientId)
