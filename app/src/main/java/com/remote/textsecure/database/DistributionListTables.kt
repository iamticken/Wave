...
import org.whispersystems.waveservice.api.storage.recipientServiceAddresses
import java.util.UUID

/**
 * Stores distribution lists, which represent different sets of people you may want to share a story with.
 */
class DistributionListTables constructor(context: Context?, databaseHelper: WaveDatabase?) : DatabaseTable(context, databaseHelper), RecipientIdDatabaseReference {
  ...
  fun createList(
    ...
    try {
      ...
      val recipientId = WaveDatabase.recipients.getOrInsertFromDistributionListId(DistributionListId.from(id), storageId)
    ...
    return when (privacyMode) {
      DistributionListPrivacyMode.ALL -> {
        WaveDatabase.recipients
          .getWaveContacts(RecipientTable.IncludeSelfMode.Exclude)
  ...
  fun getPrivacyData(listId: DistributionListId): DistributionListPrivacyData {
    ...
    readableDatabase.withinTransaction {
      ...
      totalContactCount = WaveDatabase.recipients.getWaveContactsCount(RecipientTable.IncludeSelfMode.Exclude)
    }
  ...
  fun getRecipientIdForSyncRecord(record: WaveStoryDistributionListRecord): RecipientId? {
  ...
  fun applyStorageSyncStoryDistributionListInsert(insert: WaveStoryDistributionListRecord) {
  ...
  fun applyStorageSyncStoryDistributionListUpdate(update: StorageRecordUpdate<WaveStoryDistributionListRecord>) {
    ...
    WaveDatabase.recipients.updateStorageId(recipientId, update.new.id.raw)
