...
import org.wave.core.util.IntSerializer
import org.wave.core.util.Serializer
...
import org.wave.core.util.flatten
 ...
 */
class CallTable(context: Context, databaseHelper: WaveDatabase) : DatabaseTable(context, databaseHelper), RecipientIdDatabaseReference {
  ...
  fun insertOneToOneCall(callId: Long, timestamp: Long, peer: RecipientId, type: Type, direction: Direction, event: Event) {
    ...
    writableDatabase.withinTransaction {
      val result = WaveDatabase.messages.insertCallLog(peer, messageType, timestamp, direction == Direction.OUTGOING)
    ...
    return writableDatabase.withinTransaction {
      ...
      if (call != null) {
        ...
        if (call.messageId == null) {
          ...
          WaveDatabase.messages.updateCallLog(call.messageId, call.messageType)
        }
  ...
  fun markCallDeletedFromSyncEvent(call: Call) {
    ...
    writableDatabase.withinTransaction { db ->
      ...
      if (call.messageId != null) {
        WaveDatabase.messages.deleteMessage(call.messageId)
      }
    }
  ...
  fun insertAcceptedGroupCall(
    ...
    writableDatabase.withinTransaction { db ->
      val messageId: MessageId? = if (type == Type.GROUP_CALL) {
        WaveDatabase.messages.insertGroupCall(
  ...
  private fun handleGroupUpdate(
    ...
    writableDatabase.withinTransaction {
      ...
      val messageId: MessageId = if (call != null) {
        ...
        WaveDatabase.messages.updateGroupCall(
          call.messageId,
          peekGroupCallEraId,
          peekJoinedUuids,
          isCallFull,
  ...
  fun updateGroupCallFromPeek(
    ...
    val recipientId = WaveDatabase.threads.getRecipientIdForThreadId(threadId)
    ...
    WaveDatabase.messages.updatePreviousGroupCall(
      threadId = threadId,
  ...
  private fun createEventFromRingState(
    ...
    writableDatabase.withinTransaction { db ->
      val messageId = WaveDatabase.messages.insertGroupCall(
  ...
  fun setTimestamp(callId: Long, recipientId: RecipientId, timestamp: Long) {
    writableDatabase.withinTransaction { db ->
      ...
      if (call.messageId != null) {
        WaveDatabase.messages.updateCallTimestamps(call.messageId, timestamp)
      }
    }
  ...
  fun deleteNonAdHocCallEvents(callRowIds: Set<Long>) {
    ...
    WaveDatabase.messages.deleteCallUpdates(messageIds)
  ...
  fun markRingingCallsAsMissed() {
    writableDatabase.withinTransaction { db ->
      ...
      WaveDatabase.messages.clearIsRingingOnLocalDeviceFlag(messageIds)
    }
  }
