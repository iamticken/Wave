 ...
 */
class SenderKeySharedTable internal constructor(context: Context?, databaseHelper: WaveDatabase?) : DatabaseTable(context, databaseHelper) {
  companion object {
...
/**
   ...
   */
  fun markAsShared(distributionId: DistributionId, addresses: Collection<WaveProtocolAddress>) {
   ...
   */
  fun getSharedWith(distributionId: DistributionId): Set<WaveProtocolAddress> {
   ...
   */
  fun delete(distributionId: DistributionId, addresses: Collection<WaveProtocolAddress>) {
    writableDatabase.withinTransaction { db ->
      for (address in addresses) {
   ...
   */
  fun deleteAllFor(addresses: Collection<WaveProtocolAddress>) {
    writableDatabase.withinTransaction { db ->
      for (address in addresses) {
        db.delete(TABLE_NAME)
