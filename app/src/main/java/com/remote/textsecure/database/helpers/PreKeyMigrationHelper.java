  ...
  public static boolean migratePreKeys(Context context, SQLiteDatabase database) {
    ...
    if (oneTimePreKeyIndex.exists()) {
      try {
        ...
        WaveStore.account().aciPreKeys().setNextEcOneTimePreKeyId(index.nextPreKeyId);
      } catch (IOException e) {
        Log.w(TAG, e);
      }
    }
    ...
    if (signedPreKeyIndex.exists()) {
      try {
        ...
        WaveStore.account().aciPreKeys().setNextSignedPreKeyId(index.nextSignedPreKeyId);
        WaveStore.account().aciPreKeys().setActiveSignedPreKeyId(index.activeSignedPreKeyId);
      } catch (IOException e) {
        Log.w(TAG, e);
      }
    }
