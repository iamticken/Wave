...
class NameCollisionTables(
  ...
  fun markCollisionsForThreadRecipientDismissed(threadRecipientId: RecipientId) {
    writableDatabase.withinTransaction { db ->
      val threadId = WaveDatabase.threads.getThreadIdFor(threadRecipientId) ?: return@withinTransaction
  ...
  fun getCollisionsForThreadRecipientId(recipientId: RecipientId): List<ReviewRecipient> {
    val threadId = WaveDatabase.threads.getThreadIdFor(recipientId) ?: return emptyList()
    ...
    val groupMembers: Optional<List<RecipientId>> = WaveDatabase.groups.getGroup(recipientId).map { it.members }
  ...
  fun handleIndividualNameCollision(recipientId: RecipientId) {
    writableDatabase.withinTransaction { db ->
      val similarRecipients = WaveDatabase.recipients.getSimilarRecipientIds(Recipient.resolved(recipientId))
      ...
      if (similarRecipients.size == 1) {
        val threadId = WaveDatabase.threads.getThreadIdFor(recipientId) ?: -1
        if (threadId > 0L) {
  ...
  fun handleGroupNameCollisions(groupId: GroupId.V2, changed: Set<RecipientId>) {
    writableDatabase.withinTransaction {
      val threadRecipientId = WaveDatabase.recipients.getByGroupId(groupId).orNull() ?: return@withinTransaction
      handleNameCollisions(
        threadRecipientId = threadRecipientId,
  ...
  private fun collisionExists(threadRecipientId: RecipientId): Boolean {
    val threadId = WaveDatabase.threads.getThreadIdFor(threadRecipientId) ?: return false
  ...
  private fun getDuplicatedGroupRecipients(groupId: V2, toCheck: Set<RecipientId>): List<ReviewRecipient> {
    ...
    val members: MutableList<Recipient> = WaveDatabase.groups.getGroupMembers(groupId, GroupTable.MemberSet.FULL_MEMBERS_INCLUDING_SELF).toMutableList()
  ...
  private fun getProfileChangeRecordsForGroup(groupId: V2): List<MessageRecord> {
    val groupRecipientId = WaveDatabase.recipients.getByGroupId(groupId).get()
    val groupThreadId = WaveDatabase.threads.getThreadIdFor(groupRecipientId)

    return if (groupThreadId == null) {
      emptyList()
    } else {
      WaveDatabase.messages.getProfileChangeDetailsRecords(
        groupThreadId,
