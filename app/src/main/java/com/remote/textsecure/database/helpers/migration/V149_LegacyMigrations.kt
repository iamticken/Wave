 ...
 */
@Suppress("ClassName")
object V149_LegacyMigrations : WaveDatabaseMigration {

  private val TAG: String = WaveDatabaseMigrations.TAG
  ...
  override fun migrate(context: Application, db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
    ...
    if (oldVersion < NOTIFICATION_CHANNELS && !SqlUtil.columnExists(db, "recipient_preferences", "notification_channel")) {
      ...
      db.rawQuery("SELECT recipient_ids, system_display_name, wave_profile_name, notification, vibrate FROM recipient_preferences WHERE notification NOT NULL OR vibrate != 0", null).use { cursor ->
        while (cursor != null && cursor.moveToNext()) {
          val rawAddress: String = cursor.getString(cursor.getColumnIndexOrThrow("recipient_ids"))
          val address: String = WaveE164Util.formatAsE164(rawAddress) ?: ""
          val systemName: String = cursor.getString(cursor.getColumnIndexOrThrow("system_display_name"))
          val profileName: String = cursor.getString(cursor.getColumnIndexOrThrow("wave_profile_name"))
          ...
          val vibrateEnabled: Boolean = if (vibrateState == 0) WaveStore.settings.isMessageVibrateEnabled else vibrateState == 1
    ...
    if (oldVersion < STORAGE_SERVICE_ACTIVE) {
      ...
      db.execSQL("UPDATE recipient set group_type = 1 WHERE group_id NOT NULL AND group_id LIKE '__wave_mms_group__%'")
    ...
    if (oldVersion < PROFILE_DATA_MIGRATION) {
      ...
      if (localNumber != null) {
        ...
        db.execSQL("UPDATE recipient SET wave_profile_name = ?, profile_family_name = ?, profile_joined_name = ? WHERE phone = ?", arrayOf(profileName.givenName, profileName.familyName, profileName.toString(), localNumber))
      }
    }
    ...
    if (oldVersion < AVATAR_LOCATION_MIGRATION) {
      ...
      db.execSQL("UPDATE recipient SET wave_profile_avatar = NULL")
