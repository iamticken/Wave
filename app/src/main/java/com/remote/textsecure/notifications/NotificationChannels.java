...
public class NotificationChannels {
  ...
  public synchronized void restoreContactNotificationChannels() {
    ...
    RecipientTable db = WaveDatabase.recipients();
  ...
  public synchronized @Nullable String createChannelFor(@NonNull Recipient recipient) {
    ...
    boolean      vibrationEnabled = vibrateState == VibrateState.DEFAULT ? WaveStore.settings().isMessageVibrateEnabled() : vibrateState == VibrateState.ENABLED;
                                                        ...
                                                        @Nullable String shortcutId)
  {
    ...
    setLedPreference(channel, WaveStore.settings().getMessageLedColor());
  ...
  public synchronized void updateMessageRingtone(@NonNull Recipient recipient, @Nullable Uri uri) {
    ...
    WaveDatabase.recipients().setNotificationChannel(recipient.getId(), success ? newChannelId : null);
    ensureCustomChannelConsistency();
  }
  ...
  public boolean updateWithShortcutBasedChannel(@NonNull Recipient recipient) {
    if (Build.VERSION.SDK_INT >= CONVERSATION_SUPPORT_VERSION && TextUtils.isEmpty(recipient.getNotificationChannel())) {
      ...
      if (channel.isPresent()) {
        ...
        WaveDatabase.recipients().setNotificationChannel(recipient.getId(), channel.get().getId());
        return true;
      }
    }
    return false;
  ...
  private void onCreate(@NonNull NotificationManager notificationManager) {
    ...
    NotificationChannel joinEvents                     = new NotificationChannel(JOIN_EVENTS, context.getString(R.string.NotificationChannel_contact_joined_wave), NotificationManager.IMPORTANCE_DEFAULT);
    ...
    setVibrationEnabled(messages, WaveStore.settings().isMessageVibrateEnabled());
    messages.setSound(WaveStore.settings().getMessageNotificationSound(), getRingtoneAudioAttributes());
    setLedPreference(messages, WaveStore.settings().getMessageLedColor());
  ...
  private void updateAllRecipientChannelLedColors(@NonNull NotificationManager notificationManager, @NonNull String color) {
    RecipientTable database = WaveDatabase.recipients();
