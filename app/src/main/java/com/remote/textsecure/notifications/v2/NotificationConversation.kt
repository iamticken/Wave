...
data class NotificationConversation(
  ...
  fun getSlideBigPictureUri(context: Context): Uri? {
    return if (notificationItems.size == 1 && WaveStore.settings.messageNotificationsPrivacy.isDisplayMessage && !KeyCachingService.isLocked(context)) {
  ...
  fun getContentText(context: Context): CharSequence? {
    val privacy: NotificationPrivacyPreference = WaveStore.settings.messageNotificationsPrivacy
  ...
  fun getConversationTitle(context: Context): CharSequence? {
    if (WaveStore.settings.messageNotificationsPrivacy.isDisplayContact) {
      return if (isGroup) getDisplayName(context) else null
    }
    return context.getString(R.string.SingleRecipientNotificationBuilder_wave)
  }
