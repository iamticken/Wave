...
data class FontVersion(val id: Long, val path: String) {
  ...
  companion object {
    ...
    fun get(context: Context): FontVersion {
      ...
      val version: FontVersion = if (System.currentTimeMillis() - WaveStore.story.lastFontVersionCheck > VERSION_CHECK_INTERVAL) {
        ...
        if (fromDisk == null && fromNetwork == null) {
          ...
          if (fromDisk.id < fromNetwork.id) {
            ...
            WaveStore.story.lastFontVersionCheck = System.currentTimeMillis()
            fromDisk
          }
        } else {
