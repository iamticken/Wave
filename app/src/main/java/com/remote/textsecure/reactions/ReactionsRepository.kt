...
class ReactionsRepository {
  ...
  private fun fetchReactionDetails(messageId: MessageId): List<ReactionDetails> {
    val reactions: List<ReactionRecord> = WaveDatabase.reactions.getReactions(messageId)
  ...
  fun sendReactionRemoval(messageId: MessageId) {
    val oldReactionRecord = WaveDatabase.reactions.getReactions(messageId).firstOrNull { it.author == Recipient.self().id } ?: return
