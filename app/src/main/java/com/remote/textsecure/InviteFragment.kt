...
/**
 * Fragment when inviting someone to use Wave
 */
class InviteFragment : ComposeFragment() {
  ...
  private fun onShare(inviteText: String) {
    ...
    if (sendIntent.resolveActivity(requireContext().packageManager) != null) {
      startActivity(Intent.createChooser(sendIntent, getString(R.string.InviteActivity_invite_to_wave)))
    } else {
...
fun InviteScreen(
  ...
  val default = stringResource(R.string.InviteActivity_lets_switch_to_wave, stringResource(R.string.install_url))
