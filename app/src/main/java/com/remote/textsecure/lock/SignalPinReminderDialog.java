...
public final class WavePinReminderDialog {

  private static final String TAG = Log.tag(WavePinReminderDialog.class);
  ...
  public static void show(@NonNull Context context, @NonNull Launcher launcher, @NonNull Callback mainCallback) {
    ...
    AlertDialog dialog = new MaterialAlertDialogBuilder(context, R.style.ThemeOverlay_Wave_MaterialAlertDialog_Wide)
    ...
    WaveStore.pin().getKeyboardType().applyInputTypeTo(pinEditText);
    ...
    pinEditText.addTextChangedListener(new SimpleTextWatcher() {
      ...
      private final String localHash = Objects.requireNonNull(WaveStore.svr().getLocalPinHash());
  ...
  private static final class V2PinVerifier implements PinVerifier {
    ...
    V2PinVerifier() {
      localPinHash = WaveStore.svr().getLocalPinHash();
