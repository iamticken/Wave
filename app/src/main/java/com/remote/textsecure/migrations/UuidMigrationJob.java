...
public class UuidMigrationJob extends MigrationJob {
  ...
  private static void ensureSelfRecipientExists(@NonNull Context context) {
    WaveDatabase.recipients().getOrInsertFromE164(Objects.requireNonNull(WaveStore.account().getE164()));
  }

  private static void fetchOwnUuid(@NonNull Context context) throws IOException {
    RecipientId self      = Recipient.self().getId();
    ACI         localUuid = ACI.parseOrNull(AppDependencies.getWaveServiceAccountManager().getWhoAmI().getAci());

    if (localUuid == null) {
      throw new IOException("Invalid UUID!");
    }

    WaveDatabase.recipients().markRegisteredOrThrow(self, localUuid);
    WaveStore.account().setAci(localUuid);
  }
