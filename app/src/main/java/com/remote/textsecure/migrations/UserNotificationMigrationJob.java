...
public class UserNotificationMigrationJob extends MigrationJob {
  ...
  void performMigration() {
    if (!WaveStore.account().isRegistered()   ||
        WaveStore.account().getE164() == null ||
        WaveStore.account().getAci() == null)
    {
      Log.w(TAG, "Not registered! Skipping.");
      return;
    }

    if (!WaveStore.settings().isNotifyWhenContactJoinsWave()) {
    ...
    Set<RecipientId>  registered               = WaveDatabase.recipients().getRegistered();
    List<RecipientId> systemContacts           = WaveDatabase.recipients().getSystemContacts();
    ...
    String message = context.getResources().getQuantityString(R.plurals.UserNotificationMigrationJob_d_contacts_are_on_wave,
