...
public class ApplyUnknownFieldsToSelfMigrationJob extends MigrationJob {
  ...
  public void performMigration() {
    ...
    try {
      self     = Recipient.self();
      settings = WaveDatabase.recipients().getRecordForSync(self.getId());
    ...
    try {
      ...
      WaveAccountRecord waveAccountRecord = new WaveAccountRecord(storageId, accountRecord);

      Log.d(TAG, "Applying potentially now known unknowns");
      StorageSyncHelper.applyAccountStorageSyncUpdates(context, self, waveAccountRecord, false);
    } catch (IOException e) {
      Log.w(TAG, e);
    }
  }
