...
internal class E164FormattingMigrationJob(
  ...
  companion object {
    ...
    fun fixE164Formatting() {
      ...
      val e164sByRecipientId = WaveDatabase.recipients.getAllE164sByRecipientId()
      val invalidE164s: MutableList<RecipientId> = mutableListOf()
      stopwatch.split("fetch")

      for ((id, e164) in e164sByRecipientId) {
        val formattedE164: String? = WaveE164Util.formatNonShortCodeAsE164(e164)
        ...
        try {
          ...
          WaveDatabase.recipients.setE164(id, formattedE164)
          ...
          val existing: Optional<RecipientId> = WaveDatabase.recipients.getByE164(formattedE164)
          if (existing.isPresent) {
            Log.w(TAG, "Merging ${existing.get()} and $id", true)
            WaveDatabase.rawDatabase.withinTransaction {
              WaveDatabase.recipients.mergeForMigration(existing.get(), id)
            }
    ...
    private fun attemptToGetRidOfE164(entries: List<RecipientId>): Int {
      ...
      for (id in entries) {
        if (WaveDatabase.recipients.removeE164IfAnotherIdentifierIsPresent(id)) {
        ...
        if (WaveDatabase.recipients.deleteRecipientIfItHasNoMessages(id)) {
