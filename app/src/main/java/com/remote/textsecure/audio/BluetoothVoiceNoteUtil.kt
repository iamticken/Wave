...
import org.thoughtcrime.securesms.webrtc.audio.WaveAudioHandler
...
private class BluetoothVoiceNoteUtilLegacy(val context: Context, val listener: (Boolean) -> Unit, val bluetoothPermissionDeniedHandler: () -> Unit) : BluetoothVoiceNoteUtil {
  private val commandAndControlThread: HandlerThread = WaveExecutors.getAndStartHandlerThread("voice-note-audio", ThreadUtil.PRIORITY_IMPORTANT_BACKGROUND_THREAD)
  private val uiThreadHandler = Handler(context.mainLooper)
  private val audioHandler: WaveAudioHandler = WaveAudioHandler(commandAndControlThread.looper)
  private val deviceUpdatedListener: AudioDeviceUpdatedListener = object : AudioDeviceUpdatedListener {
    override fun onAudioDeviceUpdated() {
      when (waveBluetoothManager.state) {
        WaveBluetoothManager.State.CONNECTED -> {
          Log.d(TAG, "Bluetooth SCO connected. Starting voice note recording on UI thread.")
          uiThreadHandler.post { listener(true) }
        }
        WaveBluetoothManager.State.ERROR,
        WaveBluetoothManager.State.PERMISSION_DENIED -> {
          Log.w(TAG, "Unable to complete Bluetooth connection due to ${waveBluetoothManager.state}. Starting voice note recording anyway on UI thread.")
          uiThreadHandler.post { listener(false) }
        }
        else -> Log.d(TAG, "Current Bluetooth connection state: ${waveBluetoothManager.state}.")
      }
    }
  }
  private val waveBluetoothManager: WaveBluetoothManager = WaveBluetoothManager(context, deviceUpdatedListener, audioHandler)
  ...
  override fun connectBluetoothScoConnection() {
    audioHandler.post {
      ...
      val currentState = waveBluetoothManager.state
      if (currentState == WaveBluetoothManager.State.AVAILABLE) {
        Log.d(TAG, "Bluetooth manager state is AVAILABLE. Starting SCO connection.")
        waveBluetoothManager.startScoAudio()
      } else {
        Log.d(TAG, "Recording from phone mic because bluetooth state was " + currentState + ", not " + WaveBluetoothManager.State.AVAILABLE)
        uiThreadHandler.post {
          if (currentState == WaveBluetoothManager.State.PERMISSION_DENIED && !hasWarnedAboutBluetooth) {
  ...
  override fun disconnectBluetoothScoConnection() {
    audioHandler.post {
      if (waveBluetoothManager.state == WaveBluetoothManager.State.CONNECTED) {
