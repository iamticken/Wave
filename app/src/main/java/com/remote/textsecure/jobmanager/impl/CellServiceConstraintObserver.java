...
public class CellServiceConstraintObserver implements ConstraintObserver {
  ...
  private CellServiceConstraintObserver(@NonNull Application application) {
    ...
    if (Build.VERSION.SDK_INT >= 31) {
      telephonyManager.registerTelephonyCallback(WaveExecutors.BOUNDED, new ServiceStateListenerApi31());
    } else {
      HandlerThread handlerThread = WaveExecutors.getAndStartHandlerThread("CellServiceConstraintObserver", Thread.NORM_PRIORITY);
