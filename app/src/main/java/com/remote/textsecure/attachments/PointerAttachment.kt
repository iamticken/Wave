...
class PointerAttachment : Attachment {
  ...
  companion object {
    @JvmStatic
    fun forPointers(pointers: Optional<List<WaveServiceAttachment>>): List<Attachment> {
    ...
    @JvmOverloads
    fun forPointer(
      pointer: Optional<WaveServiceAttachment>,
    ...
    fun forPointer(quotedAttachment: DataMessage.Quote.QuotedAttachment): Optional<Attachment> {
      val thumbnail: WaveServiceAttachment? = try {
        if (quotedAttachment.thumbnail != null) {
          AttachmentPointerUtil.createWaveAttachmentPointer(quotedAttachment.thumbnail)
        } else {
          null
        }
