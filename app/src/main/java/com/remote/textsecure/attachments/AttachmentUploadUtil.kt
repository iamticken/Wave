...
object AttachmentUploadUtil {
  ...
  /**
   * Builds a [WaveServiceAttachmentStream] from the provided data, which can then be provided to various upload methods.
   */
  @Throws(IOException::class)
  fun buildWaveServiceAttachmentStream(
    context: Context,
    attachment: Attachment,
    uploadSpec: ResumableUpload,
    cancellationWave: (() -> Boolean)? = null,
    progressListener: ProgressListener? = null
  ): WaveServiceAttachmentStream {
    val inputStream = PartAuthority.getAttachmentStream(context, attachment.uri!!)
    val builder = WaveServiceAttachment.newStreamBuilder()
      ...
      .withCancelationWave(cancellationWave)
