...
public final class GroupManager {
      ...
      throws GroupChangeBusyException, GroupChangeFailedException, IOException
  {
    ...
    WaveDatabase.recipients().getByGroupId(groupId).ifPresent(id -> WaveDatabase.messages().deleteScheduledMessages(id));
  }
  ...
  {
    if (!WaveDatabase.groups().groupExists(groupId)) {
      ...
      throws GroupChangeBusyException, GroupChangeFailedException, GroupNotAMemberException, GroupInsufficientRightsException, IOException
  {
    try (GroupManagerV2.GroupEditor editor = new GroupManagerV2(context).edit(groupId.requireV2())) {
      editor.acceptInvite();
      WaveDatabase.groups().setActive(groupId, true);
    }
  }
  ...
  {
    GroupTable.V2GroupProperties groupProperties = WaveDatabase.groups().requireGroup(groupId).requireV2GroupProperties();
