...
import org.whispersystems.waveservice.api.groupsv2.GroupHistoryPage
...
class GroupsV2StateProcessor private constructor(
  ...
  private val groupsApi = AppDependencies.waveServiceAccountManager.getGroupsV2Api()
  ...
  fun updateGroupSendEndorsements() {
    ...
    if (receivedGroupSendEndorsements != null) {
      ...
      WaveDatabase.groups.updateGroupSendEndorsements(groupId, receivedGroupSendEndorsements)
    } else {
  ...
  fun updateLocalGroupToRevision(
    ...
    groupRecord: Optional<GroupRecord> = WaveDatabase.groups.getGroup(groupId),
  ...
  private fun canApplyP2pChange(
    ...
    if (WaveStore.internal.gv2IgnoreP2PChanges) {
      ...
      if (groupRecord.map { it.isActive }.orNull() == false && updatedGroupState != null && updatedGroupState == remoteGroupStateDiff.previousGroupState) {
        ...
        if (currentLocalState != null && applyGroupStateDiffResult.remainingRemoteGroupChanges.isEmpty()) {
          ...
          if (endorsements != null) {
            ...
            WaveDatabase.groups.updateGroupSendEndorsements(groupId, endorsements)
          }
        }
  ...
  private fun localIsAtLeast(groupRecord: Optional<GroupRecord>, revision: Int): Boolean {
    if (revision == LATEST || groupRecord.isEmpty || WaveDatabase.groups.isUnknownGroup(groupRecord)) {
      return false
    }
  ...
  private fun saveGroupUpdate(
    ...
    if (forceSave && updatedGroupState != null) {
      ...
      if (groupSendEndorsements != null) {
        ...
        WaveDatabase.groups.updateGroupSendEndorsements(groupId, groupSendEndorsements)
      }
  ...
  private fun saveGroupState(groupStateDiff: GroupStateDiff, updatedGroupState: DecryptedGroup, groupSendEndorsements: ReceivedGroupSendEndorsements?) {
    ...
    val needsAvatarFetch = if (previousGroupState == null) {
      val groupId = WaveDatabase.groups.create(groupMasterKey, updatedGroupState, groupSendEndorsements)

      if (groupId == null) {
        Log.w(TAG, "$logPrefix Group create failed, trying to update")
        WaveDatabase.groups.update(groupMasterKey, updatedGroupState, groupSendEndorsements)
      }
  ...
  internal class ProfileAndMessageHelper(private val aci: ACI, private val masterKey: GroupMasterKey, private val groupId: GroupId.V2) {
      ...
      if (selfAsMember != null) {
        ...
        if (addedBy != null) {
          ...
          if (addedBy.isBlocked && (previousGroupState == null || !DecryptedGroupUtil.isRequesting(previousGroupState, aci))) {
            ...
            WaveDatabase.recipients.setProfileSharing(Recipient.externalGroupExact(groupId).id, true)
            if (needsAvatarFetch) {
    ...
    fun leaveGroupLocally(serviceIds: ServiceIds) {
      if (!WaveDatabase.groups.isActive(groupId)) {
        Log.w(TAG, "Group $groupId has already been left.")

        val groupRecipient = Recipient.externalGroupExact(groupId)
        val threadId = WaveDatabase.threads.getThreadIdFor(groupRecipient.id)

        if (threadId != null) {
          WaveDatabase.drafts.clearDrafts(threadId)
          WaveDatabase.threads.update(threadId, unarchive = false, allowDeletion = false)
        }
      ...
      val decryptedGroup = WaveDatabase
        .groups
      ...
      try {
        val threadId = WaveDatabase.threads.getOrCreateThreadIdFor(groupRecipient)
        val id = WaveDatabase.messages.insertMessageOutbox(leaveMessage, threadId, false, null).messageId
      ...
      WaveDatabase.groups.setActive(groupId, false)
      WaveDatabase.groups.remove(groupId, Recipient.self().id)
    }
    ...
    fun persistLearnedProfileKeys(profileKeys: ProfileKeySet) {
      val updated = WaveDatabase.recipients.persistProfileKeySet(profileKeys)
    ...
    fun storeMessage(decryptedGroupV2Context: DecryptedGroupV2Context, timestamp: Long, serverGuid: String?) {
      ...
      val serviceIds = WaveStore.account.getServiceIds()
      ...
      if (outgoing) {
        ...
        try {
          val recipientId = WaveDatabase.recipients.getOrInsertFromGroupId(groupId)
          ...
          val messageId = WaveDatabase.messages.insertMessageOutbox(outgoingMessage, threadId, false, null).messageId
        ...
        try {
          ...
          val insertResult = WaveDatabase.messages.insertMessageInbox(groupMessage)

          if (insertResult.isPresent) {
            WaveDatabase.threads.update(insertResult.get().threadId, unarchive = false, allowDeletion = false)
