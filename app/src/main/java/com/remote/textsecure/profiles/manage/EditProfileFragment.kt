...
class EditProfileFragment : LoggingFragment() {
  ...
  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    ...
    binding.manageProfileUsernameContainer.setOnClickListener { v: View ->
      if (!viewModel.isRegisteredAndUpToDate) {
        onClickWhenUnregisteredOrDeprecated()
      } else if (WaveStore.account.username != null) {
        MaterialAlertDialogBuilder(requireContext(), R.style.ThemeOverlay_Wave_MaterialAlertDialog_List)
  ...
  private fun presentUsername(username: String?) {
    ...
    if (WaveStore.account.usernameSyncState == AccountValues.UsernameSyncState.USERNAME_AND_LINK_CORRUPTED) {
      binding.usernameErrorIndicator.visibility = View.VISIBLE
    } else {
      binding.usernameErrorIndicator.visibility = View.GONE
    }

    if (WaveStore.account.username != null && WaveStore.account.usernameSyncState != AccountValues.UsernameSyncState.USERNAME_AND_LINK_CORRUPTED) {
      ...
      if (WaveStore.tooltips.showProfileSettingsQrCodeTooltop()) {
        ...
        binding.linkTooltipCloseButton.setOnClickListener {
          ...
          WaveStore.tooltips.markProfileSettingsQrCodeTooltipSeen()
        }
      }
  ...
  private fun displayConfirmUsernameDeletionDialog() {
    MaterialAlertDialogBuilder(requireContext())
      ...
      .setMessage(requireContext().getString(R.string.ManageProfileFragment__delete_username_dialog_body, WaveStore.account.username))
