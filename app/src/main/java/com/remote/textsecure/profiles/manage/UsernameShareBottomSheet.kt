...
class UsernameShareBottomSheet : DSLSettingsBottomSheetFragment() {
  ...
  private fun getConfiguration(recipient: Recipient): DSLConfiguration {
    return configure {
      noPadTextPref(
        title = DSLSettingsText.from(
          ...
          DSLSettingsText.ColorModifier(
            ContextCompat.getColor(requireContext(), R.color.wave_colorOnSurfaceVariant)
          )
        )
      )
      ...
      customPref(
        CopyButton.Model(
          text = getString(R.string.wave_me_username_url, Base64.encodeUrlSafeWithoutPadding(Username(username).hash)),
          onClick = {
            copyToClipboard(it)
          }
        )
