...
class ReviewCardViewHolder extends RecyclerView.ViewHolder {
  ...
  private final Runnable                        onWaveConnectionClicked;
                              ...
                              @NonNull Callbacks callbacks)
  {
    ...
    onWaveConnectionClicked = callbacks::onWaveConnectionClicked;
  }
  ...
  private void presentReviewTextRows(@NonNull List<ReviewTextRow> reviewTextRows, @NonNull Context context, @NonNull ReviewCard reviewCard) {
    ...
    for (int i = 0; i < Math.min(reviewTextRows.size(), subtextGroups.size()); i++) {
      ...
      switch (row) {
        case SIGNAL_CONNECTION -> presentWaveConnection(group.getFirst(), group.getSecond(), context, reviewCard);
  ...
  private void presentWaveConnection(@NonNull TextView line, @NonNull ImageView icon, @NonNull Context context, @NonNull ReviewCard reviewCard) {
    ...
    SpannableStringBuilder builder = new SpannableStringBuilder(context.getString(R.string.AboutSheet__wave_connection));
    ...
    line.setOnClickListener(v -> onWaveConnectionClicked.run());
  }
