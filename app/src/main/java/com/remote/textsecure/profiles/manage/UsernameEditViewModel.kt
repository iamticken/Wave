...
internal class UsernameEditViewModel private constructor(private val mode: UsernameEditMode) : ViewModel() {
  ...
  private val uiState: RxStore<State> = RxStore(
    defaultValue = State(
      ...
      usernameState = WaveStore.account.username?.let { UsernameState.Set(Username(it)) } ?: UsernameState.NoUsername
    ),
  ...
  private val stateMachineStore = RxStore<UsernameEditStateMachine.State>(
    defaultValue = UsernameEditStateMachine.NoUserEntry(
      nickname = WaveStore.account.username?.split(Usernames.DELIMITER)?.first() ?: "",
      discriminator = WaveStore.account.username?.split(Usernames.DELIMITER)?.last() ?: "",
  ...
  init {
    ...
    if (mode == UsernameEditMode.RECOVERY) {
      onNicknameUpdated(WaveStore.account.username?.split(Usernames.DELIMITER)?.first() ?: "")
      onDiscriminatorUpdated(WaveStore.account.username?.split(Usernames.DELIMITER)?.last() ?: "")
    }
  }
  ...
  fun onNicknameUpdated(nickname: String) {
    uiState.update { state: State ->
      if (nickname.isBlank() && WaveStore.account.username != null) {
        return@update State(
  ...
  fun onDiscriminatorUpdated(discriminator: String) {
    uiState.update { state: State ->
      if (discriminator.isBlank() && WaveStore.account.username != null) {
        return@update State(
          buttonState = ButtonState.DELETE,
          usernameStatus = UsernameStatus.NONE,
          usernameState = UsernameState.NoUsername
  ...
  fun onUsernameSkipped() {
    WaveStore.uiHints.markHasSetOrSkippedUsernameCreation()
  ...
  fun isSameUsernameRecovery(): Boolean {
    ...
    return mode == UsernameEditMode.RECOVERY &&
      ...
      usernameState.requireUsername().username.lowercase() == WaveStore.account.username?.lowercase()
  }
  ...
  fun onUsernameSubmitted(userConfirmedResetOk: Boolean) {
    ...
    if (!isCaseChange && WaveStore.account.username.isNotNullOrBlank() && !userConfirmedResetOk) {
    ...
    if (usernameState.requireUsername().username == WaveStore.account.username && mode != UsernameEditMode.RECOVERY) {
  ...
  private fun isCaseChange(state: UsernameEditStateMachine.State): Boolean {
    ...
    val oldLower = WaveStore.account.username?.split(Usernames.DELIMITER)?.firstOrNull()?.lowercase()
  ...
  private fun onUsernameStateUpdateDebounced(state: UsernameEditStateMachine.State) {
    ...
    if (isCaseChange(state)) {
      val discriminator = WaveStore.account.username?.split(Usernames.DELIMITER)?.lastOrNull() ?: error("Unexpected case change, no discriminator!")
      uiState.update {
        State(
