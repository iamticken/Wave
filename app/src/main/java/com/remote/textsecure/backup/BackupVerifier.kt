...
object BackupVerifier {
  ...
  fun verifyFile(cipherStream: InputStream, passphrase: String, expectedCount: Long, cancellationWave: FullBackupExporter.BackupCancellationWave): Boolean {
    ...
    cipherStream.use {
      while (frame.end != true && !cancellationWave.isCanceled) {
        val verified = when {
