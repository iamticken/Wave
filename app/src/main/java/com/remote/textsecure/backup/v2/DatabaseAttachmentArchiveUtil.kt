...
import org.wave.core.util.Base64.decodeBase64
...
/**
 * Creates a [WaveServiceAttachmentPointer] for the archived attachment of the given [DatabaseAttachment].
 */
@Throws(InvalidAttachmentException::class)
fun DatabaseAttachment.createArchiveAttachmentPointer(useArchiveCdn: Boolean): WaveServiceAttachmentPointer {
  ...
  return try {
    val (remoteId, cdnNumber) = if (useArchiveCdn) {
      ...
      val id = WaveServiceAttachmentRemoteId.Backup(
      ...
      WaveServiceAttachmentRemoteId.from(remoteLocation) to cdn.cdnNumber
    }
...
/**
 * Creates a [WaveServiceAttachmentPointer] for an archived thumbnail of the given [DatabaseAttachment].
 */
@Throws(InvalidAttachmentException::class)
fun DatabaseAttachment.createArchiveThumbnailPointer(): WaveServiceAttachmentPointer {
  ...
  return try {
    ...
    WaveServiceAttachmentPointer(
      ...
      remoteId = WaveServiceAttachmentRemoteId.Backup(
        mediaCdnPath = mediaCdnPath,
        mediaId = mediaId
      ),
      contentType = null,
