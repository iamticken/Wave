...
import org.wave.core.util.asList
...
class ChatItemArchiveImporter(
  ...
  private fun ChatItem.toMessageInsert(fromRecipientId: RecipientId, chatRecipientId: RecipientId, threadId: Long): MessageInsert {
    ...
    if (this.updateMessage != null) {
      if (this.updateMessage.individualCall != null && this.updateMessage.individualCall.callId != null) {
        ...
        followUps += { endPollMessageId ->
          val pollMessageId = WaveDatabase.messages.getMessageFor(updateMessage.pollTerminate.targetSentTimestamp, fromRecipientId)?.id ?: -1
          val pollId = WaveDatabase.polls.getPollId(pollMessageId)
          ...
          if (pollId != null) {
            WaveDatabase.polls.endPoll(pollId = pollId, endingMessageId = endPollMessageId)
          }
        }
      }
    }
      ...
      if (contact != null) {
        ...
        followUps += { messageRowId ->
          val attachmentMap = if (contactAttachment != null) {
            WaveDatabase.attachments.insertAttachmentsForMessage(messageRowId, listOf(contactAttachment), emptyList())
          } else {
            emptyMap()
          }
          db.update(
            ...
            contentValuesOf(
              MessageTable.SHARED_CONTACTS to WaveDatabase.messages.getSerializedSharedContacts(attachmentMap, listOf(contact))
            ),
            "${MessageTable.ID} = ?",
    ...
    if (this.directStoryReplyMessage != null) {
      ...
      if (longTextAttachment != null) {
        followUps += { messageRowId ->
          val ids = WaveDatabase.attachments.insertAttachmentsForMessage(messageRowId, listOf(longTextAttachment), emptyList())
          ids.values.firstOrNull()?.let { attachmentId ->
            WaveDatabase.attachments.setTransferState(messageRowId, attachmentId, AttachmentTable.TRANSFER_PROGRESS_DONE)
          }
        }
      }
    }
      ...
      if (hasAttachments || linkPreviews.isNotEmpty()) {
        followUps += { messageRowId ->
          val attachmentMap = if (hasAttachments) {
            WaveDatabase.attachments.insertAttachmentsForMessage(messageRowId, attachments + linkPreviewAttachments + longTextAttachment.asList(), quoteAttachments)
          } else {
            emptyMap()
          }
          ...
          if (linkPreviews.isNotEmpty()) {
            db.update(MessageTable.TABLE_NAME)
              .values(MessageTable.LINK_PREVIEWS to WaveDatabase.messages.getSerializedLinkPreviews(attachmentMap, linkPreviews))
              .where("${MessageTable.ID} = ?", messageRowId)
              .run()
          }
        }
    ...
    if (this.stickerMessage != null) {
      ...
      if (attachment != null) {
        followUps += { messageRowId ->
          WaveDatabase.attachments.insertAttachmentsForMessage(messageRowId, listOf(attachment), emptyList())
        }
      }
    }
    ...
    if (this.poll != null) {
      ...
      followUps += { messageRowId ->
        val pollId = WaveDatabase.polls.insertPoll(
        ...
        val localOptionIds = WaveDatabase.polls.getPollOptionIds(pollId)
        poll.options.forEachIndexed { index, option ->
          ...
          WaveDatabase.polls.addPollVotes(pollId = pollId, optionId = localOptionIds[index], voters = localVoters)
        }
        ...
        if (poll.hasEnded) {
          ...
          WaveDatabase.polls.endPoll(pollId = pollId, endingMessageId = -1)
        }
      }
    }
  ...
  private fun StandardMessage.parseBodyText(importState: ImportState): Pair<String?, Attachment?> {
    if (this.longText != null) {
      return null to this.longText.toLocalAttachment(contentType = "text/x-wave-plain")
    }
  ...
  private fun DirectStoryReplyMessage.parseBodyText(importState: ImportState): Pair<String?, Attachment?> {
    if (this.textReply?.longText != null) {
      return null to this.textReply.longText.toLocalAttachment(contentType = "text/x-wave-plain")
    }
  ...
  private fun tryRestorePayment(chatItem: ChatItem, chatRecipientId: RecipientId): UUID? {
    ...
    return WaveDatabase.payments.restoreFromBackup(
      chatRecipientId,
  ...
  private fun ContentValues.addFailedPaymentNotification(chatItem: ChatItem, amount: Money, fee: Money, chatRecipientId: RecipientId) {
    val uuid = WaveDatabase.payments.restoreFromBackup(
      chatRecipientId,
      0,
      0,
      0,
