...
class BackupMediaSnapshotTableTest {
  ...
  fun givenAnEmptyTable_whenIWriteToTable_thenIExpectEmptyTable() {
    WaveDatabase.backupMediaSnapshots.writePendingMediaEntries(generateArchiveMediaItemSequence(count = 100))
  ...
  fun givenAnEmptyTable_whenIWriteToTableAndCommit_thenIExpectFilledTableWithThumbnails() {
    ...
    WaveDatabase.backupMediaSnapshots.writePendingMediaEntries(generateArchiveMediaItemSequence(count = inputCount))
    WaveDatabase.backupMediaSnapshots.writePendingMediaEntries(generateArchiveMediaItemSequence(count = inputCount, thumbnail = true))
    WaveDatabase.backupMediaSnapshots.commitPendingRows()
  ...
  fun givenAFilledTable_whenIReinsertObjects_thenIExpectUncommittedOverrides() {
    ...
    WaveDatabase.backupMediaSnapshots.writePendingMediaEntries(generateArchiveMediaItemSequence(count = initialCount))
    WaveDatabase.backupMediaSnapshots.commitPendingRows()
    ...
    WaveDatabase.backupMediaSnapshots.writePendingMediaEntries(generateArchiveMediaItemSequence(count = additionalCount))
  ...
  fun givenAFilledTable_whenIInsertSimilarIdsAndCommitThenDelete_thenIExpectOnlyCommittedOverrides() {
    ...
    val page = WaveDatabase.backupMediaSnapshots.getPageOfOldMediaObjects(pageSize = 1_000)
    WaveDatabase.backupMediaSnapshots.deleteOldMediaObjects(page)
  ...
  fun getMediaObjectsWithNonMatchingCdn_noMismatches() {
    ...
    WaveDatabase.backupMediaSnapshots.writePendingMediaEntries(localData)
    WaveDatabase.backupMediaSnapshots.commitPendingRows()

    val mismatches = WaveDatabase.backupMediaSnapshots.getMediaObjectsWithNonMatchingCdn(remoteData)
  ...
  fun getMediaObjectsThatCantBeFound_allFound() {
    ...
    val notFound = WaveDatabase.backupMediaSnapshots.getMediaObjectsThatCantBeFound(remoteData)
  ...
  fun getCurrentSnapshotVersion_emptyTable() {
    val version = WaveDatabase.backupMediaSnapshots.getCurrentSnapshotVersion()
  ...
  fun getMediaObjectsLastSeenOnCdnBeforeSnapshotVersion_noneMarkedSeen() {
    ...
    val notSeenCount = WaveDatabase.backupMediaSnapshots.getMediaObjectsLastSeenOnCdnBeforeSnapshotVersion(1).count
  ...
  fun getMediaObjectsLastSeenOnCdnBeforeSnapshotVersion_someMarkedSeen() {
    ...
    WaveDatabase.backupMediaSnapshots.writePendingMediaEntries(fullSizeItems)
    WaveDatabase.backupMediaSnapshots.writePendingMediaEntries(thumbnailItems)
    ...
    WaveDatabase.backupMediaSnapshots.markSeenOnRemote(fullSizeIdsToMarkSeen, 1)
    WaveDatabase.backupMediaSnapshots.markSeenOnRemote(thumbnailIdsToMarkSeen, 1)
  ...
  private fun getTotalItemCount(includeThumbnails: Boolean): Int {
    return if (includeThumbnails) {
      WaveDatabase.backupMediaSnapshots.readableDatabase
        .count()
  ...
  private fun getCountForLatestSnapshot(includeThumbnails: Boolean): Int {
    ...
    return WaveDatabase.backupMediaSnapshots.readableDatabase
      .count()
      .from(BackupMediaSnapshotTable.TABLE_NAME)
