...
class DatabaseConsistencyTest {
  ...
  fun testUpgradeConsistency() {
    val currentVersionStatements = WaveDatabase.rawDatabase.getAllCreateStatements()
    val testHelper = InMemoryTestHelper(AppDependencies.application).also {
      it.onUpgrade(it.writableDatabase, 181, WaveDatabaseMigrations.DATABASE_VERSION)
    }
  ...
  fun testForeignKeyIndexCoverage() {
    ...
    val foreignKeys: List<ForeignKeyConstraint> = WaveDatabase.rawDatabase.getForeignKeys()
    val indexesByFirstColumn: List<Index> = WaveDatabase.rawDatabase.getIndexes()
  ...
  private class InMemoryTestHelper(private val application: Application) : SQLiteOpenHelper(application, null, null, 1) {
    ...
    override fun onUpgrade(db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
      WaveDatabaseMigrations.migrate(application, WaveSQLiteDatabase(db), 181, WaveDatabaseMigrations.DATABASE_VERSION)
    }
