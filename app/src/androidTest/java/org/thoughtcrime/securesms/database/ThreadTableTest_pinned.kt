...
class ThreadTableTest_pinned {
  ...
  fun givenAPinnedThread_whenIDeleteTheLastMessage_thenIDoNotDeleteOrUnpinTheThread() {
    ...
    WaveDatabase.threads.pinConversations(listOf(threadId))

    // WHEN
    WaveDatabase.messages.deleteMessage(messageId)

    // THEN
    val pinned = WaveDatabase.threads.getPinnedThreadIds()
  ...
  fun givenAPinnedThread_whenIDeleteTheLastMessage_thenIExpectTheThreadInUnarchivedCount() {
    ...
    // THEN
    val unarchivedCount = WaveDatabase.threads.getUnarchivedConversationListCount(ConversationFilter.OFF, allChats)
  ...
  fun givenAPinnedThread_whenIDeleteTheLastMessage_thenIExpectPinnedThreadInUnarchivedList() {
    ...
    // THEN
    WaveDatabase.threads.getUnarchivedConversationList(ConversationFilter.OFF, true, 0, 1, allChats).use {
      it.moveToFirst()
