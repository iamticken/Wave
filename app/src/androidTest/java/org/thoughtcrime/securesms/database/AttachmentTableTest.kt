...
import org.wave.core.util.stream.NullOutputStream
...
import org.whispersystems.waveservice.api.crypto.AttachmentCipherOutputStream
...
class AttachmentTableTest {
  ...
  @Before
  fun setUp() {
    WaveDatabase.attachments.deleteAllAttachments()
  }
  ...
  fun givenABlob_whenIInsert2AttachmentsForPreUpload_thenIExpectDistinctIdsButSameFileName() {
    ...
    val attachment = WaveDatabase.attachments.insertAttachmentForPreUpload(highQualityImage)
    val attachment2 = WaveDatabase.attachments.insertAttachmentForPreUpload(highQualityImage)
  ...
  fun givenABlobAndDifferentTransformQuality_whenIInsert2AttachmentsForPreUpload_thenIExpectDifferentFileInfos() {
    ...
    val attachment = WaveDatabase.attachments.insertAttachmentForPreUpload(highQualityImage)
    val attachment2 = WaveDatabase.attachments.insertAttachmentForPreUpload(lowQualityImage)

    WaveDatabase.attachments.updateAttachmentData(
      attachment,
    ...
    val attachment1Info = WaveDatabase.attachments.getDataFileInfo(attachment.attachmentId)
    val attachment2Info = WaveDatabase.attachments.getDataFileInfo(attachment2.attachmentId)
  ...
  fun doNotDeleteDedupedFileIfUsedByAnotherAttachmentWithADifferentTransformProperties() {
    ...
    val previousDatabaseAttachmentId: AttachmentId = WaveDatabase.attachments.insertAttachmentsForMessage(1, listOf(previousAttachment), emptyList()).values.first()

    val standardQualityPreUpload = createAttachment(1, blobUncompressed, AttachmentTable.TransformProperties.empty())
    val standardDatabaseAttachment = WaveDatabase.attachments.insertAttachmentForPreUpload(standardQualityPreUpload)
    ...
    val highDatabaseAttachment = WaveDatabase.attachments.insertAttachmentForPreUpload(highQualityPreUpload)

    // WHEN
    WaveDatabase.attachments.updateAttachmentData(standardDatabaseAttachment, createMediaStream(compressedData))

    // THEN
    val previousInfo = WaveDatabase.attachments.getDataFileInfo(previousDatabaseAttachmentId)!!
    val standardInfo = WaveDatabase.attachments.getDataFileInfo(standardDatabaseAttachment.attachmentId)!!
    val highInfo = WaveDatabase.attachments.getDataFileInfo(highDatabaseAttachment.attachmentId)!!
  ...
  fun doNotDedupedFileIfUsedByAnotherAttachmentWithADifferentTransformProperties() {
    ...
    val secondHighDatabaseAttachment = WaveDatabase.attachments.insertAttachmentForPreUpload(secondHighQualityPreUpload)

    // THEN
    val standardInfo = WaveDatabase.attachments.getDataFileInfo(standardDatabaseAttachment.attachmentId)!!
    val highInfo = WaveDatabase.attachments.getDataFileInfo(highDatabaseAttachment.attachmentId)!!
    val secondHighInfo = WaveDatabase.attachments.getDataFileInfo(secondHighDatabaseAttachment.attachmentId)!!
  ...
  fun resetArchiveTransferStateByPlaintextHashAndRemoteKey_singleMatch() {
    ...
    val attachmentId = WaveDatabase.attachments.insertAttachmentsForMessage(-1L, listOf(attachment), emptyList()).values.first()
    WaveDatabase.attachments.finalizeAttachmentAfterUpload(attachmentId, AttachmentTableTestUtil.createUploadResult(attachmentId))
    WaveDatabase.attachments.setArchiveTransferState(attachmentId, AttachmentTable.ArchiveTransferState.FINISHED)

    // Reset the transfer state by plaintextHash+remoteKey
    val plaintextHash = WaveDatabase.attachments.getAttachment(attachmentId)!!.dataHash!!.decodeBase64OrThrow()
    val remoteKey = WaveDatabase.attachments.getAttachment(attachmentId)!!.remoteKey!!.decodeBase64OrThrow()
    WaveDatabase.attachments.resetArchiveTransferStateByPlaintextHashAndRemoteKeyIfNecessary(plaintextHash, remoteKey)

    // Verify it's been reset
    assertThat(WaveDatabase.attachments.getAttachment(attachmentId)!!.archiveTransferState).isEqualTo(AttachmentTable.ArchiveTransferState.NONE)
  }
  ...
  fun given10NewerAnd10OlderAttachments_whenIGetEachBatch_thenIExpectProperBucketing() {
    ...
    (newMessages + oldMessages).forEach {
      WaveDatabase.messages.insertMessageInbox(it)
    }

    val firstAttachmentsToDownload = WaveDatabase.attachments.getLast30DaysOfRestorableAttachments(500)
    val nextAttachmentsToDownload = WaveDatabase.attachments.getOlderRestorableAttachments(500)

    assertThat(firstAttachmentsToDownload).hasSize(10)
    val resultNewMessages = WaveDatabase.messages.getMessages(firstAttachmentsToDownload.map { it.mmsId })
    ...
    val resultOldMessages = WaveDatabase.messages.getMessages(nextAttachmentsToDownload.map { it.mmsId })
  ...
  fun givenAnAttachmentWithAMessageThatExpiresIn5Minutes_whenIGetAttachmentsThatNeedArchiveUpload_thenIDoNotExpectThatAttachment() {
    ...
    val messageId = WaveDatabase.messages.insertMessageInbox(message).map { it.messageId }.get()
    WaveDatabase.attachments.setArchiveTransferState(AttachmentId(1L), AttachmentTable.ArchiveTransferState.NONE)
    WaveDatabase.attachments.setTransferState(messageId, AttachmentId(1L), AttachmentTable.TRANSFER_PROGRESS_DONE)
    WaveDatabase.attachments.finalizeAttachmentAfterUpload(AttachmentId(1L), AttachmentTableTestUtil.createUploadResult(AttachmentId(1L)))

    // WHEN
    val attachments = WaveDatabase.attachments.getAttachmentsThatNeedArchiveUpload()
  ...
  fun givenAnAttachmentWithAMessageWithExpirationStartedThatExpiresIn5Days_whenIGetAttachmentsThatNeedArchiveUpload_thenIDoExpectThatAttachment() {
    ...
    val message = createIncomingMessage(serverTime = 0.days, attachment = attachment, expiresIn = 5.days)
    val messageId = WaveDatabase.messages.insertMessageInbox(message).map { it.messageId }.get()
    WaveDatabase.messages.markExpireStarted(messageId)
    WaveDatabase.attachments.setArchiveTransferState(AttachmentId(1L), AttachmentTable.ArchiveTransferState.NONE)
    WaveDatabase.attachments.setTransferState(messageId, AttachmentId(1L), AttachmentTable.TRANSFER_PROGRESS_DONE)
    WaveDatabase.attachments.finalizeAttachmentAfterUpload(AttachmentId(1L), AttachmentTableTestUtil.createUploadResult(AttachmentId(1L)))
  ...
  fun givenAnDuplicateAttachmentPriorToCopyToArchive_whenICopyFirstAttachmentToArchive_thenIDoNotExpectBothAttachmentsToChangeArchiveStateToFinished() {
    ...
    // Insert Message 1
    val message1Result = WaveDatabase.messages.insertMessageInbox(createIncomingMessage(serverTime = 0.days, attachment = attachment1)).get()
    ...
    WaveDatabase.attachments.setTransferState(message1Id, attachment1Id, AttachmentTable.TRANSFER_PROGRESS_STARTED)

    // Insert Message 2
    val message2Result = WaveDatabase.messages.insertMessageInbox(createIncomingMessage(serverTime = 1.days, attachment = attachment2)).get()
    val message2Id = message2Result.messageId
    val attachment2Id = message2Result.insertedAttachments!![attachment2]!!
    // AttachmentDownloadJob#onAdded
    WaveDatabase.attachments.setTransferState(message2Id, attachment2Id, AttachmentTable.TRANSFER_PROGRESS_STARTED)

    // Finalize Attachment 1 download
    WaveDatabase.attachments.finalizeAttachmentAfterDownload(message1Id, attachment1Id, ByteArrayInputStream(data))
    // CopyAttachmentToArchiveJob#onAdded
    WaveDatabase.attachments.setArchiveTransferState(attachment1Id, AttachmentTable.ArchiveTransferState.COPY_PENDING)

    // Verify Attachment 1 data matches original Attachment 1 data from insert
    var dbAttachment1 = WaveDatabase.attachments.getAttachment(attachment1Id)!!
    ...
    WaveDatabase.attachments.finalizeAttachmentAfterDownload(message2Id, attachment2Id, ByteArrayInputStream(data))

    // Verify Attachment 1 data matches Attachment 2 data from insert and dedupe in finalize
    dbAttachment1 = WaveDatabase.attachments.getAttachment(attachment1Id)!!
    var dbAttachment2 = WaveDatabase.attachments.getAttachment(attachment2Id)!!
    ...
    WaveDatabase.attachments.setArchiveTransferState(attachment1Id, attachment1InitialRemoteKey, attachment1InitialPlaintextHash, AttachmentTable.ArchiveTransferState.FINISHED)

    dbAttachment1 = WaveDatabase.attachments.getAttachment(attachment1Id)!!
    dbAttachment2 = WaveDatabase.attachments.getAttachment(attachment2Id)!!
    ...
    WaveDatabase.attachments.setArchiveTransferState(attachment2Id, attachment2InitialRemoteKey, attachment2InitialPlaintextHash, AttachmentTable.ArchiveTransferState.FINISHED)
  ...
  private fun createAttachmentPointer(key: ByteArray, size: Int): Attachment {
    return PointerAttachment.forPointer(
      pointer = Optional.of(
        WaveServiceAttachmentPointer(
          cdnNumber = 3,
          remoteId = WaveServiceAttachmentRemoteId.V4("asdf"),
