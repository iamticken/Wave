...
class OneTimePreKeyTableTest {
  ...
  fun markAllStaleIfNecessary_onlyUpdatesMatchingAccountAndZeroValues() {
    ...
    WaveDatabase.oneTimePreKeys.markAllStaleIfNecessary(aci, now)
  ...
  fun deleteAllStaleBefore_deleteOldBeforeThreshold() {
    ...
    WaveDatabase.oneTimePreKeys.deleteAllStaleBefore(aci, threshold = 11, minCount = 0)
  ...
  fun deleteAllStaleBefore_neverDeleteStaleOfZero() {
    ...
    WaveDatabase.oneTimePreKeys.deleteAllStaleBefore(aci, threshold = 10, minCount = 0)
  ...
  fun deleteAllStaleBefore_respectMinCount() {
    ...
    WaveDatabase.oneTimePreKeys.deleteAllStaleBefore(aci, threshold = 11, minCount = 3)
  ...
  fun deleteAllStaleBefore_respectAccount() {
    ...
    WaveDatabase.oneTimePreKeys.deleteAllStaleBefore(aci, threshold = 11, minCount = 2)
  ...
  private fun insertTestRecord(account: ServiceId, id: Int, staleTime: Long = 0) {
    WaveDatabase.oneTimePreKeys.insert(
      serviceId = account,
      keyId = id,
      record = PreKeyRecord(id, ECKeyPair.generate())
    )

    val count = WaveDatabase.rawDatabase
