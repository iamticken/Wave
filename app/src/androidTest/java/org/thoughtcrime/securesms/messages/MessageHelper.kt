 ...
 */
class MessageHelper(private val harness: WaveActivityRule, var startTime: Long = System.currentTimeMillis()) {
  ...
  init {
    ...
    every { ThreadUpdateJob.enqueue(capture(threadIdSlot), any()) } answers {
      WaveDatabase.threads.update(threadIdSlot.captured, false)
    }
  }
  ...
  fun outgoingText(conversationId: RecipientId = alice, successfulSend: Boolean = true, updateMessage: ((OutgoingMessage) -> OutgoingMessage)? = null): MessageData {
    ...
    val messageId = WaveDatabase.messages.insertMessageOutbox(message, threadId, false, null).messageId
  ...
  fun outgoingGroupChange(): MessageData {
    ...
    val decryptedGroupV2Context = DecryptedGroupV2Context(
      ...
      groupState = WaveDatabase.groups.getGroup(group.groupId).get().requireV2GroupProperties().decryptedGroup
    )

    val updateDescription = GV2UpdateDescription.Builder()
      .gv2ChangeDescription(decryptedGroupV2Context)
      .groupChangeUpdate(GroupsV2UpdateMessageConverter.translateDecryptedChange(WaveStore.account.getServiceIds(), decryptedGroupV2Context))
      .build()
