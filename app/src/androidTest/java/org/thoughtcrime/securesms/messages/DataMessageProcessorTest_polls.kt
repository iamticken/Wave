...
class DataMessageProcessorTest_polls {
  ...
  fun handlePollCreate_whenIHaveAValidPollProto_createPoll() {
    ...
    val poll = WaveDatabase.polls.getPoll(insertResult!!.messageId)
    assert(poll != null)
  ...
  fun handlePollTerminate_whenIHaveValidProto_endPoll() {
    ...
    val poll = WaveDatabase.polls.getPoll(pollMessageId)
    assert(poll != null)
  ...
  private fun insertPoll(allowMultiple: Boolean = true): Long {
    ...
    val messageId = WaveDatabase.messages.insertMessageInbox(pollMessage).get()
    WaveDatabase.polls.insertPoll("question?", allowMultiple, listOf("a", "b", "c"), alice.id.toLong(), messageId.messageId)
