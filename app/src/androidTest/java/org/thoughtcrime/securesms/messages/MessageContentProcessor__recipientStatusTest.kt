...
class MessageContentProcessor__recipientStatusTest {
  ...
  fun syncGroupSentTextMessageWithRecipientUpdateFollowup() {
    ...
    val threadId = WaveDatabase.threads.getThreadIdFor(groupRecipientId)!!
    val firstSyncMessages = MessageTableTestUtils.getMessages(threadId)
    val firstMessageId = firstSyncMessages[0].id
    val firstReceiptInfo = WaveDatabase.groupReceipts.getGroupReceiptInfo(firstMessageId)
    ...
    val secondReceiptInfo = WaveDatabase.groupReceipts.getGroupReceiptInfo(firstMessageId)
