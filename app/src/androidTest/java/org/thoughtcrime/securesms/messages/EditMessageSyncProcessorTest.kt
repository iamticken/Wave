...
class EditMessageSyncProcessorTest {
  ...
  fun textMessage() {
    ...
    for (i in 1..10) {
      ...
      WaveDatabase.recipients.setExpireMessages(toRecipient.id, content.dataMessage?.expireTimer ?: 0, content.dataMessage?.expireTimerVersion ?: 1)
      ...
      WaveDatabase.recipients.setExpireMessages(toRecipient.id, (content.dataMessage?.expireTimer ?: 0) / 1000, content.dataMessage?.expireTimerVersion ?: 1)
      ...
      val originalMessageId = WaveDatabase.messages.insertMessageOutbox(originalTextMessage, threadId, false, null).messageId
      WaveDatabase.messages.markAsSent(originalMessageId, true)
      if ((content.dataMessage?.expireTimer ?: 0) > 0) {
        WaveDatabase.messages.markExpireStarted(originalMessageId, originalTimestamp)
      }
      ...
      val editMessageId = WaveDatabase.messages.insertMessageOutbox(editMessage, threadId, false, null).messageId
      WaveDatabase.messages.markAsSent(editMessageId, true)

      if ((content.dataMessage?.expireTimer ?: 0) > 0) {
        WaveDatabase.messages.markExpireStarted(editMessageId, originalTimestamp)
      }
  ...
  private inner class TestResults {
    ...
    fun cleanup() {
      WaveDatabase.rawDatabase.withinTransaction { db ->
