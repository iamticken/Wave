...
class SyncMessageProcessorTest_synchronizeDeleteForMe {
  ...
  fun singleMessageDelete() {
    ...
    var messageCount = WaveDatabase.messages.getMessageCountForThread(threadId)
    ...
    // THEN
    messageCount = WaveDatabase.messages.getMessageCountForThread(threadId)
  ...
  fun allMessagesDelete() {
    ...
    val threadRecord = WaveDatabase.threads.getThreadRecord(threadId)
  ...
  fun multipleConversationMessagesDelete() {
    ...
    val aliceThreadId = WaveDatabase.threads.getThreadIdFor(messageHelper.alice)!!
    var aliceMessageCount = WaveDatabase.messages.getMessageCountForThread(aliceThreadId)
    assertThat(aliceMessageCount).isEqualTo(2)

    val bobThreadId = WaveDatabase.threads.getThreadIdFor(messageHelper.bob)!!
    var bobMessageCount = WaveDatabase.messages.getMessageCountForThread(bobThreadId)
    ...
    // THEN
    aliceMessageCount = WaveDatabase.messages.getMessageCountForThread(aliceThreadId)
    assertThat(aliceMessageCount).isEqualTo(1)

    bobMessageCount = WaveDatabase.messages.getMessageCountForThread(bobThreadId)
  ...
  fun singleConversationDelete() {
    ...
    val threadId = WaveDatabase.threads.getThreadIdFor(messageHelper.alice)!!
    assertThat(WaveDatabase.messages.getMessageCountForThread(threadId)).isEqualTo(20)
    ...
    // THEN
    assertThat(WaveDatabase.messages.getMessageCountForThread(threadId)).isEqualTo(0)
    assertThat(WaveDatabase.threads.getThreadRecord(threadId)).isNull()
  }
  ...
  fun singleConversationNoRecentsFoundDelete() {
    ...
    // THEN
    assertThat(WaveDatabase.messages.getMessageCountForThread(threadId)).isEqualTo(20)
    assertThat(WaveDatabase.threads.getThreadRecord(threadId)).isNotNull()
  ...
  fun localOnlyRemainingAfterConversationDeleteWithFullDelete() {
    ...
    WaveDatabase.messages.insertProfileNameChangeMessages(alice, "new name", "previous name")
    Log.v(TAG, "Adding call message")
    WaveDatabase.calls.insertOneToOneCall(1, System.currentTimeMillis(), alice.id, CallTable.Type.AUDIO_CALL, CallTable.Direction.OUTGOING, CallTable.Event.ACCEPTED)

    val threadId = WaveDatabase.threads.getThreadIdFor(messageHelper.alice)!!
    assertThat(WaveDatabase.messages.getMessageCountForThread(threadId)).isEqualTo(23)
  ...
  fun localOnlyRemainingAfterConversationDeleteWithoutFullDelete() {
    ...
    // THEN
    assertThat(WaveDatabase.messages.getMessageCountForThread(threadId)).isEqualTo(3)
    assertThat(WaveDatabase.threads.getThreadRecord(threadId)).isNotNull()
  }
  ...
  fun multipleConversationDelete() {
    ...
    val threadIds = allMessages.keys.map { WaveDatabase.threads.getThreadIdFor(it)!! }
    threadIds.forEach { assertThat(WaveDatabase.messages.getMessageCountForThread(it)).isEqualTo(20) }
    ...
    // THEN
    threadIds.forEach {
      assertThat(WaveDatabase.messages.getMessageCountForThread(it)).isEqualTo(0)
      assertThat(WaveDatabase.threads.getThreadRecord(it)).isNull()
    }
  }
  ...
  fun singleLocalOnlyConversation() {
    ...
    val aliceThreadId = WaveDatabase.threads.getOrCreateThreadIdFor(messageHelper.alice, isGroup = false)

    IdentityUtil.markIdentityVerified(harness.context, alice, true, false)
    WaveDatabase.calls.insertOneToOneCall(1, System.currentTimeMillis(), alice.id, CallTable.Type.AUDIO_CALL, CallTable.Direction.OUTGOING, CallTable.Event.ACCEPTED)
    WaveDatabase.messages.insertProfileNameChangeMessages(alice, "new name", "previous name")
    WaveDatabase.messages.markAsSentFailed(messageHelper.outgoingText().messageId)

    // Cleanup and confirm setup
    WaveDatabase.messages.deleteMessage(messageId = oneToOnePlaceHolderMessage, threadId = aliceThreadId, notify = false, updateThread = false)
    assertThat(WaveDatabase.messages.getMessageCountForThread(aliceThreadId)).isGreaterThan(0)

    // WHEN
    messageHelper.syncDeleteForMeLocalOnlyConversation(messageHelper.alice)

    // THEN
    assertThat(WaveDatabase.messages.getMessageCountForThread(aliceThreadId)).isEqualTo(0)
    assertThat(WaveDatabase.threads.getThreadRecord(aliceThreadId)).isNull()
  }
  ...
  fun multipleLocalOnlyConversation() {
    ...
    val aliceThreadId = WaveDatabase.threads.getOrCreateThreadIdFor(messageHelper.alice, isGroup = false)
    val groupThreadId = WaveDatabase.threads.getOrCreateThreadIdFor(messageHelper.group.recipientId, isGroup = true)
    ...
    // Calls
    WaveDatabase.calls.insertOneToOneCall(1, System.currentTimeMillis(), alice.id, CallTable.Type.AUDIO_CALL, CallTable.Direction.OUTGOING, CallTable.Event.ACCEPTED)
    WaveDatabase.calls.insertOneToOneCall(2, System.currentTimeMillis(), alice.id, CallTable.Type.VIDEO_CALL, CallTable.Direction.INCOMING, CallTable.Event.MISSED)
    WaveDatabase.calls.insertOneToOneCall(3, System.currentTimeMillis(), alice.id, CallTable.Type.AUDIO_CALL, CallTable.Direction.INCOMING, CallTable.Event.MISSED_NOTIFICATION_PROFILE)

    WaveDatabase.calls.insertAcceptedGroupCall(4, messageHelper.group.recipientId, CallTable.Direction.INCOMING, System.currentTimeMillis())
    WaveDatabase.calls.insertDeclinedGroupCall(5, messageHelper.group.recipientId, System.currentTimeMillis())

    // Detected changes
    WaveDatabase.messages.insertProfileNameChangeMessages(alice, "new name", "previous name")
    WaveDatabase.messages.insertLearnedProfileNameChangeMessage(alice, null, "username.42")
    WaveDatabase.messages.insertNumberChangeMessages(alice.id)
    WaveDatabase.messages.insertSmsExportMessage(alice.id, WaveDatabase.threads.getThreadIdFor(messageHelper.alice)!!)
    WaveDatabase.messages.insertSessionSwitchoverEvent(alice.id, aliceThreadId, SessionSwitchoverEvent())

    // Sent failed
    WaveDatabase.messages.markAsSending(messageHelper.outgoingText().messageId)
    WaveDatabase.messages.markAsSentFailed(messageHelper.outgoingText().messageId)
    messageHelper.outgoingText().let {
      WaveDatabase.messages.markAsSending(it.messageId)
      WaveDatabase.messages.markAsRateLimited(it.messageId)
    }
    ...
    // Cleanup and confirm setup
    WaveDatabase.messages.deleteMessage(messageId = oneToOnePlaceHolderMessage, threadId = aliceThreadId, notify = false, updateThread = false)
    WaveDatabase.messages.deleteMessage(messageId = groupPlaceholderMessage, threadId = aliceThreadId, notify = false, updateThread = false)

    WaveDatabase.rawDatabase.withinTransaction {
      assertThat(WaveDatabase.messages.getMessageCountForThread(aliceThreadId)).isEqualTo(16)
      assertThat(WaveDatabase.messages.getMessageCountForThread(groupThreadId)).isEqualTo(10)
    }
    ...
    assertThat(WaveDatabase.messages.getMessageCountForThread(groupThreadId)).isEqualTo(0)
    assertThat(WaveDatabase.threads.getThreadRecord(groupThreadId)).isNull()
  }
  ...
  fun singleAttachmentDeletes() {
    ...
    var attachments = WaveDatabase.attachments.getAttachmentsForMessage(message1.messageId)
    assertThat(attachments).hasSize(4)

    val threadId = WaveDatabase.threads.getThreadIdFor(messageHelper.alice)!!
    assertThat(WaveDatabase.messages.getMessageCountForThread(threadId)).isEqualTo(1)

    // Has all three
    WaveDatabase.attachments.finalizeAttachmentAfterUpload(
    ...
    WaveDatabase.rawDatabase.update(AttachmentTable.TABLE_NAME).values(AttachmentTable.DATA_HASH_END to null).where("${AttachmentTable.ID} = ?", attachments[2].attachmentId).run()
    ...
    attachments = WaveDatabase.attachments.getAttachmentsForMessage(message1.messageId)
    ...
    assertThat(WaveDatabase.messages.getMessageCountForThread(threadId)).isEqualTo(1)
    var updatedAttachments = WaveDatabase.attachments.getAttachmentsForMessage(message1.messageId)
    ...
    assertThat(WaveDatabase.messages.getMessageCountForThread(threadId)).isEqualTo(1)
    updatedAttachments = WaveDatabase.attachments.getAttachmentsForMessage(message1.messageId)
  ...
  private fun Attachment.toUploadResult(
    ...
    return AttachmentUploadResult(
      remoteId = WaveServiceAttachmentRemoteId.V4(this.remoteLocation ?: "some-location"),
