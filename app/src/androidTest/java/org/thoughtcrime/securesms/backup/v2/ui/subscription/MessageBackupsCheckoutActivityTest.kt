...
class MessageBackupsCheckoutActivityTest {
  ...
  @get:Rule val activityRule = WaveActivityRule()
  ...
  fun e2e_paid_happy_path() {
    ...
    val iap = WaveDatabase.inAppPayments.getLatestInAppPaymentByType(InAppPaymentType.RECURRING_BACKUP)
  ...
  fun e2e_free_happy_path() {
    ...
    assertThat(WaveStore.backup.backupTier).isEqualTo(MessageBackupTier.FREE)
  }
  ...
  private fun e2e_shared_happy_path(context: Context, scenario: ActivityScenario<MessageBackupsCheckoutActivity>) {
    ...
    composeTestRule.onNodeWithText(context.getString(R.string.RemoteBackupsSettingsFragment__wave_backups)).assertIsDisplayed()
    ...
    scenario.onActivity {
      val backupKeyString = WaveStore.account.accountEntropyPool.displayValue.chunked(4).joinToString("  ")
    ...
    composeTestRule.onNodeWithTag("message-backups-key-verify-screen-backup-key-input-field").performTextInput(
      WaveStore.account.accountEntropyPool.displayValue
    )
