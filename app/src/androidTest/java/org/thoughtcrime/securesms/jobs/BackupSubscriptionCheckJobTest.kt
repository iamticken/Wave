...
import org.wave.core.util.billing.BillingPurchaseState
...
class BackupSubscriptionCheckJobTest {
  ...
  fun givenDeletionStateIsNotNone_whenIRun_thenIExpectSuccessAndEarlyExit() {
    DeletionState.entries.filter { it != DeletionState.NONE }.forEach { deletionState ->
      WaveStore.backup.deletionState = deletionState
  ...
  fun givenAPendingPayment_whenIRun_thenIExpectSuccessAndEarlyExit() {
    ...
    assertThat(WaveStore.backup.subscriptionStateMismatchDetected).isFalse()
  }
  ...
  fun givenInactiveSubscription_whenIRun_thenIExpectStateMismatchDetected() {
    ...
    assertThat(result.isSuccess).isTrue()
    assertThat(WaveStore.backup.subscriptionStateMismatchDetected).isTrue()
  }
  ...
  @Test
  fun givenActiveWaveSubscriptionWithTokenMismatch_whenIRun_thenIExpectTokenRedemption() {
    mockActivePurchase()
  ...
  @Test
  fun givenGooglePlayBillingCanceledWithoutActiveWaveSubscription_whenIRun_thenIExpectValidCancelState() {
  ...
  @Test
  fun givenGooglePlayBillingCanceledWithFailedWaveSubscription_whenIRun_thenIExpectValidCancelState() {
    mockCanceledPurchase()
