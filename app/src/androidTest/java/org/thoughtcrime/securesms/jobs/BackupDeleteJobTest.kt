...
class BackupDeleteJobTest {
  ...
  fun givenLinkedDevice_whenIRun_thenIExpectFailure() {
    mockkObject(WaveStore) {
      ...
      every { WaveStore.account.isLinkedDevice } returns true
  ...
  fun givenDeletionStateNone_whenIRun_thenIExpectFailure() {
    WaveStore.backup.deletionState = DeletionState.NONE
  ...
  fun givenDeletionStateClearLocalState_whenIRun_thenIDeleteLocalState() {
    ...
    assertThat(WaveStore.backup.backupTier).isNull()
  ...
  fun givenMediaOffloaded_whenIRun_thenIExpectAwaitingMediaDownload() {
    ...
    every { WaveDatabase.attachments.getRemainingRestorableAttachmentSize() } returns 1
    every { WaveDatabase.attachments.getOptimizedMediaAttachmentSize() } returns 1
    every { WaveDatabase.attachments.clearAllArchiveData() } returns Unit
    ...
    assertThat(WaveStore.backup.deletionState).isEqualTo(DeletionState.AWAITING_MEDIA_DOWNLOAD)
    assertThat(result.isRetry).isTrue()
  }

  @Test
  fun givenMediaDownloadFinished_whenIRun_thenIExpectDeletion() {
    WaveStore.backup.deletionState = DeletionState.MEDIA_DOWNLOAD_FINISHED
    ...
    assertThat(WaveStore.backup.deletionState).isEqualTo(DeletionState.COMPLETE)
  }
