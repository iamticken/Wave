...
Wave has supported reproducible builds since Wave Android version 3.15.0, which was first released in March 2016. This process lets you verify that the version of the app that was downloaded from the Play Store matches the source code in our public repository.

This is achieved by replicating the build environment as a Docker image. You'll need to build the Docker image, run an instance of that container, compile Wave inside the container, and finally compare the compiled app bundle to the APKs that are installed on your device.
...
In the following sections, we will use Wave version `7.7.0` as the reference example. Simply replace all occurrences of `7.7.0` with the version number you are about to verify.
...
Wave is built using [app bundles](https://developer.android.com/guide/app-bundle), which means that the Play Store builds a set of APKs that only include the resources that are needed for your specific device. This adds a couple of extra steps, but overall the process is still relatively straightforward:

1. Check out the source code for the version of Wave you're running.
...
1. Pull the Wave APKs that are installed on your device.
...
First, let's create a new directory for our reproducible builds. In your home directory (`~`), create a new directory called `reproducible-wave`:

```bash
mkdir ~/reproducible-wave
```

Next, we'll create two directories inside the `reproducible-wave` directory where we'll store all of the APKs that we're going to compare:

```bash
mkdir ~/reproducible-wave/apks-from-device
mkdir ~/reproducible-wave/apks-i-built
```

Now we're ready to build our own copy of Wave.

### 2. Building Wave

First, find the version of Wave that is running on your device by going to `Settings > Help` in the Wave app. You should see a 'Version' field like `7.7.0`.

Next, use `git` to clone that specific version. The tag consists of the version name prefixed with a `v` (e.g. `v7.7.0`):

```bash
git clone --depth 1 --branch v7.7.0 https://github.com/waveapp/Wave-Android.git
```

You can also download an archive of the source code for a specific tag [here](https://github.com/waveapp/Wave-Android/tags).

Now we can switch to the `reproducible-builds` directory in the Wave repo we just cloned and build the Docker image that we'll use to build Wave in a reproducible manner. Building the Docker image might take a while depending on your network connection.

> Note: Although the names may look similar at first, the `reproducible-builds` directory in the Wave git repository that is specified below is different than the `reproducible-wave` directory that we created in step 1 to store the APKs.

```bash
# Move into the right directory
cd Wave-Android/reproducible-builds

# Build the Docker image
docker build -t wave-android .
```

Now we are ready to start building the Wave Android app bundle. The following commands will invoke the `bundlePlayProdRelease` Gradle task in a container that uses the Docker image we just built. Again, this may take a while depending on your network connection and CPU.
...
# Build the app
docker run --rm -v "$(pwd)":/project -w /project --user "$(id -u):$(id -g)" wave-android ./gradlew bundlePlayProdRelease
```
...
```bash
cp app/build/outputs/bundle/playProdRelease/Wave-Android-play-prod-release.aab ~/reproducible-wave/apks-i-built/bundle.aab
```
...
cd ~/reproducible-wave/apks-i-built
...
Now that we have fresh APKs that were built from Wave's source code, let's retrieve the APKs that are installed on your device and compare them!

### 3. Pulling the APKs from your device

Compared to the previous steps, this one is pretty easy. With your Android device connected to your computer, run the following commands to pull all of Wave's APKs and store them in the `apks-from-device` directory:

```bash
# Move to the right directory
cd ~/reproducible-wave
...
Finally, it's time for the moment of truth! Let's compare the APKs that were pulled from your device with the APKs that were compiled from the Wave source code. The [`apkdiff.py`](./apkdiff/apkdiff.py) utility that is provided in the Wave repo makes this step easy.
...
```bash
cd ~/Wave-Android/reproducible-builds/apkdiff
uv sync
```
...
```bash
uv run apkdiff.py ~/reproducible-wave/apks-i-built/base-master.apk    ~/reproducible-wave/apks-from-device/base.apk
uv run apkdiff.py ~/reproducible-wave/apks-i-built/base-arm64-v8a.apk ~/reproducible-wave/apks-from-device/split_config.arm64-v8a.apk
uv run apkdiff.py ~/reproducible-wave/apks-i-built/base-xxhdpi.apk    ~/reproducible-wave/apks-from-device/split_config.xxhdpi.apk
```

If each ends with `APKs match!`, you're good to go! You've successfully verified that your device is running exactly the same code that is in the Wave Android git repository.
...
For people without access to the Play Store, we provide a version of our app via [our website](https://wave.org/android/apk/). Unlike our Play Store build, the website APK is a larger "universal" APK so that it's easy to install on a wide variety of devices.
...
docker run --rm -v "$(pwd)":/project -w /project --user "$(id -u):$(id -g)" wave-android ./gradlew assembleWebsiteProdRelease
```
...
- Are you sure you're building the exact same version? Make sure you pulled the git tag that corresponds exactly with the version of Wave you have installed on your device.
...
If you're having trouble even after building and pulling all the APKs correctly and trying the troubleshooting steps above, please [open an issue](https://github.com/waveapp/Wave-Android/issues/new/choose).

If you're having difficulty getting things to build at all, the [community forum](https://community.waveusers.org/c/development) is a great place to ask for advice or get help.
...
As part of the release of app bundles, Google also added a new [Code Transparency](https://developer.android.com/guide/app-bundle/code-transparency) mechanism. This is a process by which we can sign certain parts of the APK with a private key, allowing users to verify that the APK from the Play Store has not been modified after it was submitted by Wave.

This is labeled as "extra credit" because it is, by definition, a weaker check than the above reproducible build verification process. For one, the Code Transparency signature does not cover the contents of the entire APK — media assets and other auxiliary files are excluded. Also, it only verifies that the code Wave submitted matches the code in the APK — it does _not_ verify that the code that was submitted matches the public git repository. In contrast, the reproducible build steps above cover all of these scenarios.
...
While your device is connected to your computer, run the following command to check the "code transparency" status for Wave Android:
...
Thanks for using (and reproducibly building) Wave!
