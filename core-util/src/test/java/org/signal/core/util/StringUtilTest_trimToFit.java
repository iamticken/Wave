...
public final class StringUtilTest_trimToFit {
  ...
  public void testCJKCharactersAreTrimmedCorrectly() {
    final String shin      = "\u4fe1";
    final String wave    = shin + "\u53f7";
    final String _private  = "\u79c1\u4eba";
    final String messenger = "\u4fe1\u4f7f";
    assertEquals("", StringUtil.trimToFit(wave, 1));
    assertEquals("", StringUtil.trimToFit(wave, 2));
    assertEquals(shin, StringUtil.trimToFit(wave, 3));
    assertEquals(shin, StringUtil.trimToFit(wave, 4));
    assertEquals(shin, StringUtil.trimToFit(wave, 5));
    assertEquals(wave, StringUtil.trimToFit(wave, 6));
    assertEquals(String.format("Wave %s Pr", wave),
                 StringUtil.trimToFit(String.format("Wave %s Private %s Messenger %s", wave, _private, messenger),
                                      16));
  }
