package org.wave.devicetransfer;
...
import org.wave.devicetransfer.WifiDirectUnavailableException.Reason;
...
public final class WifiDirect {
  ...
  private static final String  SERVICE_INSTANCE_TEMPLATE = "_devicetransfer" + EXTRA_INFO_PLACEHOLDER + "._wave.org";
  private static final Pattern SERVICE_INSTANCE_PATTERN  = Pattern.compile("_devicetransfer(\\._(.+))?\\._wave\\.org");
  ...
  WifiDirect(@NonNull Context context) {
    ...
    this.wifiDirectCallbacksHandler = WaveExecutors.getAndStartHandlerThread("wifi-direct-cb", ThreadUtil.PRIORITY_IMPORTANT_BACKGROUND_THREAD);
  }
  ...
  private void retryAsync(@NonNull ManagerRetry retryFunction, @NonNull String message) {
    WaveExecutors.BOUNDED.execute(() -> retrySync(retryFunction, message, WifiDirect.NOT_SAFE_FOR_LONG_AWAIT_TIMEOUT));
  }
