 ...
 */
sealed class ServiceId(val libWaveServiceId: org.wave.libwave.protocol.ServiceId) {
  companion object {
    private const val TAG = "ServiceId"

    @JvmStatic
    fun fromLibWave(serviceId: org.wave.libwave.protocol.ServiceId): ServiceId {
      return when (serviceId) {
        is org.wave.libwave.protocol.ServiceId.Aci -> ACI(serviceId)
        is org.wave.libwave.protocol.ServiceId.Pni -> PNI(serviceId)
        else -> throw IllegalArgumentException("Unknown libwave ServiceId type!")
      }
    }
    ...
    fun parseOrNull(raw: String?, logFailures: Boolean = true): ServiceId? {
      ...
      return try {
        fromLibWave(org.wave.libwave.protocol.ServiceId.parseFromString(raw))
      } catch (e: IllegalArgumentException) {
        if (logFailures) {
          Log.w(TAG, "[parseOrNull(String)] Illegal argument!", e)
        }
        null
      } catch (e: org.wave.libwave.protocol.ServiceId.InvalidServiceIdException) {
        if (logFailures) {
    ...
    fun parseOrNull(raw: ByteArray?): ServiceId? {
      ...
      return try {
        if (raw.size == 17) {
          fromLibWave(org.wave.libwave.protocol.ServiceId.parseFromFixedWidthBinary(raw))
        } else {
          fromLibWave(org.wave.libwave.protocol.ServiceId.parseFromBinary(raw))
        }
      } catch (e: IllegalArgumentException) {
  ...
  val rawUuid: UUID = libWaveServiceId.rawUUID

  val isUnknown: Boolean = rawUuid == UuidUtil.UNKNOWN_UUID

  val isValid: Boolean = !isUnknown

  fun toProtocolAddress(deviceId: Int): WaveProtocolAddress = WaveProtocolAddress(libWaveServiceId.toServiceIdString(), deviceId)

  fun toByteString(): ByteString = ByteString.Companion.of(*libWaveServiceId.toServiceIdBinary())

  fun toByteArray(): ByteArray = libWaveServiceId.toServiceIdBinary()

  fun logString(): String = libWaveServiceId.toLogString()
   ...
   */
  override fun toString(): String = libWaveServiceId.toServiceIdString()

  data class ACI(val libWaveAci: org.wave.libwave.protocol.ServiceId.Aci) : ServiceId(libWaveAci) {
    companion object {
      @JvmField
      val UNKNOWN = from(UuidUtil.UNKNOWN_UUID)

      @JvmStatic
      fun from(uuid: UUID): ACI = ACI(org.wave.libwave.protocol.ServiceId.Aci(uuid))

      @JvmStatic
      fun fromLibWave(aci: org.wave.libwave.protocol.ServiceId.Aci): ACI = ACI(aci)
  ...
  data class PNI(val libWavePni: org.wave.libwave.protocol.ServiceId.Pni) : ServiceId(libWavePni) {
    companion object {
      @JvmField
      var UNKNOWN = from(UuidUtil.UNKNOWN_UUID)

      @JvmStatic
      fun from(uuid: UUID): PNI = PNI(org.wave.libwave.protocol.ServiceId.Pni(uuid))
      ...
      fun parseOrNull(raw: String?): PNI? {
        return if (raw == null) {
          ...
          if (uuid != null) {
            PNI(org.wave.libwave.protocol.ServiceId.Pni(uuid))
          } else {
            null
          }
        }
