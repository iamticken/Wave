...
class WaveLogDetector : Detector(), Detector.UastScanner {
  ...
  override fun visitMethodCall(context: JavaContext, node: UCallExpression, method: PsiMethod) {
    ...
    if (evaluator.isMemberInClass(method, "android.util.Log")) {
      context.report(
        ...
        message = "Using 'android.util.Log' instead of a Wave Logger",
        quickfixData = quickFixIssueLog(node)
      )
    }

    if (evaluator.isMemberInClass(method, "org.wave.glide.Log")) {
      context.report(
        issue = LOG_NOT_SIGNAL,
        scope = node,
        location = context.getLocation(node),
        message = "Using 'org.wave.glide.Log' instead of a Wave Logger",
        quickfixData = quickFixIssueLog(node)
      )
    }

    if (evaluator.isMemberInClass(method, "org.wave.libwave.protocol.logging.Log")) {
      context.report(
        issue = LOG_NOT_APP,
        scope = node,
        location = context.getLocation(node),
        message = "Using Wave server logger instead of app level Logger",
        quickfixData = quickFixIssueLog(node)
      )
    }

    if (evaluator.isMemberInClass(method, "org.wave.core.util.logging.Log")) {
  ...
  private fun quickFixIssueLog(logCall: UCallExpression): LintFix {
    ...
    var fixSource = "org.wave.core.util.logging.Log."
  ...
  companion object {
    val LOG_NOT_SIGNAL: Issue = Issue.create(
      id = "LogNotWave",
      briefDescription = "Logging call to Android Log instead of Wave's Logger",
      explanation = "Wave has its own logger which must be used.",
      category = MESSAGES,
      priority = 5,
      severity = ERROR,
      implementation = Implementation(WaveLogDetector::class.java, JAVA_FILE_SCOPE)
    )

    val LOG_NOT_APP: Issue = Issue.create(
      id = "LogNotAppWave",
      briefDescription = "Logging call to Wave Service Log instead of App level Logger",
      explanation = "Wave app layer has its own logger which must be used.",
      category = MESSAGES,
      priority = 5,
      severity = ERROR,
      implementation = Implementation(WaveLogDetector::class.java, JAVA_FILE_SCOPE)
