...
import org.wave.libwave.net.AppExpiredException
import org.wave.libwave.net.AuthenticatedChatConnection
import org.wave.libwave.net.ChatConnection
import org.wave.libwave.net.ChatConnectionListener
import org.wave.libwave.net.ChatServiceException
import org.wave.libwave.net.ConnectionInvalidatedException
import org.wave.libwave.net.DeviceDeregisteredException
import org.wave.libwave.net.Network
import org.wave.libwave.net.UnauthenticatedChatConnection
...
import org.whispersystems.waveservice.internal.util.whenComplete
...
import org.wave.libwave.net.ChatConnection.Request as LibWaveRequest

/**
 * Implements the WebSocketConnection interface via libwave-net
 *
 ...
 * - [org.whispersystems.waveservice.api.websocket.WebSocketConnectionState] reporting is implemented
 *   as close as possible to the original implementation in
 *   [org.whispersystems.waveservice.internal.websocket.OkHttpWebSocketConnection].
...
class LibWaveChatConnection(
  ...
  // One of the more nasty parts of this is that libwave-net does not expose, nor does it ever
  ...
  // The layer above this, WaveWebSocket.java, is written to handle HTTP Requests and Responses
  ...
  // any bugs with underlying libwave-net layer.
  // So, we lie.
  // We assign our own "pseudo IDs" for each incoming request in this layer, provide that ID
  // up the stack to the WaveWebSocket, and then we store it. Eventually, WaveWebSocket will
  // tell us to send a response for that ID, and then we use the pseudo ID as a handle to find
  // the callback given to us earlier by libwave-net, and we call that callback.
  ...
  companion object {
    ...
    const val SIGNAL_SERVICE_ENVELOPE_TIMESTAMP_HEADER_KEY = "X-Wave-Timestamp"

    private val TAG = Log.tag(LibWaveChatConnection::class.java)

    private val KEEP_ALIVE_REQUEST = LibWaveRequest(
      "GET",
      "/v1/keepalive",
      emptyMap(),
      ByteArray(0),
    ...
    private fun WebSocketRequestMessage.toLibWaveRequest(timeout: Duration = WebSocketConnection.DEFAULT_SEND_TIMEOUT): LibWaveRequest {
      return LibWaveRequest(
  ...
  val stateMonitor = state
    ...
    .subscribe { nextState ->
      CHAT_SERVICE_LOCK.withLock {
        ...
        stateChangedOrMessageReceivedCondition.waveAll()
      }
    }
  ...
  private fun sendRequestInternal(request: WebSocketRequestMessage, timeoutSeconds: Long, single: SingleSubject<WebsocketResponse>) {
    CHAT_SERVICE_LOCK.withLock {
      ...
      val internalRequest = request.toLibWaveRequest(timeout = timeoutSeconds.seconds)
  ...
  private fun handleConnectionFailure(throwable: Throwable) {
    CHAT_SERVICE_LOCK.withLock {
      ...
      // Internally, libwave-net will throw this DeviceDeregisteredException when the HTTP CONNECT
  ...
  override fun isDead(): Boolean {
    CHAT_SERVICE_LOCK.withLock {
      return when (state.value) {
        ...
        null -> throw IllegalStateException("LibWaveChatConnection.state can never be null")
      }
    }
  }
    ...
    CHAT_SERVICE_LOCK.withLock {
      ...
      return when (state.value) {
        ...
        WebSocketConnectionState.CONNECTED -> {
          ...
          throw IllegalStateException("LibWaveChatConnection.state was neither dead, CONNECTING, or CONNECTED.")
        }
  ...
  override fun sendResponse(response: WebSocketResponseMessage) {
    if (response.status == 200 && response.message.equals("OK")) {
      ...
      // libwave-net only supports sending {200: OK} responses
  ...
  private val listener = LibWaveChatListener()

  private inner class LibWaveChatListener : ChatConnectionListener {
