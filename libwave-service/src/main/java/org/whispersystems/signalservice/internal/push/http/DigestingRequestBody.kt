...
import org.wave.libwave.protocol.incrementalmac.ChunkSizeChoice
import org.wave.libwave.protocol.logging.Log
import org.whispersystems.waveservice.api.crypto.DigestingOutputStream
import org.whispersystems.waveservice.api.crypto.SkippingOutputStream
...
class DigestingRequestBody(
  ...
  private val progressListener: WaveServiceAttachment.ProgressListener?,
  private val cancelationWave: CancelationWave?,
  ...
  override fun writeTo(sink: BufferedSink) {
    ...
    while (inputStream.read(buffer, 0, buffer.size).also { read = it } != -1) {
      if (cancelationWave?.isCanceled == true) {
