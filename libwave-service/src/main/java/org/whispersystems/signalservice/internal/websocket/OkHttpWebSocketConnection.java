...
public class OkHttpWebSocketConnection extends WebSocketListener implements WebSocketConnection {
  ...
  public OkHttpWebSocketConnection(String name,
                                   WaveServiceConfiguration serviceConfiguration,
                                   Optional<CredentialsProvider> credentialsProvider,
                                   String waveAgent,
                                   HealthMonitor healthMonitor,
                                   boolean allowStories) {
    this(name, serviceConfiguration, credentialsProvider, waveAgent, healthMonitor, "", allowStories);
  }
                                   ...
                                   boolean allowStories)
  {
    ...
    this.trustStore          = serviceConfiguration.getWaveServiceUrls()[0].getTrustStore();
    ...
    this.waveProxy         = serviceConfiguration.getWaveProxy();
    ...
    this.serviceUrls         = serviceConfiguration.getWaveServiceUrls();
  ...
  private Pair<WaveServiceUrl, String> getConnectionInfo() {
    WaveServiceUrl serviceUrl = serviceUrls[random.nextInt(serviceUrls.length)];
  ...
  public synchronized Observable<WebSocketConnectionState> connect() {
    ...
    if (client == null) {
      Pair<WaveServiceUrl, String> connectionInfo = getConnectionInfo();
      WaveServiceUrl               serviceUrl     = connectionInfo.getFirst();
      ...
      if (waveProxy.isPresent()) {
        clientBuilder.socketFactory(new TlsProxySocketFactory(waveProxy.get().getHost(), waveProxy.get().getPort(), dns));
      }
      ...
      requestBuilder.addHeader("X-Wave-Receive-Stories", allowStories ? "true" : "false");
