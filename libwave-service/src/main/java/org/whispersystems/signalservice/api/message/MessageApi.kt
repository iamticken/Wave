...
class MessageApi(
   ...
   * - 404: Message is not a story and recipient is not a registered Wave user
   ...
   * - 404: Message is not a story and some of the recipients are not registered Wave users
  ...
  fun sendGroupMessage(body: ByteArray, sealedSenderAccess: SealedSenderAccess, timestamp: Long, online: Boolean, urgent: Boolean, story: Boolean): NetworkResult<SendGroupMessageResponse> {
    val request = WebSocketRequestMessage.putCustom(
      ...
      body = body,
      headers = mapOf("content-type" to "application/vnd.wave-messenger.mrm")
    )
