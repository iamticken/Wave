...
import org.wave.libwave.net.CdsiLookupRequest;
import org.wave.libwave.net.CdsiLookupResponse;
...
import org.whispersystems.waveservice.api.push.exceptions.CdsiInvalidArgumentException;
...
public final class CdsiV2Service {
  ...
  public CdsiV2Service(@Nonnull Network network) {
    this.cdsiRequestHandler = (username, password, request, tokenSaver) -> {
      try {
        Future<CdsiLookupResponse> cdsiRequest = network.cdsiLookup(username, password, buildLibwaveRequest(request), tokenSaver);
        return Single.fromFuture(cdsiRequest)
                     .onErrorResumeNext((Throwable err) -> {
                       ...
                       return Single.error(mapLibwaveError(err));
                     })
                     .map(CdsiV2Service::parseLibwaveResponse)
                     .toObservable();
      } catch (Exception exception) {
        return Observable.error(mapLibwaveError(exception));
      }
    };
  }
  ...
  private static CdsiLookupRequest buildLibwaveRequest(Request request) {
    HashMap<org.wave.libwave.protocol.ServiceId, ProfileKey> serviceIds = new HashMap<>(request.serviceIds.size());
    request.serviceIds.forEach((key, value) -> serviceIds.put(key.getLibWaveServiceId(), value));
    return new CdsiLookupRequest(request.previousE164s, request.newE164s, serviceIds, Optional.ofNullable(request.token));
  }

  private static Response parseLibwaveResponse(CdsiLookupResponse response) {
  ...
  private static Throwable mapLibwaveError(Throwable lookupError) {
    if (lookupError instanceof org.wave.libwave.net.CdsiInvalidTokenException) {
      return new CdsiInvalidTokenException();
    } else if (lookupError instanceof org.wave.libwave.net.RetryLaterException) {
      org.wave.libwave.net.RetryLaterException e = (org.wave.libwave.net.RetryLaterException) lookupError;
    ...
    } else if (lookupError instanceof org.wave.libwave.net.CdsiProtocolException) {
