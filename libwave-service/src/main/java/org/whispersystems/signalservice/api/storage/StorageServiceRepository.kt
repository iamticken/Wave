...
class StorageServiceRepository(private val storageServiceApi: StorageServiceApi) {
  ...
  /**
   * Fetches and returns the fully-decrypted [WaveStorageManifest], if possible.
  ...
  /**
   * Fetches and returns the fully-decrypted [WaveStorageManifest] if the remote version is higher than the [manifestVersion] passed in.
  ...
  /**
   * Fetches and returns the fully-decrypted [WaveStorageRecord]s matching the list of provided [storageIds]
   */
  fun readStorageRecords(storageKey: StorageKey, recordIkm: RecordIkm?, storageIds: List<StorageId>): StorageRecordResult {
    ...
    val unknownRecordPlaceholders = storageIds
      .filter { it.isUnknown }
      .map { WaveStorageRecord.forUnknown(it) }
  ...
  fun writeStorageRecords(
    ...
    waveManifest: WaveStorageManifest,
    insertItems: List<WaveStorageRecord>,
    deleteRawIds: List<ByteArray>
  ): WriteStorageRecordsResult {
    return writeStorageRecords(storageKey, waveManifest, insertItems, deleteRawIds, clearAllExisting = false)
  }
   ...
   */
  fun writeUnchangedManifest(storageKey: StorageKey, manifest: WaveStorageManifest): WriteStorageRecordsResult {
  ...
  private fun writeStorageRecords(
    ...
    val manifestIds = waveManifest.storageIds.map { id ->
    ...
    val manifestRecord = ManifestRecord(
      sourceDevice = waveManifest.sourceDeviceId,
      version = waveManifest.version,
      identifiers = manifestIds,
      recordIkm = waveManifest.recordIkm?.value?.toByteString() ?: ByteString.EMPTY
    )

    val manifestKey = storageKey.deriveManifestKey(waveManifest.version)

    val encryptedManifest = StorageManifest(
      version = manifestRecord.version,
      value_ = WaveStorageCipher.encrypt(manifestKey, manifestRecord.encode()).toByteString()
    )

    val writeOperation = WriteOperation.Builder().apply {
      manifest = encryptedManifest
      insertItem = insertItems.map { it.toRemote(storageKey, waveManifest.recordIkm) }
  ...
  private fun StorageManifest.toLocal(storageKey: StorageKey): WaveStorageManifest {
    val rawRecord = WaveStorageCipher.decrypt(storageKey.deriveManifestKey(this.version), this.value_.toByteArray())
  ...
  @Throws(IOException::class, InvalidKeyException::class)
  private fun StorageItem.toLocal(type: Int, storageKey: StorageKey, recordIkm: RecordIkm?): WaveStorageRecord {
    val rawId = this.key.toByteArray()
    val key = recordIkm?.deriveStorageItemKey(rawId) ?: storageKey.deriveItemKey(rawId)
    val rawRecord = WaveStorageCipher.decrypt(key, this.value_.toByteArray())
    val record = StorageRecord.ADAPTER.decode(rawRecord)
    val id = StorageId.forType(rawId, type)

    return WaveStorageRecord(id, record)
  }

  private fun WaveStorageRecord.toRemote(storageKey: StorageKey, recordIkm: RecordIkm?): StorageItem {
    val key = recordIkm?.deriveStorageItemKey(this.id.raw) ?: storageKey.deriveItemKey(this.id.raw)
    val encryptedRecord = WaveStorageCipher.encrypt(key, this.proto.encode())
  ...
  sealed interface ManifestResult {
    data class Success(val manifest: WaveStorageManifest) : ManifestResult
  ...
  sealed interface ManifestIfDifferentVersionResult {
    data class DifferentVersion(val manifest: WaveStorageManifest) : ManifestIfDifferentVersionResult
  ...
  sealed interface StorageRecordResult {
    data class Success(val records: List<WaveStorageRecord>) : StorageRecordResult
