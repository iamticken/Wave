...
import org.whispersystems.waveservice.api.util.OptionalUtil.emptyIfStringEmpty
import org.whispersystems.waveservice.internal.push.BodyRange
import java.util.LinkedList
import java.util.Optional
import org.whispersystems.waveservice.internal.push.DataMessage.Payment as PaymentProto
import org.whispersystems.waveservice.internal.push.DataMessage.Quote as QuoteProto

/**
 * Represents a decrypted Wave Service data message.
 *
 ...
 */
class WaveServiceDataMessage private constructor(
  val timestamp: Long,
  val groupContext: Optional<WaveServiceGroupV2>,
  val attachments: Optional<List<WaveServiceAttachment>>,
  ...
  val previews: Optional<List<WaveServicePreview>>,
  ...
  class Builder {
    ...
    private var groupV2: WaveServiceGroupV2? = null
    private val attachments: MutableList<WaveServiceAttachment> = LinkedList<WaveServiceAttachment>()
    ...
    private val previews: MutableList<WaveServicePreview> = LinkedList<WaveServicePreview>()
    ...
    fun asGroupMessage(group: WaveServiceGroupV2?): Builder {
      groupV2 = group
      return this
    }

    fun withAttachments(attachments: List<WaveServiceAttachment>?): Builder {
    ...
    fun withPreviews(previews: List<WaveServicePreview>?): Builder {
    ...
    fun build(): WaveServiceDataMessage {
      if (timestamp == 0L) {
        timestamp = System.currentTimeMillis()
      }

      return WaveServiceDataMessage(
        timestamp = timestamp,
  ...
  data class Quote(
    ...
    data class QuotedAttachment(val contentType: String, val fileName: String?, val thumbnail: WaveServiceAttachment?)
  }
  class Sticker(val packId: ByteArray?, val packKey: ByteArray?, val stickerId: Int, val emoji: String?, val attachment: WaveServiceAttachment?)
