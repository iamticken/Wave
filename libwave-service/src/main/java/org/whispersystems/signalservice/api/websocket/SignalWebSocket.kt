...
import org.wave.libwave.net.BadRequestError
...
import org.whispersystems.waveservice.api.messages.EnvelopeResponse
...
import org.whispersystems.waveservice.internal.websocket.WebSocketResponseMessage
...
sealed class WaveWebSocket(
  ...
  companion object {
    private val TAG = Log.tag(WaveWebSocket::class)

    const val SERVER_DELIVERED_TIMESTAMP_HEADER = "X-Wave-Timestamp"
  ...
  /**
   * Executes the given callback with the underlying libwave chat connection when available.
   *
   * This is only supported for LibWave-based connections.
   *
   ...
   */
  suspend fun <T> runWithChatConnection(callback: (org.wave.libwave.net.ChatConnection) -> T): T {
  ...
  protected fun WebSocketRequestMessage.isWaveServiceEnvelope(): Boolean {
  ...
  private fun WebSocketRequestMessage.getWebSocketResponse(): WebSocketResponseMessage {
    return if (this.isWaveServiceEnvelope()) {
   ...
   */
  class UnauthenticatedWebSocket(connectionFactory: WebSocketFactory, canConnect: CanConnect, sleepTimer: SleepTimer, disconnectTimeoutMs: Long) : WaveWebSocket(connectionFactory, canConnect, sleepTimer, disconnectTimeoutMs.milliseconds) {
   ...
   */
  class AuthenticatedWebSocket(connectionFactory: WebSocketFactory, canConnect: CanConnect, sleepTimer: SleepTimer, disconnectTimeoutMs: Long) : WaveWebSocket(connectionFactory, canConnect, sleepTimer, disconnectTimeoutMs.milliseconds) {
    ...
    fun readMessageBatch(timeout: Long, batchSize: Int, callback: MessageReceivedCallback): Boolean {
      ...
      if (!hitEndOfQueue) {
        for (i in 1 until batchSize) {
          ...
          if (request != null) {
            if (request.isWaveServiceEnvelope()) {
