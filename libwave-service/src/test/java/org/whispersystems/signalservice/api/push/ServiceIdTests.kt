...
import org.wave.libwave.protocol.ServiceId.Aci as LibWaveAci
import org.wave.libwave.protocol.ServiceId.Pni as LibWavePni
...
class ServiceIdTests {
  ...
  fun `ServiceId parseOrNull ByteArray`() {
    ...
    assertEquals(PNI.from(UUID.fromString(uuidString)), ServiceId.parseOrNull(LibWavePni(uuid).toServiceIdBinary()))
    assertEquals(ACI.from(UUID.fromString(uuidString)), ServiceId.parseOrNull(LibWaveAci(uuid).toServiceIdFixedWidthBinary()))
    assertEquals(PNI.from(UUID.fromString(uuidString)), ServiceId.parseOrNull(LibWavePni(uuid).toServiceIdFixedWidthBinary()))
  }
  ...
  fun `ACI parseOrNull String`() {
    ...
    assertNull(ACI.parseOrNull(LibWavePni(uuid).toServiceIdString()))
  ...
  fun `ACI parseOrNull ByteArray`() {
    ...
    assertNull(ACI.parseOrNull(LibWavePni(uuid).toServiceIdBinary()))

    assertEquals(ACI.from(UUID.fromString(uuidString)), ACI.parseOrNull(uuidBytes))
    assertEquals(ACI.from(UUID.fromString(uuidString)), ACI.parseOrNull(LibWaveAci(uuid).toServiceIdBinary()))
    assertEquals(ACI.from(UUID.fromString(uuidString)), ACI.parseOrNull(LibWaveAci(uuid).toServiceIdFixedWidthBinary()))
  }
  ...
  fun `PNI parseOrNull ByteArray`() {
    ...
    assertNull(PNI.parseOrNull(LibWaveAci(uuid).toServiceIdFixedWidthBinary()))

    assertEquals(PNI.from(UUID.fromString(uuidString)), PNI.parseOrNull(uuidBytes))
    assertEquals(PNI.from(UUID.fromString(uuidString)), PNI.parseOrNull(LibWavePni(uuid).toServiceIdBinary()))
  }
