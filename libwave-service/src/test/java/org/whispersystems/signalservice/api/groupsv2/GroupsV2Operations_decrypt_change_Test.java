...
public final class GroupsV2Operations_decrypt_change_Test {
  ...
  private ByteString encryptProfileKey(ACI aci, ProfileKey profileKey) {
    return ByteString.of(new ClientZkGroupCipher(groupSecretParams).encryptProfileKey(profileKey, aci.getLibWaveAci()).serialize());
  }
  ...
  GroupCandidate groupCandidate(ACI aci, ProfileKey profileKey) {
    try {
      ...
      ProfileKeyCommitment                 commitment                           = profileKey.getCommitment(aci.getLibWaveAci());
      ProfileKeyCredentialRequestContext   requestContext                       = profileOperations.createProfileKeyCredentialRequestContext(aci.getLibWaveAci(), profileKey);
